...
function generateQuestionId(gradeLevel, phase, difficulty, subtag) {
  const timestamp = Date.now();
  const random = Math.random().toString(36).substring(2, 8);
  return `${gradeLevel}_${phase}_d${difficulty}_${subtag}_${timestamp}_${random}`;
}
__name(generateQuestionId, "generateQuestionId");
function getWriteClient(env) {
  ...
  console.log(`\u{1F517} [getWriteClient] Creating Supabase client with URL: ${env.VITE_SUPABASE_URL}`);
  return createClient(env.VITE_SUPABASE_URL, key);
}
__name(getWriteClient, "getWriteClient");
var GRADE_LEVEL_CONTEXT = {
...
function validateQuestions(parsed) {
  return parsed.map((q, index) => {
    ...
    return {
      ...
      explanation: q.explanation ? String(q.explanation) : void 0
    };
  });
}
__name(validateQuestions, "validateQuestions");
async function generateQuestionsWithAI(apiKey, specs, gradeLevel, phase) {
  console.log(`\u{1F916} Generating ${specs.length} questions via OpenRouter...`);
  const questionsDescription = specs.map(
    (spec, i) => `Question ${i + 1}: Difficulty ${spec.difficulty} (${DIFFICULTY_DESCRIPTIONS[spec.difficulty]}), Type: ${spec.subtag.replace(/_/g, " ")}`
  ).join("\n");
...
__name(generateQuestionsWithAI, "generateQuestionsWithAI");
var MIDDLE_SCHOOL_FALLBACKS = {
  numerical_reasoning: [
...
var HIGH_SCHOOL_FALLBACKS = {
  numerical_reasoning: [
    ...
    { text: "What is the value of 5\xB2 + 3\xB2?", options: { A: "30", B: "32", C: "34", D: "36" }, correctAnswer: "C" }
  ],
  logical_reasoning: [
  ...
  data_interpretation: [
    ...
    { text: "If a pie chart shows 25% for Category A, what angle does it represent?", options: { A: "45\xB0", B: "90\xB0", C: "180\xB0", D: "270\xB0" }, correctAnswer: "B" },
...
function getFallbackQuestion(gradeLevel, phase, difficulty, subtag, excludeTexts = /* @__PURE__ */ new Set()) {
  ...
  if (availableFallbacks.length > 0) {
    ...
    const allSubtags = Object.keys(fallbacks);
...
__name(getFallbackQuestion, "getFallbackQuestion");
async function getCachedQuestions(env, supabase, gradeLevel, phase, difficulty, subtag, limit = 10, excludeIds = []) {
  console.log(`\u{1F50D} [getCachedQuestions] Querying cache:`, {
    gradeLevel,
    phase,
    difficulty,
    subtag,
  ...
  let query = supabase.from("adaptive_aptitude_questions_cache").select("*").eq("grade_level", gradeLevel).eq("phase", phase).eq("is_active", true);
  ...
  if (!data || data.length === 0) {
    console.log("\u{1F4ED} [getCachedQuestions] No cached questions found");
    return [];
  }
  console.log(`\u2705 [getCachedQuestions] Found ${data.length} cached questions`);
  const questions = data.map((record) => ({
    ...
    phase: record.phase,
    explanation: record.explanation || void 0,
    createdAt: record.created_at
  }));
  if (questions.length > 0) {
    const questionIds = questions.map((q) => q.id);
    console.log(`\u{1F4CA} [getCachedQuestions] Triggering usage tracking for ${questionIds.length} questions`);
    trackQuestionUsage(env, questionIds).catch(
      (err) => console.warn("\u26A0\uFE0F [getCachedQuestions] Failed to track question usage:", err)
    );
  }
  return questions;
}
__name(getCachedQuestions, "getCachedQuestions");
async function trackQuestionUsage(env, questionIds) {
  if (questionIds.length === 0)
    return;
  console.log(`\u{1F4CA} [trackQuestionUsage] Tracking usage for ${questionIds.length} cached questions...`);
  const serviceKey = env.SUPABASE_SERVICE_ROLE_KEY || env.SUPABASE_SERVICE_KEY;
  console.log(`\u{1F511} [trackQuestionUsage] Service role key available: ${!!serviceKey}`);
  const writeClient = getWriteClient(env);
  const updatePromises = questionIds.map(async (questionId, index) => {
    console.log(`\u{1F4DD} [trackQuestionUsage] Processing question ${index + 1}/${questionIds.length}: ${questionId}`);
    const { data: currentData, error: fetchError } = await writeClient.from("adaptive_aptitude_questions_cache").select("usage_count").eq("question_id", questionId).single();
    ...
    console.log(`\u{1F4C8} [trackQuestionUsage] Current usage_count for ${questionId}: ${currentData.usage_count}`);
    const updateData = {
      usage_count: (currentData.usage_count || 0) + 1,
      last_used_at: (/* @__PURE__ */ new Date()).toISOString()
    };
    console.log(`\u{1F4BE} [trackQuestionUsage] Updating ${questionId} with:`, updateData);
    const { error: updateError } = await writeClient.from("adaptive_aptitude_questions_cache").update(updateData).eq("question_id", questionId);
    if (updateError) {
...
__name(trackQuestionUsage, "trackQuestionUsage");
async function cacheQuestions(env, questions) {
  if (questions.length === 0) {
    console.log("\u26A0\uFE0F [cacheQuestions] No questions to cache (empty array)");
    return;
  }
  try {
    console.log(`\u{1F4BE} [cacheQuestions] Starting cache operation for ${questions.length} questions`);
    const serviceKey = env.SUPABASE_SERVICE_ROLE_KEY || env.SUPABASE_SERVICE_KEY;
    console.log(`\u{1F511} [cacheQuestions] Service role key available: ${!!serviceKey}`);
    const writeClient = getWriteClient(env);
    ...
    console.log(`\u{1F4DD} [cacheQuestions] Sample record to cache:`, {
    ...
    console.log(`\u{1F4BE} [cacheQuestions] Attempting upsert to adaptive_aptitude_questions_cache...`);
    const { data, error } = await writeClient.from("adaptive_aptitude_questions_cache").upsert(records, { onConflict: "question_id" });
    if (error) {
      ...
      console.log(`\u{1F4CA} [cacheQuestions] Upsert response data:`, data);
    }
  } catch (err) {
    console.error("\u274C [cacheQuestions] Unexpected error during caching:", err);
  }
}
__name(cacheQuestions, "cacheQuestions");
function selectSubtagsForScreener(totalQuestions, minSubtags) {
...
__name(selectSubtagsForScreener, "selectSubtagsForScreener");
function reorderToPreventConsecutiveSubtags(questions, maxConsecutive) {
...
__name(reorderToPreventConsecutiveSubtags, "reorderToPreventConsecutiveSubtags");
async function generateDiagnosticScreenerQuestions(env, gradeLevel, excludeQuestionIds = [], excludeQuestionTexts = []) {
  console.log("\u{1F3AF} generateDiagnosticScreenerQuestions:", { gradeLevel, excludeCount: excludeQuestionIds.length, excludeTextsCount: excludeQuestionTexts.length });
  ...
  const phase = "diagnostic_screener";
  ...
  const usedQuestionIds = new Set(excludeQuestionIds);
  const allQuestions = new Array(6);
  const missingSpecs = [];
  let cachedCount = 0;
  console.log("\u{1F4E6} Checking cache sequentially to prevent duplicates...");
  ...
  console.log(`\u{1F4CA} Cache hit: ${cachedCount}/6, need to generate: ${missingSpecs.length}`);
  ...
  if (missingSpecs.length > 0) {
    try {
      ...
      if (batchGenerated.length > 0) {
        cacheQuestions(env, batchGenerated).catch(
          (err) => console.warn("Background caching failed:", err)
        );
      }
    } catch (genError) {
  ...
  const usedFallbackTexts = new Set(excludeQuestionTexts || []);
  allQuestions.forEach((q) => {
    if (q)
      usedFallbackTexts.add(q.text);
  });
  ...
  const validQuestions = allQuestions.filter((q) => q !== void 0);
...
function generateDifficultyRange(startingDifficulty, count) {
  const difficulties = [startingDifficulty];
...
__name(generateDifficultyRange, "generateDifficultyRange");
function generateBalancedSubtagSequence(count, maxConsecutive) {
  const sequence = [];
...
__name(generateBalancedSubtagSequence, "generateBalancedSubtagSequence");
async function generateAdaptiveCoreQuestions(env, gradeLevel, startingDifficulty, count = 10, excludeQuestionIds = [], excludeQuestionTexts = []) {
  console.log("\u{1F3AF} generateAdaptiveCoreQuestions:", { gradeLevel, startingDifficulty, count, excludeCount: excludeQuestionIds.length, excludeTextsCount: excludeQuestionTexts.length });
  const startTime = Date.now();
  const supabase = createClient(env.VITE_SUPABASE_URL, env.VITE_SUPABASE_ANON_KEY);
  const apiKey = env.OPENROUTER_API_KEY || env.VITE_OPENROUTER_API_KEY;
  if (!apiKey) {
  ...
  const usedQuestionIds = new Set(excludeQuestionIds);
  const usedQuestionTexts = new Set(excludeQuestionTexts);
  const allQuestions = new Array(questionCount);
  const missingSpecs = [];
  let cachedCount = 0;
  console.log("\u{1F4E6} Checking cache sequentially to prevent duplicates...");
  for (let index = 0; index < questionSpecs.length; index++) {
...
function generateStabilityDifficulties(provisionalBand, count) {
  const difficulties = [];
  const minDifficulty = Math.max(1, provisionalBand - 1);
  const maxDifficulty = Math.min(5, provisionalBand + 1);
...
__name(generateStabilityDifficulties, "generateStabilityDifficulties");
function generateMixedFormatSubtags(count, maxConsecutive) {
  const dataFormats = ["data_interpretation", "pattern_recognition"];
  const logicFormats = ["logical_reasoning", "numerical_reasoning", "verbal_reasoning", "spatial_reasoning"];
  const sequence = [];
...
__name(generateMixedFormatSubtags, "generateMixedFormatSubtags");
async function generateStabilityConfirmationQuestions(env, gradeLevel, provisionalBand, count = 4, excludeQuestionIds = [], excludeQuestionTexts = []) {
  console.log("\u{1F3AF} generateStabilityConfirmationQuestions:", { gradeLevel, provisionalBand, count, excludeCount: excludeQuestionIds.length, excludeTextsCount: excludeQuestionTexts.length });
  const startTime = Date.now();
  const supabase = createClient(env.VITE_SUPABASE_URL, env.VITE_SUPABASE_ANON_KEY);
  const apiKey = env.OPENROUTER_API_KEY || env.VITE_OPENROUTER_API_KEY;
  if (!apiKey) {
  ...
  const phase = "stability_confirmation";
    ...
    if ((path === "/generate/single" || path === "/api/generate/single") && request.method === "POST") {
      try {
        ...
        if (generated.length > 0) {
          cacheQuestions(env, generated).catch(() => {
          });
        }
        return jsonResponse({
          questions: generated,
