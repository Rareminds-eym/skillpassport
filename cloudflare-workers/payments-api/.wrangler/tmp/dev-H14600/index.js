...
// .wrangler/tmp/bundle-v4dOYC/checked-fetch.js
...
// .wrangler/tmp/bundle-v4dOYC/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-v4dOYC/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
async function handleDeactivateSubscription(request, env) {
  ...
  const { data: updated, error: updateError } = await supabase.from("subscriptions").update({
    ...
    auto_renew: false,
    cancellation_reason
  }).eq("id", subscription_id).eq("user_id", user.id).select("id, status, cancelled_at, subscription_end_date").single();
  if (updateError) {
    return jsonResponse({ error: "Failed to cancel subscription" }, 500);
  }
  return jsonResponse({
      ...
      switch (path) {
        ...
        case "/health":
          ...
          return jsonResponse({
            ...
            timestamp: (/* @__PURE__ */ new Date()).toISOString(),
            config: configStatus,
            message: allConfigured ? "All required secrets are configured" : "Some required secrets are missing. Set them via Cloudflare dashboard or wrangler secret put."
          });
        default:
