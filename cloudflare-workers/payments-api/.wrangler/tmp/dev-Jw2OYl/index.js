...
// .wrangler/tmp/bundle-doDdnb/checked-fetch.js
...
// .wrangler/tmp/bundle-doDdnb/strip-cf-connecting-ip-header.js
...
__name(getRazorpayKeyId, "getRazorpayKeyId");
...
async function handleCreateOrder(request, env) {
  ...
  const { count } = await supabase.from("razorpay_orders").select("*", { count: "exact", head: true }).eq("user_id", user.id).gte("created_at", oneMinuteAgo);
  if (count && count >= 5) {
...
__name(calculateSubscriptionEndDate, "calculateSubscriptionEndDate");
async function handleVerifyPayment(request, env) {
  ...
  const { data: existingSubscription } = await supabaseAdmin.from("subscriptions").select("*").eq("razorpay_payment_id", razorpay_payment_id).maybeSingle();
  if (existingSubscription) {
    const { data: transaction } = await supabaseAdmin.from("payment_transactions").select("payment_method, amount").eq("razorpay_payment_id", razorpay_payment_id).maybeSingle();
    return jsonResponse({
      success: true,
      verified: true,
  ...
  const { data: order, error: orderError } = await supabaseAdmin.from("razorpay_orders").select("*").eq("order_id", razorpay_order_id).maybeSingle();
  if (orderError || !order) {
  ...
  await supabaseAdmin.from("razorpay_orders").update({ status: "paid", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("order_id", razorpay_order_id);
  const { data: userData } = await supabaseAdmin.from("users").select("firstName, lastName, email, phone").eq("id", order.user_id).maybeSingle();
  ...
  const { data: subscription, error: subError } = await supabaseAdmin.from("subscriptions").insert([subscriptionData]).select().single();
  if (subError) {
  ...
  const { error: txnError } = await supabaseAdmin.from("payment_transactions").insert([transactionData]);
  if (txnError) {
...
async function handleWebhook(request, env) {
  ...
  switch (event) {
    case "payment.captured":
      await supabaseAdmin.from("payment_transactions").update({ status: "success", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("razorpay_payment_id", payload.payment.entity.id);
      break;
    case "payment.failed":
      await supabaseAdmin.from("payment_transactions").update({ status: "failed", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("razorpay_payment_id", payload.payment.entity.id);
      break;
...
async function handleCancelSubscription(request, env) {
  ...
  await supabase.from("subscriptions").update({
    ...
    auto_renew: false,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("razorpay_subscription_id", subscription_id).eq("user_id", user.id);
  return jsonResponse({
    success: true,
...
async function handleDeactivateSubscription(request, env) {
  const auth = await authenticateUser(request, env);
  if (!auth)
    return jsonResponse({ error: "Unauthorized" }, 401);
  const { user, supabase } = auth;
  ...
  const { data: subscription, error: fetchError } = await supabase.from("subscriptions").select("id, user_id, status, subscription_end_date, cancelled_at").eq("id", subscription_id).maybeSingle();
  ...
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const { data: updated, error: updateError } = await supabase.from("subscriptions").update({
    status: "cancelled",
    auto_renew: false,
    cancelled_at: now,
    cancellation_reason,
    updated_at: now
  }).eq("id", subscription_id).eq("user_id", user.id).select("*").single();
  if (updateError) {
    return jsonResponse({ error: "Failed to cancel subscription" }, 500);
  }
...
__name(handleDeactivateSubscription, "handleDeactivateSubscription");
async function handlePauseSubscription(request, env) {
  ...
  const { data: subscription, error: fetchError } = await supabase.from("subscriptions").select("*").eq("id", subscription_id).maybeSingle();
  if (fetchError || !subscription) {
    return jsonResponse({ error: "Subscription not found" }, 404);
  }
  if (subscription.user_id !== user.id) {
...
__name(handlePauseSubscription, "handlePauseSubscription");
...
__name(handleResumeSubscription, "handleResumeSubscription");
async function handleGetSubscription(request, env) {
  const auth = await authenticateUser(request, env);
  if (!auth)
    return jsonResponse({ error: "Unauthorized" }, 401);
  const { user, supabase } = auth;
  const { data: subscription, error } = await supabase.from("subscriptions").select("*").eq("user_id", user.id).in("status", ["active", "paused"]).order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (error) {
    return jsonResponse({ error: "Failed to fetch subscription" }, 500);
  }
  if (!subscription) {
    const { data: recentSub } = await supabase.from("subscriptions").select("*").eq("user_id", user.id).order("created_at", { ascending: false }).limit(1).maybeSingle();
    return jsonResponse({
      success: true,
...
var MAX_EVENT_AMOUNT_RUPEES = 1e7;
var TEST_MODE_MAX_AMOUNT = 5e6;
async function handleCreateEventOrder(request, env) {
  const body = await request.json();
  ...
  const supabaseUrl = getSupabaseUrl(env);
  const supabaseAdmin = createClient(supabaseUrl, env.SUPABASE_SERVICE_ROLE_KEY);
  const { data: registration, error: regError } = await supabaseAdmin.from("event_registrations").select("id, payment_status").eq("id", registrationId).maybeSingle();
  ...
  await supabaseAdmin.from("event_registrations").update({ razorpay_order_id: order.id }).eq("id", registrationId);
  return jsonResponse({
    success: true,
    id: order.id,
    amount: order.amount,
...
__name(handleCreateEventOrder, "handleCreateEventOrder");
async function handleExpireSubscriptions(env) {
  const supabaseUrl = getSupabaseUrl(env);
  const supabaseAdmin = createClient(supabaseUrl, env.SUPABASE_SERVICE_ROLE_KEY);
...
__name(handleExpireSubscriptions, "handleExpireSubscriptions");
var src_default = {
...
// .wrangler/tmp/bundle-doDdnb/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-doDdnb/middleware-loader.entry.ts
