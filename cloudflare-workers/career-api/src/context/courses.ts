// Course Context Builder - Cloudflare Workers Version

import type { SupabaseClient } from '@supabase/supabase-js';
import type { CourseContext, CourseEnrollment, AvailableCourse } from '../types/career-ai';
...
export async function buildCourseContext(
  ...
  try {
    ...
    // Note: educator_name column doesn't exist in courses table, removed from query
    ...
    const transformedAvailable: AvailableCourse[] = (availableCourses || []).map(c => ({
      ...
      educatorName: undefined // educator_name column doesn't exist in courses table
    }));

    const inProgress = transformedEnrollments.filter(e => e.status === 'active' && e.progress < 100).length;
    const completed = transformedEnrollments.filter(e => e.status === 'completed' || e.progress === 100).length;
