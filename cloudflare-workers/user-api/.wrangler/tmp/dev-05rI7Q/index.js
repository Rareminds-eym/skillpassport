...
// .wrangler/tmp/bundle-1DDHki/checked-fetch.js
...
// .wrangler/tmp/bundle-1DDHki/strip-cf-connecting-ip-header.js
...
__name(checkEmailExists, "checkEmailExists");
async function deleteAuthUser(supabaseAdmin, userId) {
  await supabaseAdmin.auth.admin.deleteUser(userId);
}
__name(deleteAuthUser, "deleteAuthUser");

// src/utils/email.ts
var LOGIN_URL = "https://skillpassport.rareminds.in/login";
async function sendWelcomeEmail(env, email, name, password, role, additionalInfo) {
...
async function sendPasswordResetEmail(env, email, otp) {
  if (!env.RESEND_API_KEY) {
    console.log("RESEND_API_KEY not configured, skipping email");
    return false;
  }
...
async function sendInterviewReminderEmail(env, recipientEmail, recipientName, interviewDetails) {
  if (!env.RESEND_API_KEY) {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
    <h1 style="margin: 0;">\u{1F3AF} Interview Reminder</h1>
  </div>
  <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px;">
    <p>Dear ${recipientName},</p>
    <p>This is a friendly reminder about your upcoming interview!</p>
    <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #667eea;">
      <h3 style="margin-top: 0; color: #667eea;">\u{1F4C5} Interview Details</h3>
    ...
    ${meetingLink ? `<div style="text-align: center;"><a href="${meetingLink}" style="background: #667eea; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">\u{1F517} Join Meeting</a></div>` : ""}
...
__name(sendInterviewReminderEmail, "sendInterviewReminderEmail");
function generateWelcomeEmailHtml(name, email, password, roleDisplay, additionalInfo) {
  return `
<!DOCTYPE html>
<html>
        ...
        <table style="width: 600px; max-width: 100%; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          <tr>
            <td style="padding: 40px; text-align: center; background: linear-gradient(135deg, #3B82F6 0%, #1D4ED8 100%); border-radius: 12px 12px 0 0;">
              <h1 style="margin: 0; color: #ffffff; font-size: 28px;">\u{1F389} Welcome to Skill Passport!</h1>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding: 24px 40px; background-color: #F9FAFB; border-radius: 0 0 12px 12px; text-align: center;">
              <p style="margin: 0; color: #9CA3AF; font-size: 12px;">\xA9 ${(/* @__PURE__ */ new Date()).getFullYear()} Skill Passport by Rareminds</p>
            </td>
          </tr>
        </table>
      </td>
...
// src/handlers/school.ts
async function handleSchoolAdminSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    ...
    const { data: existingSchool } = await supabaseAdmin.from("schools").select("id").eq("code", body.schoolCode).maybeSingle();
    if (existingSchool) {
    ...
    try {
      ...
      const { data: school, error: schoolError } = await supabaseAdmin.from("schools").insert({
...
__name(handleSchoolAdminSignup, "handleSchoolAdminSignup");
async function handleEducatorSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    ...
    const { data: school, error: schoolError } = await supabaseAdmin.from("schools").select("id, name").eq("id", body.schoolId).single();
    if (schoolError || !school) {
      return jsonResponse({ error: "Invalid school selected" }, 400);
    }
    const { data: existingEducator } = await supabaseAdmin.from("school_educators").select("id").eq("email", body.email.toLowerCase()).maybeSingle();
    if (existingEducator) {
    ...
    try {
      ...
      const { data: educator, error: educatorError } = await supabaseAdmin.from("school_educators").insert({
        user_id: userId,
        school_id: body.schoolId,
        email: body.email.toLowerCase(),
        first_name: body.firstName,
...
__name(handleEducatorSignup, "handleEducatorSignup");
async function handleStudentSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    ...
    const { data: existingStudent } = await supabaseAdmin.from("students").select("id").eq("email", body.email.toLowerCase()).maybeSingle();
    if (existingStudent) {
...
__name(handleStudentSignup, "handleStudentSignup");

// src/handlers/college.ts
async function handleCollegeAdminSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    const requiredFields = [
    ...
    const { data: existingCollege } = await supabaseAdmin.from("colleges").select("id").eq("code", body.collegeCode).maybeSingle();
    if (existingCollege) {
    ...
    try {
      ...
      const { data: college, error: collegeError } = await supabaseAdmin.from("colleges").insert({
...
__name(handleCollegeAdminSignup, "handleCollegeAdminSignup");
async function handleCollegeEducatorSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    ...
    const { data: college, error: collegeError } = await supabaseAdmin.from("colleges").select("id, name").eq("id", body.collegeId).single();
    if (collegeError || !college) {
      return jsonResponse({ error: "Invalid college selected" }, 400);
    }
    const { data: existingEducator } = await supabaseAdmin.from("college_lecturers").select("id").eq("metadata->>email", body.email.toLowerCase()).maybeSingle();
    if (existingEducator) {
      return jsonResponse({ error: "An educator with this email already exists" }, 400);
    }
    const { data: authUser, error: authError } = await supabaseAdmin.auth.admin.createUser({
    ...
    try {
      ...
      const { data: educator, error: educatorError } = await supabaseAdmin.from("college_lecturers").insert({
        user_id: userId,
        userId,
...
__name(handleCollegeEducatorSignup, "handleCollegeEducatorSignup");
async function handleCollegeStudentSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
...
__name(handleCollegeStudentSignup, "handleCollegeStudentSignup");

// src/handlers/university.ts
async function handleUniversityAdminSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    ...
    const { data: existingUniversity } = await supabaseAdmin.from("universities").select("id").eq("code", body.universityCode).maybeSingle();
    ...
    try {
      ...
      const { data: university, error: universityError } = await supabaseAdmin.from("universities").insert({
...
__name(handleUniversityAdminSignup, "handleUniversityAdminSignup");
async function handleUniversityEducatorSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    ...
    const { data: university, error: universityError } = await supabaseAdmin.from("universities").select("id, name").eq("id", body.universityId).single();
    if (universityError || !university) {
    ...
    try {
      ...
      const { data: educator, error: educatorError } = await supabaseAdmin.from("university_educators").insert({
        user_id: userId,
        university_id: body.universityId,
        email: body.email.toLowerCase(),
        first_name: body.firstName,
...
__name(handleUniversityEducatorSignup, "handleUniversityEducatorSignup");
async function handleUniversityStudentSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
...
__name(handleUniversityStudentSignup, "handleUniversityStudentSignup");

// src/handlers/recruiter.ts
async function handleRecruiterAdminSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    ...
    const { data: existingCompany } = await supabaseAdmin.from("companies").select("id").eq("code", body.companyCode).maybeSingle();
    if (existingCompany) {
...
__name(handleRecruiterAdminSignup, "handleRecruiterAdminSignup");
async function handleRecruiterSignup(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    ...
    const { data: company, error: companyError } = await supabaseAdmin.from("companies").select("id, name").eq("id", body.companyId).single();
    if (companyError || !company) {
      return jsonResponse({ error: "Invalid company selected" }, 400);
    }
    const { data: existingRecruiter } = await supabaseAdmin.from("recruiters").select("id").eq("email", body.email.toLowerCase()).maybeSingle();
    ...
    try {
      ...
      const { data: recruiter, error: recruiterError } = await supabaseAdmin.from("recruiters").insert({
        user_id: userId,
        company_id: body.companyId,
        name: body.name,
        email: body.email.toLowerCase(),
...
__name(handleRecruiterSignup, "handleRecruiterSignup");

// src/handlers/utility.ts
async function handleGetSchools(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const { data: schools, error } = await supabaseAdmin.from("schools").select("id, name, city, state, country, code").order("name", { ascending: true });
    if (error) {
...
__name(handleGetSchools, "handleGetSchools");
async function handleGetColleges(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const { data: colleges, error } = await supabaseAdmin.from("colleges").select("id, name, city, state, country, code").order("name", { ascending: true });
    if (error) {
...
__name(handleGetColleges, "handleGetColleges");
async function handleGetUniversities(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const { data: universities, error } = await supabaseAdmin.from("universities").select("id, name, district, state, code").order("name", { ascending: true });
    if (error) {
...
__name(handleGetUniversities, "handleGetUniversities");
async function handleGetCompanies(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const { data: companies, error } = await supabaseAdmin.from("companies").select("id, name, hqCity, hqState, hqCountry, code").order("name", { ascending: true });
    if (error) {
...
__name(handleGetCompanies, "handleGetCompanies");
async function handleCheckSchoolCode(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    if (!body.code) {
    ...
    const { data: existingSchool } = await supabaseAdmin.from("schools").select("id").eq("code", body.code).maybeSingle();
    return jsonResponse({
      success: true,
...
__name(handleCheckSchoolCode, "handleCheckSchoolCode");
async function handleCheckCollegeCode(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    if (!body.code) {
    ...
    const { data: existingCollege } = await supabaseAdmin.from("colleges").select("id").eq("code", body.code).maybeSingle();
    return jsonResponse({
      success: true,
...
__name(handleCheckCollegeCode, "handleCheckCollegeCode");
async function handleCheckUniversityCode(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    if (!body.code) {
    ...
    const { data: existingUniversity } = await supabaseAdmin.from("universities").select("id").eq("code", body.code).maybeSingle();
    return jsonResponse({
      success: true,
...
__name(handleCheckUniversityCode, "handleCheckUniversityCode");
async function handleCheckCompanyCode(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  try {
    const body = await request.json();
    if (!body.code) {
    ...
    const { data: existingCompany } = await supabaseAdmin.from("companies").select("id").eq("code", body.code).maybeSingle();
    return jsonResponse({
      success: true,
...
__name(handleCheckCompanyCode, "handleCheckCompanyCode");
async function handleCheckEmail(request, env) {
  ...
  try {
    ...
    return jsonResponse({
      success: true,
      exists: emailExists,
      message: emailExists ? "An account with this email already exists" : "Email is available"
    });
  } catch (error) {
...
async function handleCreateStudent(request, env) {
  ...
  const { data: currentUserData } = await supabaseAdmin.from("users").select("id, organizationId, role").eq("email", userEmail).maybeSingle();
  ...
  if (collegeId) {
    ...
    if (userRole === "college_admin") {
      const { data: college } = await supabaseAdmin.from("colleges").select("id").ilike("deanEmail", userEmail).maybeSingle();
      if (college?.id) {
        collegeId = college.id;
        institutionType = "college";
      }
      ...
      const { data: educatorData } = await supabaseAdmin.from("school_educators").select("school_id").eq("email", userEmail).maybeSingle();
...
async function handleCreateTeacher(request, env) {
  const auth = await authenticateUser(request, env);
  if (!auth)
    return jsonResponse({ error: "Unauthorized" }, 401);
  ...
  const { data: currentUserData } = await supabaseAdmin.from("users").select("organizationId").eq("id", user.id).single();
  ...
  const { data: existingTeacher } = await supabaseAdmin.from("school_educators").select("id").eq("email", teacher.email.toLowerCase()).maybeSingle();
  if (existingTeacher) {
  ...
  try {
    ...
    const { data: teacherRecord, error: teacherError } = await supabaseAdmin.from("school_educators").insert({
      user_id: authUser.user.id,
      school_id: schoolId,
      email: teacher.email.toLowerCase(),
      first_name: teacher.first_name,
...
async function handleCreateEventUser(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  const body = await request.json();
  ...
  if (existingUser) {
    await supabaseAdmin.from("event_registrations").update({ user_id: existingUser.id }).eq("id", registrationId);
    return jsonResponse({
      success: true,
  ...
  const { error: updateError } = await supabaseAdmin.from("event_registrations").update({ user_id: newUser.user.id }).eq("id", registrationId);
  const planName = metadata?.plan || "Skill Passport";
...
__name(handleCreateEventUser, "handleCreateEventUser");
async function handleSendInterviewReminder(request, env) {
  const supabaseAdmin = getSupabaseAdmin(env);
  const body = await request.json();
...
__name(handleSendInterviewReminder, "handleSendInterviewReminder");
...
async function handleResetPassword(request, env) {
  ...
  if (action === "send") {
    ...
    const { error: dbError } = await supabaseAdmin.from("reset_tokens").insert({ email, token, expires_at: expiresAt2.toISOString() });
    if (dbError) {
  ...
  if (action === "verify") {
    ...
    const { data: tokens } = await supabaseAdmin.from("reset_tokens").select("*").eq("email", email).eq("token", otp).gt("expires_at", (/* @__PURE__ */ new Date()).toISOString()).order("created_at", { ascending: false }).limit(1);
    if (!tokens || tokens.length === 0) {
      return jsonResponse({ error: "Invalid or expired OTP" }, 400);
    }
    const {
      data: { users }
    } = await supabaseAdmin.auth.admin.listUsers({ page: 1, perPage: 1e3 });
...
// .wrangler/tmp/bundle-1DDHki/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1DDHki/middleware-loader.entry.ts
