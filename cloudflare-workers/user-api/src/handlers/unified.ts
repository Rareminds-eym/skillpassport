...
import { capitalizeFirstLetter, jsonResponse, validateEmail } from '../utils/helpers';
...
export interface UnifiedSignupRequest {
  ...
  lastName: string;
  phone?: string;
  role: 'school_student' | 'college_student' | 'school_educator' | 'college_educator' | 'recruiter' | 'school_admin' | 'college_admin' | 'university_admin';
  dateOfBirth?: string;
  country?: string;
  state?: string;
  city?: string;
 ...
 */
export async function handleUnifiedSignup(request: Request, env: Env): Promise<Response> {
