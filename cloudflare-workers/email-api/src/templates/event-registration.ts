...
import type { EventConfirmationData, OTPEmailData } from '../types';

/**
 * Generate user confirmation email HTML
 */
export function generateUserConfirmationHtml(data: EventConfirmationData): string {
          ...
          <tr>
            <td style="padding:40px;">
              ...
              <div style="margin-top:24px;padding:20px;background:#eff6ff;border-radius:12px;text-align:center;border:2px solid #3b82f6;">
                ...
                <a href="https://skillpassport.rareminds.in/api/email/download-receipt/${orderId}" style="display:inline-block;background:#1e40af;color:#ffffff;text-decoration:none;padding:12px 32px;border-radius:8px;font-size:14px;font-weight:600;margin-bottom:8px;">Download PDF Receipt</a>
 ...
 */
export function generateAdminNotificationHtml(data: EventConfirmationData): string {
  const { name, email, phone, amount, orderId, campaign } = data;
