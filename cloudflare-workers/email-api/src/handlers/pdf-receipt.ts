...
import type { Env, PreRegistration } from '../types';

/**
 * Generate PDF receipt and return as downloadable file
 * GET /download-receipt/:orderId
 */
export async function handlePDFReceipt(orderId: string, env: Env): Promise<Response> {
  if (!orderId) {
  ...
  try {
    ...
    // Generate PDF
    const pdfBytes = await generatePDF(data as PreRegistration, orderId);
    ...
    return errorResponse('Failed to generate PDF receipt', (error as Error).message, 500);
  }
}
...
// ═══════════════════════════════════════════════════════════
function drawCentered(page: PDFPage, text: string, y: number, font: PDFFont, size: number, color: RGB, pageWidth: number): void {
...
// ═══════════════════════════════════════════════════════════
function drawRow(page: PDFPage, label: string, value: string, y: number, labelFont: PDFFont, valueFont: PDFFont, labelColor: RGB, valueColor: RGB, x: number, gap: number): void {
