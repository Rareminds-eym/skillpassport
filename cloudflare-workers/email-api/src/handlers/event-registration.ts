...
import {
  ...
  getOTPSubject
} from '../templates/event-registration';
import { sendEmail } from '../services/mailer';
import { sendEmailWithAttachment } from '../services/mailer-with-attachments';
import { successResponse, errorResponse } from '../utils/response';
import { DEFAULT_FROM_EMAIL, DEFAULT_FROM_NAME } from '../config/constants';
import type { Env, EventConfirmationRequest, EventOTPRequest } from '../types';
 ...
 */
export async function handleEventConfirmation(body: EventConfirmationRequest, env: Env): Promise<Response> {
  ...
  try {
    ...
    return errorResponse('Failed to send confirmation emails', (error as Error).message);
  }
}
 ...
 * POST /event-otp
 */
export async function handleEventOTP(body: EventOTPRequest, env: Env): Promise<Response> {
  ...
  try {
    ...
    return errorResponse('Failed to send OTP email', (error as Error).message);
  }
}
