...
import type { Env, CountdownEmailRequest } from '../types';

export async function handleCountdownEmail(body: CountdownEmailRequest, env: Env): Promise<Response> {
  ...
  try {
    ...
    return errorResponse('Failed to send countdown email', (error as Error).message);
  }
}
