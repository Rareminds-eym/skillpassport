...
async function generateAptitudeQuestions(env, streamId, questionsPerCategory = 5, studentId, attemptId) {
  ...
  async function generateBatch(batchNum, questionsPerCat) {
    ...
    const prompt = APTITUDE_PROMPT.replace(/\{\{QUESTIONS_PER_CATEGORY\}\}/g, String(questionsPerCat)).replace(/\{\{CATEGORIES\}\}/g, categoriesText).replace(/\{\{STREAM_NAME\}\}/g, streamContext.name).replace(/\{\{STREAM_CONTEXT\}\}/g, streamContext.context);
    let jsonText;
    if (claudeKey) {
      console.log(`\u{1F511} Batch ${batchNum}: Using Claude API`);
      try {
      ...
      console.log(`\u{1F511} Batch ${batchNum}: Using OpenRouter with retry`);
      jsonText = await callOpenRouterWithRetry(openRouterKey, [
        { role: "system", content: "You are an expert psychometric assessment creator. Generate ONLY valid JSON." },
        { role: "user", content: prompt }
      ]);
  ...
  console.log("\u{1F4E6} Generating batch 1 (25 questions)...");
  ...
  await delay(3e3);
  console.log("\u{1F4E6} Generating batch 2 (25 questions)...");
