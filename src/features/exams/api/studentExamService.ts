...
const logger = getLogger('StudentExamService');
...
export const getStudentExams = async (studentId: string): Promise<StudentExam[]> => {
  try {
    ...
    if (studentError || !student?.school_class_id) {
      logger.info('Student not found or not assigned to a class', { studentId });
      return [];
    }
    ...
    logger.error('Error fetching student exams', error, { studentId });
    throw error;
  }
};
...
export const getGroupedStudentExams = async (studentId: string): Promise<GroupedExam[]> => {
  try {
    ...
    logger.error('Error fetching grouped student exams', error, { studentId });
    throw error;
  }
};
...
export const getStudentResults = async (studentId: string): Promise<StudentResult[]> => {
  try {
    ...
    logger.error('Error fetching student results', error, { studentId });
    throw error;
  }
};
...
export const getStudentResultStats = async (studentId: string): Promise<{
  ...
  try {
    ...
    logger.error('Error calculating result stats', error, { studentId });
    return {
      totalExams: 0,
      passed: 0,
      failed: 0,
