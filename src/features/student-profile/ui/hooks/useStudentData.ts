...
const logger = getLogger('student-data');
...
export const useStudentData = (student: Student | null, isOpen: boolean) => {
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      if (projectsError) {
        logger.info('No projects table found, using student.projects field');
        setProjects([]);
      } else {
  ...
  const fetchCertificates = async () => {
    ...
    try {
      ...
      if (certificatesError) {
        logger.info('No certificates table found, using student.certificates field');
        setCertificates([]);
      } else {
