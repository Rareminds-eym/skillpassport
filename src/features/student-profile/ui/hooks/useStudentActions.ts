...
export const useStudentActions = (student: Student | null) => {
  ...
  const getCurrentSemester = () => {
    ...
    if (checkIsCollegeStudent(student) && student.enrollmentDate) {
  ...
  const getTotalSemesters = () => {
    ...
    if (checkIsSchoolStudent(student)) {
      return 12;
    }
  ...
  const handlePromotion = async () => {
    ...
    try {
      ...
      if (promotionError) {
        logger.error('Promotion error', promotionError);

        // Handle specific error cases
        if (promotionError.code === '23503') { // Foreign key constraint violation
          logger.warn('Skipping promotion record due to foreign key constraint. Proceeding with semester update.');
      ...
      if (studentUpdateError) {
        ...
        logger.warn('Promotion record created but student semester update failed');
      }
