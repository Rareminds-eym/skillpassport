// src/components/educator/LearnerProfileDrawer.tsx
...
const logger = getLogger('LearnerProfileDrawer');
...
const ExportModal = ({ isOpen, onClose, learner }) => {
  ...
  const generatePDF = (learner, settings) => {
    ...
    doc.text(`LEARNER PROFILE - ${learner.name}`, margin, yPos);
    yPos += 10;
    ...
    const basicFields = [
      ...
      { label: 'Registration Number', value: learner.registration_number },
      { label: 'Date of Birth', value: learner.date_of_birth },
      { label: 'Age', value: learner.age },
      { label: 'University', value: learner.university || learner.college },
      { label: 'College/School', value: learner.college_school_name || learner.college },
      { label: 'Department', value: learner.dept || learner.branch_field },
      { label: 'Course', value: learner.course },
      { label: 'District', value: learner.district_name || learner.location },
      { label: 'Year', value: learner.year },
    ];
    ...
    if (isFullProfile) {
      basicFields.push(
        ...
        { label: 'Alternate Number', value: learner.alternate_number }
      );
    }
    ...
    const academicFields = [
      { label: 'Branch/Field', value: learner.branch_field },
      { label: 'Trainer Name', value: learner.trainer_name },
      { label: 'learner ID', value: learner.nm_id },
      { label: 'Academic Score', value: learner.ai_score_overall || learner.academic_score },
      { label: 'CGPA', value: learner.cgpa },
      { label: 'Year of Passing', value: learner.year_of_passing },
      { label: 'Employability Score', value: learner.employability_score },
    ];
    ...
    if (learner.skills && learner.skills.length > 0) {
      ...
      const skillsText = learner.skills.join(', ');
    ...
    if (learner.projects && learner.projects.length > 0) {
      ...
      learner.projects.forEach((project, index) => {
        checkNewPage();
        doc.setFont('helvetica', 'bold');
        doc.text(`${index + 1}. ${project.title || project.name || 'Untitled Project'}`, margin + 5, yPos);
        yPos += lineHeight;
        ...
        if (project.status) {
          checkNewPage();
          doc.text(`Status: ${project.status}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (project.duration) {
          checkNewPage();
          doc.text(`Duration: ${project.duration}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (project.start_date) {
          checkNewPage();
          doc.text(`Start Date: ${new Date(project.start_date).toLocaleDateString()}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (project.end_date) {
          checkNewPage();
          doc.text(`End Date: ${new Date(project.end_date).toLocaleDateString()}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        yPos += lineHeight / 2; // Add space between projects
      });
    ...
    if (learner.certificates && learner.certificates.length > 0) {
      ...
      learner.certificates.forEach((cert, index) => {
        checkNewPage();
        doc.setFont('helvetica', 'bold');
        doc.text(`${index + 1}. ${cert.title || cert.name || 'Untitled Certificate'}`, margin + 5, yPos);
        yPos += lineHeight;

        doc.setFont('helvetica', 'normal');
        if (cert.issuer) {
          checkNewPage();
          doc.text(`Issuer: ${cert.issuer}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (cert.issued_on) {
          checkNewPage();
          doc.text(`Issue Date: ${new Date(cert.issued_on).toLocaleDateString()}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (cert.level) {
          checkNewPage();
          doc.text(`Level: ${cert.level}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (cert.credential_id) {
          checkNewPage();
          doc.text(`Credential ID: ${cert.credential_id}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (cert.description) {
          checkNewPage();
          const descLines = doc.splitTextToSize(cert.description, pageWidth - (margin * 2) - 10);
          descLines.forEach(line => {
            checkNewPage();
            doc.text(line, margin + 10, yPos);
            yPos += lineHeight;
        ...
        if (cert.status) {
          checkNewPage();
          doc.text(`Status: ${cert.status}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        yPos += lineHeight / 2; // Add space between certificates
      });
    ...
    if (learner.assessments && learner.assessments.length > 0) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      doc.text('ASSIGNMENTS & ASSESSMENTS', margin, yPos);
      yPos += lineHeight + 2;
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');

      learner.assessments.forEach((assessment: any, index: number) => {
        const assignment = assessment.assignments;
        if (!assignment) return;

        checkNewPage();
        doc.setFont('helvetica', 'bold');
        doc.text(`${index + 1}. ${assignment.title || 'Untitled Assignment'}`, margin + 5, yPos);
        yPos += lineHeight;

        doc.setFont('helvetica', 'normal');
        if (assignment.course_name) {
          checkNewPage();
          doc.text(`Course: ${assignment.course_name}${assignment.course_code ? ` (${assignment.course_code})` : ''}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (assignment.assignment_type) {
          checkNewPage();
          doc.text(`Type: ${assignment.assignment_type}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (assessment.status) {
          checkNewPage();
          doc.text(`Status: ${assessment.status.replace('-', ' ')}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (assessment.grade_percentage) {
          checkNewPage();
          doc.text(`Grade: ${assessment.grade_percentage}%`, margin + 10, yPos);
          yPos += lineHeight;
        } else if (assessment.grade_received) {
          checkNewPage();
          doc.text(`Points: ${assessment.grade_received}/${assignment.total_points || 100}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (assignment.due_date) {
          checkNewPage();
          doc.text(`Due Date: ${new Date(assignment.due_date).toLocaleDateString()}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (assessment.submission_date) {
          checkNewPage();
          doc.text(`Submitted: ${new Date(assessment.submission_date).toLocaleDateString()}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        if (assessment.instructor_feedback) {
          checkNewPage();
          const feedbackLines = doc.splitTextToSize(`Feedback: ${assessment.instructor_feedback}`, pageWidth - (margin * 2) - 15);
        ...
        if (assessment.is_late) {
          checkNewPage();
          doc.text(`Late Submission${assessment.late_penalty ? ` (Penalty: ${assessment.late_penalty}%)` : ''}`, margin + 10, yPos);
          yPos += lineHeight;
        }

        yPos += lineHeight / 2; // Space between assignments
      });
    ...
    // Verification Badges
    if (learner.badges && learner.badges.length > 0) {
      checkNewPage();
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.text(learner.badges.join(', '), margin + 5, yPos);
      yPos += lineHeight + 3;
    }
  ...
  const generateCSV = (learner, settings) => {
    const isFullProfile = settings.type === 'full_profile';
    ...
    const row = [
      ...
      escapeCSV(learner.registration_number),
      escapeCSV(learner.date_of_birth),
      escapeCSV(learner.age),
      escapeCSV(learner.university || learner.college),
      escapeCSV(learner.college_school_name || learner.college),
      escapeCSV(learner.dept || learner.branch_field),
      escapeCSV(learner.course),
      escapeCSV(learner.branch_field),
      escapeCSV(learner.district_name || learner.location),
      escapeCSV(learner.year),
    ];

    if (isFullProfile) {
      row.push(
        escapeCSV(learner.email),
        escapeCSV(learner.phone),
        escapeCSV(learner.alternate_number),
        escapeCSV(learner.trainer_name),
        escapeCSV(learner.nm_id)
      );
    }

    // Calculate assessments summary
    const assessments = learner.assessments || [];
    const assignmentsCount = assessments.length;
    const completedAssignments = assessments.filter((a: any) => a.status === 'graded').length;
    const gradedAssessments = assessments.filter((a: any) => a.grade_percentage);
    const averageGrade = gradedAssessments.length > 0
      ? (gradedAssessments.reduce((sum: number, a: any) => sum + (a.grade_percentage || 0), 0) / gradedAssessments.length).toFixed(1)
      : '';

    row.push(
      escapeCSV(learner.ai_score_overall || learner.academic_score),
      escapeCSV(learner.cgpa),
      escapeCSV(learner.year_of_passing),
      escapeCSV(learner.employability_score),
      escapeCSV(learner.skills?.join('; ')),
      escapeCSV(assignmentsCount),
      escapeCSV(completedAssignments),
      escapeCSV(averageGrade),
      escapeCSV(learner.badges?.join('; ')),
      escapeCSV(learner.verified ? 'Yes' : 'No')
    );
      ...
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-medium text-gray-900">Export Learner Report</h3>
...
const MessageModal = ({ isOpen, onClose, learner }) => {
  ...
  const handleWhatsApp = () => {
    const phone = learner.phone?.replace(/[^0-9]/g, '') || '';
    const message = encodeURIComponent(`Hi ${learner.name}, I wanted to reach out regarding your academic progress...`);
          ...
          <div className="mt-4 p-3 bg-gray-50 rounded-lg">
            <div className="text-xs text-gray-600 space-y-1">
              <div className="flex items-center">
                ...
                <span>{learner.phone || 'Not available'}</span>
              </div>
              <div className="flex items-center">
...
const AddMentorNoteModal = ({ isOpen, onClose, learner, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // await saveMentorNote(learner.id, note);
...
const VerifyAssignmentModal = ({ isOpen, onClose, learner, onSuccess }) => {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // await verifyAssignments(learner.id, selectedAssignments);
    ...
    <div className="fixed inset-0 z-[60] overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
          ...
          <div className="mb-4">
            <p className="text-sm text-gray-600">Learner: <span className="font-medium">{learner.name}</span></p>
          </div>
...
const LearnerProfileDrawer = ({ learner, isOpen, onClose }) => {
  ...
  useEffect(() => {
    // Prefer the primary learner row id for related tables, then fallback to other identifiers
    const baseLearner: any = learner || {};
    const learnerId = baseLearner.id || baseLearner.learner_id || baseLearner.user_id;
    if (!learnerId) return;
  ...
  const learnerName = (learner as any).name || (learner as any).learner_name || '';
  const learnerId = (learner as any).id || (learner as any).learner_id || '';
  const learnerEmail = (learner as any).email || (learner as any).learner_email || '';

  const qrCodeValue = `${window.location.origin}/learner/profile/${learnerId}`;

  // Parse profile data
  let profileData: any = learner;
  let rawProfile: any = {};

  if (learner.profile && typeof learner.profile === 'string') {
    try {
      rawProfile = JSON.parse(learner.profile);
      profileData = { ...learner, ...rawProfile };
    } catch (e) {
      // Fall back to using the full learner object as profile source
      rawProfile = learner || {};
      profileData = { ...learner };
    }
  } else if (learner.profile && typeof learner.profile === 'object') {
    rawProfile = learner.profile;
    profileData = { ...learner, ...rawProfile };
  } else {
    // No separate profile field; use the learner object itself so overview has fields to show
    rawProfile = learner || {};
    profileData = { ...learner };
  }

  // Prefer live-fetched projects/certificates but fall back to joined data on the learner object
  const projectsData = projects.length > 0 ? projects : ((learner as any).projects || []);
  const certificatesData = certificates.length > 0 ? certificates : ((learner as any).certificates || []);
  ...
  const rawSkills = Array.isArray((learner as any).skills) ? (learner as any).skills : [];
  ...
  const experienceData = Array.isArray((learner as any).experience) ? (learner as any).experience : [];
  const trainingData = Array.isArray((learner as any).trainings)
    ? (learner as any).trainings.map((t: any) => ({
      course: t.title,
      skill: undefined,
  ...
  const excludedFields = new Set([
    '_', 'imported_at', 'contact_number_dial_code', 'id', 'learner_id', 'user_id',
              ...
              <div className="px-6 py-4 bg-primary-50 border-b border-primary-200">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center">
                      <h2 className="text-xl font-semibold text-gray-900">{profileData.name || learner.name}</h2>
                      <div className="ml-3 flex items-center">
                        <StarIcon className="h-5 w-5 text-yellow-400 fill-current" />
                        <span className="text-lg font-bold text-gray-900 ml-1">{learner.ai_score_overall || learner.academic_score || 0}</span>
                        <span className="text-sm text-gray-600 ml-1">Academic Score</span>
                      </div>
                    </div>
                    <div className="mt-2 flex items-center text-sm text-gray-600">
                      <AcademicCapIcon className="h-4 w-4 mr-1" />
                      <span>{profileData.college_school_name || learner.college} • {profileData.branch_field || learner.dept}</span>
                    </div>
                    <div className="mt-1 flex items-center text-sm text-gray-600">
                      <MapPinIcon className="h-4 w-4 mr-1" />
                      <span>{profileData.district_name || learner.location} • Age {profileData.age || 'N/A'}</span>
                    </div>
              ...
              <div className="px-6 py-3 bg-gray-50 border-b border-gray-200">
                <div className="flex items-center space-x-4 text-sm">
                  <div className="flex items-center text-gray-600">
                    ...
                    <span>{profileData.contact_number || learner.phone || 'Not provided'}</span>
                  </div>
                  <div className="flex items-center text-gray-600">
                    <EnvelopeIcon className="h-4 w-4 mr-1" />
                    <span>{profileData.email || learner.email}</span>
                  </div>
                </div>
              </div>
                ...
                {activeTab === 'verification' && (
                  <div className="p-6">
                    ...
                    <div className="mb-6">
                      ...
                      <div className="space-y-3">
                        {learner.badges && learner.badges.length > 0 ? (
                          learner.badges.map((badge: string, index: number) => {
                    ...
                    <div className="space-y-3">
                      {mentorNotes.length > 0 ? (
                        ...
                        <div className="text-center py-12">
                          ...
                          <p className="text-gray-400 text-sm mt-1">Add feedback or observations about this learner</p>
                        </div>
                      )}
                    </div>
                  </div>
