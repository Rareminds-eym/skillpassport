...
const LearnerStatusBadge = ({ status }: { status: string }) => {
    ...
    return (
        <span className={`px-2 py-1 text-xs font-medium rounded-full border ${colors[status] || colors.todo}`}>
...
const GradingModal = ({ isOpen, onClose, assignment, onGradeSubmitted }: GradingModalProps) => {
    ...
    const loadlearners = async () => {
        ...
        try {
            setLoading(true);
            const learnersData = await getAssignmentLearners(assignment.id);
            setlearners(learnersData);
        } catch (error) {
            logger.error('Error loading learners', error as Error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleGradeSubmit = async () => {
        if (!selectedLearner || !gradeData.grade_received || !assignment) {
        ...
        try {
            ...
            await gradeAssignment(selectedLearner.learner_assignment_id, {
                grade_received: gradeValue,
                instructor_feedback: gradeData.instructor_feedback,
                graded_by: educatorId
            });

            setlearners(learners.map(s =>
                s.learner_assignment_id === selectedLearner.learner_assignment_id
                    ? { ...s, status: 'graded', grade_received: gradeValue, grade_percentage: gradePercentage, instructor_feedback: gradeData.instructor_feedback }
                    : s
            ));
    ...
    const filteredlearners = useMemo(() => {
        if (filterStatus === 'all') return learners;
        return learners.filter(s => s.status === filterStatus);
    }, [learners, filterStatus]);
                    ...
                    <div className="w-full md:w-2/5 lg:w-1/3 border-r border-gray-200 flex flex-col min-h-0">
                        <div className="p-3 md:p-4 border-b border-gray-200 bg-gray-50 shrink-0">
                            ...
                            >
                                ...
                                <option value="submitted">Submitted ({learners.filter(s => s.status === 'submitted').length})</option>
                                <option value="graded">Graded ({learners.filter(s => s.status === 'graded').length})</option>
                                <option value="in-progress">In Progress ({learners.filter(s => s.status === 'in-progress').length})</option>
                                <option value="todo">Not Started ({learners.filter(s => s.status === 'todo').length})</option>
                            </select>
                        </div>
                            ...
                            {loading ? (
                                ...
                                <div className="divide-y divide-gray-100">
                                    {filteredlearners.map((learner) => (
                                        <button
                                            key={learner.learner_assignment_id}
                                            onClick={() => {
                                                setSelectedLearner(learner);
                                                setGradeData({
                                                    grade_received: learner.grade_received?.toString() || '',
                                                    instructor_feedback: learner.instructor_feedback || ''
                                                });
                                            }}
                                            className={`w-full text-left p-3 md:p-4 hover:bg-gray-50 transition-colors ${selectedLearner?.learner_assignment_id === learner.learner_assignment_id
                                        ...
                                        >
                                            <div className="flex items-start justify-between gap-2 mb-2">
                                                <div className="flex-1 min-w-0">
                                                    <h4 className="font-medium text-gray-900 text-sm md:text-base truncate">{learner.learner?.name}</h4>
                                                    <p className="text-xs text-gray-500 mt-0.5 truncate">{learner.learner?.email}</p>
                                                </div>
                                                <LearnerStatusBadge status={learner.status} />
                                            </div>

                                            {learner.learner?.university && (
                                                <p className="text-xs text-gray-600 mb-1 truncate">{learner.learner.university}</p>
                                            )}

                                            <div className="flex items-center justify-between gap-2 mt-2">
                                                {learner.status === 'graded' && learner.grade_received !== null && learner.grade_received !== undefined ? (
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-sm font-semibold text-emerald-600">
                                                            {learner.grade_received}/{assignment.totalPoints}
                                                        </span>
                                                        <span className="text-xs text-gray-500">
                                                            ({learner.grade_percentage?.toFixed(0)}%)
                                                        </span>
                                                    </div>
                                                ) : learner.submission_date ? (
                                                    <span className="text-xs text-gray-500 truncate">
                                                        Submitted {new Date(learner.submission_date).toLocaleDateString()}
                                                    </span>
                                                ) : (
                        ...
                        {selectedLearner ? (
                            <>
                                <div className="flex-1 overflow-y-auto p-4 md:p-6 min-h-0">
                                    <div className="mb-6">
                                        ...
                                        <p className="text-sm text-gray-600 truncate">{selectedLearner.learner?.email}</p>
                                        {selectedLearner.learner?.registration_number && (
                                            <p className="text-xs text-gray-500 mt-1">
                                                Reg: {selectedLearner.learner.registration_number}
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                            ...
                                            <div className="space-y-2 text-sm">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <LearnerStatusBadge status={selectedLearner.status} />
                                                </div>
                                                {selectedLearner.submission_date && (
                                                    <div className="flex justify-between items-start gap-2">
                                                        <span className="text-gray-600 shrink-0">Submitted:</span>
                                                        <span className="text-gray-900 text-right">
                                                            {new Date(selectedLearner.submission_date).toLocaleString()}
                                                        </span>
                                                    </div>
                                                )}
                                                {selectedLearner.is_late && (
                                                    <div className="flex justify-between">
                                                        <span className="text-gray-600">Late Submission:</span>
                                                        <span className="text-red-600 font-medium">Yes</span>
                                                    </div>
                                                )}
                                                {selectedLearner.submission_content && (
                                                ...
                                                {selectedLearner.submission_files && selectedLearner.submission_files.length > 0 && (
                                                    <div className="mt-3 pt-3 border-t border-gray-200">
                                                        <span className="text-gray-600 block mb-2">Submitted Files:</span>
                                                        <div className="space-y-2">
                                                            {selectedLearner.submission_files.map((file, index) => (
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => setGradeData({ ...gradeData, grade_received: e.target.value })}
                                                min="0"
                                            ...
                                            {gradeData.grade_received && (
                                                <div className="flex items-center justify-between mt-2">
                                                    <p className="text-sm text-gray-600">
                                                        Percentage: {((parseFloat(gradeData.grade_received) / assignment.totalPoints) * 100).toFixed(1)}%
                                                    </p>
                                        ...
                                        <div>
                                            ...
                                            <textarea
                                                ...
                                                onChange={(e) => setGradeData({ ...gradeData, instructor_feedback: e.target.value })}
                                                rows={6}
                                        ...
                                        {/* Previous Feedback */}
                                        {selectedLearner.status === 'graded' && selectedLearner.instructor_feedback && (
                                            <div className="bg-emerald-50 rounded-lg p-4 border border-emerald-200">
                                                <div className="flex items-start gap-2">
                                                    <ChatBubbleBottomCenterTextIcon className="h-5 w-5 text-emerald-600 mt-0.5 shrink-0" />
                                                    <div className="flex-1 min-w-0">
                                                        <h4 className="text-sm font-semibold text-emerald-900 mb-1">Previous Feedback</h4>
                                                        <p className="text-sm text-emerald-800">{selectedLearner.instructor_feedback}</p>
                                                        {selectedLearner.graded_date && (
                                                            <p className="text-xs text-emerald-600 mt-2">
                                                                Graded on {new Date(selectedLearner.graded_date).toLocaleString()}
                                                            </p>
                                                        )}
                                                    </div>
                                                </div>
                                ...
                                <div className="border-t border-gray-200 p-4 md:p-6 bg-gray-50 shrink-0">
                                    <div className="flex gap-3">
                                        ...
                                        >
                                            {submitting ? 'Submitting...' : selectedLearner.status === 'graded' ? 'Update Grade' : 'Submit Grade'}
                                        </button>
                                    </div>
                                </div>
                            </>
                            ...
                            <div className="flex-1 flex items-center justify-center text-gray-400">
                                <div className="text-center p-4">
                                    ...
                                    <p className="text-sm">Select a learner to grade their submission</p>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                {!showMobileGrading ? (
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden min-h-0">
                        <div className="p-3 border-b border-gray-200 bg-gray-50 shrink-0">
                            <select
                                ...
                                className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                            >
                                <option value="all">All ({learners.length})</option>
                                <option value="submitted">Submitted ({learners.filter(s => s.status === 'submitted').length})</option>
                                <option value="graded">Graded ({learners.filter(s => s.status === 'graded').length})</option>
                                <option value="in-progress">In Progress ({learners.filter(s => s.status === 'in-progress').length})</option>
                                <option value="todo">Not Started ({learners.filter(s => s.status === 'todo').length})</option>
                                ...
                                <div className="divide-y divide-gray-100">
                                    {filteredlearners.map((learner) => (
                                        ...
                                        >
                                            <div className="flex items-start justify-between gap-2 mb-2">
                                                <div className="flex-1 min-w-0">
                                                    <h4 className="font-medium text-gray-900 text-sm truncate">{learner.learner?.name}</h4>
                                                    <p className="text-xs text-gray-500 mt-0.5 truncate">{learner.learner?.email}</p>
                                                </div>
                                                <LearnerStatusBadge status={learner.status} />
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between gap-2 mt-2">
                                                {learner.status === 'graded' && learner.grade_received !== null && learner.grade_received !== undefined ? (
                                                    ...
                                                    <span className="text-xs text-gray-500">
                                                        {new Date(learner.submission_date).toLocaleDateString()}
                                                    </span>
                                                ) : (
                                                    <span className="text-xs text-gray-400">Not submitted</span>
                                                )}
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden min-h-0">
                        <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200 bg-gray-50 shrink-0">
                            ...
                            <h3 className="text-sm font-semibold text-gray-900 truncate">Grade Learner</h3>
                        </div>
                            ...
                            <>
                                            ...
                                            {gradeData.grade_received && (
                                                <div className="flex items-center justify-between mt-2">
                                                    <p className="text-xs text-gray-600">
                                                        {((parseFloat(gradeData.grade_received) / assignment.totalPoints) * 100).toFixed(1)}%
                                                    </p>
                                        ...
                                        {selectedLearner.status === 'graded' && selectedLearner.instructor_feedback && (
                                            <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-200">
                                                <div className="flex items-start gap-2">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <h4 className="text-xs font-semibold text-emerald-900 mb-1">Previous Feedback</h4>
                                                        <p className="text-xs text-emerald-800">{selectedLearner.instructor_feedback}</p>
                                                        {selectedLearner.graded_date && (
                                                            <p className="text-xs text-emerald-600 mt-1.5">
                                                                {new Date(selectedLearner.graded_date).toLocaleDateString()}
                                                            </p>
                                                        )}
                                                    </div>
                                                </div>
                                ...
                                <div className="border-t border-gray-200 p-4 bg-gray-50 shrink-0">
                                    <div className="flex gap-2">
                                        ...
                                        >
                                            {submitting ? 'Saving...' : selectedLearner.status === 'graded' ? 'Update' : 'Submit'}
                                        </button>
                                    </div>
                                </div>
                            </>
