...
export const useSubscriptionStore = create<SubscriptionState>()(
  immer((set, get) => ({
    ...
    fetchSubscription: async (userId: string) => {
      ...
      console.log('📦 [SubStore] fetchSubscription called. userId=', userId, 'state:', {
      ...
      if (
        ...
        console.log('📦 [SubStore] Data is fresh, skipping fetch');
        if (state.isLoading) {
      ...
      if (state._fetchSubPromise && state._currentUserId === userId) {
        console.log('📦 [SubStore] Fetch already in progress, returning existing promise');
      ...
      const fetchPromise = (async () => {
        ...
        console.log('📦 [SubStore] Starting fetch. isRefetch=', isRefetch);
        set((s) => {
        ...
        try {
          console.log('📦 [SubStore] Calling getActiveSubscription()...');
          const result = await getActiveSubscription();
          console.log('📦 [SubStore] getActiveSubscription returned:', {
          ...
          if (get()._currentUserId !== userId) {
            console.log('📦 [SubStore] User changed during fetch, discarding');
            return;
          }
          ...
          if (result.success && result.data) {
            ...
            console.log('📦 [SubStore] Subscription found. accessState:', accessState);
            ...
            // No subscription
            console.log('📦 [SubStore] No subscription found. Setting hasAccess=false, isLoading=false');
            set((s) => {
              s.subscription = null;
              s.hasAccess = false;
          ...
          console.error('📦 [SubStore] fetchSubscription CAUGHT error:', err);
          ...
          console.log('📦 [SubStore] fetchSubscription finally. Store state:', {
