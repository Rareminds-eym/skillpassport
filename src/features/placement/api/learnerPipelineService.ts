...
export class LearnerPipelineService {
  ...
  static async getlearnerPipelineStatus(learnerId, learnerEmail = null) {
    try {
      ...
      if (error) {
        logger.error('Error fetching pipeline status', error, { learnerId, learnerEmail });
        throw error;
      }
  ...
  static async getlearnerPipelineActivities(learnerId) {
    try {
      // Direct query using learner_id (leverages idx_pipeline_activities_learner_id index)
      ...
      logger.info('getlearnerPipelineActivities completed', { 
  ...
  static async getlearnerApplicationsWithPipeline(learnerId, learnerEmail = null) {
    try {
      logger.info('Starting getlearnerApplicationsWithPipeline', { learnerId });
      const startTime = performance.now();
      ...
      const endTime = performance.now();
      logger.info('getlearnerApplicationsWithPipeline completed', { 
        duration: `${(endTime - startTime).toFixed(2)}ms`,
