...
interface LearnerMessagingExampleProps {
  learnerId: string;
  recruiterId?: string;
...
/**
 * Complete example of learner messaging with:
 * - Zustand state management
 * - React Query for non-blocking data fetching
 * - Supabase realtime for instant updates
 * - Hot-toast notifications with smooth animations
 */
export const LearnerMessagingExample: React.FC<LearnerMessagingExampleProps> = ({
  learnerId,
  recruiterId = 'rec_001',
  applicationId
}) => {
  ...
  // Use the new consolidated hook - gets messages, conversations, and unread count all in one
  const {
    messages,
    conversations,
    unreadCount,
  ...
  } = useLearnerMessages(learnerId, {
    conversationId: selectedConversationId,
    enableRealtime: true,
  });

  // Setup message notifications (exclude current conversation)
              ...
              <div className="space-y-2">
                {conversations.map((conv) => (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-lg transition-colors ${selectedConversationId === conv.id
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      ...
                      {conv.learner_unread_count > 0 && (
                        <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">
                          {conv.learner_unread_count}
                        </span>
                      )}
                    </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {loadingMessages ? (
                  ...
                  messages.map((message) => {
                    const isOwnMessage = message.sender_id === learnerId;
                    return (
                      <div
                        key={message.id}
                      ...
                      >
                        <div
                          className={`max-w-md px-4 py-3 rounded-2xl ${isOwnMessage
...
export default LearnerMessagingExample;
