...
const MessageModal = ({ learner, isOpen, onClose, onSend, isLoading }) => {
  ...
  // Predefined subjects for admin-learner conversations
  const subjects = [
    'General Inquiry',
    'Academic Support',
    'Attendance Issue',
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <img
              src={`https://ui-avatars.com/api/?name=${encodeURIComponent(learner.name || learner.email)}&background=3B82F6&color=fff`}
              alt={learner.name}
...
const NewLearnerConversationModal = ({ isOpen, onClose, schoolId, onConversationCreated }) => {
  ...
  // Fetch learners from the school when modal opens or search query changes
  useEffect(() => {
    if (isOpen && schoolId) {
      fetchlearners();
    }
  ...
  const filteredlearners = learners.filter(learner => {
    ...
    return (learner.name && learner.name.toLowerCase().includes(query)) ||
           (learner.email && learner.email.toLowerCase().includes(query)) ||
           (learner.contact_number && learner.contact_number.toLowerCase().includes(query)) ||
           (learner.university && learner.university.toLowerCase().includes(query)) ||
           (learner.branch_field && learner.branch_field.toLowerCase().includes(query));
  });
              ...
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-sm">
                    {searchQuery ? `No learners found for "${searchQuery}"` : 'No learners found'}
                  </p>
                  <p className="text-gray-400 text-xs mt-2">
                    {searchQuery ? 'Try a different search term' : 'Start typing to search for learners'}
                  </p>
                </div>
              ) : (
...
export default NewLearnerConversationModal;
