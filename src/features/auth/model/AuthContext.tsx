...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const restoreUserFromStorage = useCallback((sessionUser: User) => {
  ...
  const checkSessionValidity = useCallback(async (): Promise<Session | null> => {
    try {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        if (event === 'SIGNED_IN' && session?.user) {
          ...
          if (typeof window !== 'undefined' && (window as any).facultyOnboardingInProgress) {
