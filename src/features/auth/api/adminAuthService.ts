 ...
 */
const checkSchoolAdmin = async (user: any, correlationId: string) => {
  try {
 ...
 */
const checkCollegeAdmin = async (user: any, correlationId: string) => {
  try {
    const { data: org, error } = await withTimeout(
      supabase
        .from('organizations')
 ...
 */
const checkUniversityAdmin = async (user: any, correlationId: string) => {
  try {
    const { data: org, error } = await withTimeout(
      supabase
        .from('organizations')
