...
const logger = getLogger('delete-learner');
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || 'Failed to delete learner');
      }
    } catch (err) {
      logger.error('Error deleting learner', err instanceof Error ? err : new Error(String(err)));
        ...
        <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
            ...
            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
              ...
              <div className="mt-2">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  This action will soft delete the learner from the system. The record will be marked as deleted
          ...
          <div className="mt-4 p-4 bg-gray-50 border border-gray-200 rounded-md">
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-gray-900">{learner?.email || 'N/A'}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Department:</span>
                <span className="font-medium text-gray-900">{learner?.dept || 'N/A'}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">College:</span>
                <span className="font-medium text-gray-900">{learner?.college || 'N/A'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
            <p className="text-xs text-yellow-800">
              <strong>Note:</strong> This is a soft delete. The learner data will not be permanently removed
