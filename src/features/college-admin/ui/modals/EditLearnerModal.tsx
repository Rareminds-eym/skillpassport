...
type PincodeModule = typeof import('indian-pincodes');
type PincodeApi = PincodeModule & { default?: PincodeModule };

let pincodeModulePromise: Promise<PincodeModule> | null = null;

const loadPincodeModule = () => {
  pincodeModulePromise ??= import('indian-pincodes');
  return pincodeModulePromise;
};

const getPincodeApi = async () => {
  const pincodeModule = await loadPincodeModule() as PincodeApi;
  return pincodeModule.default ?? pincodeModule;
};
...
const EditLearnerModal: React.FC<EditlearnerModalProps> = ({
  isOpen,
  onClose,
  learner,
  onSuccess
}) => {
  const [formData, setFormData] = useState<UpdatelearnerData>({});
  ...
  useEffect(() => {
    if (isOpen && learner) {
      const fetchAcademicInfo = async () => {
        let universityName = learner.university || '';
        let collegeName = learner.college_school_name || learner.college || '';
        let branchName = learner.branch_field || learner.dept || '';
        let courseName = learner.course_name || '';
        ...
        // Get date of birth from learner data
        const dob = learner.date_of_birth || '';
        
        // Calculate age from date of birth if available
        const calculatedAge = dob ? calculateAge(dob) : (learner.age || undefined);
        
        // Initialize form with learner data
        setFormData({
          name: learner.name || '',
          email: learner.email || '',
          contact_number: learner.contact_number || learner.phone || '',
          alternate_number: learner.alternate_number || '',
          date_of_birth: dob,
          age: calculatedAge,
          gender: learner.gender || '',
          district_name: learner.district_name || learner.location || '',
          university: universityName,
          university_main: learner.university_main || '',
      ...
      if (learner.country) {
        const selectedCountry = countries.find(c => c.name === learner.country);
        if (selectedCountry) {
          getStatesOfCountry(selectedCountry.isoCode).then((countryStates) => {
            ...
            if (learner.state) {
              ...
              if (selectedState) {
                getCitiesOfState(selectedCountry.isoCode, selectedState.isoCode).then((stateCities) => {
      ...
      setCountrySearch(learner.country || '');
      setStateSearch(learner.state || '');
      setCitySearch(learner.city || '');
      setPincodeSearch(learner.pincode || '');
  ...
  const handlePincodeSearch = async (searchValue: string) => {
    ...
    try {
      const pincodeModule = await getPincodeApi();
      const trimmedSearch = searchValue.trim();
      const results = /^\d+$/.test(trimmedSearch)
        ? pincodeModule
            .getAllPincodes()
            .filter((item) => item.pincode.toString().includes(trimmedSearch))
        : pincodeModule.searchPincodes(trimmedSearch);

      setFilteredPincodes(results.slice(0, 100));
    } catch (error) {
      logger.error('Error searching pincodes', error instanceof Error ? error : new Error(String(error)));
      setFilteredPincodes([]);
    } finally {
  ...
  const handlePincodeSelect = async (pincodeData: any) => {
    ...
    if (selectedCountry) {
      try {
        const countryStates = await getStatesOfCountry(selectedCountry.isoCode);
        setStates(countryStates);
        setFilteredStates(countryStates);

        const selectedState = countryStates.find(s => s.name === newState);
        if (selectedState) {
          const stateCities = await getCitiesOfState(selectedCountry.isoCode, selectedState.isoCode);
          setCities(stateCities);
          setFilteredCities(stateCities);
        }
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateLearner(learner.id, formData);
      ...
      if (result.success) {
        ...
        logger.error('Failed to update learner', new Error(result.error));
        setError(result.error || 'Failed to update learner');
      }
    } catch (err) {
      logger.error('Error updating learner', err instanceof Error ? err : new Error(String(err)));
      ...
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full sm:p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-xl font-semibold text-gray-900">Edit Learner Profile</h3>
              <p className="text-sm text-gray-500 mt-1">
                Update information for <span className="font-medium">{learner?.name}</span>
              </p>
            </div>
            <button
              onClick={onClose}
                ...
                <div>
                  ...
                  <div className="relative" ref={pincodeDropdownRef}>
                    <input
                      ...
                      value={pincodeSearch}
                      onChange={(e) => {
                        void handlePincodeSearch(e.target.value);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    rows={3}
                    maxLength={500}
                    placeholder="Brief description about the learner..."
                    className={`w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 ${
