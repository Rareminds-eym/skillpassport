import React, { ChangeEvent, useEffect, useMemo, useState } from "react"
import { XMarkIcon, UserPlusIcon, UserMinusIcon } from "@heroicons/react/24/outline"
import Papa from "papaparse"
import toast from "react-hot-toast"
import {
  addlearnerToClass,
  EducatorClass,
  fetchlearnerDirectory,
  removelearnerFromClass,
  LearnerDirectoryEntry
} from "@/features/college-admin"

interface Props {
  isOpen: boolean
  onClose: () => void
  classItem: EducatorClass | null
  onlearnersUpdated: (updatedClass: EducatorClass) => void
  schoolId?: string | null
  collegeId?: string | null
  educatorType?: 'school' | 'college'
}

type AddMode = "select" | "csv"
type CSVLearner = { name: string; email: string; progress?: number }

const ManageLearnersModal: React.FC<Props> = ({ isOpen, onClose, classItem, onlearnersUpdated, schoolId, collegeId, educatorType = 'school' }) => {
  const [loading, setLoading] = useState(false)
  const [loadingDirectory, setLoadingDirectory] = useState(false)
  const [directory, setDirectory] = useState<LearnerDirectoryEntry[]>([])
  const [addMode, setAddMode] = useState<AddMode>("select")
  const [selectedLearnerIds, setSelectedLearnerIds] = useState<string[]>([])
  const [csvlearners, setCsvlearners] = useState<CSVLearner[]>([])
  const [csvError, setCsvError] = useState("")
  const [directorySearch, setDirectorySearch] = useState("")

  useEffect(() => {
    if (!isOpen) return
    setSelectedLearnerIds([])
    setAddMode("select")
    setCsvlearners([])
    setCsvError("")
    setDirectorySearch("")
    if (!directory.length && (schoolId || collegeId)) {
      loadDirectory()
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, schoolId, collegeId])

  const loadDirectory = async () => {
    if (!schoolId && !collegeId) return
    
    setLoadingDirectory(true)
    const { data, error } = await fetchlearnerDirectory(schoolId || undefined, collegeId || undefined)
    if (error || !data) {
      toast.error(error || "Unable to load learners")
    } else {
      setDirectory(data)
    }
    setLoadingDirectory(false)
  }

  const availablelearners = useMemo(() => {
    if (!classItem) return []
    const existing = new Set(classItem.learners.map((learner) => learner.id))
    return directory.filter((entry) => !existing.has(entry.id))
  }, [directory, classItem])

  const filteredlearners = useMemo(() => {
    if (!directorySearch.trim()) return availablelearners
    const query = directorySearch.toLowerCase()
    return availablelearners.filter(
      (learner) =>
        learner.name.toLowerCase().includes(query) || learner.email.toLowerCase().includes(query)
    )
  }, [availablelearners, directorySearch])

  useEffect(() => {
    setSelectedLearnerIds((prev) => prev.filter((id) => availablelearners.some((learner) => learner.id === id)))
  }, [availablelearners])

  if (!isOpen || !classItem) return null

  const togglelearnerSelection = (learnerId: string) => {
    setSelectedLearnerIds((prev) =>
      prev.includes(learnerId) ? prev.filter((id) => id !== learnerId) : [...prev, learnerId]
    )
  }

  const handleCSVChange = (event: ChangeEvent<HTMLInputElement>) => {
    const input = event.target
    const file = input.files?.[0]
    if (!file) {
      input.value = ""
      return
    }
    setCsvError("")
    setCsvlearners([])
    Papa.parse<Record<string, string>>(file, {
      header: true,
      skipEmptyLines: true,
      complete: (results) => {
        const learners = results.data
          .map((row) => {
            if (!row) return null
            const normalized = Object.entries(row).reduce<Record<string, string>>((acc, [key, value]) => {
              acc[key.trim().toLowerCase()] = typeof value === "string" ? value.trim() : value ? String(value).trim() : ""
              return acc
            }, {})
            const name = normalized.name || ""
            const email = normalized.email || ""
            const progressText = normalized.progress || ""
            if (!name || !email) return null
            const progressValue = progressText ? Number(progressText) : undefined
            if (progressValue !== undefined && (Number.isNaN(progressValue) || progressValue < 0 || progressValue > 100)) {
              return null
            }
            return {
              name,
              email,
              ...(progressValue !== undefined ? { progress: progressValue } : {})
            }
          })
          .filter((learner): learner is CSVLearner => Boolean(learner))
        if (!learners.length) {
          setCsvError("No valid rows found")
          return
        }
        setCsvlearners(learners)
        setCsvError(results.errors && results.errors.length ? "Some rows could not be parsed" : "")
      },
      error: () => {
        setCsvlearners([])
        setCsvError("Unable to parse CSV")
      }
    })
    input.value = ""
  }

  const handleAddLearner = async () => {
    if (loading) return
    if (addMode === "select") {
      if (!selectedLearnerIds.length) {
        toast.error("Choose at least one learner to add")
        return
      }
      const entries = selectedLearnerIds
        .map((id) => directory.find((entry) => entry.id === id))
        .filter((entry): entry is LearnerDirectoryEntry => Boolean(entry))
      if (!entries.length) {
        toast.error("Selected learners not found")
        return
      }
      setLoading(true)
      let latestClass: EducatorClass | null = null
      for (const entry of entries) {
        const { data, error } = await addlearnerToClass({
          classId: classItem.id,
          learner: {
            id: entry.id,
            name: entry.name,
            email: entry.email,
            progress: entry.defaultProgress
          },
          educatorType
        })
        if (error || !data) {
          toast.error(error || "Unable to add learners")
          setLoading(false)
          return
        }
        latestClass = data
      }
      setLoading(false)
      if (latestClass) {
        toast.success(
          entries.length === 1 ? `${entries[0].name} added` : `${entries.length} learners added`
        )
        onlearnersUpdated(latestClass)
        setSelectedLearnerIds([])
      }
    } else if (addMode === "csv") {
      if (!csvlearners.length) {
        toast.error("Upload a CSV with learners")
        return
      }
      const learnersToAdd = csvlearners
      setLoading(true)
      let latestClass: EducatorClass | null = null
      for (const learner of learnersToAdd) {
        const { data, error } = await addlearnerToClass({
          classId: classItem.id,
          learner: {
            name: learner.name,
            email: learner.email,
            progress: learner.progress
          },
          educatorType
        })
        if (error || !data) {
          toast.error(error || "Unable to add learners")
          setLoading(false)
          return
        }
        latestClass = data
      }
      setLoading(false)
      if (latestClass) {
        toast.success(
          learnersToAdd.length === 1
            ? `${learnersToAdd[0].name} added`
            : `${learnersToAdd.length} learners added`
        )
        onlearnersUpdated(latestClass)
        setCsvlearners([])
        setCsvError("")
      }
    }
  }

  const handleRemoveLearner = async (learnerId: string, name: string) => {
    if (loading) return
    setLoading(true)
    const { data, error } = await removelearnerFromClass(classItem.id, learnerId, educatorType)
    setLoading(false)
    if (error || !data) {
      toast.error(error || "Unable to remove learner")
      return
    }
    toast.success(`${name} removed`)
    onlearnersUpdated(data)
  }

  const learnerCount = classItem.learners.length

  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 py-8 text-center sm:block sm:p-0">
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 transition-opacity" aria-hidden="true" onClick={onClose} />
        <div className="inline-block w-full max-w-5xl align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:p-6">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center space-x-3">
                <h2 className="text-lg font-semibold text-gray-900">Manage Learners</h2>
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-indigo-100 text-indigo-700">{learnerCount}</span>
              </div>
              <p className="mt-1 text-sm text-gray-500">{classItem.name}</p>
            </div>
            <button onClick={onClose} className="text-gray-400 hover:text-gray-600" type="button">
              <XMarkIcon className="h-6 w-6" />
              <span className="sr-only">Close</span>
            </button>
          </div>

          <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              <div className="border border-gray-200 rounded-lg divide-y divide-gray-200">
                {learnerCount === 0 && (
                  <div className="py-10 px-6 text-center text-sm text-gray-500">
                    No learners yet. Add learners to start tracking progress.
                  </div>
                )}
                {classItem.learners.map((learner) => (
                  <div key={learner.id} className="px-6 py-4 flex items-center justify-between hover:bg-gray-50">
                    <div>
                      <p className="text-sm font-medium text-gray-900">{learner.name}</p>
                      <p className="text-xs text-gray-500">{learner.email}</p>
                    </div>
                    <div className="flex items-center space-x-4">
                      <div className="text-right">
                        <p className="text-xs text-gray-500">Progress</p>
                        <p className="text-sm font-semibold text-gray-900">{learner.progress}%</p>
                      </div>
                      <button
                        onClick={() => handleRemoveLearner(learner.id, learner.name)}
                        className="inline-flex items-center px-3 py-1.5 border border-gray-300 rounded-md text-xs font-medium text-gray-700 hover:bg-gray-100"
                        type="button"
                      >
                        <UserMinusIcon className="h-4 w-4 mr-1" />
                        Remove
                      </button>
                    </div>
                  </div>
                ))}
              </div>
            </div>

            <div className="lg:col-span-1">
              <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                <h3 className="text-sm font-medium text-gray-900">Add Learner</h3>
                <div className="mt-4">
                  <div className="flex rounded-md shadow-sm">
                    <button
                      type="button"
                      onClick={() => setAddMode("select")}
                      className={`flex-1 px-3 py-2 text-xs font-medium border ${
                        addMode === "select" ? "border-indigo-600 text-indigo-700 bg-white" : "border-gray-200 text-gray-600 bg-gray-100"
                      } rounded-l-md`}
                    >
                      From Directory
                    </button>
                    <button
                      type="button"
                      onClick={() => setAddMode("csv")}
                      className={`flex-1 px-3 py-2 text-xs font-medium border -ml-px ${
                        addMode === "csv" ? "border-indigo-600 text-indigo-700 bg-white" : "border-gray-200 text-gray-600 bg-gray-100"
                      } rounded-r-md`}
                    >
                      Upload CSV
                    </button>
                  </div>
                </div>

                <div className="mt-4 space-y-4">
                  {addMode === "select" ? (
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Choose Learners</label>
                      <input
                        value={directorySearch || ""}
                        onChange={(e) => setDirectorySearch(e.target.value)}
                        type="text"
                        className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 mb-2"
                        placeholder="Search by name or email"
                      />
                      <div className="max-h-64 overflow-y-auto rounded-md border border-gray-200 bg-white">
                        {loadingDirectory ? (
                          <div className="px-3 py-2 text-sm text-gray-500">Loading learners...</div>
                        ) : availablelearners.length === 0 ? (
                          <div className="px-3 py-2 text-sm text-gray-500">No learners available</div>
                        ) : filteredlearners.length === 0 ? (
                          <div className="px-3 py-2 text-sm text-gray-500">No learners match your search</div>
                        ) : (
                          filteredlearners.map((learner) => (
                            <label
                              key={learner.id}
                              className="flex items-start gap-3 border-b border-gray-100 px-3 py-2 last:border-none hover:bg-gray-50"
                            >
                              <input
                                type="checkbox"
                                checked={selectedLearnerIds.includes(learner.id)}
                                onChange={() => togglelearnerSelection(learner.id)}
                                className="mt-0.5 h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                              />
                              <div>
                                <p className="text-sm font-medium text-gray-900">{learner.name}</p>
                                <p className="text-xs text-gray-500">{learner.email}</p>
                              </div>
                            </label>
                          ))
                        )}
                      </div>
                      {selectedLearnerIds.length > 0 && (
                        <p className="mt-2 text-xs text-gray-500">{selectedLearnerIds.length} learner{selectedLearnerIds.length === 1 ? "" : "s"} selected</p>
                      )}
                    </div>
                  ) : (
                    <>
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">Upload CSV</label>
                        <input
                          type="file"
                          accept=".csv,text/csv"
                          onChange={handleCSVChange}
                          className="w-full text-xs text-gray-600"
                        />
                        <p className="mt-1 text-xs text-gray-500">CSV should have columns: name, email, progress (optional)</p>
                      </div>
                      {csvError && <p className="text-xs text-red-600">{csvError}</p>}
                      {csvlearners.length > 0 && (
                        <div className="mt-3 max-h-64 overflow-y-auto rounded-md border border-gray-200 bg-white">
                          {csvlearners.map((learner, index) => (
                            <div
                              key={`${learner.email}-${index}`}
                              className="flex items-center justify-between px-3 py-2 border-b border-gray-100 last:border-none"
                            >
                              <div>
                                <p className="text-sm font-medium text-gray-900">{learner.name}</p>
                                <p className="text-xs text-gray-500">{learner.email}</p>
                              </div>
                              <div className="text-right">
                                <p className="text-xs text-gray-500">Progress</p>
                                <p className="text-sm font-semibold text-gray-900">
                                  {learner.progress !== undefined ? `${learner.progress}%` : "N/A"}
                                </p>
                              </div>
                            </div>
                          ))}
                        </div>
                      )}
                      {csvlearners.length > 0 && (
                        <p className="text-xs text-gray-500">{csvlearners.length} learner{csvlearners.length === 1 ? "" : "s"} ready to add</p>
                      )}
                    </>
                  )}
                </div>

                <button
                  onClick={handleAddLearner}
                  disabled={
                    loading ||
                    (addMode === "select"
                      ? selectedLearnerIds.length === 0
                      : csvlearners.length === 0)
                  }
                  className="mt-6 w-full inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
                  type="button"
                >
                  <UserPlusIcon className="h-4 w-4 mr-2" />
                  {loading ? "Saving..." : "Add Learner"}
                </button>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

export default ManageLearnersModal