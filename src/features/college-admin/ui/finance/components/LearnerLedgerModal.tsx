...
export const LearnerLedgerModal: React.FC<Props> = ({ isOpen, onClose, learner }) => {
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-3xl bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
          <div className="sticky top-0 bg-white flex items-center justify-between border-b border-gray-200 px-6 py-4 z-10">
            <h3 className="text-lg font-semibold text-gray-900">Learner Fee Ledger</h3>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-start justify-between p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
              <div>
                <h4 className="text-xl font-bold text-gray-900">{learner.learner_name}</h4>
                <p className="text-gray-600">Roll No: {learner.roll_number}</p>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="p-4 bg-gray-50 rounded-xl text-center">
                ...
                <p className="text-xl font-bold text-gray-900">₹{learner.total_due.toLocaleString()}</p>
              </div>
              <div className="p-4 bg-green-50 rounded-xl text-center">
                <p className="text-sm text-green-600 mb-1">Total Paid</p>
                <p className="text-xl font-bold text-green-700">₹{learner.total_paid.toLocaleString()}</p>
              </div>
              <div className="p-4 bg-red-50 rounded-xl text-center">
                <p className="text-sm text-red-600 mb-1">Balance</p>
                <p className="text-xl font-bold text-red-700">₹{learner.balance.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="border rounded-lg overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody>
                    {learner.ledger_entries.map((entry) => {
