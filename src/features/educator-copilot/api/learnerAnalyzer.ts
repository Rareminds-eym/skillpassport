import { LearnerProfile, Opportunity } from './educatorDataService';

/**
 * Learner Analyzer Service
 * Provides deep analysis of individual learner profiles
 */
...
class LearnerAnalyzerService {
  /**
   * Analyze a single learner comprehensively
   */
  analyzeLearner(learner: LearnerProfile): LearnerAnalysis {
  ...
  /**
   * Match a learner to opportunities
   */
  matchlearnerToOpportunities(
    learner: LearnerProfile,
    opportunities: Opportunity[]
  ): OpportunityMatch[] {
    const profile = learner.profile;
    const learnerSkills = (profile.technicalSkills || [])
      .filter(s => s.enabled !== false)
      .map(s => s.name.toLowerCase());
    
    const learnerSkillLevels = new Map(
    ...
    return opportunities.map(opp => {
      ...
      const matchingSkills: SkillMatch[] = requiredSkills.map(reqSkill => {
        const hasSkill = learnerSkills.includes(reqSkill);
        const level = learnerSkillLevels.get(reqSkill) || 0;
        return {
          skill: reqSkill,
          learnerLevel: level,
          hasSkill,
  ...
  private identifySkillGaps(skills: any[], training: any[]): string[] {
    ...
    const learnerSkills = skills.map(s => s.name.toLowerCase());
    
    const missing = commonSkills.filter(cs => !learnerSkills.some(ss => ss.includes(cs)));
...
export const learnerAnalyzer = new LearnerAnalyzerService();
