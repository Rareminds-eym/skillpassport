...
} from '../../../../services/geminiAssessmentService';
...
export const useAssessmentResults = () => {
  ...
  const fetchStudentInfo = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        if (studentData) {
          const rawName =
            studentData.name || user.user_metadata?.full_name || user.email?.split('@')[0] || '—';
          ...
          console.log(
            ...
            studentData.grade,
            'from school_classes:',
            studentData.school_classes?.grade,
            'year:',
            studentData.year,
            'semester:',
          ...
          console.log(
            'Institution detection - hasSchoolId:',
            hasSchoolId,
            'hasCollegeId:',
            hasCollegeId,
            'isSchoolGrade:',
          ...
          console.log(
            'Derived gradeLevel from student data:',
            derivedGradeLevel,
            'grade:',
            studentGrade,
            'school_id:',
          ...
          // If we have branch_field or course_name, derive the stream
            ...
            if (
              fieldText.includes('science') ||
              fieldText.includes('engineering') ||
              fieldText.includes('tech') ||
              fieldText.includes('bca') ||
              fieldText.includes('computer') ||
              fieldText.includes('physics') ||
              fieldText.includes('chemistry') ||
              fieldText.includes('biology') ||
              fieldText.includes('mathematics') ||
              fieldText.includes('medical') ||
              fieldText.includes('mbbs') ||
              fieldText.includes('bsc') ||
              fieldText.includes('b.sc') ||
              fieldText.includes('b.tech') ||
              fieldText.includes('m.tech') ||
              fieldText.includes('mtech')
            ) {
              derivedStream = 'SCIENCE';
            }
            // Commerce stream indicators
            else if (
              fieldText.includes('commerce') ||
              fieldText.includes('business') ||
              fieldText.includes('bba') ||
              fieldText.includes('bcom') ||
              fieldText.includes('b.com') ||
              fieldText.includes('finance') ||
              fieldText.includes('accounting') ||
              fieldText.includes('economics') ||
              fieldText.includes('management') ||
              fieldText.includes('marketing')
            ) {
              derivedStream = 'COMMERCE';
            }
            // Arts stream indicators
            else if (
              fieldText.includes('arts') ||
              fieldText.includes('humanities') ||
              fieldText.includes('ba ') ||
              fieldText.includes('b.a') ||
              fieldText.includes('law') ||
              fieldText.includes('llb') ||
              fieldText.includes('english') ||
              fieldText.includes('history') ||
              fieldText.includes('political') ||
              fieldText.includes('sociology') ||
              fieldText.includes('psychology') ||
              fieldText.includes('literature')
            ) {
              derivedStream = 'ARTS';
            }

            console.log(
              '📚 Derived stream from database:',
              derivedStream,
              'from field:',
              fieldText
            );
          ...
          if (studentData.grade_start_date) {
            ...
            console.log(
              'Months in grade:',
              months,
              'from grade_start_date:',
              studentData.grade_start_date
            );
            ...
            if (yearMatch) {
              ...
              setMonthsInGrade(months);
              console.log(
                'Months in grade (estimated):',
                months,
                'from academic_year:',
                studentData.school_classes.academic_year
              );
            }
          }
  ...
  const loadResults = async () => {
    ...
    if (attemptId) {
      ...
      try {
        ...
        console.log(
          ...
          Array.isArray(attempt?.results) ? 'array' : typeof attempt?.results
        );
        ...
        if (result && result.id) {
          ...
          if (
            result.gemini_results &&
            typeof result.gemini_results === 'object' &&
            Object.keys(result.gemini_results).length > 0
          ) {
            const geminiResults = result.gemini_results;

            // Validate that AI analysis is complete (has RIASEC scores)
            const hasValidRiasec =
              geminiResults.riasec?.scores &&
              Object.keys(geminiResults.riasec.scores).length > 0 &&
              Object.values(geminiResults.riasec.scores).some((score) => score > 0);

            if (!hasValidRiasec) {
              console.log(
                '⚠️ Database result has gemini_results but RIASEC scores are empty/invalid'
              );
            ...
            if (retryCompleted || alreadyRetried) {
              ...
              if (!result.gemini_results || Object.keys(result.gemini_results).length === 0) {
                ...
                setError(
                  new Error(
                    "Unable to retrieve assessment report. Please click 'Retry' to regenerate."
                  )
                );
              }
          ...
          setError(
            'Your assessment was saved but the results are missing. Click "Try Again" to generate your personalized career report.'
          );
          setLoading(false);
          return;
        }
    ...
    try {
      ...
      if (user) {
        ...
        if (
          latestResult?.gemini_results &&
          typeof latestResult.gemini_results === 'object' &&
          Object.keys(latestResult.gemini_results).length > 0
        ) {
  ...
  const handleRetry = useCallback(async () => {
    try {
      ...
      const isAIAssessment = ['after10', 'after12', 'college', 'higher_secondary'].includes(
        storedGradeLevel
      );
        ...
        try {
          ...
          if (user) {
            ...
            if (aptitudeAnswerKeys.length > 0) {
              ...
              const aiAptitudeQuestions = await fetchAIAptitudeQuestions(
                user.id,
                aptitudeAnswerKeys
              );
            ...
            if (knowledgeAnswerKeys.length > 0) {
              ...
              const aiKnowledgeQuestions = await fetchAIKnowledgeQuestions(
                user.id,
                knowledgeAnswerKeys
              );
  ...
  const validateResults = () => {
    ...
    if (
      ...
      if (!riasec || !riasec.topThree || riasec.topThree.length === 0)
        missingFields.push('Interest Explorer');
      // For high school and higher secondary, check aptitude sampling
      if (
        (gradeLevel === 'highschool' || gradeLevel === 'higher_secondary') &&
        (!aptitude || !aptitude.scores)
      )
        missingFields.push('Aptitude Sampling');
      return missingFields;
    }

    // For after12, check all comprehensive assessment fields (keep as-is)
    if (!riasec || !riasec.topThree || riasec.topThree.length === 0)
      missingFields.push('RIASEC Interests');
    if (!bigFive || typeof bigFive.O === 'undefined') missingFields.push('Big Five Personality');
    if (!workValues || !workValues.topThree || workValues.topThree.length === 0)
      missingFields.push('Work Values');
    if (!employability || !employability.strengthAreas) missingFields.push('Employability Skills');
    if (!knowledge || typeof knowledge.score === 'undefined')
      missingFields.push('Knowledge Assessment');
    if (!careerFit || !careerFit.clusters || careerFit.clusters.length === 0)
      missingFields.push('Career Fit');
    if (!skillGap || !skillGap.priorityA || skillGap.priorityA.length === 0)
      missingFields.push('Skill Gap Analysis');
    if (!roadmap || !roadmap.projects || roadmap.projects.length === 0)
      missingFields.push('Action Roadmap');
