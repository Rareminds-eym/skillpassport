...
const ReportHeader = ({ studentInfo, gradeLevel }) => {
  ...
  const infoItems = [
    ...
    { label: rollNumberLabel, value: studentInfo.regNo },
    {
      ...
      value: formatStreamDisplay(studentInfo.stream || studentInfo.branchField || '—'),
    },
            ...
            <div>
              ...
              <div className="relative h-[2px] w-40 md:w-56 mb-2 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    background:
                      'linear-gradient(90deg, #1E3A8A, #3B82F6, #60A5FA, #93C5FD, #BFDBFE)',
                    backgroundSize: '200% 100%',
                    animation: 'shimmer 3s linear infinite',
                  }}
                />
              </div>
              <p className="text-slate-300 text-sm md:text-base">AI-Powered Career Assessment</p>
            </div>
            <img
              src="/logo.png"
              alt="SkillPassport"
        ...
        <div className="relative px-6 md:px-8 py-6 bg-gray-50">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-4">
            {infoItems.map((item, index) => (
              ...
              >
                <p className="text-sm text-blue-600 font-medium mb-1.5">{item.label}</p>
                <p
                  className={`font-medium text-slate-900 text-base md:text-lg ${item.truncate ? 'line-clamp-2' : ''}`}
                >
                  {item.value}
                </p>
              </div>
