...
const SkillGapCourseCard = ({ course, onClick }) => {
  ...
  const { course_id, title, duration, skills = [], relevance_score, why_this_course } = course;
      ...
      <div className="mt-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span className="text-base">
            No platform courses currently address this skill. Check back later for new courses.
          </span>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-red-200 shadow-sm overflow-hidden">
        <div className="bg-red-50 p-5 border-b border-red-100">
          <div className="flex items-center justify-between">
            ...
            <span className="px-3 py-1 bg-red-500 text-white rounded-full text-base font-bold">
              {priorityA.length} Skills
            </span>
          </div>
        </div>
              ...
              <div key={idx} className="bg-gray-50 rounded-lg p-5">
                <div className="flex flex-wrap justify-between items-start gap-3 mb-3">
                  <div className="flex items-center gap-2">
                    <span className="w-10 h-10 rounded-lg bg-red-500 flex items-center justify-center text-white font-bold text-lg">
                      {idx + 1}
                    </span>
      ...
      <div className="bg-white rounded-xl p-6 border border-yellow-200 shadow-sm">
        ...
        <div className="grid md:grid-cols-2 gap-2">
          {priorityB.length > 0 ? (
            priorityB.map((item, idx) => (
              <div
                key={idx}
                className="flex items-center gap-2 p-3 bg-yellow-50 rounded-lg border border-yellow-100"
              >
                <span className="w-7 h-7 rounded bg-yellow-500 flex items-center justify-center text-white text-sm font-bold">
                  {idx + 1}
                </span>
      ...
      <div className="bg-slate-800 rounded-xl p-6">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          {learningTracks.map((track, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`rounded-lg p-5 ${isRecommended ? 'bg-indigo-500/20 border-2 border-indigo-400' : 'bg-white/5 border border-white/10'}`}
              >
                {isRecommended && (
                ...
                <div className="space-y-1 text-base">
                  <p className="text-gray-400">
                    <span className="text-gray-300">Best if:</span> {track.suggestedIf}
                  </p>
                  <p className="text-gray-400">
                    <span className="text-gray-300">Topics:</span> {track.topics}
                  </p>
                </div>
              </div>
            );
        ...
        <div className="flex items-center justify-between p-4 bg-white/10 rounded-lg">
          ...
          <span className="font-bold text-indigo-400 text-lg">
            {skillGap?.recommendedTrack || 'Not determined'}
          </span>
        </div>
      </div>
    </div>
