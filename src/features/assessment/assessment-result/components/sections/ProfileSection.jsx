      ...
      <div className="grid grid-cols-3 gap-4">
        {riasec?.topThree?.slice(0, 3).map((code, idx) => {
          ...
          return (
            ...
            >
              <div
                className={`w-14 h-14 rounded-lg flex items-center justify-center text-white font-bold text-2xl mb-3 ${scoreColor.bg}`}
              >
                {code}
              </div>
              <p className="font-bold text-gray-800 text-lg">{riasecNames[code]}</p>
              <p className="text-base text-gray-500">
                #{idx + 1} Interest • {score}/{maxScore}
              </p>
            </div>
          );
        })}
      ...
      <div className="grid lg:grid-cols-2 gap-5">
                ...
                <div key={code}>
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <span
                        className={`w-8 h-8 rounded flex items-center justify-center text-white font-bold text-base ${scoreColor.bg}`}
                      >
                        {code}
                      </span>
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-base font-bold px-2 py-1 rounded ${scoreColor.light} ${scoreColor.text}`}
                      >
                        {score}/{maxScore}
                      </span>
        ...
        <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
          ...
          {profileSnapshot?.aptitudeStrengths && profileSnapshot.aptitudeStrengths.length > 0 ? (
                ...
                {Object.entries(aptitude.scores).map(([domain, data]) => {
                  ...
                  return (
                    <div key={domain} className="flex items-center gap-3">
                      <div
                        className={`w-10 h-10 rounded-lg ${scoreColor.light} flex items-center justify-center shrink-0`}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className={`text-base font-bold ${scoreColor.text} ml-2`}>
                            {scoreText}
                          </span>
                        </div>
                        <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
                      ...
                      <span
                        className={`text-base font-bold px-2 py-1 rounded ${scoreColor.light} ${scoreColor.text} shrink-0`}
                      >
                        {pct}%
                      </span>
                    </div>
              ...
              {aptitude?.topStrengths && (
                <div className="mt-4 pt-4 border-t border-gray-100 flex flex-wrap gap-2">
                  {aptitude.topStrengths.map((s, i) => (
                    <span
                      key={i}
                      className="px-3 py-1.5 bg-green-100 text-green-700 rounded-full text-base font-semibold flex items-center gap-1"
                    >
            ...
            <p className="text-gray-500 italic text-center py-4 text-base">
            ...
            {workValues?.topThree?.map((val, idx) => {
              ...
              return (
                <div key={idx} className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
                  <div
                    className={`w-10 h-10 rounded-lg ${scoreColor.bg} flex items-center justify-center text-white`}
                  >
                  ...
                  <span
                    className={`px-3 py-1 rounded text-base font-bold ${scoreColor.light} ${scoreColor.text}`}
                  >
                    {val.score}/5
                  </span>
                </div>
