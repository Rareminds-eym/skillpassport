...
const CourseRecommendationCard = ({ course, onClick }) => {
  ...
  const { course_id, title, duration, skills = [], relevance_score, match_reasons = [] } = course;
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-0 left-0 h-1 ${matchColor.bg}`}
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-start gap-3 flex-1 min-w-0">
            <div
              className={`w-12 h-12 rounded-lg ${matchColor.light} flex items-center justify-center flex-shrink-0`}
            >
              <BookOpen className={`w-6 h-6 ${matchColor.text}`} />
            </div>
            <div className="min-w-0 flex-1">
              <h4 className="font-bold text-gray-900 text-base line-clamp-2 group-hover:text-indigo-600 transition-colors">
                {title}
              </h4>
              {duration && (
        ...
        {displaySkills.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {displaySkills.map((skill, idx) => (
              <span key={idx} className="px-2.5 py-1 bg-gray-100 text-gray-600 rounded text-sm">
                {skill}
              </span>
            ))}
