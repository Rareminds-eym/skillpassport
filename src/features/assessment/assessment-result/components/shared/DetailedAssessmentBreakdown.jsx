  ...
  const allStages = [
    {
      ...
      scores: riasec?.scores
        ? Object.entries(riasec.scores).map(([code, score]) => ({
        ...
        ? [
            {
              ...
              value: bigFive.O || 0,
              max: 5,
              percentage: Math.round(((bigFive.O || 0) / 5) * 100),
            },
            {
              label: 'Conscientiousness',
              value: bigFive.C || 0,
              max: 5,
              percentage: Math.round(((bigFive.C || 0) / 5) * 100),
            },
            {
              label: 'Extraversion',
              value: bigFive.E || 0,
              max: 5,
              percentage: Math.round(((bigFive.E || 0) / 5) * 100),
            },
            {
              label: 'Agreeableness',
              value: bigFive.A || 0,
              max: 5,
              percentage: Math.round(((bigFive.A || 0) / 5) * 100),
            },
            {
              label: 'Neuroticism',
              value: bigFive.N || 0,
              max: 5,
              percentage: Math.round(((bigFive.N || 0) / 5) * 100),
            },
          ]
        : [],
      avgPercentage: bigFive
        ? Math.round(
            (((bigFive.O || 0) +
              (bigFive.C || 0) +
              (bigFive.E || 0) +
    ...
    },
    {
      ...
      data: workValues,
      scores:
        workValues?.topThree?.map((val) => ({
          label: val.value,
          value: val.score,
          max: 5,
          percentage: Math.round((val.score / 5) * 100),
        })) || [],
      avgPercentage: workValues?.topThree
        ? Math.round(
            (workValues.topThree.reduce((sum, val) => sum + val.score, 0) /
              workValues.topThree.length /
              5) *
              100
          )
        : 0,
    ...
    },
    {
      ...
      data: employability,
      scores: employability?.strengthAreas
        ...
        : [],
      avgPercentage: employability?.strengthAreas
        ? Math.round(
            (employability.strengthAreas.reduce((sum, area) => sum + (area.score || 4), 0) /
              employability.strengthAreas.length /
              5) *
              100
          )
        : 0,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
          <div>
            ...
            <span
              style={{ fontSize: '10px', fontWeight: 'bold', color: '#ffffff', marginLeft: '5px' }}
            >
          ...
          <div>
            ...
            >
              {Math.round(
                stages.filter((s) => s.data).reduce((sum, s) => sum + s.avgPercentage, 0) /
                  stages.filter((s) => s.data).length
              )}
              %
            </span>
          </div>
          ...
          >
            ...
            >
              <div>
                ...
                <h4 style={{ fontSize: '9px', fontWeight: 'bold', margin: 0 }}>{stage.name}</h4>
              </div>
      ...
      >
        ...
        >
          This detailed breakdown is included in the PDF export for internal tracking and quality
          assurance. It provides granular visibility into the assessment scoring logic, helping
          developers verify calculation accuracy and identify potential data quality issues.{' '}
          <strong style={{ color: '#fbbf24' }}>Color coding:</strong>{' '}
          <strong style={{ color: '#22c55e' }}>Green (≥70%)</strong> = Excellent,{' '}
          <strong style={{ color: '#eab308' }}>Yellow (40-69%)</strong> = Good,{' '}
          <strong style={{ color: '#ef4444' }}>Red (&lt;40%)</strong> = Needs Improvement.
        </p>
      </div>
    </div>
  );
