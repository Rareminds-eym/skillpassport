...
import { STREAM_KNOWLEDGE_PROMPTS } from './streamPrompts';
 ...
 *
 * @param programName - The program name from learner profile
 ...
 */
export function getFallbackKnowledgeQuestions(streamId: string): any[] {
  const streamInfo = STREAM_KNOWLEDGE_PROMPTS[streamId];
  if (!streamInfo) return [];

  return streamInfo.topics.map((topic: string, idx: number) => ({
