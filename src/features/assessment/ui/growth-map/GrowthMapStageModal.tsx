...
type SubTabId = 'overview' | 'parent' | 'instructional' | 'actionSteps';
...
/** Per-tab accent theme, matching the Bolt reference's color-coding
 * (blue=parent, indigo=instructional, emerald=actionSteps) — visual/
 * interaction pattern only, not fabricated content. */
const GUIDANCE_THEME: Record<
  'parent' | 'instructional' | 'actionSteps',
  {
    icon: typeof Heart;
    iconBg: string;
    cardBorder: string;
...
/**
 * A stage_guidance block is only trusted if it has all 4 required fields in
 * the right shape — this mirrors the backend's isValidStageGuidanceBlock
 * (report-generator.ts) as a defensive frontend check, since gemini_results
 * is passed through as `any` end-to-end and a malformed/partial block must
 * never render as if it were real content (fail safely, not with fake
 * fallback text).
 */
function isValidGuidanceBlock(block: unknown): block is StageGuidanceBlock {
  ...
  return (
    ...
    b.highlights.every((h) => typeof h === 'string' && h.length > 0)
  );
}

/**
 * Which real sidebar items exist for a given stage. "Overview" is always
 * present (it's whatever renderStageOverview returns for that stage).
 * Parent Translation / Instructional Implications / Actionable Next Steps
 * come from stage_guidance[stage.id] — generated by the same AI pipeline as
 * every other real field here — and only appear when that stage's block is
 * present AND passes isValidGuidanceBlock, so an existing report generated
 * before stage_guidance existed simply shows Overview only, never a broken
 * or fake tab. The old capability_insights-driven "Insights & Next Steps"
 * tab has been removed from this modal (the underlying capability_insights
 * data itself is untouched in the backend/report pipeline — this is a
 * modal UI change only).
 */
function getAvailableSubTabs(id: StageId, reports: StageReports): SubTabMeta[] {
  ...
  const guidance = reports.stage_guidance?.[id];
  if (guidance && isValidGuidanceBlock(guidance.parent)) {
    tabs.push({
      id: 'parent',
      label: guidance.parent.title,
      sublabel: guidance.parent.subtitle,
      icon: GUIDANCE_THEME.parent.icon,
    });
  }
  if (guidance && isValidGuidanceBlock(guidance.instructional)) {
    tabs.push({
      id: 'instructional',
      label: guidance.instructional.title,
      sublabel: guidance.instructional.subtitle,
      icon: GUIDANCE_THEME.instructional.icon,
    });
  }
  if (guidance && isValidGuidanceBlock(guidance.actionSteps)) {
    tabs.push({
      id: 'actionSteps',
      label: guidance.actionSteps.title,
      sublabel: guidance.actionSteps.subtitle,
      icon: GUIDANCE_THEME.actionSteps.icon,
    });
  }
...
/**
 * Renders one Parent / Instructional / Action Steps block using the Bolt
 * reference's real visual pattern: a themed gradient hero card (icon +
 * subtitle + title + desc) followed by a numbered 3-column grid of
 * highlight cards — adapted to our own real stage_guidance content, not
 * Bolt's mock text. Returns null (never fake fallback content) if the
 * block fails validation.
 */
function renderGuidanceBlock(
  kind: 'parent' | 'instructional' | 'actionSteps',
  block: StageGuidanceBlock | undefined
): ReactNode {
  if (!isValidGuidanceBlock(block)) return null;
  const theme = GUIDANCE_THEME[kind];
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-sm leading-relaxed text-slate-700">{block.desc}</p>
      </div>

      <div className="grid gap-4 sm:grid-cols-3">
        {block.highlights.map((point, idx) => (
          <div
            key={idx}
...
export const GrowthMapStageModal: FC<Props> = ({
  ...
  const stageGuidance = reports.stage_guidance?.[stage.id];

  const content = (() => {
    switch (modalSubTab) {
      case 'parent':
        return renderGuidanceBlock('parent', stageGuidance?.parent);
      case 'instructional':
        return renderGuidanceBlock('instructional', stageGuidance?.instructional);
      case 'actionSteps':
        return renderGuidanceBlock('actionSteps', stageGuidance?.actionSteps);
      default:
        return renderStageOverview(stage.id, reports);
    }
  })();
  ...
  // actual guidance block's own title) instead of a static label.
  const headerSubtitle =
    modalSubTab === 'overview'
      ? 'Interactive Exploration'
      : modalSubTab === 'parent'
        ? stageGuidance?.parent?.title || 'Parent Translation'
        : modalSubTab === 'instructional'
          ? stageGuidance?.instructional?.title || 'Instructional Implications'
          : stageGuidance?.actionSteps?.title || 'Actionable Next Steps';
