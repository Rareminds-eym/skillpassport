...
import { formatSalaryRange, getRoleName, getRoleSalary } from '@/shared/lib/salaryFormatter';
...
describe('CareerSection Data Transformation', () => {
  ...
  describe('Mixed Role Arrays', () => {
    ...
    it('should process arrays with string roles correctly', () => {
      fc.assert(
        fc.property(
          fc.array(stringRoleArbitrary, { minLength: 1, maxLength: 5 }),
          (roles) => {
            roles.forEach(role => {
              const name = getRoleName(role);
              const salary = getRoleSalary(role);
  ...
  describe('Specific Examples', () => {
    ...
    it('should correctly process Explore Later role data', () => {
      const exploreLaterRoles = [
        { name: 'Technical Writer', salary: { min: 3, max: 6 } }
      ];
