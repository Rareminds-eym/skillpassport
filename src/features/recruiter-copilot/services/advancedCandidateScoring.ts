...
class AdvancedCandidateScoringService {
  ...
  private enrichCandidate(
    ...
    const overallScore = Math.round(
      technicalScore * 0.35 + educationScore * 0.2 + experienceScore * 0.25 + engagementScore * 0.2
    );
  ...
  private detectRedFlags(student: any, skills: any[], applications: any[]): string[] {
    ...
    if (applications.length > 10) {
      const progressed = applications.filter(
        (a) => a.application_status !== 'applied' && a.application_status !== 'rejected'
      );
    ...
    if (daysSinceUpdate > 180) {
      flags.push(
        `🕐 Inactive profile (last updated ${Math.floor(daysSinceUpdate / 30)} months ago)`
      );
    }
  ...
  private detectDataQualityIssues(student: any, skills: any[]): string[] {
    ...
    const suspiciousSkills = skills.filter(
      (s) => /life|evaluation|test|testing/i.test(s.name) && s.name.split(' ').length <= 2
    );
   ...
   */
  private categorizeSkills(skills: any[]): {
    technical: string[];
    soft: string[];
    tools: string[];
  } {
