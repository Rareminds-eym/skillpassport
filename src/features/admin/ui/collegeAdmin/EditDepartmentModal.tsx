...
import { DepartmentWithStats, departmentService } from '@/shared/api/college/departmentService';
...
const EditDepartmentModal: React.FC<EditDepartmentModalProps> = ({
  ...
  // Validate department code with debouncing (only if code changed from original)
  useEffect(() => {
    if (!code.trim() || !collegeId || !department) {
      setCodeValidation(null);
      return;
    }

    // Don't validate if code hasn't changed
    if (code.trim().toUpperCase() === originalCode.toUpperCase()) {
      setCodeValidation(null);
      return;
    }
  ...
  }, [code, collegeId, department, originalCode]);

  useEffect(() => {
    if (department && isOpen) {
      setName(department.name || "");
      setCode(department.code || "");
      setOriginalCode(department.code || ""); // Store original code for comparison
      setDescription(department.description || "");
      // Convert database status (lowercase) to UI format (capitalized)
      const dbStatus = department.status || "active";
      setStatus(dbStatus.charAt(0).toUpperCase() + dbStatus.slice(1).toLowerCase());
      setError(null);
      setSubmitting(false);
      setCodeValidation(null);
      setIsValidatingCode(false);
      ...
      if (hodName && hodName !== 'Not Assigned') {
        ...
        if (faculty) {
          ...
          // If faculty not found in list, create a placeholder
          setSelectedFaculty({
            id: department.metadata?.hod_id || '',
            name: hodName,
            email: hodEmail,
            designation: 'Head of Department',
            specialization: ''
          });
          setFacultySearchTerm(hodName);
        }
      } else {
  ...
  const handleSubmit = () => {
    ...
    // Check code validation (only if code was changed)
    if (code.trim().toUpperCase() !== originalCode.toUpperCase()) {
    ...
    setTimeout(() => {
      const updated: Department = {
        ...
        // Convert UI status (capitalized) back to database format (lowercase)
