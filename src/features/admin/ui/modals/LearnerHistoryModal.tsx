import { AttendanceRecord, AttendanceSession, Learner } from '@/features/college-admin';
...
const LearnerHistoryModal = ({
    ...
    onExportHistory: (learner: Learner, session: AttendanceSession, allRecords: AttendanceRecord[]) => void;
}) => {
    const [dateRange, setDateRange] = useState({ from: "", to: "" });

    if (!isOpen || !learner || !session) return null;

    const learnerHistory = allRecords.filter((record) => record.learnerId === learner.id && record.subject === session.subject);

    const stats = {
        present: learnerHistory.filter((r) => r.status === "present").length,
        absent: learnerHistory.filter((r) => r.status === "absent").length,
        late: learnerHistory.filter((r) => r.status === "late").length,
        excused: learnerHistory.filter((r) => r.status === "excused").length,
        total: learnerHistory.length,
        percentage: learnerHistory.length > 0 ? ((learnerHistory.filter((r) => r.status === "present" || r.status === "late" || r.status === "excused").length / learnerHistory.length) * 100).toFixed(1) : "0",
    };

    const filteredHistory = learnerHistory.filter((record) => {
                ...
                <div className="relative w-full max-w-4xl transform overflow-hidden rounded-2xl bg-white shadow-2xl transition-all">
                    ...
                    <div className="flex items-start justify-between border-b border-gray-200 px-6 py-5 bg-gradient-to-r from-indigo-50 to-purple-50">
                        <div className="flex-1">
                            <h2 className="text-2xl font-bold text-gray-900 mb-2">
                                {learner.name} - Attendance History
                            </h2>
                            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                                ...
                                <span className="flex items-center gap-1">
                                    ...
                                    {learner.course} - Sem {learner.semester}
                                </span>
                            </div>
                        </div>
                        <button
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                            ...
                            <ReactApexChart
                                options={{
                                    ...
                                    xaxis: {
                                        categories: learnerHistory.slice(-7).map(r => formatDate(r.date)),
                                ...
                                series={[
                                    {
                                        name: "Attendance",
                                        data: learnerHistory.slice(-7).map(r => r.status === "present" || r.status === "late" || r.status === "excused" ? 1 : 0),
                                    },
                                ]}
                                type="line"
                                height={200}
                    ...
                    <div className="border-t border-gray-200 px-6 py-4 bg-gray-50 flex items-center justify-between">
                        <div className="text-sm text-gray-600">
                            Showing {filteredHistory.length} of {learnerHistory.length} records
                        </div>
                        <div className="flex gap-3">
                            ...
                            <button
                                onClick={() => onExportHistory(learner, session, allRecords)}
