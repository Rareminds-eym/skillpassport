...
const CoursePlayer = () => {
  ...
  const saveProgress = async () => {
    ...
    try {
      ...
      const result = await enrollmentService.updateProgress(user.email, courseId, lessonsArray);
  ...
  const saveVideoPosition = useCallback(async (position, duration) => {
    ...
    const result = await progressService.saveVideoPosition(
      user.id,
      courseId,
      currentLesson.id,
      Math.floor(position),
  ...
  useEffect(() => {
    ...
    const loadVideoPosition = async () => {
      ...
      const saved = await progressService.getVideoPosition(user.id, courseId, currentLesson.id);
  ...
  const initializeLessonProgress = async (targetLesson) => {
    ...
    try {
      ...
      await progressService.saveRestorePoint(
        user.id,
        courseId,
        currentModuleIndex,
        currentLessonIndex,
