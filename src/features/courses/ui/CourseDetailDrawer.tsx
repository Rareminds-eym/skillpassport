...
import { addResource, deleteResource, addLesson, updateLesson, deleteLesson as deleteLessonFromDB } from '@/features/educator';
import toast from 'react-hot-toast';
import { getLogger } from '@/shared/config/logging';

const logger = getLogger('course-detail-drawer');
...
const CourseDetailDrawer: React.FC<CourseDetailDrawerProps> = ({
  ...
  const handleResourcesAdded = async (newResources: Resource[]) => {
    if (!selectedModuleId || !selectedLessonId) {
      logger.error('Missing selectedModuleId or selectedLessonId');
    ...
    try {
      ...
      logger.error('Error saving resources', error instanceof Error ? error : new Error(String(error)));
  ...
  const handleDeleteResource = async (moduleId: string, lessonId: string, resourceId: string) => {
    ...
    try {
      ...
      logger.error('Error deleting resource', error instanceof Error ? error : new Error(String(error)));
