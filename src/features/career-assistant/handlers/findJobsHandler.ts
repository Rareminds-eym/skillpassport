  ...
  try {
    ...
    if (!opportunities || opportunities.length === 0) {
      return {
        success: true,
        message:
          "I couldn't find any active job opportunities at the moment. New opportunities are added regularly, so please check back soon! In the meantime, would you like help preparing your resume or developing new skills?",
      };
    }
...
async function matchJobsWithAI(
  ...
  try {
    ...
    console.log(
      'Skills:',
      studentContext.technical_skills.length,
      'technical,',
      studentContext.soft_skills.length,
      'soft'
    );
...
function createFallbackMatches(
  ...
  return opportunities.slice(0, topN).map((opp, idx) => ({
    ...
    key_matching_skills:
      studentProfile.profile?.technicalSkills?.slice(0, 3).map((s: any) => s.name) || [],
    skills_gap: [],
    recommendation:
      'Review the job requirements and consider applying if it aligns with your career goals.',
    opportunity: opp,
  }));
}
