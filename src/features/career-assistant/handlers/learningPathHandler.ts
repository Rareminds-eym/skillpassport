...
export async function handleLearningPath(message: string, studentId: string): Promise<AIResponse> {
  try {
    ...
    console.log('✅ Student profile loaded:', {
      ...
      completedTraining:
        studentProfile.profile?.training?.filter((t: any) => t.status === 'completed').length || 0,
    });
...
async function generateLearningPath(
  ...
  try {
    ...
    const completion = await client.chat.completions.create({
      ...
      temperature: 0.8,
      max_tokens:
        queryType === 'quick-answer' ? 800 : queryType === 'detailed-guidance' ? 1500 : 2500,
    });
