/**
 * 🎯 LEARNER PROBLEM SOLVER SERVICE
 * 
 * Addresses REAL learner struggles:
...
const logger = getLogger('learner-problem-solver');

export type LearnerProblem =
  | 'imposter-syndrome'
  | 'learning-overwhelm'
  | 'time-management'
  | 'project-completion'
...
export interface ProblemSolution {
  problemIdentified: LearnerProblem;
...
class LearnerProblemSolverService {
  
  /**
   * 🎯 Identify and Solve Learner Problem
   * Detects underlying issues and provides actionable solutions
   */
  async solvelearnerProblem(
    ...
    try {
      const prompt = `You are an empathetic learner mentor who understands the REAL struggles learners face. Not just technical problems, but emotional and psychological challenges.
...
- Skills: ${learnerContext.skills.join(', ')}
- Learning for: ${learnerContext.learningDuration || 'Unknown'}
- Recent activity: ${learnerContext.recentActivity || 'Unknown'}` : ''}
...
**OUTPUT (JSON):**
{
  ...
  "encouragement": "The fact that you're worried about being good enough actually proves you care about quality - that's a STRENGTH, not a weakness. You belong here. Every line of code you write, every bug you fix, every concept you grasp is proof that you're not an imposter - you're a developer in the making. Keep going. You've got this. 💪"
      ...
      const completion = await client.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an exceptionally empathetic learner mentor. You understand that learning to code is as much emotional as it is technical. You provide practical, actionable advice while validating feelings. You are warm, genuine, and never dismissive.'
          },
      ...
      logger.error('Failed to analyze learner problem', error instanceof Error ? error : new Error(String(error)), {
        learnerMessage: learnerMessage.substring(0, 100)
      });
      throw new Error('Failed to analyze learner problem');
    }
  }
  
  /**
   * 💪 Generate Motivational Boost
   * For when learners just need encouragement
   */
  async generateMotivation(
    ...
    try {
      const prompt = `Generate a personalized, genuine motivational message for a learner.

${learnerContext.recentAchievements ? `Recent wins: ${learnerContext.recentAchievements.join(', ')}` : ''}
${learnerContext.currentChallenge ? `Current challenge: ${learnerContext.currentChallenge}` : ''}
Skill level: ${learnerContext.skillLevel || 'Learning'}
...
Be personal, honest, and energizing. Like a mentor who really cares.`;
   ...
   * Fast check if message contains common learner problems
   */
  detectProblemType(message: string): LearnerProblem | null {
...
export default new LearnerProblemSolverService();
