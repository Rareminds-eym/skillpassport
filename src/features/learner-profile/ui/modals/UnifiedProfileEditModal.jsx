  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      if (singleEditMode && processedItems.length > 0) {
        ...
        const validFieldNames = config.fields.map(f => f.name);
        const metadataFields = ['id', 'learner_id', 'created_at', 'updated_at', 'approval_status', 'enabled', 'verified', 'has_pending_edit', 'verified_data', 'pending_edit_data'];
  ...
  const saveItem = async () => {
    ...
    if (editingIndex !== null) {
      ...
      const updatedItem = {
        ...
        id: existingItem.id,
        learner_id: existingItem.learner_id,
                ...
                if (usesSeparateModal.includes(type)) {
                  return (
                    ...
                    >
                      ...
                      <span className="whitespace-nowrap">{config.addButtonText}</span>
                    </Button>
                  );
                }
                return null;
