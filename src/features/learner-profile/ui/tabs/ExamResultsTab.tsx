...
const ExamResultsTab: React.FC<ExamResultsTabProps> = ({ learner, loading }) => {
      ...
      (markEntries || []).forEach((entry: any, index: number) => {
        ...
        if (subjects.length === 0) {
          ...
          results.push({
            ...
            remarks: entry.is_absent ? 'Learner was absent' : (entry.remarks || ''),
  ...
  const EmptyState = () => (
    <div className="text-center py-16">
      ...
      <p className="text-xs text-gray-500 max-w-sm mx-auto">
        No exam results have been recorded for this learner yet.
      </p>
    </div>
  );
