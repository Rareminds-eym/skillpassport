...
import { getlearnerDocuments, getlearnerDocumentUrl } from '@/features/learner-profile/api';
...
const DocumentsTab: React.FC<DocumentsTabProps> = ({ learner, loading: externalLoading = false }) => {
  ...
  const loadDocuments = async () => {
    try {
      setLoading(true);
      setError(null);
      const docs = await getlearnerDocuments(learner.id);
      setDocuments(docs);
    } catch (err) {
  ...
  const handleViewDocument = (document: Document) => {
    const viewUrl = getlearnerDocumentUrl(document.url, 'inline');
  ...
  const handleDownloadDocument = async (document: Document) => {
    try {
      ...
      const downloadUrl = getlearnerDocumentUrl(document.url, 'download');
    ...
    return (
      <div className="p-6">
        <div className="text-center py-20">
          ...
          <p className="mt-1 text-sm text-gray-500">This learner hasn't uploaded any documents yet.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h3 className="text-lg font-medium text-gray-900 mb-2">Learner Documents</h3>
        <p className="text-sm text-gray-500">View and download documents uploaded by the learner</p>
      </div>
