...
import { CurriculumData, LessonPlan } from '@/features/learner-profile/model';
import { LessonSection } from '..';

interface CurriculumTabProps {
  curriculumData: CurriculumData[];
  lessonPlans: LessonPlan[];
  learnerAcademicYear: string | null;
  loading: boolean;
  learner?: any;
}
  ...
  const EmptyState = () => (
    <div className="text-center py-12">
      ...
      <p className="text-sm text-gray-500">
        No curriculum found for Grade {learner?.grade || learner?.class}. Curriculum may not be created or approved yet.
      </p>
    </div>
  );
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">
            Grade {learner?.grade || learner?.class} • {uniqueSubjects.length} subject{uniqueSubjects.length !== 1 ? 's' : ''} • {
                        ...
                        <div className="flex-1">
                          ...
                          {primaryCurriculum.lessonCount !== undefined && (
                            <p className="text-xs text-gray-500 mt-2">
                              {learnerAcademicYear && primaryCurriculum.academic_year === learnerAcademicYear 
                                ? `Learner's academic year curriculum (${primaryCurriculum.lessonCount} lesson${primaryCurriculum.lessonCount !== 1 ? 's' : ''})`
