...
import {
  ...
  normalizeStreamId,
} from '../../services/careerAssessmentAIService';
...
const AssessmentTest = () => {
  ...
  const [adaptiveQuestionTimer, setAdaptiveQuestionTimer] = useState(
    TIMERS.ADAPTIVE_QUESTION_TIME_LIMIT
  );
    ...
    const fetchStudentGrade = async () => {
      ...
      try {
        ...
        let { data: student, error } = await supabase
          .from('students')
          .select(
            'id, grade, grade_start_date, school_class_id, school_id, university_college_id, program_id, course_name, school_classes:school_class_id(grade, academic_year), program:program_id(name, code)'
          )
          .eq('user_id', user.id)
          .maybeSingle();
    ...
    const saveAdaptiveSessionLink = async () => {
      if (adaptiveAptitude.session?.id && currentAttempt?.id) {
        try {
          ...
          console.log(
            '✅ Linked adaptive session to assessment attempt:',
            adaptiveAptitude.session.id
          );
        } catch (linkErr) {
  ...
  const skipToAptitude = () => {
    ...
    console.log(
      'Test Mode: Skipped to Aptitude section, filled',
      Object.keys(filledAnswers).length,
      'answers'
    );
  };
  ...
  const skipToKnowledge = () => {
    ...
    console.log(
      'Test Mode: Skipped to Knowledge section, filled',
      Object.keys(filledAnswers).length,
      'answers'
    );
  };
  ...
  const loadQuestionsFromDatabase = async (streamId, gradeLevel) => {
    ...
    try {
      ...
      if (totalQuestions === 0 && !usesAIQuestions) {
        ...
        setQuestionsError(
          `No assessment questions available for this grade level (${gradeLevel}). Please contact support or try a different grade level.`
        );
      ...
      try {
        ...
        if (studentId) {
          ...
          if (!eligibility.canTake) {
            ...
            setError(
              `You can retake the assessment after ${nextDate.toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
          ...
          if (existingAttempt) {
            ...
            const hasProgress =
              existingAttempt.current_section_index > 0 ||
  ...
  const handleResumeAssessment = async () => {
    ...
    try {
      ...
      if (pendingAttempt.restoredResponses) {
        console.log(
          'Restoring answers:',
          Object.keys(pendingAttempt.restoredResponses).length,
          'answers'
        );
      ...
      if (pendingAttempt.adaptive_aptitude_session_id) {
        console.log(
          '🔄 Found linked adaptive aptitude session:',
          pendingAttempt.adaptive_aptitude_session_id
        );
        try {
  ...
  const getQuestionsForSection = (sectionId) => {
    ...
    switch (sectionId) {
      case 'riasec':
        return aiPoweredRiasecQuestions;
      case 'bigfive':
        return aiPoweredBigFiveQuestions;
      case 'values':
        return aiPoweredWorkValuesQuestions;
      case 'employability':
        return aiPoweredEmployabilityQuestions;
      default:
        return [];
    }
  };
    ...
    if (gradeLevel === 'middle') {
      return [
        ...
        },
        {
          ...
          instruction:
            'Rate each statement: 1 = not like me, 2 = sometimes, 3 = mostly me, 4 = very me',
        },
        {
        ...
        },
        {
          ...
          description:
            "A smart test that adjusts to your skill level. It gets easier or harder based on how you're doing!",
          color: 'indigo',
    ...
    if (gradeLevel === 'highschool') {
      return [
        ...
        },
        {
          ...
          icon: <img src="/RMLogo.webp" alt="RM Logo" className="w-6 h-6 object-contain" />,
          description:
            'An intelligent test that adapts to your ability level for accurate aptitude measurement.',
          color: 'indigo',
          questions: [], // Questions are generated dynamically by the adaptive engine
          isAdaptive: true, // Flag to indicate this is the adaptive aptitude section
          instruction:
            'Answer each question carefully. The test will adapt to your performance level.',
        },
      ];
    }
      ...
      return [
        {
          ...
          icon: <Heart className="w-6 h-6 text-rose-500" />,
          description:
            'Discover what types of work environments and activities appeal to you most.',
          color: 'rose',
        ...
        },
        {
          ...
          description:
            'Measure your cognitive strengths across verbal, numerical, logical, spatial, and clerical domains.',
          color: 'amber',
    ...
    if (gradeLevel === 'after10' || gradeLevel === 'after12' || gradeLevel === 'college') {
      return [
        ...
        },
        {
          ...
          isAptitude: true,
          individualTimeLimit: 60, // 1 minute per question
          get individualQuestionCount() {
            return this.questions.length;
          }, // All questions have individual timers
          instruction: 'Choose the correct answer. You have 1 minute per question.',
        },
        {
          id: 'knowledge',
  ...
  const categoryStreams = selectedCategory
    ? streamsByCategory[selectedCategory]
    : [
  ...
  useEffect(() => {
    ...
    currentQuestionIndex,
    showSectionIntro,
    currentSection?.individualTimeLimit,
  ]);
  ...
  useEffect(() => {
    if (
      currentSection?.isAdaptive &&
      adaptiveQuestionTimer === 0 &&
      adaptiveAptitude.currentQuestion &&
      !adaptiveAptitude.submitting
    ) {
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      if (useDatabase && currentAttempt?.id) {
        ...
        console.log(
          'Saving progress on page unload - timer:',
          timerToSave,
          'elapsed:',
          elapsedToSave
        );
  ...
  const handleGradeSelect = async (level) => {
    ...
    if (level === 'middle') {
      ...
      if (studentRecordId) {
        try {
          console.log(
            'Creating assessment attempt for after10/higher_secondary with studentRecordId:',
            studentRecordId
          );
  ...
  const handleCategorySelect = async (categoryId) => {
    ...
    console.log(
      '📚 handleCategorySelect - gradeLevel:',
      gradeLevel,
      'effectiveGradeLevel:',
      effectiveGradeLevel,
      'categoryId:',
    ...
    if (effectiveGradeLevel === 'after12') {
      ...
      if (studentRecordId) {
        try {
          ...
          setUseDatabase(true);
          console.log(
            'Assessment attempt created in database for category:',
            categoryId,
            'gradeLevel:',
            effectiveGradeLevel
          );
  ...
  const generateCourseRecommendations = (analysisResults) => {
    try {
      ...
      const courseMatches = allCourses.map((course) => {
        ...
        // Determine category
        const category = streamsByCategory.science.find((s) => s.id === course.id)
          ? 'Science'
          : streamsByCategory.commerce.find((s) => s.id === course.id)
            ? 'Commerce'
            : 'Arts';
    ...
    try {
      ...
      const questionBanks = {
        ...
        streamKnowledgeQuestions: {
          [studentStream]: getQuestionsForSection(getSectionId('knowledge')),
        },
      };
      ...
      if (gradeLevel === 'highschool' && answers.adaptive_aptitude_results) {
        console.log(
          '📊 Including adaptive aptitude results in analysis:',
          answers.adaptive_aptitude_results
        );
      ...
      if (geminiResults) {
        ...
        throw new Error(
          'AI analysis returned no results. Please check your API key configuration.'
        );
      }
    } catch (err) {
  ...
  const showLoading =
    checkingExistingAttempt || questionsLoading || (!assessmentStarted && dbLoading);
  if (showLoading) {
    return (
            ...
            {showSectionComplete && !error && !isSubmitting ? (
              ...
              >
                ...
                >
                  {currentSection.id === 'knowledge' ? (
                    <>
                      ...
                      <span className="text-sm font-medium">
                        Knowledge Test - Answers will be scored
                      </span>
                    </>
                    ...
                    <>
                      ...
                      <span className="text-sm font-medium">
                        Aptitude Test - Speed & accuracy matter
                      </span>
                    </>
                  ) : (
                    <>
                ...
                {currentSection.id === 'employability' && (
                  ...
                  >
                    <p className="text-xs font-bold text-green-800 mb-2">
                      2 Parts in this section:
                    </p>
                    <div className="space-y-2 text-xs">
                      <div className="bg-white/70 rounded-lg p-2">
                        <p className="font-semibold text-green-700">
                          Part A: Self-Rating Skills (25 Qs)
                        </p>
                        <p className="text-green-600 text-[10px]">
                          Communication, Teamwork, Problem Solving, Adaptability, Leadership,
                          Digital Fluency, Professionalism, Career Readiness
                        </p>
                      </div>
                      <div className="bg-white/70 rounded-lg p-2">
                        <p className="font-semibold text-rose-700">
                          Part B: Situational Judgement Test (6 Qs)
                        </p>
                        <p className="text-rose-600 text-[10px]">
                          Choose BEST and WORST response for workplace scenarios
                        </p>
                      </div>
                    </div>
                  </motion.div>
                    ...
                    <div className="space-y-2 text-xs">
                      <div className="bg-white/70 rounded-lg p-2">
                        <p className="text-indigo-600">
                          This intelligent test adapts to your ability level in real-time, providing
                          an accurate measurement of your aptitude across different reasoning areas.
                        </p>
                      </div>
                    </div>
                  </motion.div>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      {currentSection?.isAdaptive ? '18-22' : currentSection.questions.length}{' '}
                      questions
                    </span>
                  </div>
                  {currentSection?.isAdaptive && (
              ...
              >
                ...
                <div className="md:w-1/3 bg-gray-50 p-8 border-r border-gray-100">
                  <div className="mb-8">
                    <div
                      className={`w-12 h-12 rounded-xl bg-${currentSection.color}-100 flex items-center justify-center mb-4 shadow-sm`}
                    >
                      {currentSection.icon}
                    </div>
                      ...
                      <div className="mb-4 space-y-1">
                        {aptitudeModules.map((mod) => {
                          ...
                          return (
                            <div
                              key={mod.id}
                              className={`flex items-center gap-2 text-sm px-2 py-1 rounded ${isCurrentModule ? 'bg-amber-100 font-semibold' : 'opacity-60'}`}
                            >
                              <div
                                className={`w-2 h-2 rounded-full ${moduleColors[mod.color]}`}
                              ></div>
                      ...
                      <div className="mb-4 space-y-1">
                        {(() => {
                          ...
                          return (
                            <>
                              <div
                                className={`text-sm font-semibold px-2 py-1 ${moduleInfo.part === 'A' ? 'text-green-700' : 'text-gray-400'}`}
                              >
                                Part A: Self-Rating
                              </div>
                              {partADomains.map((domain, idx) => {
                                const isCurrentDomain =
                                  moduleInfo.part === 'A' && moduleInfo.domain === domain;
                                return (
                                  <div
                                    key={domain}
                                    className={`flex items-center gap-2 text-xs px-2 py-0.5 rounded ml-2 ${isCurrentDomain ? 'bg-green-100 font-semibold' : 'opacity-50'}`}
                                  >
                                    <div
                                      className={`w-1.5 h-1.5 rounded-full bg-${domainColors[idx]}-500`}
                                    ></div>
                              ...
                              <div
                                className={`text-sm font-semibold px-2 py-1 mt-1 ${moduleInfo.part === 'B' ? 'text-rose-700' : 'text-gray-400'}`}
                              >
                    ...
                    <div className="flex items-center gap-3 text-sm text-gray-600">
                      ...
                      <span>
                        ...
                        {currentSection?.isAdaptive
                          ...
                          : gradeLevel === 'after12' && currentSection.id === 'aptitude'
                            ? 50
                            : gradeLevel === 'after12' && currentSection.id === 'knowledge'
                              ? 20
                              : currentSection.questions.length}
                      </span>
                    </div>
                      ...
                      currentSection.isAptitude ? (
                        aptitudePhase === 'individual' ? (
                          <div className="flex flex-col gap-2">
                            <div
                              className={`flex items-center gap-3 text-sm font-semibold ${aptitudeQuestionTimer <= 10 ? 'text-red-600' : 'text-orange-600'}`}
                            >
                              <Clock className="w-4 h-4" />
                            ...
                            <div className="text-xs text-gray-500">
                              ...
                              {Math.min(
                                currentSection.individualQuestionCount || 30,
                                currentSection.questions.length
                              )}{' '}
                              (1 min each)
                            </div>
                          ...
                          <div className="flex flex-col gap-2">
                            <div
                              className={`flex items-center gap-3 text-sm font-semibold ${timeRemaining !== null && timeRemaining <= 60 ? 'text-red-600' : 'text-orange-600'}`}
                            >
                              <Clock className="w-4 h-4" />
                    ...
                    {error ? (
                      ...
                      >
                        ...
                        <p className="text-gray-500">
                          Rareminds is generating your personalized career roadmap.
                        </p>
                      </motion.div>
                    ) : !currentQuestion ? (
                      <motion.div
                      ...
                      <motion.div
                        key={
                          currentSection?.isAdaptive
                            ? `adaptive-${adaptiveAptitude.currentQuestion?.id}`
                          ...
                          {currentSection?.isAdaptive ? (
                            <div className="space-y-4">
                              ...
                              <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                                ...
                                >
                                  <Clock className="w-4 h-4" />
                                  {Math.floor(adaptiveQuestionTimer / 60)}:
                                  {(adaptiveQuestionTimer % 60).toString().padStart(2, '0')}
                                </div>
                              </div>
                            ...
                            <div className="space-y-4">
                              <div className="p-3 bg-rose-50 rounded-lg border border-rose-200 mb-4">
                                <p className="text-sm font-medium text-rose-700">
                                  Select the <span className="font-bold text-green-700">BEST</span>{' '}
                                  response and the{' '}
                                  <span className="font-bold text-red-700">WORST</span> response for
                                  this scenario.
                                </p>
                              </div>
                              ...
                              {currentQuestion.options.map((option, idx) => {
                                const optionLabel =
                                  currentQuestion.optionLabels?.[idx] ||
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-start gap-3">
                                      ...
                                      <p className="flex-1 text-gray-700 font-medium text-lg">
                                        {option}
                                      </p>
                                    </div>
                            ...
                            <div className="space-y-4">
                              <div className="p-3 bg-green-50 rounded-lg border border-green-200 mb-4">
                                <p className="text-sm font-medium text-green-700">
                                  Select up to{' '}
                                  <span className="font-bold">{currentQuestion.maxSelections}</span>{' '}
                                  options that feel most like you
                            ...
                            >
                              {currentSection.responseScale.map((option) => (
                                <div
                                  key={option.value}
                                  onClick={() => handleAnswer(option.value)}
                                  className={`flex items-center space-x-3 border rounded-xl p-4 transition-all cursor-pointer hover:bg-gray-50 ${
                                    answers[questionId] === option.value
                                ...
                                >
                                  ...
                                  <Label
                                    ...
                                    className="flex-1 cursor-pointer font-medium text-gray-700 text-lg"
                                  >
                                    {option.label}
                                  </Label>
                                </div>
