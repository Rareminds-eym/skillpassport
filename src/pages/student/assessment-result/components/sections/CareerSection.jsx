        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                {careerFit.clusters.map((cluster, idx) => {
                    ...
                    return (
                        <div key={idx} className={`bg-white rounded-xl p-5 border ${style.border} shadow-sm`}>
                            {/* Header */}
                            <div className="flex flex-wrap justify-between items-start gap-4 mb-5">
                                <div className="flex items-center gap-3">
                                    <div className={`w-12 h-12 rounded-lg ${style.bg} flex items-center justify-center text-white font-bold text-xl`}>
                                        {idx + 1}
                                    </div>
                                    <div>
                                        <h3 className="text-lg font-bold text-gray-900">{cluster.title}</h3>
                                        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${style.light} ${style.text}`}>
                                            {cluster.fit} Fit
                                        </span>
                                    </div>
                                </div>
                                <div className="flex flex-col items-center">
                                    <ProgressRing value={cluster.matchScore} size={70} strokeWidth={6} color={style.ring} />
                            ...
                            <div className="bg-gray-50 rounded-lg p-4 mb-4">
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h4 className="font-semibold text-gray-800 text-sm">Why This Fits You</h4>
                                </div>
                                {cluster.evidence ? (
                            ...
                            <div className="grid md:grid-cols-2 gap-4">
                                <div>
                                    <h5 className="text-xs font-bold text-gray-500 uppercase mb-2">Career Progression</h5>
                                    <div className="space-y-2">
                                        <div className="flex items-start gap-2">
                                            <span className="w-12 text-xs font-medium text-gray-500 pt-1">Entry</span>
                                            <div className="flex-1 flex flex-wrap gap-1">
                                                {cluster.roles?.entry?.length > 0 ? cluster.roles.entry.map((r, i) => (
                                                    <span key={i} className="px-2 py-1 bg-green-100 text-green-700 rounded text-xs">{r}</span>
                                                )) : <span className="text-gray-400 text-xs">Not specified</span>}
                                            </div>
                                        </div>
                                        <div className="flex items-start gap-2">
                                            <span className="w-12 text-xs font-medium text-gray-500 pt-1">Mid</span>
                                            <div className="flex-1 flex flex-wrap gap-1">
                                                {cluster.roles?.mid?.length > 0 ? cluster.roles.mid.map((r, i) => (
                                                    <span key={i} className="px-2 py-1 bg-indigo-100 text-indigo-700 rounded text-xs">{r}</span>
                                                )) : <span className="text-gray-400 text-xs">Not specified</span>}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div>
                                    <h5 className="text-xs font-bold text-gray-500 uppercase mb-2">Related Domains</h5>
                                    <div className="flex flex-wrap gap-1">
                                        {cluster.domains?.length > 0 ? cluster.domains.map((d, i) => (
                                            <span key={i} className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">{d}</span>
                                        )) : <span className="text-gray-400 text-xs">Not specified</span>}
                                    </div>
                                </div>
                            </div>
            ...
            <div className="bg-slate-800 rounded-xl p-6">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h3 className="font-bold text-white">Career Options Ranked</h3>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-green-500/10 rounded-lg p-4 border border-green-500/30">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <h4 className="font-bold text-green-400 text-sm">High Fit</h4>
                        </div>
                        <div className="space-y-2">
                            {careerFit.specificOptions.highFit.map((role, idx) => (
                                <div key={idx} className="flex items-center gap-2 p-2 bg-white/5 rounded">
                                    <span className="w-5 h-5 rounded bg-green-500/30 flex items-center justify-center text-green-400 font-bold text-xs">{idx + 1}</span>
                    ...
                    <div className="bg-yellow-500/10 rounded-lg p-4 border border-yellow-500/30">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <h4 className="font-bold text-yellow-400 text-sm">Medium Fit</h4>
                        </div>
                        <div className="space-y-2">
                            {careerFit.specificOptions.mediumFit.map((role, idx) => (
                                <div key={idx} className="flex items-center gap-2 p-2 bg-white/5 rounded">
                                    <span className="w-5 h-5 rounded bg-yellow-500/30 flex items-center justify-center text-yellow-400 font-bold text-xs">{idx + 1}</span>
                    ...
                    <div className="bg-red-500/10 rounded-lg p-4 border border-red-500/30">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <h4 className="font-bold text-red-400 text-sm">Explore Later</h4>
                        </div>
                        <div className="space-y-2">
                            {careerFit.specificOptions.exploreLater.map((role, idx) => (
                                <div key={idx} className="flex items-center gap-2 p-2 bg-white/5 rounded">
                                    <span className="w-5 h-5 rounded bg-red-500/30 flex items-center justify-center text-red-400 font-bold text-xs">{idx + 1}</span>
