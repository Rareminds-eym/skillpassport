        ...
        <div className="mb-6 space-y-3">
          ...
          {showFilters && (
            <div className="flex flex-wrap items-center gap-3 p-4 bg-white rounded-xl border border-gray-100 shadow-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={approvalFilter} onChange={(e) => setApprovalFilter(e.target.value)} className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm cursor-pointer">
          ...
          {hasActiveFilters && !showFilters && (
            <div className="flex flex-wrap items-center gap-2">
              ...
              {statusFilter !== 'all' && <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-medium">{STATUS_OPTIONS.find(o => o.value === statusFilter)?.label}<button onClick={() => setStatusFilter('all')}><X className="w-3 h-3" /></button></span>}
              {approvalFilter !== 'all' && <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-green-50 text-green-700 rounded-full text-xs font-medium">{APPROVAL_OPTIONS.find(o => o.value === approvalFilter)?.label}<button onClick={() => setApprovalFilter('all')}><X className="w-3 h-3" /></button></span>}
              {searchTerm && <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-purple-50 text-purple-700 rounded-full text-xs font-medium">"{searchTerm}"<button onClick={() => setSearchTerm('')}><X className="w-3 h-3" /></button></span>}
            </div>
          )}
        </div>
