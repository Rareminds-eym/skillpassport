...
const GradeCalculation: React.FC = () => {
  ...
  const filteredCalculations = gradeCalculations.filter((calc) => {
    if (
      searchTerm &&
      !calc.examination.toLowerCase().includes(searchTerm.toLowerCase()) &&
      !calc.college.toLowerCase().includes(searchTerm.toLowerCase())
    )
      return false;
    if (collegeFilter && !calc.college.toLowerCase().includes(collegeFilter.toLowerCase()))
      return false;
  ...
  const tabs = [
    {
      ...
      count: gradeCalculations.filter((c) => c.status === 'Pending').length,
    },
    {
      id: 'progress',
      label: 'In Progress',
      count: gradeCalculations.filter((c) => c.status === 'In Progress').length,
    },
    {
      id: 'completed',
      label: 'Completed',
      count: gradeCalculations.filter((c) => c.status === 'Completed').length,
    },
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-3xl font-bold">
                {gradeCalculations.filter((c) => c.status === 'Pending').length}
              </p>
            </div>
        ...
        <div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl p-6 text-white shadow-lg">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-3xl font-bold">
                {gradeCalculations.filter((c) => c.status === 'In Progress').length}
              </p>
            </div>
            <div className="p-3 bg-white/10 rounded-xl">
        ...
        <div className="bg-gradient-to-br from-green-600 to-green-700 rounded-2xl p-6 text-white shadow-lg">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-3xl font-bold">
                {gradeCalculations.filter((c) => c.status === 'Completed').length}
              </p>
            </div>
            <div className="p-3 bg-white/10 rounded-xl">
        ...
        <div className="bg-gradient-to-br from-purple-600 to-purple-700 rounded-2xl p-6 text-white shadow-lg">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-3xl font-bold">
                {gradeCalculations
                  .reduce((sum, calc) => sum + calc.totalStudents, 0)
                  .toLocaleString()}
              </p>
            </div>
            <div className="p-3 bg-white/10 rounded-xl">
                          ...
                          <span
                            className={`px-3 py-1 rounded-full text-sm font-medium ${
                              range.grade === 'F'
                                ...
                                : range.grade.includes('A')
                                  ? 'bg-green-100 text-green-700'
                                  : range.grade.includes('B')
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredCalculations.map((calc) => (
                    <tr key={calc.id} className="hover:bg-gray-50">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-16 bg-gray-200 rounded-full h-2">
                            <div
                              ...
                              style={{
                                width: `${(calc.calculatedStudents / calc.totalStudents) * 100}%`,
                              }}
                            ></div>
                          </div>
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusBadge(calc.status)}`}
                        >
                          {calc.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-900">{calc.dueDate}</td>
                      <td className="px-4 py-3">
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-gray-400">
                  {activeTab === 'pending'
                    ...
                    : activeTab === 'progress'
                      ? 'No calculations currently in progress'
