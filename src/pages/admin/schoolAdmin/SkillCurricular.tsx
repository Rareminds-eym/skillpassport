...
export default function ClubsActivitiesPage() {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        console.log(
          `✅ [SkillCurricular] Loaded ${mappedStudents.length} students from school ${schoolId}`
        );
  ...
  const joinedClubIds = useMemo(() => {
    return new Set(
      clubs.filter((c) => c.members.includes(currentStudent.id)).map((c) => c.club_id)
    );
    ...
    try {
      ...
      const updated = clubs.map((c) =>
        c.club_id === club.club_id
          ? { ...c, members: c.members.filter((m) => m !== currentStudent.id) }
          : c
      );
      setClubs(updated);
  ...
  const handleStudentEnroll = async (studentId, club) => {
    ...
    try {
      ...
      const updated = clubs.map((c) =>
        c.club_id === club.club_id ? { ...c, members: [...c.members, studentId] } : c
      );
      setClubs(updated);
  ...
  const handleStudentLeave = async (studentId, club) => {
    ...
    try {
      ...
      const updated = clubs.map((c) =>
        c.club_id === club.club_id ? { ...c, members: c.members.filter((m) => m !== studentId) } : c
      );
      setClubs(updated);
      setStudentDrawer({ open: true, club: updated.find((c) => c.club_id === club.club_id) });
  ...
  const buildClubParticipationRows = async () => {
    try {
      ...
      const { data, error } = await supabase.from('club_participation_report').select('*');
  ...
  const buildCompetitionPerformanceRows = async () => {
    try {
      ...
      const { data, error } = await supabase.from('competition_performance_report').select('*');
  ...
  const exportClubsPDF = async () => {
    const rows = await buildClubParticipationRows();
    if (rows && rows.length > 0) {
      const html = `<table><thead><tr>${Object.keys(rows[0])
        .map((k) => `<th>${k}</th>`)
                  ...
                  {competitionSearchQuery ? (
                    <>
                      ...
                      <p className="text-gray-500 mb-4">
                        No competitions match your search "{competitionSearchQuery}"
                      </p>
                      <button
                        onClick={() => setCompetitionSearchQuery('')}
                ...
                <div className="space-y-6">
                  ...
                  {totalCompetitionPages > 1 && (
                    <div className="mt-8 flex items-center justify-between bg-white rounded-xl border border-gray-200 p-4">
                      <button
                        onClick={() =>
                          setCurrentCompetitionPage(Math.max(1, currentCompetitionPage - 1))
                        }
                      ...
                      <button
                        onClick={() =>
                          setCurrentCompetitionPage(
                            Math.min(totalCompetitionPages, currentCompetitionPage + 1)
                          )
                        }
                ...
                <div className="flex gap-2 mt-2 flex-wrap">
                  {selectedClub.members.length > 0 ? (
                    selectedClub.members.map((m) => (
                      <div key={m} className="px-2 py-1 bg-gray-100 rounded-full text-xs">
                        {m}
                      </div>
                    ))
                  ) : (
          ...
          {viewComp && (
            <div className="space-y-4">
              <div className="flex items-start justify-between">
                ...
                <span
                  className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                    viewComp.status === 'upcoming'
                      ? 'bg-blue-100 text-blue-700'
                      : viewComp.status === 'ongoing'
                        ? 'bg-green-100 text-green-700'
                        : viewComp.status === 'completed'
                          ? 'bg-gray-100 text-gray-700'
                          : 'bg-red-100 text-red-700'
                  }`}
                >
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm font-semibold text-orange-900">
                      {viewComp.participatingClubs?.length ?? 0}{' '}
                      {viewComp.participatingClubs?.length === 1 ? 'Club' : 'Clubs'}
                    </div>
                  </div>
