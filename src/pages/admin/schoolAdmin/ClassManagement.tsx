        ...
        <div className="flex items-start justify-between border-b border-gray-200 px-6 py-5">
          <div className="max-w-xl">
            ...
            <p className="text-xs text-gray-400">
              Section {classItem.section} • {classItem.academic_year}
            </p>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingClass) {
        ...
        const { error: insertError } = await supabase.from('school_classes').insert([
          {
            school_id: schoolId,
            name: name.trim(),
            grade: grade.trim(),
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                {editingClass
                  ...
                  : 'Enter class details to add it to your school.'}
              </p>
            </div>
            <button onClick={onClose} className="text-gray-400 hover:text-gray-600" type="button">
              <XMarkIcon className="h-6 w-6" />
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center bg-white border border-dashed border-gray-300 rounded-lg p-10">
      ...
      <p className="mt-2 text-sm text-gray-500 max-w-sm">
        You haven't created any classes yet. Create your first class to start managing students and
        educators.
      </p>
      <div className="mt-6">
        <button
  ...
  const filteredClasses = useMemo(() => {
    ...
    return classes.filter((item) => {
      const matchesSearch = query
        ? [item.name, item.grade, item.section, item.academic_year].some((field) =>
            field.toLowerCase().includes(query)
          )
        : true;

      const matchesGrade = filters.grades.length
        ? filters.grades.includes(item.grade.toLowerCase())
        : true;
      const matchesSection = filters.sections.length
        ? filters.sections.includes(item.section.toLowerCase())
        : true;
      ...
      const matchesStatus = filters.statuses.length
        ? filters.statuses.includes(item.account_status.toLowerCase())
        : true;
        ...
        <div className="max-w-7xl mx-auto">
          <div className="bg-white rounded-lg shadow-sm p-12 text-center">
            ...
            <p className="mt-2 text-gray-500">
              Unable to load school information. Please ensure your account is properly configured
    ...
    <div className="flex overflow-y-auto mb-4 flex-col h-screen">
      <div className="p-4 sm:p-6 lg:p-8 mb-2">
        ...
        <p className="text-base md:text-lg mt-2 text-gray-600">
          Manage classes, assign educators, and organize students
        </p>
      </div>
                ...
                <div className="space-y-0">
                  <FilterSection title="Grade" defaultOpen>
                    <CheckboxGroup
                      ...
                      onChange={(values: string[]) =>
                        setFilters((prev) => ({ ...prev, grades: values }))
                      }
                    />
                  </FilterSection>
                  ...
                  <FilterSection title="Section">
                    <CheckboxGroup
                      ...
                      onChange={(values: string[]) =>
                        setFilters((prev) => ({ ...prev, sections: values }))
                      }
                    />
                  </FilterSection>
...
import {
  AssignEducatorModal as AssignEducatorModalComponent,
  ManageStudentsModal as ManageStudentsModalComponent,
} from '../../../components/admin/modals/ClassManagementModals';
