...
import {
  ...
  ValidationConflict,
} from '../../../../utils/timetableValidation';
...
const TimetableBuilderEnhanced: React.FC = () => {
  ...
  const loadRooms = () => {
    ...
    const uniqueRooms = [
      ...new Set(classes.map((c) => c.room_no).filter((room) => room && room.trim() !== '')),
    ].sort();
  ...
  const handleAddSlot = async () => {
    ...
    if (warnings.length > 0) {
      ...
      const confirmWithWarnings = confirm(
        `Warning:\n${warningMessages}\n\nDo you want to continue anyway?`
      );
  ...
  const getFilteredSlots = () => {
    return allSlots.filter((slot) => {
      ...
      if (filterSubject && !slot.subject_name.toLowerCase().includes(filterSubject.toLowerCase()))
        return false;
      if (filterStatus) {
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">Timetable Builder</h1>
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-200">
            ...
            <div className="text-sm text-indigo-700">
              {getFilteredSlots().length === allSlots.length
                ? 'Total Slots'
                : `Filtered (${allSlots.length} total)`}
            </div>
          </div>
            ...
            <div className="text-2xl font-bold text-yellow-900">
              {
                getFilteredSlots().filter((s) => {
                  ...
                  return slotConflicts.some(
                    (c) =>
                      c.severity === 'warning' &&
                      !slotConflicts.some((c2) => c2.severity === 'error')
                  );
                }).length
              }
            </div>
            ...
            <div>
              ...
              {allSlots.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  No slots added yet. Switch to "Teacher Load & Add Slots" tab to start building
                  your timetable.
                </div>
              ) : (
                <>
                  ...
                  {getFilteredSlots().length > itemsPerPage && (
                    <div className="mt-6 flex items-center justify-between">
                      <div className="text-sm text-gray-700">
                        ...
                        {Math.min(currentPage * itemsPerPage, getFilteredSlots().length)} of{' '}
                        {getFilteredSlots().length} slots
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                  ...
                  {periods.map((period) => (
                    <tr key={period}>
                      ...
                      {days.map((day, dayIndex) => {
                        ...
                        const isFiltered =
                          slot && !getFilteredSlots().some((s) => s.id === slot.id);
                            ...
                            {slot ? (
                              isFiltered ? (
                                ...
                                (() => {
                                  const slotConflicts = slot.id ? conflicts.get(slot.id) || [] : [];
                                  const hasConflict = slotConflicts.length > 0;
                                  const hasError = slotConflicts.some(
                                    (c) => c.severity === 'error'
                                  );
      ...
      {showTeacherClassModal && selectedCell && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6">
            ...
            <p className="text-sm text-gray-600 mb-6">Time: {timeSlots[selectedCell.period - 1]}</p>
                ...
                {newSlot.teacher_id && (
                  <div className="p-4 bg-indigo-50 rounded-lg border border-indigo-200">
                    <p className="text-sm font-medium text-indigo-900 mb-2">
                      Selected: {teachers.find((t) => t.id === newSlot.teacher_id)?.first_name}{' '}
                      {teachers.find((t) => t.id === newSlot.teacher_id)?.last_name}
                    </p>
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-3 bg-gray-200 rounded-full overflow-hidden">
                        <div
                          className={`h-full transition-all ${
                            (getTeacherLoad(newSlot.teacher_id) / 30) * 100 > 100
                              ? 'bg-red-500'
                              : (getTeacherLoad(newSlot.teacher_id) / 30) * 100 > 80
                                ? 'bg-yellow-500'
                                : 'bg-green-500'
                          }`}
                          style={{
                            width: `${Math.min((getTeacherLoad(newSlot.teacher_id) / 30) * 100, 100)}%`,
                          }}
                        />
                      </div>
                ...
                {newSlot.class_id && (
                  <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                    <p className="text-sm font-medium text-green-900 mb-1">
                      ...
                      {classes.find((c) => c.id === newSlot.class_id)?.section
                        ? `-${classes.find((c) => c.id === newSlot.class_id)?.section}`
                        : ''}
                    </p>
                    <p className="text-xs text-green-600">
                      {classes.find((c) => c.id === newSlot.class_id)?.name}
                    </p>
            ...
            <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-3 mb-4">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-indigo-900">
                  {teachers.find((t) => t.id === newSlot.teacher_id)?.first_name}{' '}
                  {teachers.find((t) => t.id === newSlot.teacher_id)?.last_name}
                </span>
              </div>
