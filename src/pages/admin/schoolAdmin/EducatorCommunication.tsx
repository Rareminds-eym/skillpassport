...
const EducatorCommunication = () => {
    ...
    const createConversationWithEducator = async () => {
      ...
      try {
        ...
        const existingConversation = activeConversations.find(
          (conv) => conv.educator_id === targetEducator.targetEducatorId
        );
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async ({ conversationId }) => {
      ...
      const previousActive = queryClient.getQueryData([
        'school-admin-educator-conversations',
        schoolId,
        'active',
      ]);
  ...
  const handleNewConversation = useCallback(
    async (educatorId: string, subject: string) => {
      if (!schoolId) return;
      ...
      try {
        ...
        // Check if conversation already exists
        const existingConversation = activeConversations.find(
          (conv) => conv.educator_id === educatorId
        );
                ...
                <div className="flex flex-col flex-1">
                  ...
                  {searchQuery && (
                    <p className="text-sm text-gray-500 mt-1">
                      {filteredContacts.length} conversation
                      {filteredContacts.length !== 1 ? 's' : ''} found for "{searchQuery}"
                    </p>
                  )}
                </div>
                {!showArchived && (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3">
                        ...
                        // @ts-expect-error - Auto-suppressed for migration
                        {isAnyoneTyping() && (
                          <p className="text-xs text-blue-600 italic">{getTypingText()}</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center h-full">
                      <div className="text-center">
                        ...
                        <p className="text-gray-400 text-sm">
                          Start the conversation with this educator
                        </p>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="flex-1 flex items-center justify-center bg-gray-50">
                <div className="text-center">
                  ...
                  <p className="text-gray-600 mb-6">
                    Select a conversation to start messaging with educators
                  </p>
                  <button
                    onClick={() => setShowNewConversationModal(true)}
