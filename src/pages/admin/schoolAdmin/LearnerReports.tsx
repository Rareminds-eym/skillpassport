...
const logger = getLogger('school-admin-learner-reports');

const LearnerReports: React.FC = () => {
  const [learners, setlearners] = useState<LearnerReportData[]>([]);
  ...
  const fetchSchoolIdAndReports = async () => {
    try {
      ...
      setlearners(enrichedlearners);
    } catch (error) {
      logger.error('Error fetching learner reports', error as Error);
    } finally {
      setLoading(false);
    }
  };

  const generateReport = async (learnerId: string, type: string) => {
    if (!schoolId) {
    ...
    try {
      ...
      switch (type) {
        case 'attendance':
          result = await learnerReportService.generateAttendanceReport(learnerId, schoolId, academicYear);
          break;
        case 'academic':
          result = await learnerReportService.generateAcademicReport(learnerId, schoolId, academicYear);
          break;
        case 'career_readiness':
          result = await learnerReportService.generateCareerReadinessReport(learnerId, schoolId, academicYear);
          break;
      }
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `learner_reports_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading learner reports...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base mt-2 text-gray-600">
              Comprehensive learner performance and attendance reports
            </p>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 mt-1">
                {learners.length > 0
                  ? (learners.reduce((sum, s) => sum + s.attendancePercentage, 0) / learners.length).toFixed(1)
                  : 0}%
              </p>
            </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 mt-1">
                {learners.length > 0
                  ? (learners.reduce((sum, s) => sum + s.averageScore, 0) / learners.length).toFixed(1)
                  : 0}%
              </p>
            </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 mt-1">
                {learners.reduce((sum, s) => sum + s.assessmentCount, 0)}
              </p>
            </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Learner Performance Overview</h2>
        </div>
                ...
                <tr key={learner.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      ...
                      <div className="text-sm text-gray-500">Roll: {learner.rollNumber}</div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-2 text-xs text-gray-500">
                        ({learner.presentDays}/{learner.totalDays})
                      </div>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                      <div
                        className={`h-1.5 rounded-full ${
                          learner.attendancePercentage >= 75 ? 'bg-green-600' : 'bg-red-600'
                        }`}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {learner.assessmentCount}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      learner.averageScore >= 75
                        ? 'bg-green-100 text-green-800'
                        : learner.averageScore >= 50
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="flex gap-2">
                      <button
                        onClick={() => generateReport(learner.id, 'attendance')}
                      ...
                      <button
                        onClick={() => generateReport(learner.id, 'academic')}
        ...
        {learners.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              No learner data available for reports.
            </p>
          </div>
        )}
      </div>
