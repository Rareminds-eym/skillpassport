...
const ExamsAssessments: React.FC = () => {
  ...
  const checkDateOverlap = useCallback((newExam: Partial<Exam>) => {
  ...
  // Event handlers
  const handleSaveExam = useCallback(async (examData: Partial<Exam>) => {
    try {
  ...
  const handleCreateInvigilationAssignment = useCallback(async (examId: string, assignment: unknown) => {
