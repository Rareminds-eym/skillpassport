...
export const exportFeeStructurePDF = (structure: FeeStructure, collegeName?: string) => {
  ...
  const infoData = [
    ...
    ["Effective From", structure.effective_from ? new Date(structure.effective_from).toLocaleDateString() : "-"],
    ["Effective To", structure.effective_to ? new Date(structure.effective_to).toLocaleDateString() : "Ongoing"],
  ];
  ...
  const discountData = [
    ...
    ["Scholarship Amount", structure.scholarship_applicable ? `₹${(structure.scholarship_amount || 0).toLocaleString()}` : "N/A"],
