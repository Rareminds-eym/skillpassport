...
const MentorAllocation: React.FC = () => {
  ...
  const transformMentorToLegacy = (mentor: any): LegacyMentor => ({
    ...
    allocations:
      mentor.allocations?.map((allocation: any) => ({
  ...
  const [selectedMentorForAllocation, setSelectedMentorForAllocation] =
    useState<LegacyMentor | null>(null);
  ...
  const [allocationForConfig, setAllocationForConfig] = useState<LegacyMentorAllocation | null>(
    null
  );
  ...
  const [interventionType, setInterventionType] = useState<
    'academic' | 'personal' | 'career' | 'attendance' | 'behavioral' | 'financial' | 'other'
  >('academic');
  ...
  const filteredMentors = useMemo(() => {
    return mentors.filter(
      (m) =>
        ...
        m.allocations.some(
          (allocation) => allocation.status === 'active' && allocation.students.length > 0
        ) &&
  ...
  const filteredStatistics = useMemo(() => {
    ...
    const uniqueFilteredStudents = Array.from(uniqueStudentIds).map(
      (id) => filteredMentorStudents.find((s) => s.id === id)!
    );
    ...
    const filteredMentorUuids = filteredMentors
      .map((mentor) => {
        return dynamicMentors.find(
          (m) => parseInt(m.id.replace(/-/g, '').substring(0, 8), 16) === mentor.id
        )?.id;
      })
      .filter(Boolean);
  ...
  const getMentorCurrentLoadLegacy = (mentorId: number) => {
    const mentorUuid = dynamicMentors.find(
      (m) => parseInt(m.id.replace(/-/g, '').substring(0, 8), 16) === mentorId
    )?.id;
  ...
  const handleAllocateStudents = async (
    ...
    try {
      ...
      const studentUuids = studentIds
        .map(
          (id) =>
            dynamicUnallocatedStudents.find(
              (s) => parseInt(s.id.replace(/-/g, '').substring(0, 8), 16) === id
            )?.id
        )
      ...
      if (!activePeriod) {
        ...
        console.log(
          '🔄 [handleAllocateStudents] No matching period found, creating new period for dates:',
          allocationPeriod
        );
  ...
  const handleAddIntervention = async () => {
    ...
    try {
      ...
      const mentorUuid = dynamicMentors.find(
        (m) => parseInt(m.id.replace(/-/g, '').substring(0, 8), 16) === selectedMentor.id
      )?.id;

      const studentUuid = dynamicStudents.find(
        (s) => parseInt(s.id.replace(/-/g, '').substring(0, 8), 16) === selectedStudent.id
      )?.id;
  ...
  const handleReassignStudent = async (newMentorId: number, newPeriodId: number) => {
    ...
    try {
      ...
      const newMentorUuid = dynamicMentors.find(
        (m) => parseInt(m.id.replace(/-/g, '').substring(0, 8), 16) === newMentorId
      )?.id;

      const studentUuid = dynamicStudents.find(
        (s) => parseInt(s.id.replace(/-/g, '').substring(0, 8), 16) === studentToReassign.id
      )?.id;

      // Convert period ID to UUID
      const newPeriodUuid = periods.find(
        (p) => parseInt(p.id.replace(/-/g, '').substring(0, 8), 16) === newPeriodId
      )?.id;
      ...
      const currentMentorUuid = dynamicMentors.find((m) =>
        m.allocations?.some((allocation) => allocation.students?.some((s) => s.id === studentUuid))
      )?.id;
      ...
      if (!allocationId) {
        toast.error(
          'No active allocation found for this student. Please check the allocation status.'
        );
        return;
      }
        ...
        if (currentMentorId) {
          ...
          if (refreshedMentor) {
            console.log('✅ [handleReassignStudent] Updating drawer with refreshed mentor:', {
              ...
              totalStudents: refreshedMentor.allocations.reduce(
            ...
            console.warn(
              '⚠️ [handleReassignStudent] Could not find refreshed mentor in updated array'
            );
          }
        }
  ...
  const handleCapacityConfiguration = async (
    ...
    try {
      ...
      const mentor = dynamicMentors.find(
        (m) => parseInt(m.id.replace(/-/g, '').substring(0, 8), 16) === allocation.mentorId
      );
      ...
      console.log(
        '🔧 [handleCapacityConfiguration] Found mentor:',
        mentor.first_name,
        mentor.last_name
      );
      console.log(
        '🔧 [handleCapacityConfiguration] Mentor allocations:',
        mentor.allocations.length
      );
      ...
      const mentorAllocation = mentor.allocations.find((a) => {
        const allocationMatches =
          parseInt(a.id.replace(/-/g, '').substring(0, 8), 16) === allocationId;
        console.log(
          '🔧 [handleCapacityConfiguration] Checking allocation:',
          a.id,
          'matches:',
          allocationMatches
        );
      ...
      setTimeout(() => {
        if (currentMentorId) {
          ...
          if (refreshedMentor) {
            console.log(
              '🔄 [handleCapacityConfiguration] Refreshing mentor details with updated data'
            );
            setSelectedMentor(refreshedMentor);
      ...
      console.error(
        '❌ [handleCapacityConfiguration] Error updating allocation configuration:',
        error
      );
  ...
  const handleAddStudentsComplete = async (studentIds: number[]) => {
    ...
    try {
      // Convert legacy IDs to UUIDs
      const mentorUuid = dynamicMentors.find(
        (m) => parseInt(m.id.replace(/-/g, '').substring(0, 8), 16) === mentorForAddingStudents.id
      )?.id;
      ...
      if (!activePeriod) {
        ...
        console.log(
          '🔄 [handleAddStudentsComplete] No current period found, creating new period...'
        );
        ...
        try {
          activePeriod = await createPeriod(defaultPeriodData);
          console.log(
            '✅ [handleAddStudentsComplete] New period created successfully:',
            activePeriod
          );
        } catch (periodError) {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="space-y-6 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-purple-50 via-indigo-50 to-blue-50 rounded-2xl p-6 border border-purple-100">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">Mentor Allocation</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-yellow-600 text-sm mt-1">
                  Unable to load mentor allocation data. Please ensure you are logged in with a
                  valid college account.
                </p>
              </div>
            </div>
          </div>
                    ...
                    <div className="flex-1 mb-4">
                              ...
                              return (
                                <div>
                                  <div className="font-medium truncate">
                                    {currentAllocation.allocationPeriod.startDate} -{' '}
                                    {currentAllocation.allocationPeriod.endDate}
                                  </div>
                                  <div className="text-xs text-gray-500 truncate">
                                    {currentAllocation.academicYear}
                                  </div>
                                </div>
                              );
                            }
                              ...
                              return (
                                <div>
                                  <div className="truncate">
                                    {allocation.allocationPeriod.startDate} -{' '}
                                    {allocation.allocationPeriod.endDate}
                                  </div>
                                  <div className="text-xs text-gray-500 truncate">
                            ...
                            return (
                              <div>
                                <div className="font-medium">
                                  {activeAllocations.length} active periods
                                </div>
                                <div className="text-xs text-gray-500">
                                  Multiple allocation timeframes
                                </div>
                              </div>
                            );
                          })()}
                      ...
                      <div className="h-6 flex items-center gap-3">
                        {getMentorAtRiskStudentsLegacy(mentor.id).length > 0 && (
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-xs text-red-600 font-medium">
                              {getMentorAtRiskStudentsLegacy(mentor.id).length} at-risk student
                              {getMentorAtRiskStudentsLegacy(mentor.id).length > 1 ? 's' : ''}
                            </span>
                          </div>
                        )}
                        ...
                        {(() => {
                          ...
                          const mentorNotes = dynamicNotes.filter(
                            (n) => n.mentor_id === mentorUuid
                          );
                          const notesWithResponse = mentorNotes.filter(
                            (n) =>
                              (n.educator_response || n.action_taken) && n.status !== 'completed'
                          );
                          const pendingNotes = mentorNotes.filter(
                            (n) => !n.educator_response && !n.action_taken && n.status === 'pending'
                          );
                            ...
                            return (
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-xs text-green-600 font-medium">
                                  {notesWithResponse.length} response
                                  {notesWithResponse.length > 1 ? 's' : ''}
                                </span>
                              </div>
                            );
                          }
                              ...
                              activeAllocations.length > 0
                                ? Math.max(
                                    ...activeAllocations.map(
                                      ...
                                      (a) => a.period?.default_mentor_capacity || a.capacity || 15
                                    )
                                  )
                                : 15; // Default capacity
            ...
            {mentors.length === 0 && (
              <div className="text-center py-16 bg-gray-50 rounded-xl">
                ...
                <p className="text-gray-600 mb-4 max-w-md mx-auto">
                  No mentors are currently available for allocation. Please add mentors to get
                  started with the mentoring program.
                </p>
        ...
        {showMentorSelectionModal && (
          <MentorSelectionModal
            selectedStudents={
              selectedStudentsForAllocation.map(
                (id) => availableStudents.find((s) => s.id === id)!
              ) as any
            }
        ...
        {showFeedbackModal && selectedNoteForFeedback && (
          <InterventionFeedbackModal
            ...
            studentName={
              dynamicStudents.find((s) => s.id === selectedNoteForFeedback.student_id)?.name ||
              'Unknown Student'
            }
            mentorName={(() => {
          ...
          <MentorDetailsDrawer
            ...
            onViewConversation={(note: any) => {
              ...
              const fullNote =
                note._fullNote ||
                dynamicNotes.find(
                  (n) => parseInt(n.id.replace(/-/g, '').substring(0, 8), 16) === note.id
                );
              if (fullNote) {
        ...
        {showAddStudentsModal && mentorForAddingStudents && (
          <StudentSelectionModal
            ...
            description={`Select students to assign to ${mentorForAddingStudents.name} (${mentorForAddingStudents.designation}). Available capacity: ${(() => {
              ...
              const activeAllocations = getMentorActiveAllocationsLegacy(
                mentorForAddingStudents.id
              );
              const maxCapacity =
                activeAllocations.length > 0
                  ? Math.max(...activeAllocations.map((a) => a.capacity))
