...
const CollegeAdminCourses: React.FC = () => {
  ...
  useEffect(() => {
    const loadEducatorAndCourses = async () => {
      try {
        ...
        logger.info('✅ Courses loaded:', { value: coursesData.length, 'courses' });
  ...
  const handleArchiveCourse = async (course: Course) => {
    ...
    logger.info('Archiving course:', { value: course.id, 'New status:', newStatus });
  ...
  const handleViewAnalytics = (course: Course) => {
    navigate(`/college-admin/academics/courses/${course.id}/analytics`);
  };
