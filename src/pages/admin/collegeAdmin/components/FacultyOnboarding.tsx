...
const FacultyOnboarding: React.FC<FacultyOnboardingProps> = ({ collegeId }) => {
  ...
  const handleFileChange = async (field: keyof typeof documents, files: FileList | null) => {
    ...
    if (field === 'experience_letters') {
      ...
      try {
        ...
        setMessage({
          type: 'success',
          text: `Successfully uploaded ${successfulUploads.length} experience letters`,
        });
      } catch (error) {
        setUploadStatus((prev) => ({
          ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!collegeId) {
      setMessage({
        type: 'error',
        text: "College ID not found. Please ensure you're logged in as a college admin.",
      });
      setLoading(false);
      if (typeof window !== 'undefined') {
        window.facultyOnboardingInProgress = false;
    ...
    try {
      ...
      if (existingFaculty) {
        ...
        setMessage({
          type: 'error',
          text: 'Employee ID already exists. Please choose a different ID.',
        });
        setLoading(false);
        if (typeof window !== 'undefined') {
          window.facultyOnboardingInProgress = false;
    ...
    try {
      ...
      let uploadedExperienceUrls: string[] = (experienceUrls || []).filter(
      ...
      if (documents.experience_letters.length > 0 && uploadedExperienceUrls.length === 0) {
        try {
          const results = await uploadMultipleFiles(
            documents.experience_letters,
            `faculty/${facultyId}/experience`
          );
          uploadedExperienceUrls = results
            .map((r) => r.url)
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">Faculty Onboarding</h1>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Subject Expertise */}
          <div
            className={`bg-gray-50 rounded-xl p-6 border ${validationErrors.subjects ? 'border-red-300' : 'border-gray-200'}`}
          >
