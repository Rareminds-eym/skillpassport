...
export const useTimetableData = ({ collegeId }: UseTimetableDataProps) => {
  ...
  const loadFacultyClasses = async (facultyId: string): Promise<CollegeClass[]> => {
    ...
    if (assignments && assignments.length > 0) {
      const assignedClasses = assignments.map((a: any) => a.college_classes).filter(Boolean);
  ...
  const loadSubstitutions = async (startDate?: string, endDate?: string) => {
    ...
    if (data) {
      const processed: Substitution[] = data.map((s: any) => ({
        ...
        original_faculty_name: s.original
          ? `${s.original.first_name || ''} ${s.original.last_name || ''}`.trim()
          : '',
        substitute_faculty_id: s.substitute_faculty_id,
        substitute_faculty_name: s.substitute
          ? `${s.substitute.first_name || ''} ${s.substitute.last_name || ''}`.trim()
          : null,
        status: s.status,
      }));
  ...
  const savePeriods = async (newPeriods: TimePeriod[]) => {
    ...
    try {
      ...
      await supabase.from('college_time_periods').delete().eq('timetable_id', timetableId);
