...
const FacultyPerformanceAnalytics: React.FC<FacultyPerformanceAnalyticsProps> = ({ collegeId }) => {
  ...
  const stats = useMemo(() => {
    ...
    const avgTeachingHours =
      activeFaculty > 0
        ? Math.round(
            (facultyData.reduce((sum, f) => sum + f.teachingHours, 0) / activeFaculty) * 10
          ) / 10
        : 0;
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <div className="bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 rounded-2xl p-6 border border-indigo-100">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">Faculty Analytics</h1>
            ...
            {statusDistribution.unassigned > 0 && (
              <div className="bg-white rounded-lg p-3 border border-amber-200">
                <p className="text-sm text-amber-800">
                  <span className="font-bold">{statusDistribution.unassigned}</span> faculty members
                  have no classes assigned
                </p>
              </div>
            )}
            {statusDistribution.overloaded > 0 && (
              <div className="bg-white rounded-lg p-3 border border-red-200">
                <p className="text-sm text-red-800">
                  <span className="font-bold">{statusDistribution.overloaded}</span> faculty members
                  may be overloaded (20+ slots/week)
                </p>
              </div>
            )}
          </div>
              ...
              {filteredData.map((faculty) => (
                <tr key={faculty.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`font-semibold ${faculty.totalSlots > 0 ? 'text-indigo-600' : 'text-gray-400'}`}
                    >
                      {faculty.totalSlots}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center text-sm text-gray-600">
                    {faculty.teachingHours}h
                  </td>
                  <td className="px-4 py-3 text-center text-sm text-gray-600">
                    {faculty.classesCount}
                  </td>
                  <td className="px-4 py-3 text-center text-sm text-gray-600">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${STATUS_CONFIG[faculty.status].color}`}
                    >
