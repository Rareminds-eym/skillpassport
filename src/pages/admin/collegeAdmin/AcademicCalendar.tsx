...
const AcademicCalendar: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'all' | 'academic_year' | 'semester' | 'holiday' | 'exam_window' | 'ia_window'
  >('all');
  ...
  const handleLockEvent = async (id: string) => {
    try {
      // Mock implementation - replace with actual Supabase update
      setEvents((prev) => prev.map((e) => (e.id === id ? { ...e, status: 'locked' as const } : e)));
  ...
  const handleSaveEvent = async (data: Partial<CalendarEvent>) => {
    try {
      if (selectedEvent) {
        ...
        setEvents((prev) =>
          prev.map((e) =>
            e.id === selectedEvent.id ? { ...e, ...data, updated_at: new Date().toISOString() } : e
          )
        );
