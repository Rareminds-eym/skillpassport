...
export const CalendarTab: React.FC<Props> = ({
  ...
  const handlePrev = () =>
    calendarView === 'month' ? prevMonth() : calendarView === 'week' ? prevWeek() : prevDay();
  const handleNext = () =>
    calendarView === 'month' ? nextMonth() : calendarView === 'week' ? nextWeek() : nextDay();
  ...
  const getDayEvents = () => {
    ...
    return events.filter(
      (e) => e.start_date.split('T')[0] <= dayStr && e.end_date.split('T')[0] >= dayStr
    );
  };
        ...
        <div className="p-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
            <button
              ...
              className={`p-2 rounded ${calendarView === 'month' ? 'bg-white shadow' : 'hover:bg-gray-200'}`}
              title="Month"
            >
            ...
            <button
              ...
              className={`p-2 rounded ${calendarView === 'week' ? 'bg-white shadow' : 'hover:bg-gray-200'}`}
              title="Week"
            >
            ...
            <button
              ...
              className={`p-2 rounded ${calendarView === 'day' ? 'bg-white shadow' : 'hover:bg-gray-200'}`}
              title="Day"
            >
                      ...
                      >
                        ...
                        <div className="mb-1">
                          {isToday(dayInfo.date) ? (
                            <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-blue-500 text-white text-sm font-semibold">
                              {dayInfo.day}
                            </span>
                          ) : (
                            <span
                              className={`text-sm ${!dayInfo.isCurrentMonth ? 'text-gray-400' : 'text-gray-900'}`}
                            >
                              {dayInfo.day}
                            </span>
                          )}
                        ...
                        <div className="flex-1 flex items-center">
                          {dayIdx === 0 && (
                            <div
                              className="absolute left-0 right-0 space-y-0.5 z-10"
                              style={{ top: '50%', transform: 'translateY(-50%)' }}
                            >
                              {weekEvents.map(
                                ({ event, startCol, span, isEventStart, isEventEnd }) =>
                                  startCol === 0 && (
                                    <div
                                      key={event.id}
                                      draggable
                                    ...
                                    >
                                      <span className="truncate">
                                        ...
                                        {eventRegCounts[event.id] > 0 && isEventStart && (
                                          <span className="ml-1 opacity-75">
                                            ({eventRegCounts[event.id]}
                          ...
                          {dayIdx > 0 && (
                            ...
                            >
                              {weekEvents
                                .filter(({ startCol }) => startCol === dayIdx)
                                .map(({ event, span, isEventStart, isEventEnd }) => (
                                  <div
                                    key={event.id}
                                    draggable
                                    onDragStart={() => onDragStart(event)}
            ...
            <div className="grid grid-cols-7 gap-px bg-gray-200 rounded-lg overflow-hidden">
              {weekDays.map((date, idx) => (
                <div key={idx} className="bg-gray-50 p-2 text-center">
                  ...
                  {isToday(date) ? (
                    <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-500 text-white text-lg font-semibold mx-auto">
                      {date.getDate()}
                    </span>
                  ) : (
            ...
            <div className="space-y-2">
              {getDayEvents().length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  ...
                  <button
                    onClick={() => onQuickAdd(calendarDate)}
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      {eventRegCounts[event.id] > 0 && (
                        <span className="px-2 py-1 bg-white rounded-full text-xs font-medium">
                          {eventRegCounts[event.id]} registered
                        </span>
                      )}
                    </div>
                    {event.venue && (
      ...
      {selectedEvent && (
        <div className="w-80 bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => onSelectEvent(null)} className="p-1 hover:bg-gray-100 rounded">
          ...
          <div className="space-y-3 text-sm">
            ...
            {selectedEvent.capacity && (
              <div className="flex items-center gap-2 text-gray-600">
                ...
                <span>
                  {eventRegCounts[selectedEvent.id] || 0} / {selectedEvent.capacity} registered
                </span>
              </div>
            )}
          </div>
