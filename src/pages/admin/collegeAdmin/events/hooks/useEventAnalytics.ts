...
export interface EventAnalytics {
  ...
  // Registration stats
  registrationStats: { eventTitle: string; registered: number; capacity: number; percentage: number }[];
  // Attendance rate
...
export const useEventAnalytics = (
  ...
  return useMemo(() => {
    ...
    const publishedEvents = events.filter((e) => e.status === "published" || e.status === "completed").length;
    const avgRegistrationsPerEvent = publishedEvents > 0 ? Math.round(totalRegistrations / publishedEvents) : 0;
