...
export const useCalendar = (events: CollegeEvent[]) => {
  ...
  const getCalendarWeeks = useMemo((): CalendarDay[][] => {
    ...
    while (currentWeek.length < 7 && currentWeek.length > 0) {
      currentWeek.push({ day: nextDay, isCurrentMonth: false, date: new Date(year, month + 1, nextDay) });
      nextDay++;
    }
  ...
  const getWeekEvents = useCallback((week: CalendarDay[]): WeekEvent[] => {
    ...
    return events.filter((event) => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        if (dayStr >= eventStartStr) { startCol = i; break; }
      }
  ...
  const prevWeek = () => setCalendarDate(new Date(calendarDate.getTime() - 7 * 24 * 60 * 60 * 1000));
  const nextWeek = () => setCalendarDate(new Date(calendarDate.getTime() + 7 * 24 * 60 * 60 * 1000));
  ...
  return {
    calendarDate, calendarView, setCalendarView,
    getCalendarWeeks, getWeekDays, getWeekEvents,
    prevMonth, nextMonth, prevWeek, nextWeek, prevDay, nextDay, goToToday, isToday,
  };
};
