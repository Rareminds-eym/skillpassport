...
export const useEvents = (collegeId: string | null) => {
  ...
  const loadRegistrationCounts = useCallback(async () => {
    try {
      ...
      console.error('Failed to load registration counts');
    }
  }, []);
    ...
    try {
      ...
      if (existingEvent) {
        const { error } = await supabase
          .from('college_events')
          .update({ ...data, college_id: eventCollegeId, updated_at: new Date().toISOString() })
        ...
        const { error } = await supabase
          .from('college_events')
          .insert({ ...data, college_id: eventCollegeId, created_by: user?.id });
        if (error) throw error;
  ...
  // Drag and drop
  const handleDragStart = useCallback((event: CollegeEvent) => {
  ...
  const todayEvents = useMemo(() => {
    ...
    return events.filter(
      (e) =>
        e.start_date.split('T')[0] <= today &&
        e.end_date.split('T')[0] >= today &&
        e.status === 'published'
    );
  }, [events]);
