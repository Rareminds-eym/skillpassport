...
const LearnerCard = ({ learner, onViewProfile, onAddNote, onViewCareerPath, loadingAssessmentForLearner }: {
  learner: any;
  onViewProfile: (learner: any) => void;
  onAddNote: (learner: any) => void;
  onViewCareerPath: (learner: any) => void;
  loadingAssessmentForLearner: string | null;
}) => {
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex flex-col items-end">
          ...
          <StatusBadgeComponent status={learner.approval_status || 'pending'} />
        </div>
      </div>
...
const LearnerDataAdmission = () => {
  const logger = getLogger('college-admin-learner-data');
  ...
  const filteredAndSortedlearners = useMemo(() => {
    ...
    // Note: The useLearners hook already filters by college_id via RLS, so we get all relevant learners
    let result = learners;
    ...
    if (filters.status.length > 0) {
      result = result.filter(learner =>
        learner.approval_status && filters.status.includes(learner.approval_status.toLowerCase())
      );
    }
  ...
  const handleViewProfile = (learner: any) => {
    // Debug: Log the learner data to see what fields are available
    logger.info('Learner data passed to profile drawer:', {
      id: learner.id,
      name: learner.name,
  ...
  const handleAddNoteClick = (learner: any) => {
  ...
  const handleViewCareerPath = async (learner: any) => {
    try {
      ...
      if (result) {
        logger.info('Found assessment result for learner:', learner.name);
        setAssessmentResult(result);
        setShowAssessmentReport(true);
      } else {
        logger.warn('No assessment result found for learner:', learner.name);
  ...
  return (
    <div className="flex flex-col h-screen">
      <div className="p-4 sm:p-6 lg:p-8 mb-2 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl md:text-3xl font-bold text-gray-900">Learner Data & Admission</h1>
          <p className="text-base md:text-lg mt-2 text-gray-600">Manage learner enrollments and profiles for your college.</p>
        </div>
        <button
                          ...
                          <span className="text-sm text-gray-800">
                            {(() => {
                              ...
                              if ((learner as any).semester && (learner as any).semester > 0) {
                                currentSem = (learner as any).semester;
                              }
                              // For college learners, calculate based on enrollment date
                              else if ((learner as any).college_id && (learner as any).enrollmentDate) {
                              ...
                              // For school learners, use grade
                              else if (learner.school_id && learner.grade) {
                                currentSem = parseInt(learner.grade) || 1;
                                totalSems = 12;
                              }
                              // Fallback
                              else {
                                currentSem = parseInt((learner as any).current_semester) || 1;
                              }
                              ...
                              if (learner.school_id) {
                                ...
                                const degreeType = learner.branch_field?.toLowerCase() || learner.dept?.toLowerCase() || '';
                              ...
                              const academicYear = (learner as any).admission_academic_year 
                                ? calculateAcademicYear((learner as any).admission_academic_year, currentSem)
                                : 'N/A';
      ...
      <AssessmentReportDrawer
        learner={learnerForReport ? {
          ...
          college: learnerForReport.college || undefined,
          college_name: learnerForReport.college || undefined,
          grade: learnerForReport.grade || learnerForReport.learner_grade || undefined,
          school_name: learnerForReport.college || undefined,
          roll_number: learnerForReport.enrollmentNumber || learnerForReport.roll_number || 'N/A',
...
export default LearnerDataAdmission;