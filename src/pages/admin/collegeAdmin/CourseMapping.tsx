    ...
    if (selectedFaculty) {
      ...
      if (selectedFaculty.currentWorkload + netIncrease > selectedFaculty.maxWorkload) {
        const availableCredits =
          selectedFaculty.maxWorkload - selectedFaculty.currentWorkload + currentMappingCredits;
        ...
        <div className="mb-5 flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 p-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-blue-800">
              Editing Mode: You can change the offering type, faculty allocation, and capacity.
              Course details cannot be modified.
            </p>
          </div>
        </div>
      )}
            ...
            {selectedCourseId && !isNewCourse && (
              <div className="mt-2 flex items-start gap-2 rounded-lg border border-blue-200 bg-blue-50 p-3">
                ...
                <p className="text-xs text-blue-700">
                  Using existing course. Course details are read-only and will be populated
                  automatically.
                </p>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <select
              value={offeringType}
              onChange={(e) =>
                setOfferingType(e.target.value as 'core' | 'dept_elective' | 'open_elective')
              }
              className="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-colors"
            >
    ...
    <div className="group relative rounded-lg border border-gray-200 bg-white p-4 hover:border-gray-300 hover:shadow-sm transition-all duration-200">
      ...
      <div className="pr-16">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span
            className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium border ${getTypeColor(mapping.type || 'core')}`}
          >
        ...
        <div className="space-y-1">
          ...
          {faculty && <div className="text-xs text-gray-500">Faculty: {faculty.name}</div>}
    ...
    >
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">From Program</label>
            <select
              value={fromProgramId}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">To Program</label>
            <select
              value={toProgramId}
      ...
      <div className="mt-6 flex justify-end gap-3">
        ...
        <button
          onClick={handleSubmit}
          disabled={
            submitting || fromSemester === '' || toSemester === '' || !fromProgramId || !toProgramId
          }
...
const ElectiveManagementModal = ({
  ...
  const [tempData, setTempData] = useState<ElectiveSetupData>({
    mappingId: '',
    facultyId: '',
    capacity: '',
  });
  ...
  const electiveMappings = mappings.filter(
    (m) => m.type === 'dept_elective' || m.type === 'open_elective'
  );
  ...
  const handleEditSave = async () => {
    ...
    try {
      await onUpdateMapping(tempData.mappingId, {
        ...
        capacity:
          typeof tempData.capacity === 'string' && tempData.capacity === ''
            ? undefined
            : Number(tempData.capacity),
      });
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500">
                      {mapping.course_code} • {mapping.credits} Credits
                    </p>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setTempData((prev) => ({ ...prev, facultyId: e.target.value }))
                          }
      ...
      {electiveMappings.length > 0 && (
        <div className="mt-6 flex justify-between items-center">
          <div className="text-sm text-gray-600">
            <span className="font-medium">{electiveMappings.length}</span> elective course
            {electiveMappings.length !== 1 ? 's' : ''} •{' '}
...
const CourseMapping: React.FC = () => {
  ...
  const loadInitialData = async () => {
    try {
      ...
      const [deptData, coursesData] = await Promise.all([getDepartments(), getCourses()]);
  ...
  const loadCourseMappings = async () => {
    try {
      const mappingsData = await getCourseMappings(
        selectedProgramId,
        selectedSemester,
        searchQuery,
  ...
  const stats = useMemo(() => {
    ...
    const electiveMappings = courseMappings.filter(
      (m) => m.type === 'dept_elective' || m.type === 'open_elective'
    ).length;
      ...
      <main className="flex flex-1 flex-col lg:flex-row gap-6">
        ...
        <aside className="w-full lg:w-80 space-y-5 flex-shrink-0">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-900 mb-3">Semester</label>
              <select
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className={`text-xs ${isLocked ? 'text-red-700' : 'text-indigo-700'}`}>
                    {isLocked
        ...
        <section className="flex-1 rounded-xl bg-white border border-gray-200 shadow-sm overflow-hidden flex flex-col">
            ...
            {!selectedDeptId || !selectedProgramId ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-sm text-gray-500 max-w-sm mb-4">
                  Please select a department and program from the sidebar to view and manage course
                  mappings.
                </p>
                    ...
                    <div className="text-left">
                      ...
                      <ol className="text-sm text-amber-700 space-y-1">
                        <li className="flex items-center gap-2">
                          <span className="w-4 h-4 bg-amber-200 text-amber-800 rounded-full text-xs flex items-center justify-center font-medium">
                            1
                          </span>
                          Select a Department
                        </li>
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">No Course Mappings</h3>
          ...
          <div className="border-t border-gray-200 px-5 py-4 bg-gray-50 flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-gray-600">
              <span className="font-semibold text-gray-900">{courseMappings.length}</span> mapping
              {courseMappings.length !== 1 ? 's' : ''} total for Semester {selectedSemester}
              {!selectedDeptId || !selectedProgramId ? (
                <span className="text-amber-600 ml-2">
                  • Select department & program to manage mappings
                </span>
              ) : null}
            </p>
            <div className="text-xs text-gray-500">
              Use Edit/Delete buttons on mapping cards to manage
            </div>
          </div>
        </section>
      </main>
