...
const LearnerCollegeAdminCommunication = () => {
  ...
  // Tab management - learners and college educators
  const [activeTab, setActiveTab] = useState<'learners' | 'college_educators'>('learners');
  ...
  // The new hook handles both learner and educator conversations based on conversation type
  ...
  useEffect(() => {
    ...
    const subscription = MessageService.subscribeToUserConversations(
      ...
      (conversation: Conversation) => {
        // Handle both learner and educator conversations
        if (conversation.conversation_type === 'learner_college_admin') {
          logger.info('Learner conversation UPDATE:', conversation);

          if (conversation.deleted_by_college_admin) {
            logger.info('Ignoring deleted learner conversation:', conversation.id);
            return;
          }
  ...
  useEffect(() => {
    const createConversationWithLearner = async () => {
      if (!targetLearner?.targetLearnerId || !collegeId || loadingConversations) {
        return;
      }
  ...
  }, [targetLearner, collegeId, activeConversations, loadingConversations, refetchActive]);
  ...
  const handleNewConversation = useCallback(async (learnerId: string, subject: string, initialMessage?: string) => {
    ...
    try {
      ...
      if (initialMessage && initialMessage.trim()) {
        ...
        logger.info('Initial message sent to learner');
      }
  ...
  const filteredContacts = useMemo(() => {
    ...
    const contacts = activeConversations.map((conv: any) => {
      if (activeTab === 'learners') {
        ...
        logger.info('Learner online check:', {
    ...
    return contacts.filter(c =>
      ...
      (c.type === 'learner' && (
        c.learnerEmail.toLowerCase().includes(query) ||
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
          ...
          <div className="w-full md:w-[400px] border-r border-gray-200 flex flex-col">
            ...
            <div className="px-6 py-5 border-b border-gray-200">
                  ...
                  <div className="relative" ref={tabDropdownRef}>
                    ...
                    >
                      ...
                      <span className="text-sm font-medium text-blue-900">
                        {activeTab === 'learners' ? 'Learners' : 'College Educators'}
                      </span>
                      {(activeTab === 'learners' ? activeConversations : activeEducatorConversations).length > 0 && (
                        <span className="bg-blue-100 text-blue-600 text-xs px-2 py-0.5 rounded-full">
                          {(activeTab === 'learners' ? activeConversations : activeEducatorConversations).length}
                        </span>
                      )}
                      ...
                      <div className="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                        ...
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-gray-500">Learner communications</div>
                          </div>
                        ...
                        <button
                          ...
                          className={`w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors flex items-center gap-3 border-t border-gray-100 ${activeTab === 'college_educators' ? 'bg-blue-50 text-blue-900' : 'text-gray-700'
                            }`}
                        >
                          <UserIcon className="w-4 h-4" />
                          <div className="flex-1">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder={`Search ${activeTab === 'learners' ? 'learner' : 'college educator'} conversations...`}
                  value={searchQuery}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-gray-500">
                          {(activeTab === 'learners' ? archivedConversations : archivedEducatorConversations).length} conversation{(activeTab === 'learners' ? archivedConversations : archivedEducatorConversations).length !== 1 ? 's' : ''}
                        </p>
                      </div>
                    </div>
                ...
                <div className="flex flex-col items-center justify-center h-full p-6 text-center">
                  ...
                  <p className="text-gray-400 text-xs mt-2 mb-4">
                    {showArchived
                      ...
                      : searchQuery
                        ? 'Try searching by learner name, email, subject, or university'
                        : 'Learners will message you about college matters'
                    }
                  </p>
                  {searchQuery && (
                    <button
              ...
              <div className="flex-1 flex items-center justify-center bg-gray-50">
                <div className="text-center max-w-md px-8">
                  ...
                  <p className="text-gray-500 leading-relaxed">
                    Choose a conversation from the list to start messaging with learners
                  </p>
                </div>
              </div>
            )}
      ...
      {/* New Learner Conversation Modal */}
      {activeTab === 'learners' && (
