...
} from '../../../services/college/curriculumService';
...
const CollegeCurriculumBuilderContent: React.FC = () => {
  ...
  const [status, setStatus] = useState<
    'draft' | 'approved' | 'published' | 'pending_approval' | 'rejected'
  >('draft');
      ...
      if (unit.id && units.find((u) => u.id === unit.id)) {
        ...
        if (result.success && result.data) {
          ...
          if (status === 'approved' || status === 'published') {
            setStatus('draft');
            toast(
              'Curriculum moved to draft status. Please get approval before publishing again.',
              {
                icon: 'ℹ️',
                duration: 4000,
              }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (status === 'approved' || status === 'published') {
          setStatus('draft');
          toast(
            'Unit deleted. Curriculum moved to draft status - please get approval before publishing again.',
            {
              icon: 'ℹ️',
              duration: 4000,
            }
      ...
      if (outcome.id && learningOutcomes.find((lo) => lo.id === outcome.id)) {
        ...
        if (result.success && result.data) {
          setLearningOutcomes((prev) =>
            prev.map((lo) => (lo.id === outcome.id ? result.data! : lo))
          );
          // If curriculum was approved or published and is being edited, set to draft for re-approval
          if (status === 'approved' || status === 'published') {
            setStatus('draft');
    ...
    try {
      ...
      if (result.success) {
        ...
        if (status === 'approved' || status === 'published') {
          setStatus('draft');
          toast(
            'Learning outcome deleted. Curriculum moved to draft status - please get approval before publishing again.',
            {
              icon: 'ℹ️',
              duration: 4000,
            }
  ...
  const handleApprove = async () => {
    ...
    setConfirmModal({
      ...
      message:
        'Are you sure you want to approve this curriculum? Once approved, it will be ready for publishing.',
  ...
  const handlePublish = async () => {
    ...
    setConfirmModal({
      ...
      message:
        'Are you sure you want to publish this curriculum? It will be immediately available to students and faculty.',
  ...
  const handleRequestApproval = async (message?: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.success(
          'Curriculum submitted for approval! You will be notified when it is reviewed.'
        );
      } else {
  ...
  const confirmPublish = async () => {
    ...
    try {
      ...
      if (result.success) {
        setStatus('published');
        toast.success(
          'Curriculum published successfully! It is now active and available to students and faculty.'
        );
      } else {
  ...
  return (
    <>
      ...
      <ConfirmationModal
        ...
        type={
          confirmModal.type === 'deleteUnit' || confirmModal.type === 'deleteOutcome'
            ? 'danger'
            : 'warning'
        }
        confirmText={
          confirmModal.type === 'deleteUnit'
            ? 'Delete Unit'
            : confirmModal.type === 'deleteOutcome'
              ? 'Delete Outcome'
