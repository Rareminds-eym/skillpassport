...
import { useLearners, UICandidate } from '@/entities/learner';
...
import { EditLearnerModal } from '@/features/educator';
import { DeleteLearnerModal } from '@/features/educator';
import { BulkDeleteLearnersModal } from '@/features/educator';
...
const LearnerCard = ({ learner, onViewProfile, onEdit, onDelete, onMessage, isSelected, onToggleSelect, educatorType }: {
  learner: UICandidate;
  onViewProfile: (learner: UICandidate) => void;
  onEdit: (learner: UICandidate) => void;
  onDelete: (learner: UICandidate) => void;
  onMessage: (learner: UICandidate) => void;
  isSelected?: boolean;
      ...
      <div className="flex-1">
        ...
        <div className="flex items-start justify-between mb-3 pr-8">
          <div className="flex-1 min-w-0">
            <h3 className="font-medium text-gray-900 truncate">{learner.name}</h3>
            <p className="text-sm text-gray-500 truncate">{learner.dept}</p>
            <p className="text-xs text-gray-400 truncate">{learner.college} • {learner.location}</p>
          </div>
          <div className="flex flex-col items-end ml-3 flex-shrink-0">
            <div className="flex items-center mb-1">
              <StarIcon className="h-4 w-4 text-yellow-400 fill-current" />
              <span className="text-sm font-medium text-gray-700 ml-1">{learner.ai_score_overall}</span>
            </div>
        ...
        <div className="mb-3">
          <div className="flex flex-wrap gap-1">
            {learner.skills.slice(0, 5).map((skill, index: number) => {
            ...
            {learner.skills.length > 5 && (
              <span className="text-xs text-gray-500">+{learner.skills.length - 5} more</span>
            )}
          </div>
        </div>
...
const LearnersPage = () => {
  ...
  const [learnerToEdit, setlearnerToEdit] = useState<UICandidate | null>(null);
  const [learnerToDelete, setlearnerToDelete] = useState<UICandidate | null>(null);
  ...
  const { learners, loading, error, refetch } = useLearners({ 
  ...
  const skillOptions = useMemo(() => {
    ...
    // Apply search filter and other active filters (except skills) to get base filtered learners
    let baseFilteredlearners = [...learners];
    
    // Apply search query
    if (searchQuery && searchQuery.trim() !== '') {
      const query = searchQuery.toLowerCase().trim();
      baseFilteredlearners = baseFilteredlearners.filter(learner => {
    ...
    // Count how many learners would match each skill filter
    const skilllearnerSets: Record<string, Set<string>> = {};
    
    baseFilteredlearners.forEach(learner => {
      const skillsToCheck = learner.skills;
      if (skillsToCheck && Array.isArray(skillsToCheck)) {
        skillsToCheck.forEach(skill => {
          const skillName = typeof skill === 'string' ? skill : (skill && typeof skill === 'object' && 'name' in skill) ? skill.name : undefined;
          if (skillName) {
            const normalizedSkill = skillName.toLowerCase();
            if (!skilllearnerSets[normalizedSkill]) {
              skilllearnerSets[normalizedSkill] = new Set();
            }
            skilllearnerSets[normalizedSkill].add(learner.id);
          }
        });
      }
    });
    ...
    // Convert to counts
    Object.entries(skilllearnerSets).forEach(([skill, learnerIds]) => {
      skillCounts[skill] = learnerIds.size;
    });
    ...
    return Object.entries(skillCounts)
      ...
      .slice(0, 20);
  }, [learners, searchQuery, filters.minScore, filters.maxScore]);
  ...
  const filteredAndSortedlearners = useMemo(() => {
    ...
    // Apply filters
    result = result.filter(learner => {
      // Skill filters - OR logic: learner must have at least ONE of the selected skills
      if (filters.skills.length > 0) {
        const learnerSkills = learner.skills?.map(s => (typeof s === 'string' ? s : s.name)?.toLowerCase()) ?? [];
        const hasAnySkill = filters.skills.some(fs => learnerSkills.includes(fs.toLowerCase()));
        if (!hasAnySkill) {
          return false;
        }
      }
  ...
  const handleEditClick = (learner: UICandidate) => {
  ...
  const handleDeleteClick = (learner: UICandidate) => {
  ...
  const handleSelectAll = () => {
    ...
    const allIds = new Set(paginatedlearners.map(s => s.id));
  ...
  // Handle message learner
  const handleMessageLearner = async (learner: UICandidate) => {
    try {
      ...
      navigate(navigationPath, {
        state: {
          ...
          targetlearnerName: learner.name,
          targetlearnerEmail: learner.email
        }
      });
    } catch (error) {
  ...
  const selectedlearners = learners.filter(s => selectedLearnerIds.has(s.id));
  const allOnPageSelected = paginatedlearners.length > 0 && paginatedlearners.every(s => selectedLearnerIds.has(s.id));
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="p-4 sm:p-6 lg:p-8 mb-2 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl md:text-3xl font-bold text-gray-900">Learners Management</h1>
          <p className="text-base md:text-lg mt-2 text-gray-600">Manage your learners and their profiles.</p>
        </div>
        <button
        ...
        <div className="w-80 flex-shrink-0 pr-4 text-left">
          <div className="inline-flex items-baseline">
            ...
            <span className="ml-2 text-sm text-gray-500">
              ({totalItems} {searchQuery || filters.skills.length > 0 ? 'matching' : ''} learners)
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {selectedLearnerIds.size > 0 && educatorType !== 'college' && (
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  >
                    ...
                    Delete {selectedLearnerIds.size} Learner{selectedLearnerIds.size > 1 ? 's' : ''}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="px-4 sm:px-6 lg:px-8 flex-1 overflow-y-auto p-4">
            {viewMode === 'grid' ? (
              <div className={`grid grid-cols-1 gap-4 items-stretch ${
                ...
                {(loading || schoolLoading) && <div className="text-sm text-gray-500">Loading learners...</div>}
                {error && <div className="text-sm text-red-600">{error}</div>}
                {!loading && !schoolLoading && paginatedlearners.map((learner) => (
                  <LearnerCard
                    key={learner.id}
                    learner={learner}
                    ...
                    onMessage={handleMessageLearner}
                    isSelected={selectedLearnerIds.has(learner.id)}
                    onToggleSelect={handleToggleLearner}
                    educatorType={educatorType}
                  />
                ))}
                {!loading && !schoolLoading && paginatedlearners.length === 0 && !error && (
      ...
      {/* Delete Learner Modal - Hidden for college lecturers */}
      {educatorType !== 'college' && (
        <DeleteLearnerModal
      ...
      {/* Bulk Delete Learners Modal - Hidden for college lecturers */}
      {educatorType !== 'college' && (
        <BulkDeleteLearnersModal
