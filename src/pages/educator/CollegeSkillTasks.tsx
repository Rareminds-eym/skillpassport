...
export default function CollegeSkillTasks() {
  ...
  const filteredSections = useMemo(() => {
    ...
    const filtered = programSections.filter((section) => {
      const deptMatch =
        selectedDepartment === 'all' || section.department_id === selectedDepartment;
  ...
  const sectionsForDropdown = useMemo(() => {
    ...
    filteredSections.forEach((s, i) => {
      ...
      console.log(`     Semester: ${s.semester}, Section: ${s.section}`);
      console.log(
        `     Matches form program? ${s.program_id === taskForm.program_id ? '✅ YES' : '❌ NO'}`
      );
    });
  ...
  const handleProgramChange = async (programId: string) => {
    ...
    if (programId) {
      ...
      if (user) {
        console.log('👤 COMPONENT: User ID:', user.id);
        const { data, error } = await collegeAssignmentService.fetchEducatorCoursesByProgram(
          user.id,
          programId
        );
        if (error) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Create, manage, and evaluate skill-based assignments
              </p>
            </div>
            <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredAssignments.map((assignment) => (
                  <div
                    ...
                    className="border border-gray-200 rounded-lg p-5 hover:shadow-lg transition-shadow bg-white relative"
                  >
                    {/* 3-Dot Menu */}
                    ...
                    <div className="pr-8">
                      ...
                      <button
                        onClick={async () => {
                          console.log(
                            '🎯 Assign button clicked for assignment:',
                            assignment.assignment_id
                          );
                          console.log(
                            '📋 Assignment program_section_id:',
                            assignment.program_section_id
                          );
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleTaskFormChange('total_points', parseInt(e.target.value))
                        }
                        min="1"
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      />
                  ...
                  <div className="mt-4">
                    <label className="flex items-center">
                      <input
                        ...
                        onChange={(e) =>
                          handleTaskFormChange('allow_late_submission', e.target.checked)
                        }
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xl font-semibold text-gray-900">
                    Assign to Section Students
                  </h2>
                  <p className="text-sm text-gray-600 mt-1">
                    Select students from the chosen program section
                  </p>
                </div>
                <button
                  onClick={() => setAssignStudentsModal(false)}
                ...
                {filteredStudents.length === 0 ? (
                  <div className="p-8 text-center">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      No students are enrolled in the selected program section.
                    </p>
                  </div>
                ) : (
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    {selectedAssignment.semester && selectedAssignment.section && (
                      <p className="text-sm text-blue-700">
                        Semester {selectedAssignment.semester} - Section{' '}
                ...
                {selectedAssignment.instruction_files &&
                  selectedAssignment.instruction_files.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-2">
                        {selectedAssignment.instruction_files.map(
                          (
            ...
            <div className="inline-block w-full max-w-md align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:p-6">
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-sm text-gray-600">
                  ...
                  <span className="font-medium">{selectedAssignment.title}</span>"? This action
                  cannot be undone.
                </p>
              </div>
