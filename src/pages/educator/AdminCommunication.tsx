...
const AdminCommunication = () => {
  ...
  const handleNewConversation = useCallback(
    async (subject: string) => {
      ...
      try {
        console.log('🆕 Creating new conversation with school admin:', {
          educatorRecordId,
          schoolId,
          subject,
        });
        ...
        const existingConversation = activeConversations.find((conv) => conv.subject === subject);
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async ({ conversationId }) => {
      await queryClient.cancelQueries({
        queryKey: ['educator-admin-conversations', educatorRecordId],
      });

      const previousActive = queryClient.getQueryData([
        'educator-admin-conversations',
        educatorRecordId,
        'active',
      ]);
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3">
                        ...
                        // @ts-expect-error - Auto-suppressed for migration
                        {isAnyoneTyping() && (
                          <p className="text-xs text-green-600 italic">{getTypingText()}</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center h-full">
                      <div className="text-center">
                        ...
                        <p className="text-gray-400 text-sm">
                          Start the conversation with school administration
                        </p>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="flex-1 flex items-center justify-center bg-gray-50">
                <div className="text-center">
                  ...
                  <p className="text-gray-600 mb-6">
                    Select a conversation to start messaging with school administration
                  </p>
                  <button
                    onClick={() => setShowNewConversationModal(true)}
