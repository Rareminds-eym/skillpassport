...
const AdminCommunication = () => {
  ...
  const { messages, isLoadingMessages: loadingMessages, sendMessage, isSending } = useEducatorMessages(
    educatorRecordId || '',
    {
      ...
      enabled: !!selectedConversationId && !!educatorRecordId,
      enableRealtime: true,
    }
  );
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async ({ conversationId }) => {
      await queryClient.cancelQueries({ queryKey: queryKeys.educator.admin.all });
                      ...
                      {displayMessages.map((message) => (
                        ...
                        >
                          ...
                          >
                            ...
                            <div className={`flex items-center justify-end gap-1 mt-1 ${message.sender === 'me' ? 'text-green-100' : 'text-gray-500'
                              }`}>
