...
const EducatorAssessmentResults: React.FC = () => {
  ...
  const { data: resultsData, isLoading: loading, error: queryError } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiPost<{ results: AssessmentResult[], schoolName: string }>('/educator/actions', {
          action: 'fetch-educator-assessment-results'
        });

        return response || { results: [], schoolName: '' };
      } catch (error) {
        logger.error('Error fetching assessment results from API', error);
        throw error;
      }
    },
