...
const StudentCard = ({ student, onViewProfile, onAddNote }) => {
  const handleViewProfile = () => {
    ...
    console.log('Certificates:', student.certificates?.length || 0);
    console.log('Education:', student.education?.length || 0);
    console.log('Experience:', student.experience?.length || 0);
    console.log('Projects:', student.projects?.length || 0);
    console.log('Trainings:', student.trainings?.length || 0);
    console.log('Skills:', student.skills?.length || 0);
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
      ...
      <div className="mb-4 space-y-1">
        ...
        {student.experience && student.experience.length > 0 && (
          <p className="text-xs text-gray-600">
            💼 {student.experience[0].organization} - {student.experience[0].role}
          </p>
        )}
        {student.projects && student.projects.length > 0 && (
          <p className="text-xs text-gray-600">
        ...
        {student.education && student.education.length > 0 && (
          <p className="text-xs text-gray-600">
            🎓 {student.education[0].degree} - {student.education[0].university}
          </p>
        )}
      </div>
...
const StudentsPage = () => {
  ...
  // Debug: Log students data when loaded
  useEffect(() => {
    if (students.length > 0) {
      console.log('Students loaded in StudentsPage:', students.length);
      console.log('Sample student structure:', {
        id: students[0].id,
        name: students[0].name,
        email: students[0].email,
        phone: students[0].phone,
        certificates: students[0].certificates?.length || 0,
        education: students[0].education?.length || 0,
        experience: students[0].experience?.length || 0,
        projects: students[0].projects?.length || 0,
        trainings: students[0].trainings?.length || 0,
        skills: students[0].skills?.length || 0,
        rawData: students[0].rawData
      });
    }
  }, [students]);
      ...
      const resultsWithScores = students.map(student => {
        ...
        if (skillsToCheck && Array.isArray(skillsToCheck)) {
          const skillMatch = skillsToCheck.some((skill: any) => {
            ...
            if (skillStr.toLowerCase().includes(query)) {
              if (!isMatch) matchedField = skillStr.toLowerCase();
              isMatch = true;
              return true;
            }
            return false;
        ...
        // Projects
        if (searchInArray(student.projects, ['title', 'description', 'techStack', 'link', 'status'], 'projects')) {
          return { student, matchedField };
        }

        // Education
        if (searchInArray(student.education, ['degree', 'university', 'department', 'level', 'cgpa'], 'education')) {
          return { student, matchedField };
        }

        // Experience
        if (searchInArray(student.experience, ['organization', 'role', 'title', 'duration'], 'experience')) {
          return { student, matchedField };
        }

        // Certificates
        if (searchInArray(student.certificates, ['title', 'issuer', 'level', 'description', 'status'], 'certificates')) {
          return { student, matchedField };
        }
  ...
  // const handleViewProfileClick = (student: any) => {
  //   console.log('=== handleViewProfileClick ===');
  //   console.log('Passing student to onViewProfile:', {
  //     id: student.id,
  //     name: student.name,
  //     email: student.email,
  //     certificatesCount: student.certificates?.length || 0,
  //     educationCount: student.education?.length || 0,
  //     experienceCount: student.experience?.length || 0,
  //     projectsCount: student.projects?.length || 0,
  //     trainingsCount: student.trainings?.length || 0,
...
const handleViewProfileClick = (student: any) => {
    console.log('=== handleViewProfileClick ===');
    console.log('Full student object being passed:', student);
    console.log('Student data breakdown:', {
      id: student.id,
      name: student.name,
      age: student.age,
      email: student.email,
      ...
      certificatesData: student.certificates,
      education: student.education?.length || 0,
      educationData: student.education,
      experience: student.experience?.length || 0,
      experienceData: student.experience,
      projects: student.projects?.length || 0,
      projectsData: student.projects,
      trainings: student.trainings?.length || 0,
      trainingsData: student.trainings,
      assessments: student.assessments?.length || 0,
      assessmentsData: student.assessments,
      skills: student.skills?.length || 0,
      skillsData: student.skills,
      rawData: student.rawData
    });
    console.log('===========================');
    
    // Ensure all related data is properly structured
                    ...
                    {paginatedStudents.map((student) => (
                      <tr key={student.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex space-x-2">
                            <button
                              onClick={() => handleViewProfileClick(student)}
                              className="text-primary-600 hover:text-primary-900"
                            >
                              View
                            </button>
