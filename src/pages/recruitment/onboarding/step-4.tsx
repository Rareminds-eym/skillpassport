/**
 * Recruitment Onboarding - Step 4: Review & Complete
 * 
...
import { CheckCircle, Loader2, ArrowLeft, Edit, Building2, Users, Settings } from 'lucide-react';
...
export default function OnboardingStep4() {
    ...
    const handleComplete = async () => {
        ...
        try {
            console.log('[OnboardingStep4] Completing onboarding with data:', data);
            ...
            console.error('[OnboardingStep4] Error completing onboarding:', error);
                ...
                <div className="space-y-6">
                    ...
                    <div className="border-2 border-slate-200 rounded-xl p-6">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button
                                type="button"
                                onClick={() => navigate('/recruitment/onboarding/step-3')}
                                className="text-blue-600 hover:text-blue-700 flex items-center gap-1 text-sm font-medium"
                            >
                                <Edit className="h-4 w-4" />
                                Edit
                        ...
                        <dl className="space-y-3 text-sm">
                            <div>
                                <dt className="text-slate-500 mb-1">Job Boards</dt>
                                <dd className="font-medium text-slate-900">
                                    {data.recruitmentPreferences.jobBoards.length > 0
                                        ? data.recruitmentPreferences.jobBoards.join(', ')
                                        : 'None selected'}
                                </dd>
                            </div>
                            <div>
                                <dt className="text-slate-500 mb-1">Email Notifications</dt>
                                <dd className="flex flex-wrap gap-2">
                                    {data.recruitmentPreferences.emailNotifications.newApplications && (
                                        <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs">New Applications</span>
                                    )}
                                    {data.recruitmentPreferences.emailNotifications.dailyDigest && (
                                        <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs">Daily Digest</span>
                                    )}
                                    {data.recruitmentPreferences.emailNotifications.weeklyReport && (
                                        <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs">Weekly Report</span>
                                    )}
                                </dd>
                            </div>
                            <div>
                                <dt className="text-slate-500 mb-1">AI Auto-Matching</dt>
                                <dd className="font-medium text-slate-900">
                                    {data.recruitmentPreferences.autoMatching ? 'Enabled' : 'Disabled'}
                                </dd>
                            </div>
                        </dl>
                    </div>
