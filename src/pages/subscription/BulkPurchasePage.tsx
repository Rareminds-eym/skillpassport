...
import BulkPurchaseWizard, {
  PurchaseData,
} from '../../components/Subscription/Organization/BulkPurchaseWizard';
...
function BulkPurchasePage() {
  ...
  useEffect(() => {
    const fetchOrganizationId = async () => {
      ...
      if (user?.school_id) {
        console.log('[BulkPurchasePage] Found school_id:', user.school_id);
        setOrganizationId(String(user.school_id));
        return;
      }
      if (user?.college_id) {
        console.log('[BulkPurchasePage] Found college_id:', user.college_id);
        setOrganizationId(String(user.college_id));
        return;
      }
      if (user?.university_id) {
        console.log('[BulkPurchasePage] Found university_id:', user.university_id);
        setOrganizationId(String(user.university_id));
        return;
      }
      if (user?.schoolId) {
        console.log('[BulkPurchasePage] Found schoolId:', user.schoolId);
        setOrganizationId(String(user.schoolId));
        return;
      }
      if (user?.collegeId) {
        console.log('[BulkPurchasePage] Found collegeId:', user.collegeId);
        setOrganizationId(String(user.collegeId));
        return;
      }
      if (user?.universityId) {
        console.log('[BulkPurchasePage] Found universityId:', user.universityId);
      ...
      if (storedUser) {
        try {
          ...
          if (userData.schoolId) {
            console.log('[BulkPurchasePage] Found schoolId in localStorage:', userData.schoolId);
            setOrganizationId(userData.schoolId);
            return;
          }
          if (userData.collegeId) {
            console.log('[BulkPurchasePage] Found collegeId in localStorage:', userData.collegeId);
            setOrganizationId(userData.collegeId);
            return;
          }
          if (userData.universityId) {
            console.log(
              '[BulkPurchasePage] Found universityId in localStorage:',
      ...
      console.log(
        '[BulkPurchasePage] Fetching from database, userId:',
        userId,
        'userEmail:',
        userEmail
      );
      ...
      try {
        ...
        if (organizationType === 'school' && userId) {
          ...
          if (educatorData?.school_id) {
            console.log(
              '[BulkPurchasePage] Found school_id from school_educators:',
        ...
        if (organizationType === 'college' && userId) {
          ...
          if (lecturerData?.collegeId) {
            console.log(
              '[BulkPurchasePage] Found collegeId from college_lecturers:',
        ...
        if (userEmail) {
          console.log(
            '[BulkPurchasePage] Querying organizations by email:',
            userEmail,
            'type:',
            organizationType
          );
        ...
        if (userId) {
          ...
          console.log(
            '[BulkPurchasePage] Organizations by admin_id result:',
            orgByAdminId,
            adminError
          );
  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      console.log(
        '[BulkPurchasePage] Fetching members for org:',
        organizationId,
        'type:',
        organizationType
      );
      ...
      console.log(
        '[BulkPurchasePage] Members fetched:',
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center max-w-md">
          ...
          <p className="text-gray-600 mb-4">
            Please log in to purchase organization subscriptions.
          </p>
          <button
