...
function MySubscription() {
  ...
  const dashboardPath = useMemo(
    () => getDashboardPathFromUrl(location.pathname),
    [location.pathname]
  );
  ...
  const confirmCancelSubscription = async () => {
    ...
    try {
      ...
      const result = await deactivateSubscription(subscriptionData.id, cancelReason, token);

      if (result.success) {
        // Success - show confirmation
        alert(
          `Subscription cancelled successfully. You'll have access until ${formatDate(subscriptionData.endDate)}`
        );
  ...
  const handlePauseSubscription = async () => {
    ...
    try {
      ...
      const result = await pauseSubscription(subscriptionData.id, pauseMonths, token);

      if (result.success) {
        alert(
          `Subscription paused for ${pauseMonths} month(s). It will automatically resume after this period.`
        );
        await refreshSubscription();
  ...
  const currentPlan = useMemo(() => {
    ...
    const planId =
      planMapping[subscriptionData.plan.toLowerCase()] || subscriptionData.plan.toLowerCase();
  ...
  const { displayFeatures, totalFeaturesCount, hasMoreFeatures, hiddenCount } = useMemo(() => {
    ...
    const hasMore =
      currentPlan?.hasMoreFeatures === true ||
    ...
    currentPlan?.totalFeatures,
    currentPlan?.hasMoreFeatures,
    subscriptionData?.features,
    showingAllFeatures,
  ]);
            ...
            {(isExpiringSoon || isExpired || isPaused) && (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p
                    className={`text-sm mt-1 ${isPaused ? 'text-orange-700' : 'text-neutral-600'}`}
                  >
                    {isExpired
                  ...
                  <div className="px-6 py-5">
                    <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                      ...
                      {subscriptionData.nextBillingDate && autoRenewEnabled && (
                        <div className="sm:col-span-2">
                          <dt className="text-xs font-medium text-neutral-500 mb-1.5">
                            Next Billing Date
                          </dt>
                          <dd className="text-sm text-neutral-900">
