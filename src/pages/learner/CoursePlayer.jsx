...
const CoursePlayer = () => {
  ...
  const enrollAndLoadProgress = async () => {
    ...
    try {
      ...
      const enrollResult = await courseEnrollmentService.enrollLearner(user.email, courseId);
  ...
  const saveVideoPosition = useCallback(async (position, duration) => {
    console.log('📹 saveVideoPosition called:', { position, duration, isLearner, userId: user?.id, courseId, currentLessonId });
    if (!isLearner || !user?.id || !courseId || !currentLessonId) {
  ...
  }, [isLearner, user?.id, courseId, currentLessonId]);
  ...
  }, [isLearner, user?.id, courseId, isYouTubeVideo, currentLessonId]);
  ...
  }, [isLearner, saveVideoPosition, isYouTubeVideo]);
  ...
  const completeCourse = async () => {
    ...
    try {
      ...
      const userFullName = [learnerRecord.users?.firstName, learnerRecord.users?.lastName].filter(Boolean).join(' ');
