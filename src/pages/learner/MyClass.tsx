 ...
 * 
 * This component intelligently routes learners to the appropriate class interface
 * based on their user role (learner vs learner) and data availability.
 * 
 * Architecture:
 * - Single entry point for all learner class interfaces
 * - Role-based routing using user.role from database
 * - Graceful fallback for learners without assigned classes/programs
...
const MyClass: React.FC = () => {
  ...
  const [learnerTypeInfo, setlearnerTypeInfo] = useState<{
    isCollege: boolean;
    isSchool: boolean;
    hasProgram: boolean;
    hasClass: boolean;
  ...
  // Determine learner type and routing
  useEffect(() => {
    const determinelearnerType = async () => {
      if (authLoading) return;
      
      if (!learnerId) {
        setLoading(false);
        setError('Learner information not found. Please ensure you are logged in correctly.');
        return;
      }

      try {
        setLoading(true);
        const typeInfo = await getlearnerTypeInfo(learnerId);
  ...
  if (learnerTypeInfo.isCollege) {
    if (!learnerTypeInfo.hasProgram) {
      return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
          <div className="text-center max-w-md">
  ...
  if (learnerTypeInfo.isSchool) {
    if (!learnerTypeInfo.hasClass) {
      return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
          <div className="text-center max-w-md">
  ...
  // Fallback for unknown learner types
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="text-center max-w-md">
