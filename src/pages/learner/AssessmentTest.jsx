...
const AssessmentTest = () => {
    ...
    const [learnerStream, setlearnerStream] = useState(null);
    ...
    // Always filter by grade - show only relevant assessment options based on learner's grade level
    ...
    // Learner grade from database
    const [learnerGrade, setlearnerGrade] = useState(null);
    const [learnerSchoolClassId, setlearnerSchoolClassId] = useState(null);
    const [learnerId, setLearnerId] = useState(null); // Learner record ID for AI question saving
    const [isCollegeLearner, setIsCollegeLearner] = useState(false);
    const [learnerProgram, setlearnerProgram] = useState(null); // Program name for college learners (BBA, BCA, BSC, etc.)
    const [gradeStartDate, setGradeStartDate] = useState(null); // When learner started current grade
    ...
    // Fetch learner's grade from database using React Query
    ...
    // Update local state when learner grade data changes
    useEffect(() => {
        if (learnerGradeData) {
            // Store complete profile data for missing field analysis
            setProfileData(learnerGradeData);

            // Save learner ID for AI question saving
            setLearnerId(learnerGradeData.id);

            // Check if learner is a college learner (using centralized utility)
            const isCollege = checkIsCollegeLearner(learnerGradeData);
            setIsCollegeLearner(isCollege);

            // Set program name if available (BBA, BCA, BSC, etc.)
            // Priority: program.name > program.code > course_name
            const programName = learnerGradeData.program?.name || learnerGradeData.program?.code || learnerGradeData.course_name;
            if (programName) {
                setlearnerProgram(programName);
            }

            // Set grade_start_date and calculate months in grade
            if (learnerGradeData.grade_start_date) {
                setGradeStartDate(learnerGradeData.grade_start_date);
                const months = calculateMonthsInGrade(learnerGradeData.grade_start_date);
                setMonthsInGrade(months);
            } else if (learnerGradeData.school_classes?.academic_year) {
                // Fallback: estimate from academic year (e.g., "2024-2025" -> started June 2024)
                const academicYear = learnerGradeData.school_classes.academic_year;
            ...
            const effectiveGrade = learnerGradeData.grade || learnerGradeData.school_classes?.grade;

            setlearnerGrade(effectiveGrade);
            setlearnerSchoolClassId(learnerGradeData.school_class_id);
        }
    }, [learnerGradeData]);
    ...
    useEffect(() => {
        ...
        const checkExistingAttempt = async () => {
            // Wait for auth hook to be ready and learner ID to be loaded
            if (dbLoading || loadinglearnerGrade) return;

            setCheckingExistingAttempt(true);

            try {
                // Use learnerId (from learners table) instead of user.id (auth user)
                if (learnerId) {
                    // First check if user can take assessment (6-month restriction)
                    const eligibility = await assessmentService.canTakeAssessment(learnerId);
                    ...
                    // User logged in but no learner record found yet, wait
                }
            } catch (err) {
                setShowGradeSelection(true);
            } finally {
    ...
    }, [learnerId, user?.id, dbLoading, loadinglearnerGrade, checkInProgressAttempt, initialCheckDone, assessmentStarted]);
    ...
    const handleResumeAssessment = async () => {
        ...
        try {
            setUseDatabase(true);
            setlearnerStream(pendingAttempt.stream_id);
    ...
    // Load AI questions for after10, higher_secondary, after12 AND college learners
    useEffect(() => {
        let cancelled = false;
        const loadAIQuestions = async () => {
            // Only require gradeLevel and learnerStream - learnerId is optional for saving
            // Support 'after10', 'higher_secondary', 'after12' and 'college' grade levels
            if ((gradeLevel === 'after10' || gradeLevel === 'higher_secondary' || gradeLevel === 'after12' || gradeLevel === 'college') && learnerStream) {
                console.log({
                    gradeLevel,
                    learnerStream,
                    learnerId: learnerId || 'NOT SET',
                ...
                try {
                    ...
                    // For college learners, pass their actual course for knowledge questions
                    const learnerCourse = (gradeLevel === 'college' && learnerProgram) ? learnerProgram : null;
                    if (learnerCourse) {
                    }

                    // Pass learnerId, attemptId, and course for save/resume functionality
                    const questions = await loadCareerAssessmentQuestions(
                        learnerStream,
                        gradeLevel,
                        learnerId || null,
                        currentAttempt?.id || null,
                        learnerCourse // Pass course for college learners
                    );
    ...
    }, [gradeLevel, learnerStream, learnerId, currentAttempt?.id, learnerProgram]);
    ...
    const sections = useMemo(() => {
        ...
        // Learners have already chosen their stream, so use full assessment with AI-generated aptitude & knowledge
        ...
        return [];
    }, [dbQuestions, learnerStream, gradeLevel, aiQuestions, aiQuestionsLoading]);
    ...
    useEffect(() => {
        if (currentSection?.isAdaptive && adaptiveQuestionTimer === 0 && adaptiveAptitude.currentQuestion && !adaptiveAptitude.submitting) {
            ...
            // If learner selected an answer, submit it
    ...
    const handleGradeSelect = async (level) => {
        ...
        if (level === 'middle') {
            ...
            setAssessmentStarted(true);
            const streamId = null; // No stream yet - learner is deciding
            setlearnerStream(streamId);
            ...
            // For higher secondary (11th/12th), learners have already chosen their stream
            // Show category selection (Science/Commerce/Arts) to get their stream
            setShowCategorySelection(true);
        } else if (level === 'college') {
            // For college learners (UG/PG), use 'college' stream for program-based assessment
            setAssessmentStarted(true);

            // Use 'college' stream for college learners (program-based assessment)
    ...
    const handleCategorySelect = async (categoryId) => {
        ...
        // For after12 and higher_secondary learners: Skip program selection, go directly to assessment
        if (effectiveGradeLevel === 'after12' || effectiveGradeLevel === 'higher_secondary') {
            ...
            // Try to create a database attempt if learner record exists
            if (learnerRecordId) {
                try {
...
const generateCourseRecommendations = (analysisResults) => {
    ...
    const courseMatches = allCourses.map(course => {
        ...
        // Check if course's RIASEC types match learner's top interests
        if (course.riasec && course.riasec.length > 0) {
            ...
            course.riasec.forEach(type => {
                ...
                // Bonus if this type is in learner's top 3
        ...
        if (course.aptitudeStrengths && course.aptitudeStrengths.length > 0) {
            ...
            course.aptitudeStrengths.forEach(strength => {
                ...
                // Bonus if this is in learner's top strengths
...
const handleSubmit = async () => {
    ...
    try {
        ...
        localStorage.setItem('assessment_stream', learnerStream);
        ...
        const questionBanks = {
            ...
            streamKnowledgeQuestions: { [learnerStream]: getQuestionsForSection(getSectionId('knowledge')) }
        };
                ...
                if (knowledgeAnswerKeys.length > 0) {
                    ...
                    if (learner) {
                        ...
                        if (questionSets?.questions) {
                            ...
                            questionBanks.streamKnowledgeQuestions = { [learnerStream]: aiKnowledgeQuestions };
                        }
                    }
                }
            } catch (fetchErr) {
        ...
        const learnerContext = {
            rawGrade: learnerGrade, // Original grade string (e.g., "PG Year 1", "Grade 10")
            programName: learnerProgram, // Program name (e.g., "MCA", "B.Tech CSE")
        ...
        if (geminiResults) {
            // Enhance results with adaptive aptitude data for high school learners
            if (gradeLevel === 'highschool' && answers.adaptive_aptitude_results) {
            ...
            // Add course recommendations for after12 learners
            ...
            if (!attemptId && (learnerRecordId || user?.id)) {
                ...
                submissionSuccess = false;
                submissionError = 'No learner record found. Please log in again.';
            }
            ...
            if (submissionSuccess && attemptId) {
                ...
                try {
                    ...
                    if (currentAttempt?.id) {
                        ...
                        const learnerIdForDb = learnerRecordId || user.id;
        ...
        if (submissionSuccess && finalAttemptId) {
            navigate(`/learner/assessment/result?attemptId=${finalAttemptId}`);
        } else {
...
// Stream selection is no longer used - college learners go directly to assessment
// Keeping this commented for reference if needed in future
/*
if (showStreamSelection) {
    // Default streams for college learners
