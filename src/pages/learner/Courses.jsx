...
import {
  Badge, Button, Card, CardContent, CardHeader, CardTitle, Pagination,
  PaginationContent,
  PaginationEllipsis,
  PaginationItem,
  PaginationLink,
  PaginationNext,
  PaginationPrevious, SearchBar
} from '@/shared/ui';
...
import { downloadCertificate, getCertificateProxyUrl } from '@/features/certificate-generation';
    ...
    const fetchlearnerInfo = async () => {
      ...
      try {
        ...
        if (res?.data) {
          ...
          // Only if courses have been fetched once
          if (hasFetchedCoursesRef.current) {
            fetchCourses();
          }
        }
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-2">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    className={`relative text-left p-4 rounded-lg transition-all ${activeTab === 'courses'
                        ? 'bg-gradient-to-r from-indigo-50 to-blue-50 shadow-md'
                        : 'bg-white hover:bg-gray-50'
                      }`}
                  >
                    <div className="flex items-start gap-3">
                      <div className={`p-2 rounded-lg ${activeTab === 'courses' ? 'bg-indigo-600' : 'bg-gray-100'
                        }`}>
                        <BookOpen className={`w-6 h-6 ${activeTab === 'courses' ? 'text-white' : 'text-gray-600'
                          }`} />
                      </div>
                      <div className="flex-1">
                        <h1 className={`font-bold text-2xl ${activeTab === 'courses' ? 'text-indigo-600' : 'text-gray-900'
                          }`}>
                          Courses
                        </h1>
                  ...
                  <button
                    ...
                    className={`relative text-left p-4 rounded-lg transition-all ${activeTab === 'progress'
                        ? 'bg-gradient-to-r from-indigo-50 to-blue-50 shadow-md'
                        : 'bg-white hover:bg-gray-50'
                      }`}
                  >
                    <div className="flex items-start gap-3">
                      <div className={`p-2 rounded-lg ${activeTab === 'progress' ? 'bg-indigo-600' : 'bg-gray-100'
                        }`}>
                        <TrendingUp className={`w-6 h-6 ${activeTab === 'progress' ? 'text-white' : 'text-gray-600'
                          }`} />
                      </div>
                      <div className="flex-1">
                        <h1 className={`font-bold text-lg ${activeTab === 'progress' ? 'text-indigo-600' : 'text-gray-900'
                          }`}>
                ...
                <div className="flex gap-2 items-center w-full lg:w-auto flex-wrap">
                  ...
                  {learnerBranch && (
                    <button
                      ...
                      className={`h-12 px-4 rounded-lg transition-all shadow-sm flex items-center gap-2 whitespace-nowrap ${filterByBranch
                        ...
                        <div className="pt-2">
                          ...
                          <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                            <motion.div
                              ...
                              className={`h-full rounded-full ${isCourseCompleted(course.course_id)
                        ...
                        <div className="flex flex-col gap-2 pt-2">
                          ...
                          <div className="flex gap-2">
                            <Button
                              ...
                              className={`flex-1 flex items-center justify-center gap-2 ${getCertificateUrl(course.course_id)
                            ...
                            <Button
                              ...
                              className={`flex-1 flex items-center justify-center gap-2 ${getCertificateUrl(course.course_id) && downloadingCertificate !== course.course_id
                            ...
                            <div className="mb-4 max-w-md">
                              ...
                              <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div
                                  ...
                                  className={`h-full rounded-full transition-all duration-500 ${isCourseCompleted(course.course_id)
                                      ? 'bg-gradient-to-r from-emerald-500 to-green-500'
                                      : 'bg-gradient-to-r from-indigo-500 to-purple-500'
                                    }`}
                                />
                          ...
                          {isCourseCompleted(course.course_id) ? (
                            <div className="flex flex-wrap gap-2">
                              ...
                              <Button
                                onClick={(e) => handleViewCertificate(course.course_id, course.title, e)}
                                disabled={!getCertificateUrl(course.course_id)}
                                className={`flex items-center gap-2 ${getCertificateUrl(course.course_id)
                                    ? 'bg-blue-600 hover:bg-blue-700 text-white'
                                    : 'bg-gray-200 text-gray-400 cursor-not-allowed'
                                  }`}
                              >
                              ...
                              <Button
                                onClick={(e) => handleDownloadCertificate(course.course_id, course.title, e)}
                                disabled={!getCertificateUrl(course.course_id) || downloadingCertificate === course.course_id}
                                className={`flex items-center gap-2 ${getCertificateUrl(course.course_id) && downloadingCertificate !== course.course_id
                                    ? 'bg-emerald-600 hover:bg-emerald-700 text-white'
                                    : 'bg-gray-200 text-gray-400 cursor-not-allowed'
                                  }`}
                              >
