...
import { useLearnerProfile, useLearnerActivity, useLearnerMessages } from '@/features/learner-profile';
import { useLearnerDataByEmail, useLearnerMessageNotifications, useLearnerUnreadCount } from '@/entities/learner';
...
import { SuggestedNextSteps, RecentUpdatesCard } from '@/widgets/learner-dashboard';
import {
  suggestions as mockSuggestions
} from "@/shared/lib/test/mockData";
import { showProfileUpdateToast, showProfileErrorToast, PROFILE_UPDATE_MESSAGES } from '@/features/learner-profile';
...
const MySkills = () => {
  ...
  const { learnerData, updateTechnicalSkills, updateSoftSkills } = useLearnerDataByEmail(userEmail, false);

  const technicalSkills = learnerData?.technicalSkills || [];
  const softSkills = learnerData?.softSkills || [];
  ...
  // AI Job Matching - Get top 3 matched jobs for learner
  const {
    matchedJobs,
    loading: matchingLoading,
    error: matchingError,
  } = useAIJobMatching(learnerData, true, 3);
