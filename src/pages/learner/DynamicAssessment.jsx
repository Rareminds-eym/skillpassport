...
const DynamicAssessment = () => {
  ...
  // Get learner data
  const { learnerData } = useLearnerDataByEmail(user?.email, false);
  ...
  }, [learnerData?.id, courseName, resumeAttempt, loadedExistingAttempt, navigate]);
  ...
  const handleSubmit = async () => {
    ...
    if (learnerData?.id && courseId && UUID_PATTERN.test(courseId)) {
      ...
      if (!trainingUpdate.success) {
        console.error('❌ Failed to update training status after assessment:', trainingUpdate.error);
      }
    }
