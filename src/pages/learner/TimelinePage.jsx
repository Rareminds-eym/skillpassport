 ...
 * visual representation of a learner's educational journey, work experience,
...
const TimelinePage = () => {
  ...
  const createMilestones = () => {
    ...
    logger.info('📊 Learner Data Structure', {
      education: learnerData.education,
      experience: learnerData.experience,
      projects: learnerData.projects,
      certificates: learnerData.certificates,
      achievements: learnerData.achievements,
      availableKeys: Object.keys(learnerData)
    });
    
    const milestones = [];

    // Get data from profile or direct properties
    const education = learnerData.education || [];
    const experience = learnerData.experience || [];
    const projects = learnerData.projects || [];
    const certificates = learnerData.certificates || [];
    const achievements = learnerData.achievements || [];
    ...
    if (education.length === 0 && experience.length === 0 && projects.length === 0 && certificates.length === 0 && achievements.length === 0) {
      ...
      if (learnerData.university || learnerData.college_school_name) {
        education.push({
          degree: learnerData.branch_field || 'Current Studies',
          university: learnerData.university || learnerData.college_school_name,
      ...
      if (learnerData.approval_status === 'approved') {
        achievements.push({
          title: 'Profile Approved',
          category: 'Platform Achievement',
          date: learnerData.updated_at ? new Date(learnerData.updated_at).getFullYear() : new Date().getFullYear(),
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
            ...
            >
              <div className="w-32 h-32 rounded-full overflow-hidden ring-4 ring-indigo-500 ring-offset-4 shadow-xl">
                <img
                  src={learnerData?.profilePicture || learnerData?.profile_photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(learnerData?.name || 'Learner')}&size=200&background=6366f1&color=fff&bold=true`}
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              >
                {learnerData?.college_school_name || getRoleLabel(user?.role)}
              </motion.p>
              ...
              >
                {learnerData?.bio || 'Passionate about learning and growth'}
              </motion.p>
              ...
              >
                {learnerData?.email && (
                  <span className="flex items-center gap-2 text-gray-600 bg-gray-100 px-4 py-2 rounded-full">
                    ...
                    <span className="text-sm">{learnerData.email}</span>
                  </span>
                )}
                {(learnerData?.contact_number || learnerData?.contactNumber) && (
                  <span className="flex items-center gap-2 text-gray-600 bg-gray-100 px-4 py-2 rounded-full">
                    <Phone className="w-4 h-4" />
                    <span className="text-sm">{learnerData.contact_number || learnerData.contactNumber}</span>
                  </span>
                )}
                {(learnerData?.city || learnerData?.state || learnerData?.district_name) && (
                  <span className="flex items-center gap-2 text-gray-600 bg-gray-100 px-4 py-2 rounded-full">
                    <MapPin className="w-4 h-4" />
                    <span className="text-sm">
                      {learnerData.city && learnerData.state 
                        ? `${learnerData.city}, ${learnerData.state}` 
                        : learnerData.district_name || learnerData.city || learnerData.state
                      }
                    </span>
                  </span>
                )}
                {/* Show university/college badge only if it's a proper institution name */}
                {(learnerData?.university && 
                  learnerData.university !== 'bangalore' && 
                  learnerData.university.length > 3 &&
                  !learnerData.university.toLowerCase().includes('city')) && (
                  <span className="flex items-center gap-2 text-gray-600 bg-gray-100 px-4 py-2 rounded-full">
                    <GraduationCap className="w-4 h-4" />
                    <span className="text-sm">{learnerData.university}</span>
                  </span>
                )}
                {/* For school learners, show grade/section if available */}
                {learnerData?.learner_type === 'school-learner' && learnerData?.grade && learnerData?.section && (
                  <span className="flex items-center gap-2 text-gray-600 bg-gray-100 px-4 py-2 rounded-full">
                    <GraduationCap className="w-4 h-4" />
                    <span className="text-sm">Grade {learnerData.grade} - {learnerData.section}</span>
                  </span>
                )}
              </motion.div>
            </div>
