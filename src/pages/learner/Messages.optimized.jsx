...
const Messages = () => {
  ...
  const learnerId = useMemo(() => 
    learnerData?.id || user?.id, 
    [learnerData?.id, user?.id]
  );
  const learnerName = useMemo(() => 
    learnerData?.profile?.name || user?.name || 'Learner',
    [learnerData?.profile?.name, user?.name]
  );
  ...
  useEffect(() => {
    if (!selectedConversationId || !learnerId) return;
    ...
    const markKey = `${selectedConversationId}-${conversation?.learner_unread_count}`;
    if (markedAsReadRef.current.has(markKey)) return;
    markedAsReadRef.current.add(markKey);
    
    MessageService.markConversationAsRead(selectedConversationId, learnerId)
      .then(() => refetchConversations())
      .catch(err => {
        logger.error('Failed to mark as read', err);
        markedAsReadRef.current.delete(markKey);
      });
  }, [selectedConversationId, learnerId, conversations, refetchConversations]);
  ...
  const handleSendMessage = useCallback(async (e) => {
    ...
    if (!trimmedInput || !currentChat || !learnerId || isSending) return;
  ...
  }, [messageInput, currentChat, learnerId, isSending, sendMessage, sendNotification, selectedConversationId, setTyping]);
