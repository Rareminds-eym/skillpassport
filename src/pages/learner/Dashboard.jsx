...
const OpportunitiesCardContent = ({ opportunities, learnerData, navigate, matchedJobs = [] }) => {
  ...
  // Determine what should be shown based on learner grade
  const learnerGrade = learnerData?.grade;
  ...
  if (learnerGrade) {
    const gradeMatch = learnerGrade.match(/\d+/);
    ...
    else if ((gradeNumber && gradeNumber >= 9 && gradeNumber <= 12) || learnerGrade.toLowerCase().includes('diploma')) {
      showJobs = jobsAndInternships.length > 0;
    }
    // College (UG/PG): Industrial Visits + Jobs/Internships
    else if (learnerGrade.toLowerCase().includes('ug') || learnerGrade.toLowerCase().includes('pg')) {
      showJobs = jobsAndInternships.length > 0;
    }
  }
  ...
  if (showFactoryVisits && showJobs) {
    // For college learners, show Jobs first, then Industrial Visits
    const isCollege = learnerGrade && (
      learnerGrade.toLowerCase().includes('ug') ||
      learnerGrade.toLowerCase().includes('pg') ||
      learnerGrade.toLowerCase().includes('year')
    );
              ...
              <div className="space-y-3 max-h-[250px] overflow-y-auto pr-2 blue-scrollbar">
                ...
                <div className="text-center pt-2 pb-1">
                  <Button
                    onClick={(e) => {
                      e.stopPropagation();
                      navigate('/learner/opportunities', { state: { activeTab: 'my-jobs' } });
                    }}
                    variant="outline"
                    size="sm"
                ...
                {factoryVisits.length > 5 && (
                  <div className="text-center pt-2 pb-1">
                    <Button
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate('/learner/opportunities', { state: { activeTab: 'industrial-visits' } });
                      }}
                      variant="outline"
                      size="sm"
    ...
    // For school learners, show Industrial Visits first
    return (
      <>
        <div className="space-y-4">
...
const LearnerDashboard = () => {
  ...
  useLearnerMessageNotifications({
    learnerId,
    enabled: !!learnerId && !isViewingOthersProfile,
    playSound: true,
    onMessageReceived: () => {
  ...
  const { unreadCount } = useLearnerUnreadCount(
    learnerId,
    !!learnerId && !isViewingOthersProfile
  );
  ...
  const qrCodeValue = React.useMemo(() => {
    const learnerId = learnerData?.id || "learner";
  ...
  // Debug log for authentication and learner data
  // useEffect(() => {
  //   // Authentication and learner data loaded
  // }, [learnerData, authlearnerLoading, authlearnerError]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('learner_settings_updated', handleSettingsUpdate);

    return () => {
      window.removeEventListener('learner_settings_updated', handleSettingsUpdate);
    };
  }, [refresh]);
  ...
  // Update userData when real learner data is loaded
  useEffect(() => {
    if (learnerData) {
      setUserData({
        education: Array.isArray(learnerData.education)
          ? learnerData.education
          : [],
        training: Array.isArray(tableTraining) && tableTraining.length > 0
          ? tableTraining
          : Array.isArray(learnerData.training)
            ? learnerData.training
            : [],
        experience: Array.isArray(tableExperience) && tableExperience.length > 0
          ? tableExperience
          : Array.isArray(learnerData.experience)
            ? learnerData.experience
            : [],
        technicalSkills: Array.isArray(learnerData.technicalSkills)
          ? learnerData.technicalSkills
          : [],
        softSkills: Array.isArray(learnerData.softSkills)
          ? learnerData.softSkills
          : [],
        projects: Array.isArray(tableProjects) && tableProjects.length > 0
          ? tableProjects
          : Array.isArray(learnerData.projects)
            ? learnerData.projects
            : Array.isArray(learnerData.profile?.projects)
              ? learnerData.profile.projects
              : [],
        certificates: Array.isArray(tableCertificates) && tableCertificates.length > 0
          ? tableCertificates
          : Array.isArray(learnerData.certificates)
            ? learnerData.certificates
            : Array.isArray(learnerData.profile?.certificates)
              ? learnerData.profile.certificates
              : [],
      });
    }
  ...
  // Determine institution info from learner data (using individual columns)
  const institutionInfo = React.useMemo(() => {
    // Priority: college_id takes precedence if both exist (College → School)
    if (learnerData?.college_id && learnerData?.college) {
      // College (standalone, not part of university)
      return {
        type: 'College',
        name: learnerData.college.name,
        code: learnerData.college.code,
        city: learnerData.college.city,
        state: learnerData.college.state,
      };
    } else if (learnerData?.school_id && learnerData?.school) {
      return {
        type: 'School',
        name: learnerData.school.name,
        code: learnerData.school.code,
        city: learnerData.school.city,
        state: learnerData.school.state,
      };
    } else if (learnerData?.university_college_id && learnerData?.universityCollege) {
      // University college with parent university info
      const college = learnerData.universityCollege;
    ...
    } else if (learnerData?.university || learnerData?.college_school_name) {
      ...
      const hasUniversity = learnerData?.university;
      const hasBranch = learnerData?.branch_field;

      if (hasUniversity && learnerData?.college_school_name) {
        // Has both university and college - show college
        type = 'College';
        name = learnerData.college_school_name;
      } else if (hasUniversity) {
        // Has only university
        type = 'University';
        name = learnerData.university;
      } else if (learnerData?.college_school_name) {
      ...
      if (name) {
        return {
          ...
          name: name,
          code: 'N/A',
          city: learnerData.location || learnerData.city,
    ...
    if (learnerData?.school_id && !learnerData?.school) {
            ...
            <LampContainer>
              ...
              >
                Welcome to your profile dashboard, {(() => {
                  const firstName = learnerData?.profile?.firstName ||
                    learnerData?.profile?.first_name ||
                    learnerData?.rawData?.firstName ||
                    ...
                    <CardHeader className="px-6 py-5 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-blue-100 rounded-t-xl">
                      <div className="flex items-center w-full justify-between">
                        ...
                        <button
                          ...
                          onClick={() => navigate("/learner/suggested-steps")}
                        >
                          <Eye className="w-5 h-5 text-blue-600" />
                        </button>
                      </div>
