...
const MyTimetable: React.FC = () => {
  ...
  const loadEducatorData = async () => {
    try {
      ...
      if (userRole === 'college_educator') {
        console.error(
          'College educator profile not found in college_lecturers or school_educators'
        );
        throw new Error(
          'Your educator profile has not been set up yet. Please contact your administrator.'
        );
      }
  ...
  const handleInitiateSwap = async (slot: TimetableSlot) => {
    ...
    const { data, error } = await getAvailableSlotsForSwap(slot.id!, educatorId, isCollegeEducator);
  ...
  const getSlotForCell = (dayIndex: number, periodNum: number, date: Date) => {
    ...
    const recurringSlot = filteredSlots.find(
      (s) =>
        s.day_of_week === dayOfWeek && s.period_number === periodNum && s.is_recurring !== false
    );
            ...
            <div className="p-4 border-t border-gray-200">
              ...
              <button
                onClick={() => {
                  if (!canView) {
                    console.log(
                      '❌ [MyTimetable] Action Blocked: View All Requests - No View Permission'
                    );
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Schedule classes by date with break management
                </p>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => {
                    if (!canView) {
                      console.log(
                        '❌ [MyTimetable] Action Blocked: Swap Requests Button - No View Permission'
                      );
                      alert('❌ Access Denied: You need VIEW permission to view swap requests');
                      return;
                    }
                  ...
                  <tbody>
                    {periods.map((period) => (
                      <tr
                        ...
                        className={period.is_break ? 'bg-gray-100' : ''}
                      >
                                      ...
                                      className={`absolute bottom-1 right-1 w-2 h-2 rounded-full ${
                                        swapStatus.status === 'pending'
                                          ...
                                          : swapStatus.status === 'accepted'
                                            ? 'bg-blue-400'
                                            : swapStatus.admin_approval_status === 'approved'
                                              ? 'bg-green-400'
                                              : 'bg-gray-400'
                                      }`}
