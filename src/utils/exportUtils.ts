  ...
  sections.forEach((section, index) => {
    ...
    if (section.data && section.data.length > 0) {
      ...
      section.data.forEach(row => {
        excelContent += headers.map(header => {
          const value = row[header];
          if (typeof value === 'string' && (value.includes(',') || value.includes('"'))) {
            return `"${value.replace(/"/g, '""')}"`;
          }
 ...
 */
export const exportComprehensiveAnalytics = (data: {
  funnel: any;
  quality: any;
  speed: any;
  geography: any;
    ...
    const sections = [
      {
        ...
        data: [
          ...
          { Stage: 'Screened', Count: data.funnel.screened, 'Conversion %': ((data.funnel.screened / data.funnel.sourced) * 100).toFixed(1) },
          { Stage: 'Interviewed', Count: data.funnel.interviewed, 'Conversion %': ((data.funnel.interviewed / data.funnel.screened) * 100).toFixed(1) },
          { Stage: 'Offered', Count: data.funnel.offered, 'Conversion %': ((data.funnel.offered / data.funnel.interviewed) * 100).toFixed(1) },
          { Stage: 'Hired', Count: data.funnel.hired, 'Conversion %': ((data.funnel.hired / data.funnel.offered) * 100).toFixed(1) }
        ]
      },
      {
        name: 'Speed Metrics',
        data: [
          { Metric: 'Time to First Response', Value: `${data.speed.median_time_to_first_response} days` },
...
export const scheduleReport = (config: {
  ...
  alert(`Report "${config.name}" scheduled successfully!\n\nFrequency: ${config.frequency}\nRecipients: ${config.recipients.join(', ')}\nFormat: ${config.format.toUpperCase()}`);
