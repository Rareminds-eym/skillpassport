...
export const migrateMockDataToSupabase = async (studentId) => {
  try {
    ...
    const { error: educationError } = await supabase.from('education').upsert(educationRecords);
    ...
    const { error: trainingError } = await supabase.from('trainings').upsert(trainingRecords);
    ...
    const { error: experienceError } = await supabase.from('experience').upsert(experienceRecords);
    ...
    const { error: updatesError } = await supabase.from('recent_updates').upsert(updateRecords);
