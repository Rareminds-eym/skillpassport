  ...
  describe('Scenario 1: Viewing Organization-Provided Features', () => {
    it('should display features provided by organization', async () => {
      const getOrganizationFeatures = async (userId: string) => {
        ...
        const orgEntitlements = Array.from(state.entitlements.values())
          .filter(e => e.user_id === userId && e.granted_by_organization && e.is_active);

        // Get assignment details
        const assignment = Array.from(state.assignments.values())
          .find(a => a.user_id === userId && a.status === 'active');
        ...
        return {
          ...
          daysRemaining: assignment 
            ? Math.ceil((new Date(assignment.expires_at).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
            : 0
        };
      };
    ...
    it('should show "Managed by Organization" badge', async () => {
      const getFeatureSource = async (userId: string, featureKey: string) => {
        const entitlement = Array.from(state.entitlements.values())
          .find(e => e.user_id === userId && e.feature_key === featureKey && e.is_active);
        ...
        return {
          ...
          badge: entitlement.granted_by_organization ? 'Managed by Springfield High School' : 'Personal Purchase'
        };
      };
  ...
  describe('Scenario 2: Checking Feature Access', () => {
    it('should correctly check if user has access to a feature', async () => {
      const hasFeatureAccess = async (userId: string, featureKey: string) => {
        ...
        return { hasAccess: true, source: entitlement.granted_by_organization ? 'organization' : 'personal' };
      };
  ...
  describe('Scenario 3: Viewing Available Personal Add-ons', () => {
    it('should display add-ons available for personal purchase', async () => {
      const availableAddons = [
        { id: 'addon-1', name: 'Premium Analytics', price: 199, description: 'Advanced learning analytics' },
        { id: 'addon-2', name: 'Certificate Prep', price: 299, description: 'Certification exam preparation' },
        { id: 'addon-3', name: '1-on-1 Tutoring', price: 499, description: 'Personal tutoring sessions' }
      ];
      ...
      const getAvailableAddons = async (userId: string) => {
        ...
        return availableAddons.filter(addon => 
          !existingFeatures.includes(addon.id)
        );
      };
  ...
  describe('Scenario 5: Viewing Combined Entitlements', () => {
    ...
    it('should display both organization and personal features separately', async () => {
      const getAllEntitlements = async (userId: string) => {
        const allEntitlements = Array.from(state.entitlements.values())
          .filter(e => e.user_id === userId && e.is_active);
  ...
  describe('Scenario 7: Contacting Organization Admin', () => {
    it('should provide admin contact information', async () => {
      ...
      const getAdminContact = async (userId: string) => {
        ...
        const orgAdmin = Array.from(admins.values())
          .find(a => a.organization_id === user.organization_id);
