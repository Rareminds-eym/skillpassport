 ...
 * every role; only the data source (each role's own backend endpoints) differs.
 */
 ...
 * of a separate type — so selecting a course is a lookup, not a reshape.
 */
...
/**
 * One page of the Course Performance Overview table — `totalCount` is the
 * full course count (pre-pagination, across every page), for computing page
 * count. Identical shape for every role; only the backend query scope
 * (College/School/University/Educator learners) differs.
 */
export interface PaginatedCoursePerformance {
...
export interface DirectoryNode {
   ...
   * branch: sum of descendants). `DirectoryTreePanel` prefers this value when
   * present, falling back to a `learnerDirectory`-derived count (via
   * `buildLearnerCountByNode`, see lib/directory.ts) only if it's undefined —
   * see that component for the exact precedence.
   */
  count?: number;
...
/** A selectable option for a filter dropdown. */
 ...
 *
 * Each role's backend returns this same shape, so the widget consumes one
 * typed prop regardless of which role's page is rendering it.
 */
export interface CourseAnalyticsData {
  ...
  /** Total records matching the current filters, for the count label. */
