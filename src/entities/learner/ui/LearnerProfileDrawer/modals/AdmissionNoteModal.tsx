...
const AdmissionNoteModal: React.FC<AdmissionNoteModalProps> = ({ 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      logger.error('Failed to save admission note', error instanceof Error ? error : new Error(String(error)));
  ...
  const sendNoteAsCommunication = async () => {
    try {
      ...
      logger.error('Failed to send note to communication', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  };
