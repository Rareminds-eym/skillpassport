...
// Centralized learner type detection
...
// Types
import { LearnerProfileDrawerProps, TabConfig, ActionConfig } from '@/shared/types';

// Hooks
import { useLearnerData, useLearnerActions } from '@/features/learner-profile/ui/hooks';
...
const LearnerProfileDrawer: React.FC<LearnerProfileDrawerProps> = ({
  learner,
  isOpen,
  onClose,
  ...
  const {
    ...
    handlePromotion,
    handleGraduation,
  } = useLearnerActions(learner);

  if (!isOpen || !learner) return null;

  // Configuration based on user role and learner type
  const getTabsConfig = (): TabConfig[] => {
    ...
    // Add courses tab for all learners
    baseTabs.push({ key: 'courses', label: 'Courses' });

    // Add projects and certificates tabs for ALL learners
    baseTabs.push(
    ...
    // Add Events tab for all learners
    baseTabs.push({ key: 'events', label: 'Events' });

    // Add Documents tab for all learners
    baseTabs.push({ key: 'documents', label: 'Documents' });

    // For school educators: show school-specific tabs
    if (userRole === 'school_educator' || (userRole.includes('admin') && learner.school_id)) {
    ...
    if (userRole === 'college_educator' || (userRole.includes('admin') && checkIsCollegeLearner(learner))) {
  ...
  const getActionsConfig = (): ActionConfig => {
    ...
    const isCollegeLearner = checkIsCollegeLearner(learner); // Use centralized utility

    return {
      showApproval: Boolean(isAdmin && needsVerification() && isCollegeLearner),
      showPromotion: Boolean(isAdmin && canPromote() && isCollegeLearner),
      showGraduation: Boolean(isAdmin && canGraduate() && isCollegeLearner),
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'overview':
        return <OverviewTab learner={learner} />;
      case 'academic':
      ...
      case 'assessments':
        return (
          <AssessmentsTab
            ...
            learnerType={learner?.college_id ? 'college' : 'school'}
      ...
      case 'curriculum':
        return (
          <CurriculumTab
            ...
            learnerAcademicYear={learnerAcademicYear}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-4">
                      ...
                      {checkIsSchoolLearner(learner) ? (
                        // School Learner Fields
                        <>
                          <div>
                            <p className="text-xs text-gray-500 uppercase tracking-wide">School</p>
                            <p className="text-sm font-medium text-gray-900 mt-1">{learner.school_name || learner.college_school_name || 'N/A'}</p>
                          </div>
                          <div>
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <div className="flex items-center gap-1">
                        <p className="text-xs text-gray-500 uppercase tracking-wide">
                          {checkIsSchoolLearner(learner) ? 'Admission Status:' : 'Enrollment Status:'}
                        </p>
                        <Badge type={learner.approval_status || 'pending'} />

                      </div>

                      {/* Academic Progress Indicator */}
                      {checkIsCollegeLearner(learner) && (
              ...
              <div className="border-t border-gray-200 px-6 py-4 bg-gray-50">
                <div className="flex items-center justify-between">
                  <div className="flex space-x-2">
                    ...
                    {/* Learner Management Actions - Only for Admins */}
                    {actions.showApproval && (
                      <button
                  ...
                  <div className="flex items-center space-x-3">
                    {/* Semester Status - Only for College Learners */}
                    {checkIsCollegeLearner(learner) && (
