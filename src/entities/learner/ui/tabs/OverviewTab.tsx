...
const OverviewTab: React.FC<OverviewTabProps> = ({ learner }) => {
  ...
  // Social links from learner data
  const socialLinks = [
    { name: 'LinkedIn', url: learner.linkedin_link || learner.profile?.linkedin_link, icon: '🔗' },
    { name: 'GitHub', url: learner.github_link, icon: '💻' },
    { name: 'Portfolio', url: learner.portfolio_link, icon: '🌐' },
    { name: 'Twitter', url: learner.twitter_link, icon: '🐦' },
    { name: 'YouTube', url: learner.youtube_link, icon: '📺' },
    { name: 'Instagram', url: learner.instagram_link, icon: '📷' },
    { name: 'Facebook', url: learner.facebook_link, icon: '👤' },
  ...
  return (
    <div className="p-6 space-y-8 max-h-[calc(100vh-300px)] overflow-y-auto">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
          <div className="flex flex-col">
            ...
            <span className="font-medium text-gray-900">{learner.name || 'N/A'}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-gray-500 text-xs mb-1">Email</span>
            <span className="font-medium text-gray-900 break-all">{learner.email || 'N/A'}</span>
          </div>
          <div className="flex flex-col">
          ...
          <div className="flex flex-col">
            <span className="text-gray-500 text-xs mb-1">{learner.school_id ? 'School' : 'College'}</span>
            <span className="font-medium text-gray-900">{learner.college_school_name || learner.college || 'N/A'}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-gray-500 text-xs mb-1">{learner.school_id ? 'Grade' : 'Degree'}</span>
            <span className="font-medium text-gray-900">{learner.grade || learner.branch_field || 'N/A'}</span>
          </div>
          <div className="flex flex-col">
          ...
          {learner.currentCgpa && (
            <div className="flex flex-col">
              ...
              <span className="font-medium text-gray-900">{learner.currentCgpa}</span>
            </div>
          )}
      ...
      {/* Attendance Summary (School Learners) */}
      {learner.school_id && attendance.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-4">
      ...
      {/* Hobbies & Interests */}
      {((learner.hobbies && learner.hobbies.length > 0) || (learner.interests && learner.interests.length > 0)) && (
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <SectionHeader icon={<StarIcon className="h-5 w-5" />} title="Hobbies & Interests" />
          <div className="space-y-3">
            {learner.hobbies && learner.hobbies.length > 0 && (
              <div>
                <div className="text-xs text-gray-500 mb-2">Hobbies</div>
                <div className="flex flex-wrap gap-2">
                  {learner.hobbies.map((hobby: string, idx: number) => (
            ...
            {learner.interests && learner.interests.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {learner.interests.map((interest: string, idx: number) => (
      ...
      {/* Languages */}
      {learner.languages && learner.languages.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <SectionHeader icon={<UserGroupIcon className="h-5 w-5" />} title="Languages" count={learner.languages.length} />
          <div className="flex flex-wrap gap-2">
            {learner.languages.map((lang: any, idx: number) => (
      ...
      {(learner.guardianName || learner.guardianPhone || learner.guardianEmail) && (
        <div className="bg-white rounded-lg border border-gray-200 p-4">
      ...
      {/* Address Information */}
      {(learner.address || learner.city || learner.state) && (
        <div className="bg-white rounded-lg border border-gray-200 p-4">
      ...
      {/* Bio */}
      {(learner.bio || learner.profile?.bio) && (
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <SectionHeader icon={<DocumentTextIcon className="h-5 w-5" />} title="Bio" />
          <p className="text-sm text-gray-700 leading-relaxed">{learner.bio || learner.profile?.bio}</p>
        </div>
      )}
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                {learner.applied_date ? new Date(learner.applied_date).toLocaleDateString() : 
          ...
          <div className="flex items-start">
            <div className="flex flex-col items-center mr-4">
              <div className={`w-3 h-3 ${
                learner.approval_status === 'approved' || learner.approval_status === 'verified' ? 'bg-green-600' : 
                learner.approval_status === 'rejected' ? 'bg-red-600' : 'bg-gray-300'
              } rounded-full`}></div>
            </div>
            <div>
              <p className="text-sm font-medium text-gray-900">Application Decision</p>
              <p className="text-xs text-gray-500">
                Status: {(learner.approval_status || 'PENDING').toUpperCase()}
              </p>
            </div>
          </div>
        </div>
        ...
        {internships.length === 0 && placements.length === 0 && competitionResults.length === 0 && 
         skillBadges.length === 0 && pipelineStatus.length === 0 && interviews.length === 0 && (
          <p className="text-sm text-gray-500 italic mt-3 text-center">
            No career data available yet. Data will appear here as the learner progresses.
          </p>
        )}
      </div>
    </div>
