...
const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose, learner }) => {
  ...
  const generatePDF = (learner: Learner, _settings: any) => {
    ...
    const personalFields = [
      ...
      { label: 'Contact Number', value: learner.contact_number || learner.contactNumber || learner.phone },
    ...
    const academicFields = [
      { label: 'Applied Class', value: learner.grade },
      { label: 'Subjects', value: learner.subjects ? learner.subjects.join(', ') : 'N/A' },
      { label: 'Application Status', value: learner.admission_status?.toUpperCase() || learner.approval_status?.toUpperCase() },
  ...
  const generateCSV = (learner: Learner) => {
    const headers = [
      'Name',
      'Email',
      'Contact Number',
    ...
    const row = [
      ...
      escapeCSV(learner.contact_number || learner.contactNumber || learner.phone),
      escapeCSV(learner.date_of_birth ? new Date(learner.date_of_birth).toLocaleDateString() : ''),
      escapeCSV(learner.grade),
      escapeCSV(learner.subjects ? learner.subjects.join('; ') : ''),
      escapeCSV(learner.admission_status || learner.approval_status),
