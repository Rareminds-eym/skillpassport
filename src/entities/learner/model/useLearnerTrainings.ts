...
/** Read-cache freshness for the trainings/stats queries (no cooldown on the sync POST). */
const STALE_MS = 30_000;

async function loadTrainings(ctx): Promise<unknown[]> {
  const [, learnerId, sortBy, sortDirection, status, approvalStatus, searchTerm] =
    ctx.queryKey as [string, string, string, string, string, string, string];
...
async function loadStats(ctx): Promise<{ total: number; completed: number; ongoing: number }> {
  const [, learnerId] = ctx.queryKey as [string, string];
  const [trainingsResult, enrollmentsResult] = await Promise.all([
    apiPost('/learner-profile/actions', {
      action: 'fetch-trainings', learnerId,
...
export const useLearnerTrainings = (learnerId, options = {}) => {
  ...
  const trainingsQuery = useQuery({
    ...
    enabled: !!learnerId,
    staleTime: STALE_MS, // within the window, serve cached read without re-querying
  });

  const statsQuery = useQuery({
    queryKey: ['learner-trainings-stats', learnerId],
    queryFn: loadStats,
    enabled: !!learnerId,
    staleTime: STALE_MS,
  });
  ...
  return {
    ...
    loading: Boolean(learnerId) && (trainingsQuery.isPending || statsQuery.isPending),
    error: (trainingsQuery.error as Error | null)?.message ?? null,
    stats: statsQuery.data ?? { total: 0, completed: 0, ongoing: 0 },
    refetch: invalidate,
  };
};
