...
/**
 * Custom hook to manage learner data from Supabase (ADAPTED for existing schema)
 * Works with existing learners table that uses JSONB profile column
 * 
 * @param {string} userId - The userId (UUID) of the learner from auth.users
 * @param {boolean} useMockData - Whether to use mock data as fallback (default: false)
 */
export const useLearnerDataAdapted = (userId, useMockData = false) => {
  ...
  const fetchlearnerData = useCallback(async () => {
    ...
    try {
      ...
      const data = await getCompleteLearnerData(userId);
  ...
  const updateProfile = async (updates) => {
    try {
      const result = await updateLearnerProfile(userId, updates);
