...
const EmployabilityDebugger = () => {
  ...
  useEffect(() => {
    if (realStudentData) {
      ...
      const testData = {
        ...
        training: [{ course: 'Web Development', status: 'completed' }],
        education: [{ degree: 'Computer Science', university: 'Test University' }],
        experience: [{ role: 'Intern', organization: 'Tech Company' }],
      };
  ...
  const renderSkillsArray = (skills, title) => {
    if (!skills || !Array.isArray(skills) || skills.length === 0) {
      return (
        <p className="text-red-600">
          <strong>{title}:</strong> Empty or missing
        </p>
      );
    }
    ...
    return (
      <div className="mb-2">
        <p>
          <strong>{title}:</strong> {skills.length} items
        </p>
        <ul className="ml-4 text-xs">
          {skills.slice(0, 3).map((skill, idx) => (
            <li key={idx}>
              {skill.name || 'No name'} - Level: {skill.level || skill.rating || 'N/A'} - Verified:{' '}
              {skill.verified || skill.evidenceVerified ? 'Yes' : 'No'}
            </li>
          ))}
    ...
    <div className="p-4 bg-gray-100 m-4 rounded border-2 border-red-400">
      ...
      <div className="text-sm grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <p>
            <strong>Student Name:</strong> {realStudentData.profile?.name}
          </p>
          <p>
            <strong>Data Keys:</strong> {Object.keys(realStudentData).join(', ')}
          </p>
        ...
        <div>
          ...
          {debugResult ? (
            <div className="bg-white p-3 rounded">
              <p>
                <strong>Score:</strong> {debugResult.employabilityScore}%
              </p>
              <p>
                <strong>Level:</strong> {debugResult.level}
              </p>
              <p>
                <strong>Label:</strong> {debugResult.label}
              </p>
              <div className="mt-2">
