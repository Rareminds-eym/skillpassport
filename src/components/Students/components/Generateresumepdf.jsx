...
export const generateResumePDF = async (studentData) => {
  ...
  const hasLinks =
    ...
    profile.portfolio ||
    (studentData.link &&
      (studentData.link.linkedin || studentData.link.github || studentData.link.portfolio));
  ...
  if (projectData.length > 0) {
    ...
    projectData.forEach((project, idx) => {
      ...
      doc.setFontSize(11);
      doc.setTextColor(...textDark);
      const projectTitle = project.title || project.name || project.projectTitle || 'Project';
      doc.text(projectTitle, mainX, yMain);

      const projectDuration =
        project.duration || project.timeline || project.period || project.date || project.year;
      if (projectDuration) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
        doc.setTextColor(...textLight);
      ...
      const projectOrganization =
        project.organization || project.company || project.client || project.role;
      ...
      const projectDescription =
        project.description || project.summary || project.details || project.responsibility;
  ...
  if (certificateData.length > 0) {
    ...
    certificateData.forEach((cert, idx) => {
      ...
      const issuedDate =
        cert.date || cert.year || cert.issued_on || cert.issuedOn || cert.completionDate;
      if (issuedDate) {
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(8.5);
        doc.setTextColor(...textLight);
  ...
  if (profile.training && Array.isArray(profile.training)) {
    const completedTraining = profile.training.filter(
      (t) => t && t.enabled !== false && t.status === 'completed'
    );
