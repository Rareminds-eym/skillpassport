...
export default function AddLearningCourseModal({ isOpen, onClose, studentId, onSuccess }) {
    ...
    try {
      ...
      if (selectedPlatform && selectedPlatform.id !== 'other' && selectedPlatform.urlPattern) {
        if (!selectedPlatform.urlPattern.test(certificateUrl)) {
          const detectedPlatform = PLATFORMS.find(
            (p) => p.urlPattern && p.urlPattern.test(certificateUrl)
          );
          if (detectedPlatform) {
            setError(
              `This appears to be a ${detectedPlatform.name} certificate, but you selected ${selectedPlatform.name}.`
            );
          } else {
      ...
      const workerUrl =
        import.meta.env.VITE_CLOUDFLARE_CERTIFICATE_WORKER_URL ||
      ...
      if (!response.ok || !result?.success) {
        ...
        setError(
          errorMsg.includes('404')
            ? 'Certificate not found. Please verify the URL or ID.'
            : errorMsg
        );
        setVerificationStatus('failed');
        return false;
  ...
  const canProceedToNextStep = () => {
    switch (currentStep) {
      ...
      case 2:
        return formData.certificate_url.trim() !== '' || formData.certificate_id.trim() !== '';
      case 3:
    ...
    if (currentStep === 2 && !extractionSuccess) {
      if (formData.certificate_url) {
        ...
        if (!selectedPlatform?.verifyUrlTemplate) {
          setError(
            'This platform does not support verification by certificate ID. Please provide a certificate URL.'
          );
          return;
        }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden shadow-2xl">
        ...
        <div className="relative bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-700 px-6 py-5">
          <button
            onClick={onClose}
            className="absolute top-4 right-4 text-white/80 hover:text-white hover:bg-white/10 rounded-full p-1.5"
          >
            <X size={20} />
          </button>
            ...
            <div className="flex items-center">
              {[1, 2, 3, 4, 5].map((step, index) => (
                <div key={step} className="flex items-center flex-1 last:flex-initial">
                  ...
                  {index < 4 && (
                    <div
                      className={`flex-1 h-0.5 mx-2 ${step < currentStep ? 'bg-white/50' : 'bg-white/20'}`}
                    />
                  )}
                </div>
              ))}
            ...
            <div className="space-y-4">
              <div className="text-center mb-6">
                ...
                <p className="text-gray-500 text-sm mt-1">
                  Select the platform where you earned your certificate
                </p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {PLATFORMS.map((platform) => (
                  ...
                  >
                    ...
                    />
                    <span
                      className={`text-sm font-medium ${selectedPlatform?.id === platform.id ? 'text-gray-900' : 'text-gray-700'}`}
                    >
                      {platform.name}
                    </span>
                  </button>
          ...
          {currentStep === 2 && (
            <div className="space-y-4">
              <div className="text-center mb-6">
                ...
                <p className="text-gray-500 text-sm mt-1">
                  Provide certificate details for verification
                </p>
              </div>
                    ...
                    <input
                      ...
                      placeholder={
                        selectedPlatform?.id === 'other'
                          ...
                          : 'https://ude.my/UC-xxx or UC-794d8663-...'
                      }
              ...
              {!verificationStatus && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start gap-2">
                  ...
                  <p className="text-xs text-amber-800">
                    Enter your certificate URL or ID to verify your certificate against the
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm ${selectedPlatform?.id !== 'other' ? 'bg-gray-100 text-gray-600 cursor-not-allowed' : 'focus:ring-2 focus:ring-indigo-500'}`}
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-amber-700 mt-1">
                        Skills from external certificates will be marked as{' '}
                        <strong>"Rareminds Assessment Pending"</strong> until verified.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="border-2 border-gray-200 rounded-xl p-5 bg-gradient-to-br from-gray-50 to-white">
                <div className="flex items-start gap-4">
                  <div
                    className={`w-14 h-14 rounded-xl flex items-center justify-center ${selectedPlatform?.bgColor || 'bg-gray-100'}`}
                  >
                    <PlatformIcon
                      platformId={selectedPlatform?.id}
                      className={`w-7 h-7 ${selectedPlatform?.iconColor || 'text-gray-500'}`}
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    {formData.completion_date && (
                      <p className="text-gray-500 text-sm mt-1">
                        Completed:{' '}
                        {new Date(formData.completion_date).toLocaleDateString('en-US', {
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric',
                        })}
              ...
              {verificationResult && (
                <div
                  className={`rounded-xl p-4 ${verificationResult.isLegitimate ? 'bg-green-50 border border-green-200' : 'bg-amber-50 border border-amber-200'}`}
                >
                  <div className="flex items-start gap-3">
                    <CheckCircle
                      className={`w-5 h-5 flex-shrink-0 mt-0.5 ${verificationResult.isLegitimate ? 'text-green-600' : 'text-amber-600'}`}
                    />
                    <div>
                      <p
                        className={`text-sm font-medium ${verificationResult.isLegitimate ? 'text-green-900' : 'text-amber-900'}`}
                      >
                        {verificationResult.summary || 'Certificate verified'}
                      </p>
                      {verificationResult.credibilityScore && (
                        <p className="text-xs text-gray-600 mt-1">
                          Credibility Score: {verificationResult.credibilityScore}%
                        </p>
                      )}
                    </div>
                  </div>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium ${currentStep === 1 ? 'text-gray-400 cursor-not-allowed' : 'text-gray-700 hover:bg-gray-200'}`}
          >
          ...
          <div className="flex items-center gap-3">
            ...
            {currentStep < totalSteps ? (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium ${canProceedToNextStep() && !extracting ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
              >
                {extracting ? (
                  <>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium ${!loading && !verifying ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
              >
                {loading ? (
                  <>
                  ...
                  <>
                    <CheckCircle size={18} /> Import Certificate
                  </>
                )}
              </button>
            )}
