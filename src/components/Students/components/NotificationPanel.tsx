...
const NotificationPanel: React.FC<NotificationPanelProps> = ({ isOpen, onClose, studentEmail }) => {
  const { user } = useAuth();
  ...
  const filteredNotifications = notifications.filter((n) => {
    switch (selectedFilter) {
      ...
      case 'messages':
        return ['new_message', 'message_reply'].includes(n.type);
      default:
        return true;
    }
  });
  ...
  const getFilterCount = (filterKey: FilterKey): number => {
    ...
    if (filterKey === 'messages') {
      return notifications.filter((n) => ['new_message', 'message_reply'].includes(n.type)).length;
    }
    return 0;
  };
  ...
  return (
    <>
      ...
      <div
        ref={panelRef}
        className="absolute right-0 mt-2 w-[calc(100vw-2rem)] max-w-sm sm:w-96 bg-white rounded-xl shadow-xl border border-gray-200 z-50 overflow-hidden"
      >
        {/* Header */}
        ...
        <div className="flex gap-2 px-4 py-2 border-b border-gray-100 overflow-x-auto">
          {(
            ['all', 'unread', 'opportunities', 'interviews', 'courses', 'messages'] as FilterKey[]
          ).map((key) => {
