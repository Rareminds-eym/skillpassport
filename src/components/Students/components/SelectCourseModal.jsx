...
export default function SelectCourseModal({ isOpen, onClose, studentId, onSuccess }) {
  ...
  const fetchAvailableCourses = async () => {
    try {
      ...
      const { data: courseLessonsData, error: lessonsError } = await supabase.from('course_modules')
        .select(`
          course_id,
    ...
    try {
      ...
      if (training) {
        ...
        if (courseSkills && courseSkills.length > 0) {
          ...
          const { error: insertSkillsError } = await supabase.from('skills').insert(studentSkills);
                              ...
                              : enrollmentProgress[course.course_id]?.totalModules > 0
                                ? Math.round(
                                    ((enrollmentProgress[course.course_id]?.completedModules || 0) /
                                      enrollmentProgress[course.course_id]?.totalModules) *
                                      100
                                  )
                                : 0
                          }
