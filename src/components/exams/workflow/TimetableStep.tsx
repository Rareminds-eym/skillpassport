...
const TimetableStep: React.FC<TimetableStepProps> = ({
  ...
  const [ttEntry, setTtEntry] = useState({
    subjectId: '',
    date: '',
    startTime: '',
    endTime: '',
  ...
  const getAvailableSubjects = () => {
    return exam.subjects.filter(
      (subject) => !exam.timetable.some((tt) => tt.subjectName === subject.name)
    );
  };
      ...
      if (existing.date === entry.date) {
        ...
        if (
          ...
          if (entry.room && existing.room === entry.room) {
            conflictList.push(
              `⚠️ Room conflict: ${entry.room} is already booked for ${existing.subjectName} at ${existingStart}-${existingEnd}`
            );
          }

          // Class conflict (same exam, same time)
          conflictList.push(
            `⚠️ Class conflict: Students already have ${existing.subjectName} exam at ${existingStart}-${existingEnd}`
          );
        }
      }
    });
  ...
  const addTimetable = async () => {
    ...
    try {
      ...
      alert(
        `❌ Failed to add timetable entry: ${error?.message || 'Unknown error'}. Please try again.`
      );
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-medium text-gray-600">
          {exam.timetable.length} / {exam.subjects.length} scheduled
        </span>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-amber-700 mt-2">
                Entry was added but please review and resolve conflicts.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <div>
              ...
              <select
                value={ttEntry.subjectId}
                onChange={(e) => setTtEntry({ ...ttEntry, subjectId: e.target.value })}
            ...
            <div>
              ...
              >
                ...
                {classRoom && <option value={classRoom}>📍 {classRoom} (Class Default)</option>}

                {/* All school rooms */}
                {allSchoolRooms && allSchoolRooms.length > 0 && (
                  <>
                    {allSchoolRooms
                      .filter((room) => room.type === 'class_room')
                      .map((room) => (
                    ...
                    {allSchoolRooms
                      .filter((room) => room.type === 'exam_room')
                      .map((room) => (
                        <option key={room.id} value={room.name}>
                          📝 {room.name}
                        </option>
