...
const PublishingStep: React.FC<PublishingStepProps> = ({ exam, setActiveStep, updateExam }) => {
  ...
  const publishResults = async () => {
    ...
    const subjectsReadyForPublishing = exam.marks.filter((subjectMark) => {
      ...
      const moderationCount = subjectMark.studentMarks.filter(
    ...
    if (subjectsReadyForPublishing < exam.marks.length) {
      ...
      alert(
        `Cannot publish: The following subjects need moderation approval:\n${pendingSubjects.map((s) => `• ${s.subjectName}`).join('\n')}`
      );
      return;
    }
  ...
  const canPublish =
    exam.marks.length === exam.subjects.length && subjectsReadyForPublishing === exam.marks.length;
            ...
            <div className="p-6 border-b border-gray-100">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600">
                  You're about to publish results for{' '}
              ...
              <label className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg cursor-pointer">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-blue-700">
                    All results have been reviewed and are ready for publication
                  </p>
                </div>
              </label>
            </div>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm">
          ...
          <div className="p-6 space-y-4">
            ...
            {/* Overall Status */}
            <div
              className={`p-4 rounded-lg border-2 ${canPublish ? 'bg-green-50 border-green-200' : 'bg-amber-50 border-amber-200'}`}
            >
              <div className="flex items-center gap-3">
                {canPublish ? (
                ...
                <div>
                  <p
                    className={`font-semibold ${canPublish ? 'text-green-900' : 'text-amber-900'}`}
                  >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  {(() => {
                    ...
                    const totalPresent =
                      exam.marks.reduce(
                        (sum, subject) =>
                          sum + subject.studentMarks.filter((s) => !s.isAbsent).length,
                        0
                      ) / exam.marks.length;
                    const overallPassRate =
                      exam.marks.reduce((sum, subject) => {
                        ...
                        const passedStudents = presentStudents.filter(
                          (s) => s.marks !== null && s.marks >= (subjectData?.passingMarks || 0)
                        );
                        return (
                          sum +
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {exam.marks.map((subjectMark) => {
                      ...
                      const avgMarks =
                        presentStudents.length > 0
                          ? Math.round(
                              presentStudents.reduce((sum, s) => sum + (s.marks || 0), 0) /
                          ...
                          <div className="space-y-2 text-xs text-gray-600">
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium text-red-600">
                                {presentStudents.length - passedStudents.length}
                              </span>
                            </div>
                          </div>
            ...
            <div className="p-6 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Results are now live and accessible to students and parents
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-900">
                        {exam.publishedAt
                          ? new Date(exam.publishedAt).toLocaleDateString('en-US', {
                              year: 'numeric',
                              month: 'long',
                              day: 'numeric',
                              hour: '2-digit',
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-900">
                        {exam.marks[0]?.studentMarks.length || 0}
                      </span>
                    </div>
                    <div className="flex justify-between">
