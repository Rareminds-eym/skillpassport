...
const PerformanceTrends: React.FC<PerformanceTrendsProps> = ({ exams }) => {
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        ...
        <div className="bg-gray-50 rounded-lg p-8 text-center">
          ...
          <p className="text-xs text-gray-400">
            Performance trends will be displayed here once exams are published and results are
            available.
          </p>
        </div>
      </div>
  ...
  const calculatePerformanceData = () => {
    ...
    const allStudentExamResults: Array<{
      studentId: string;
      examId: string;
    ...
    const overallPassRate =
      totalStudentExamInstances > 0 ? (totalPassedInstances / totalStudentExamInstances) * 100 : 0;

    // Find best and worst performing subjects
    const subjectAverages = Object.entries(subjectPerformance).map(([name, data]) => ({
      name,
      average:
        data.marks.length > 0 ? data.marks.reduce((a, b) => a + b, 0) / data.marks.length : 0,
      passRate: data.total > 0 ? (data.passed / data.total) * 100 : 0,
      total: data.total,
      passed: data.passed,
      range:
        data.marks.length > 0 ? `${Math.min(...data.marks)}-${Math.max(...data.marks)}` : '0-0',
    }));

    const topPerformer = subjectAverages.reduce(
      (prev, current) => (prev.average > current.average ? prev : current),
      subjectAverages[0] || { name: 'N/A', average: 0, passRate: 0 }
    );

    const needsAttention = subjectAverages.reduce(
      (prev, current) => (prev.passRate < current.passRate ? prev : current),
      subjectAverages[0] || { name: 'N/A', passRate: 100 }
    );
              ...
              {performanceData.subjectAverages.slice(0, 3).map((subject, index) => (
                <div key={subject.name} className="bg-white rounded-lg p-3 border border-blue-200">
                  ...
                  <div className="flex justify-between text-xs text-gray-500 mt-1">
                    <span>
                      Pass Rate: {subject.passRate.toFixed(0)}% ({subject.passed}/{subject.total})
                    </span>
                    <span>
                      Avg: {subject.average.toFixed(1)} | Range: {subject.range}
                    </span>
                  </div>
                </div>
              ))}
              ...
              {Object.entries(performanceData.gradeDistribution).map(([grade, count]) => {
                const percentage =
                  performanceData.totalStudents > 0
                    ? (count / performanceData.totalStudents) * 100
                    : 0;
                const colors = {
                  'A+': 'bg-green-600',
                  A: 'bg-blue-500',
                ...
                return (
                  <div key={grade} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-xs font-bold ${colors[grade as keyof typeof colors].replace('bg-', 'text-')}`}
                      >
                        {count}
                      </span>
                    </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-600">
                  {performanceData.topPerformer.average.toFixed(1)}% average
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-amber-700">
                  {performanceData.needsAttention.name}
                </p>
                <p className="text-xs text-amber-600">
                  {(100 - performanceData.needsAttention.passRate).toFixed(0)}% failure rate
                </p>
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <ul className="space-y-1 text-xs text-gray-600">
                <li className="flex items-center gap-2">
                  ...
                  {performanceData.topPerformer.name} shows excellent performance (
                  {performanceData.topPerformer.passRate.toFixed(0)}% pass rate)
                </li>
                <li className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 bg-green-500 rounded-full"></div>
                ...
                <li className="flex items-center gap-2">
                  ...
                  {performanceData.overallAverage >= 60 ? 'above' : 'approaching'} 60%
                </li>
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-1 text-xs text-gray-600">
                <li className="flex items-center gap-2">
                  ...
                  {performanceData.needsAttention.name} needs focused attention (
                  {(100 - performanceData.needsAttention.passRate).toFixed(0)}% failure rate)
                </li>
                <li className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 bg-amber-500 rounded-full"></div>
            ...
            <button className="flex items-center gap-3 p-3 bg-white rounded-lg border border-indigo-200 hover:border-indigo-300 transition-colors text-left">
              ...
              <div>
                ...
                <p className="text-xs text-indigo-600">
                  For {performanceData.needsAttention.name} underperformers
                </p>
              </div>
            </button>
            <button className="flex items-center gap-3 p-3 bg-white rounded-lg border border-indigo-200 hover:border-indigo-300 transition-colors text-left">
