...
const WorkflowStepper: React.FC<WorkflowStepperProps> = ({ exam, onStepClick }) => {
  ...
  const getStepStatus = (stepKey: WorkflowStage) => {
    ...
    if (stepKey === "invigilation") return allSessionsCovered ? "completed" : exam.timetable.length > 0 ? "current" : "locked";
    if (stepKey === "marks") return exam.marks.length === exam.subjects.length ? "completed" : allSessionsCovered ? "current" : "locked";
