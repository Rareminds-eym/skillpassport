...
type FilterKey = "all" | "unread" | "offers" | "interviews" | "pipeline";
...
const NotificationPanel: React.FC<NotificationPanelProps> = ({
  ...
  const {
    ...
    loading,
    error,
  } = useNotifications(emailToUse); // ✅ Pass email here
  ...
  const getNotificationIcon = (type: NotificationType) => {
    ...
    switch (type) {
      ...
      case "offer_created":
        return <CheckCircleIcon className={`${base} text-blue-500`} />;
      case "offer_expiring":
    ...
    <>
      ...
      <div className="absolute right-0 mt-2 w-96 bg-white rounded-xl shadow-2xl z-50 overflow-hidden">
        ...
        <div className="flex gap-2 px-4 py-2 border-b border-gray-100">
          {(["all", "unread", "offers", "interviews", "pipeline"] as FilterKey[]).map((key) => {
