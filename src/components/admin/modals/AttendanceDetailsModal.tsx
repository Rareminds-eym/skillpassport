...
const AttendanceDetailsModal = ({
  ...
  const session =
    subjectGroup.sessions.find((s) => s.id === selectedSessionId) || subjectGroup.sessions[0];
  ...
  const sessionRecords = allRecords.filter(
    (r) => r.date === session.date && r.subject === subjectGroup.subject
  );
  ...
  const getStudentHistory = (studentId: string) => {
    return allRecords.filter(
      (record) => record.studentId === studentId && record.subject === subjectGroup.subject
    );
  };
  ...
  const getMonthlyData = (month: string) => {
    const monthRecords = allRecords.filter(
      (record) => record.subject === subjectGroup.subject && record.date.startsWith(month)
    );
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                ...
                <span className="flex items-center gap-1">
                  ...
                  {subjectGroup.totalSessions} Total Session
                  {subjectGroup.totalSessions !== 1 ? 's' : ''}
                </span>
              </div>
            </div>
            <button
                ...
                >
                  {subjectGroup.sessions.map((sess, index) => (
                    <option key={sess.id} value={sess.id}>
                      Session {subjectGroup.sessions.length - index} - {formatDate(sess.date)} (
                      {formatTime(sess.startTime)} - {formatTime(sess.endTime)}) -{' '}
                      {sess.attendancePercentage.toFixed(1)}% attendance
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-4">
              ...
              <div className="text-center p-3 bg-green-50 rounded-lg border border-green-200">
                <p className="text-2xl font-bold text-green-800">{session.presentCount}</p>
                <p className="text-xs text-green-600 mt-1">Present</p>
              </div>
              <div className="text-center p-3 bg-red-50 rounded-lg border border-red-200">
                <p className="text-2xl font-bold text-red-800">{session.absentCount}</p>
                <p className="text-xs text-red-600 mt-1">Absent</p>
              </div>
              <div className="text-center p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                <p className="text-2xl font-bold text-yellow-800">{session.lateCount}</p>
                <p className="text-xs text-yellow-600 mt-1">Late</p>
              </div>
              <div className="text-center p-3 bg-blue-50 rounded-lg border border-blue-200">
                <p className="text-2xl font-bold text-blue-800">{session.excusedCount}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
              <div>
                <strong>Course:</strong> {subjectGroup.course} - Sem {subjectGroup.semester} (
              ...
              <div>
                <strong>Total Sessions:</strong> {subjectGroup.totalSessions}
              </div>
            </div>
          </div>
                ...
                onClick={() => {
                  if (activeTab === 'today') {
                    ...
                    const subjectRecords = allRecords.filter(
                      (r) => r.subject === subjectGroup.subject
                    );
