...
const MentorDetailsDrawer: React.FC<MentorDetailsDrawerProps> = ({
  ...
  const getAllocationDisplayStatus = (allocation: MentorAllocation) => {
    ...
    const startDate = new Date(
      ...
      allocation.period?.start_date || allocation.allocationPeriod?.startDate
    );
  ...
  const shouldShowAllocation = (allocation: MentorAllocation) => {
    ...
    return (
      ...
      ['active', 'pending', 'transferred', 'available'].includes(allocation.status)
    );
  };
  ...
  const getLatestAllocation = () => {
    const visibleAllocations = mentor.allocations.filter((allocation) =>
      shouldShowAllocation(allocation)
    );
    return visibleAllocations.sort(
  ...
  return (
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        <div
          className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity inset-y-0 right-0 pl-10 max-w-full flex sm:pl-16"
          onClick={onClose}
        ></div>
              ...
              <div className="flex-1 overflow-y-auto">
                <div className="p-6 space-y-8">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-sm text-gray-500">
                          {mentor.allocations.filter((a) => shouldShowAllocation(a)).length} period
                    ...
                    {mentor.allocations.filter((allocation) => shouldShowAllocation(allocation))
                      .length === 0 ? (
                      <div className="text-center py-12 text-gray-500 bg-gray-50 rounded-xl">
                        ...
                        <p className="text-sm">
                          Allocations will appear here once students are assigned to this mentor
                        </p>
                      </div>
                    ) : (
                        ...
                        {mentor.allocations
                          ...
                          .sort((a, b) => {
                            ...
                            const dateA = new Date(
                              // @ts-expect-error - Auto-suppressed for migration
                              a.period?.start_date || a.allocationPeriod?.startDate
                            );
                            const dateB = new Date(
                              // @ts-expect-error - Auto-suppressed for migration
                              b.period?.start_date || b.allocationPeriod?.startDate
                            );
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-xs text-gray-600">
                                        // @ts-expect-error - Auto-suppressed for migration
                                        {allocation.period?.start_date ||
                                          allocation.allocationPeriod?.startDate}{' '}
                                        to{' '}
                                        // @ts-expect-error - Auto-suppressed for migration
                                        {allocation.period?.end_date ||
                                          allocation.allocationPeriod?.endDate}
                                        // @ts-expect-error - Auto-suppressed for migration
                                        {allocation.period?.academic_year &&
                                          // @ts-expect-error - Auto-suppressed for migration
                                          ` • ${allocation.period.academic_year}`}
                                        // @ts-expect-error - Auto-suppressed for migration
                                        {allocation.period?.semester &&
                                          // @ts-expect-error - Auto-suppressed for migration
                                          ` • ${allocation.period.semester}`}
                                      </p>
                                    </div>
                                    ...
                                    <div className="text-right">
                                      <div className="flex items-center gap-2 mb-1">
                                        <span className="text-sm font-medium text-gray-900">
                                          ...
                                          {allocation.capacity ||
                                            // @ts-expect-error - Auto-suppressed for migration
                                            allocation.period?.default_mentor_capacity ||
                                            15}
                                        </span>
                                        <span
                                    ...
                                    <div>
                                      ...
                                      {allocation.students.length === 0 ? (
                                        <div className="text-center py-4 text-gray-400 bg-gray-50 rounded-lg">
                                          <p className="text-sm">
                                            No students in this allocation period
                                          </p>
                                        </div>
                                      ) : (
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-sm text-gray-500">
                        {notes.length} total interventions
                      </span>
                    </div>
                    ...
                    {notes.length === 0 ? (
                      ...
                      <div className="space-y-4">
                        {notes.slice(0, 20).map((note: MentorNote) => {
                          const student = getAllocatedStudents().find(
                            (s: Student) => s.id === note.studentId
                          );
                          const hasEducatorResponse =
                            note.educator_response || note.action_taken || note.next_steps;
                            ...
                            >
                              ...
                              <div className="bg-gradient-to-r from-gray-50 to-white px-6 py-4 border-b border-gray-100">
                                <div className="flex items-start justify-between">
                                  ...
                                  <div className="flex flex-wrap gap-2 justify-end max-w-md">
                                    {note.priority && (
                                      <span
                                        className={`px-3 py-1 text-xs font-semibold rounded-full border ${getPriorityColor(note.priority)}`}
                                      >
                                        {note.priority.toUpperCase()}
                                      </span>
                                    )}
                                    <span
                                      className={`px-3 py-1 text-xs font-medium rounded-full ${getStatusColor(note.status)}`}
                                    >
                                      {note.status.replace('-', ' ').toUpperCase()}
                                    </span>
                                    <span
                                      className={`px-3 py-1 text-xs font-medium rounded-full ${getInterventionTypeColor(note.interventionType)}`}
                                    >
                                      {note.interventionType.charAt(0).toUpperCase() +
                                        note.interventionType.slice(1)}
                                    </span>
                                    {note.isPrivate && (
                              ...
                              <div className="px-6 py-5 space-y-4">
                                ...
                                {note.action_taken && (
                                  <div
                                    className={
                                      note.educator_response ? '' : 'pt-4 border-t border-gray-100'
                                    }
                                  >
                                    <div className="flex items-center gap-2 mb-2">
                              ...
                              {onViewConversation && (
                                <div className="px-6 pb-5">
                                  ...
                                  >
                                    ...
                                    {hasEducatorResponse
                                      ? 'View Full Conversation & Respond'
