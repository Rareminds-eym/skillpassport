...
>(
  (
    ...
    const handleFiles = async (files: File[]) => {
      ...
      const totalFiles =
        uploadedFiles.length + existingFilesList.length + stagedFiles.length + validFiles.length;
      if (totalFiles > maxFiles) {
        const remaining =
          maxFiles - uploadedFiles.length - existingFilesList.length - stagedFiles.length;
    ...
    const uploadFiles = async (filesToUpload: UploadedFile[]) => {
      if (!assignmentId) {
        showNotificationModal(
          'error',
          'Upload Error',
          'Assignment ID is required for file upload. Please save the assignment first.'
        );
        return;
      }
      ...
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        try {
          ...
          // Upload file
          const result = await uploadInstructionFile(assignmentId, uploadedFile.file, token);

          // Update status to success
          setUploadedFiles((prev) =>
            prev.map((f) =>
              f.file === uploadedFile.file ? { ...f, status: 'success', progress: 100, result } : f
            )
          );
    ...
    const uploadStagedFiles = async (newAssignmentId: string) => {
      ...
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        try {
          ...
          // Upload file
          const result = await uploadInstructionFile(newAssignmentId, uploadedFile.file, token);
        ...
        {stagedFiles.length > 0 && (
          <div className="mt-4 space-y-2">
            ...
            <div className="text-xs text-yellow-700 bg-yellow-50 p-2 rounded border border-yellow-200">
              <strong>Note:</strong> These files will be uploaded when you{' '}
              {existingFilesList.length > 0 ? 'update' : 'save'} the assignment.
            </div>
          </div>
        )}
