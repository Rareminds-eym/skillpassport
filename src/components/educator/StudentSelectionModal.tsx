...
const StudentSelectionModal: React.FC<StudentSelectionModalProps> = ({
    ...
    try {
      ...
      if (!error && data) {
        const names = data.map(
          (cls) => cls.name || `Grade ${cls.grade} - ${cls.section || 'General'}`
        );
        setClassNames(names);
      }
    } catch (error) {
  ...
  const fetchStudents = async () => {
    try {
      ...
      let query = supabase.from('students').select('*').eq('is_deleted', false);
      ...
      const transformedStudents = (data || []).map((student) => {
        ...
        return {
          ...
          college_school_name:
            student.college_school_name || profile.collegeName || profile.college_school_name || '',
          registration_number:
          ...
          contact_number:
            student.contact_number || student.contactNumber || profile.contact_number || '',
  ...
  const fetchFiltersData = async () => {
    try {
      ...
      let query = supabase.from('students').select('profile').eq('is_deleted', false);
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-xl flex flex-col">
        ...
        <div className="px-6 py-3 bg-white border-b border-gray-200 flex items-center justify-between">
          ...
          <div className="text-sm font-medium text-emerald-600">{selectedIds.length} selected</div>
        </div>
                ...
                return (
                  <label
                    ...
                    className={`flex items-center p-4 bg-white border rounded-lg hover:bg-gray-50 cursor-pointer transition-colors ${
                      isAlreadyAssigned ? 'border-emerald-300 bg-emerald-50/30' : 'border-gray-200'
                    }`}
                  >
                    <input
                      type="checkbox"
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
          ...
          >
            {assigning ? (
              ...
              <span>
                Assign to {selectedIds.length} {selectedIds.length === 1 ? 'Student' : 'Students'}
              </span>
            )}
          </button>
        </div>
