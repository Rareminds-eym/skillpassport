...
const ManageStudentsModal: React.FC<Props> = ({
  isOpen,
  onClose,
  classItem,
  onStudentsUpdated,
    ...
    Papa.parse<Record<string, string>>(file, {
      ...
      complete: (results) => {
        const students = results.data
          .map((row) => {
            if (!row) return null;
            const normalized = Object.entries(row).reduce<Record<string, string>>(
              (acc, [key, value]) => {
            ...
            if (
              ...
              (Number.isNaN(progressValue) || progressValue < 0 || progressValue > 100)
            ) {
              return null;
            }
            return {
                    ...
                    <>
                      <div>
                        ...
                        <p className="mt-1 text-xs text-gray-500">
                          CSV should have columns: name, email, progress (optional)
                        </p>
                      </div>
                      ...
                      {csvStudents.length > 0 && (
                        <p className="text-xs text-gray-500">
                          {csvStudents.length} student{csvStudents.length === 1 ? '' : 's'} ready to
                          add
                        </p>
                      )}
                    </>
