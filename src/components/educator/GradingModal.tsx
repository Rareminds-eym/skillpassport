                    ...
                    <div className="w-full md:w-2/5 lg:w-1/3 border-r border-gray-200 flex flex-col min-h-0">
                        <div className="p-3 md:p-4 border-b border-gray-200 bg-gray-50 shrink-0">
                            ...
                            >
                                ...
                                <option value="submitted">Submitted ({students.filter(s => s.status === 'submitted').length})</option>
                                <option value="graded">Graded ({students.filter(s => s.status === 'graded').length})</option>
                                <option value="in-progress">In Progress ({students.filter(s => s.status === 'in-progress').length})</option>
                                <option value="todo">Not Started ({students.filter(s => s.status === 'todo').length})</option>
                            </select>
                        </div>
                                ...
                                <div className="divide-y divide-gray-100">
                                    {filteredStudents.map((student) => (
                                        <button
                                            ...
                                            className={`w-full text-left p-3 md:p-4 hover:bg-gray-50 transition-colors ${selectedStudent?.student_assignment_id === student.student_assignment_id
                                        ...
                                        >
                                            <div className="flex items-start justify-between gap-2 mb-2">
                                                <div className="flex-1 min-w-0">
                                                    <h4 className="font-medium text-gray-900 text-sm md:text-base truncate">{student.student?.name}</h4>
                                                    <p className="text-xs text-gray-500 mt-0.5 truncate">{student.student?.email}</p>
                                                </div>
                                                <StudentStatusBadge status={student.status} />
                                            </div>

                                            {student.student?.university && (
                                                <p className="text-xs text-gray-600 mb-1 truncate">{student.student.university}</p>
                                            )}

                                            <div className="flex items-center justify-between gap-2 mt-2">
                                                {student.status === 'graded' && student.grade_received !== null && student.grade_received !== undefined ? (
                            ...
                            <>
                                <div className="flex-1 overflow-y-auto p-4 md:p-6 min-h-0">
                                    <div className="mb-6">
                                        ...
                                        <p className="text-sm text-gray-600 truncate">{selectedStudent.student?.email}</p>
                                            ...
                                            <div className="bg-emerald-50 rounded-lg p-4 border border-emerald-200">
                                                <div className="flex items-start gap-2">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <p className="text-sm text-emerald-800">{selectedStudent.instructor_feedback}</p>
                                ...
                                <div className="border-t border-gray-200 p-4 md:p-6 bg-gray-50 shrink-0">
                                    <div className="flex gap-3">
                                        ...
                                        >
                                            {submitting ? 'Submitting...' : selectedStudent.status === 'graded' ? 'Update Grade' : 'Submit Grade'}
                                        </button>
                                    </div>
                                </div>
                            </>
                                ...
                                <div className="divide-y divide-gray-100">
                                    {filteredStudents.map((student) => (
                                        ...
                                        >
                                            <div className="flex items-start justify-between gap-2 mb-2">
                                                <div className="flex-1 min-w-0">
                                                    <h4 className="font-medium text-gray-900 text-sm truncate">{student.student?.name}</h4>
                                                    <p className="text-xs text-gray-500 mt-0.5 truncate">{student.student?.email}</p>
                                                </div>
                                                <StudentStatusBadge status={student.status} />
                                            </div>
                            ...
                            <>
                                            ...
                                            <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-200">
                                                <div className="flex items-start gap-2">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <p className="text-xs text-emerald-800">{selectedStudent.instructor_feedback}</p>
                                                        {selectedStudent.graded_date && (
                                ...
                                <div className="border-t border-gray-200 p-4 bg-gray-50 shrink-0">
                                    <div className="flex gap-2">
                                        ...
                                        >
                                            {submitting ? 'Saving...' : selectedStudent.status === 'graded' ? 'Update' : 'Submit'}
                                        </button>
                                    </div>
                                </div>
                            </>
