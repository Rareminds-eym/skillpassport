  ...
  const totals = useMemo(() => {
    ...
    cartItems.forEach(({ type, item }) => {
      if (type === 'addon') {
        ...
        subtotal += billingPeriod === 'monthly' ? item.monthly_price : item.annual_price;
      }
    });
  ...
  return (
    <div className={`bg-white rounded-xl border border-gray-200 overflow-hidden ${className}`}>
      ...
      <div className="p-4">
        ...
        <p className="text-xs text-gray-500 text-center mt-3">Secure payment powered by Razorpay</p>
      </div>
    </div>
  );
}
