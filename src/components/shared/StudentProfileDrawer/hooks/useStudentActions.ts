...
export const useStudentActions = (student: Student | null) => {
  ...
  const canGraduate = () => {
    ...
    const readyToGraduate =
      ...
      (student.expectedGraduationDate && new Date() >= new Date(student.expectedGraduationDate));
  ...
  const handlePromotion = async () => {
    ...
    try {
      ...
      if (nextSem > totalSems) {
        toast.error(
          `Student is already in final semester (${currentSem}/${totalSems}). Cannot promote further.`
        );
        return;
      }
      ...
      console.log(
        '📅 Using academic year for promotion:',
        academicYear,
        'from admission year:',
      ...
      const promotionData: any = {
        ...
        overall_percentage: student.currentCgpa
          ? parseFloat(student.currentCgpa.toString()) * 10
          : null,
        overall_grade: student.currentCgpa
          ...
          : null,
        overall_grade_point: student.currentCgpa
          ? parseFloat(student.currentCgpa.toString())
          : null,
      };
      ...
      if (promotionError) {
        ...
        if (promotionError.code === '23503') {
          // Foreign key constraint violation
          console.warn(
            'Skipping promotion record due to foreign key constraint. Proceeding with semester update.'
          );
          // Continue with student semester update even if promotion record fails
        } else if (promotionError.code === '23505') {
