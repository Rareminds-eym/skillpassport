...
function mapToUICandidate(row: StudentRow): UICandidate {
  ...
  const rawDial =
    row.contact_dial_code || profile.contact_number_dial_code
      ? String(row.contact_dial_code || profile.contact_number_dial_code).replace(/\.0$/, '')
      : '';
  // Remove any existing + from dial code to avoid double ++
  const dial = rawDial.replace(/^\+/, '');
  const phoneNum =
    row.contact_number || profile.contact_number
      ? String(row.contact_number || profile.contact_number).replace(/\.0$/, '')
      : '';
  const altNum =
    row.alternate_number || profile.alternate_number
      ? String(row.alternate_number || profile.alternate_number).replace(/\.0$/, '')
      : '';
  const phone =
    phoneNum || altNum ? `${dial ? '+' + dial + ' ' : ''}${phoneNum || altNum}` : undefined;

  // Use direct fields for college and department
  const college =
    row.college_school_name || row.university || profile.college_school_name || profile.university;
  ...
  const skills =
    rawSkills.length > 0 ? rawSkills.map((s) => s.name || s.title || s).filter(Boolean) : subjects; // Fallback to subjects if no skills table data
  ...
  useEffect(() => {
    ...
    const fetchStudents = async () => {
      ...
      try {
        ...
        console.log(
          `✅ Fetched ${result.data?.length || 0} students for ${filterType}: ${filterId}`
        );
