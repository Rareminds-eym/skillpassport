...
import { AUTH_ERROR_CODES, generateCorrelationId, logAuthEvent } from '../utils/authErrorHandler';
  ...
  const initializeAuth = useCallback(async () => {
    ...
    try {
      ...
      if (authResult.error && retryCountRef.current < MAX_RETRY_ATTEMPTS) {
        ...
        logAuthEvent('warn', `Auth init retry ${retryCountRef.current}/${MAX_RETRY_ATTEMPTS}`, {
          correlationId,
        });
