...
export const useAddOnTracking = () => {
   ...
   */
  const trackAddOnView = useCallback(
    async (featureKey, metadata = {}) => {
      ...
      await addOnAnalyticsService.trackEvent(user?.id, 'view', featureKey, {
        ...metadata,
   ...
   * @param {Object} metadata - Additional metadata
   */
  const trackBundleView = useCallback(
    async (bundleId, metadata = {}) => {
      const viewKey = `bundle_${bundleId}_${sessionId.current}`;
      if (trackedViews.current.has(viewKey)) return;

      trackedViews.current.add(viewKey);

      await addOnAnalyticsService.trackEvent(user?.id, 'bundle_view', null, {
        ...metadata,
        bundleId,
        sessionId: sessionId.current,
      });
  ...
  const trackUpgradePrompt = useCallback(
    async (featureKey, metadata = {}) => {
      await addOnAnalyticsService.trackEvent(user?.id, 'upgrade_prompt', featureKey, {
        ...metadata,
        sessionId: sessionId.current,
  ...
  const trackPurchaseFunnel = useCallback(
    async (step, featureKey, metadata = {}) => {
      const eventTypeMap = {
        cart_add: 'view',
        checkout_start: 'view',
        payment_init: 'view',
      ...
      await addOnAnalyticsService.trackEvent(user?.id, eventTypeMap[step] || 'view', featureKey, {
        ...metadata,
        funnelStep: step,
        sessionId: sessionId.current,
  ...
  const trackPurchase = useCallback(
    async (featureKey, purchaseDetails = {}) => {
      await addOnAnalyticsService.trackEvent(user?.id, 'purchase', featureKey, {
        ...purchaseDetails,
        sessionId: sessionId.current,
   ...
   * @param {Object} purchaseDetails - Purchase details
   */
  const trackBundlePurchase = useCallback(
    async (bundleId, purchaseDetails = {}) => {
      await addOnAnalyticsService.trackEvent(user?.id, 'bundle_purchase', null, {
        ...purchaseDetails,
        bundleId,
        sessionId: sessionId.current,
        billingPeriod: purchaseDetails.billingPeriod,
  ...
  const trackActivation = useCallback(
    async (featureKey, metadata = {}) => {
      await addOnAnalyticsService.trackEvent(user?.id, 'activation', featureKey, {
        ...metadata,
        sessionId: sessionId.current,
   ...
   * @param {Object} metadata - Additional metadata
   */
  const trackCancellation = useCallback(
    async (featureKey, reason, metadata = {}) => {
      await addOnAnalyticsService.trackEvent(user?.id, 'cancellation', featureKey, {
        ...metadata,
        sessionId: sessionId.current,
   ...
   * @param {Object} metadata - Additional metadata
   */
  const trackDiscountApplied = useCallback(
    async (code, success, metadata = {}) => {
      await addOnAnalyticsService.trackEvent(user?.id, 'discount_applied', null, {
        ...metadata,
        sessionId: sessionId.current,
        discountCode: code,
        success,
   ...
   * @param {Object} metadata - Additional metadata
   */
  const trackRenewal = useCallback(
    async (featureKey, metadata = {}) => {
      await addOnAnalyticsService.trackEvent(user?.id, 'renewal', featureKey, {
        ...metadata,
        sessionId: sessionId.current,
   ...
   * @param {Object} metadata - Additional metadata
   */
  const trackGracePeriod = useCallback(
    async (type, featureKey, metadata = {}) => {
      const eventType = type === 'started' ? 'grace_period_started' : 'grace_period_ended';

      await addOnAnalyticsService.trackEvent(user?.id, eventType, featureKey, {
        ...metadata,
        sessionId: sessionId.current,
        gracePeriodDays: 7,
