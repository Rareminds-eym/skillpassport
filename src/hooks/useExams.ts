...
export const useExams = (schoolId?: string, collegeId?: string) => {
  ...
  const transformAssessmentToExam = useCallback((assessment: Assessment, timetable: ExamTimetable[] = [], markEntries: any[] = [], invigilations: any[] = []): UIExam => {
      ...
      if (subject) {
        const studentMarks: UIStudentMark[] = entries.map(entry => {
          ...
          return {
            ...
            moderationReason: hasModeration ? (entry.mark_moderation_log?.[0]?.reason || entry.moderation_reason) : null,
            moderationType: hasModeration ? entry.mark_moderation_log?.[0]?.moderation_type : null,
            markEntryId: entry.id
          };
        });

        // Check if this subject has any moderation (either in logs or in mark entries)
        const hasModeration = entries.some(entry => 
          (entry.mark_moderation_log && entry.mark_moderation_log.length > 0) ||
        ...
        const isModerated = !hasModeration || 
          (hasModeration && allSubjectLogs.length > 0 && allSubjectLogs.every((log: any) => log.approval_status === 'approved'));
    ...
    return {
      ...
      grade: assessment.target_classes?.grade || assessment.syllabus_coverage?.grade || assessment.course_code?.split('-')[0] || String(assessment.semester) || '10',
      section: assessment.target_classes?.type === 'single_section' ? assessment.target_classes.sections?.[0] : undefined,
      academicYear: assessment.academic_year,
      startDate: assessment.start_date || timetable[0]?.exam_date || new Date().toISOString().split('T')[0],
      endDate: assessment.end_date || timetable[timetable.length - 1]?.exam_date || new Date().toISOString().split('T')[0],
      subjects,
  ...
  const loadData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      for (const assessment of assessmentsData) {
        ...
        transformedExams.push(transformAssessmentToExam(assessment, timetable, allMarkEntries, invigilationsWithEducatorIds));
      }
  ...
  const createExam = useCallback(async (examData: Partial<UIExam>, userId?: string) => {
    try {
      ...
      const assessment: Partial<Assessment> = {
        ...
        course_name: examData.subjects && examData.subjects.length > 1 
        ...
        duration_minutes: examData.subjects?.reduce((total, subject) => total + (subject.duration || 0), 0) || 180,
        total_marks: examData.subjects?.reduce((total, subject) => total + (subject.totalMarks || 0), 0) || 100,
        pass_marks: examData.subjects?.reduce((total, subject) => total + (subject.passingMarks || 0), 0) || 35,
        
        // Store all subjects in syllabus_coverage JSONB field
        syllabus_coverage: examData.subjects && examData.subjects.length > 0 ? {
  ...
  const createTimetableEntry = useCallback(async (examId: string, entry: Omit<UITimetableEntry, 'id'>) => {
    try {
      ...
      if (exam.startDate && exam.endDate) {
        if (entry.date < exam.startDate || entry.date > exam.endDate) {
          throw new Error(`Exam date must be between ${new Date(exam.startDate).toLocaleDateString()} and ${new Date(exam.endDate).toLocaleDateString()}`);
        }
      }
  ...
  const deleteTimetableEntry = useCallback(async (examId: string, entryId: string) => {
    try {
  ...
  const createInvigilationAssignment = useCallback(async (examId: string, assignment: {
    timetableEntryId: string;
    teacherId: string;
    teacherName: string;
    room?: string;
  ...
  const deleteInvigilationAssignment = useCallback(async (examId: string, assignmentId: string) => {
    try {
  ...
  const approveSubjectModeration = useCallback(async (examId: string, subjectId: string, userId?: string) => {
    try {
      ...
      const result = await examsService.approveSubjectModeration(examId, subjectId, currentUserId);
  ...
  const getSections = useCallback((grade?: string, academicYear?: string) => {
    if (!grade) return [];
