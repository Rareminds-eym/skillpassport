...
import { useLearnerData } from '@/entities/learner';
import {
  EducationEditModal,
  TrainingEditModal,
  ExperienceEditModal,
 ...
 */
const DashboardWithSupabase = ({ learnerId }) => {
  const [activeModal, setActiveModal] = useState(null);

  // Use the custom hook to fetch and manage learner data
  const {
    learnerData,
    loading,
    error,
    ...
    deleteSoftSkill,
    refresh
  } = useLearnerData(learnerId, true); // true = fallback to mock data
  ...
  const training = learnerData?.training || [];
  ...
  const opportunities = learnerData?.opportunities || [];
  const recentUpdates = learnerData?.recentUpdates || [];
  const suggestions = learnerData?.suggestions || [];
