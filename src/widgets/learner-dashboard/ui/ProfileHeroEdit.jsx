...
const ProfileHeroEdit = ({ onEditClick, learnerData: propLearnerData, loading: propLoading }) => {
  ...
  useEffect(() => {
    if (reallearnerData) {
      // Pass the entire reallearnerData object which contains profile, technicalSkills, softSkills, etc.
      const scoreData = calculateEmployabilityScore(reallearnerData);
      setEmployabilityData(scoreData);

      // Generate badges based on learner data
      const badges = generateBadges(reallearnerData);
  ...
  // Calculate graduation year for school learners
  const getGraduationYear = () => {
    if (!reallearnerData) return null;

    // For college learners, check profile.classYear first
    if (reallearnerData.university_college_id && reallearnerData.profile?.classYear) {
      return reallearnerData.profile.classYear;
    }

    // For university/college learners, use expectedGraduationDate
    if (reallearnerData.expectedGraduationDate) {
      return new Date(reallearnerData.expectedGraduationDate).getFullYear();
    }

    // For school learners, calculate based on current grade
    if (reallearnerData.school_id && reallearnerData.grade) {
      const currentYear = new Date().getFullYear();
      const grade = parseInt(reallearnerData.grade);
  ...
  const displayData = reallearnerData ? {
    name: reallearnerData.name,
    email: reallearnerData.email,
    department: reallearnerData.branch_field,
    university: typeof reallearnerData.university === 'object' ? reallearnerData.university?.name : reallearnerData.university,
    classYear: graduationYear || reallearnerData.profile?.classYear || null,
    github_link: reallearnerData.github_link || reallearnerData.profile?.github_link,
    portfolio_link: reallearnerData.portfolio_link || reallearnerData.profile?.portfolio_link,
    linkedin_link: reallearnerData.linkedin_link || reallearnerData.profile?.linkedin_link,
    twitter_link: reallearnerData.twitter_link || reallearnerData.profile?.twitter_link,
    instagram_link: reallearnerData.instagram_link || reallearnerData.profile?.instagram_link,
    facebook_link: reallearnerData.facebook_link || reallearnerData.profile?.facebook_link,
    youtube_link: reallearnerData.youtube_link || reallearnerData.profile?.youtube_link,
    degree: reallearnerData.branch_field,
    // Fallback to profile JSONB for any missing data
    ...reallearnerData.profile
  } : null;

  // Debug logging for learner type detection
  // console.log('DEBUG: learner_type =', reallearnerData?.learner_type, '| school_id =', reallearnerData?.school_id, '| isSchool =', (reallearnerData?.learner_type === 'school' || reallearnerData?.school_id) ? 'YES' : 'NO');

  // Fetch institution name and location if not in relationship data
  useEffect(() => {
    const fetchInstitutionName = async () => {
      if (!reallearnerData) return;
  ...
  const institutionName = React.useMemo(() => {
    if (!reallearnerData) return "Institution";

    // Priority 1: School relationship (school_id)
    if (reallearnerData.school_id && (reallearnerData.schools?.name || fetchedInstitutionName)) {
      return reallearnerData.schools?.name || fetchedInstitutionName;
    }

    // Priority 2: University college relationship (university_college_id)
    if (reallearnerData.university_college_id) {
      if (reallearnerData.university_colleges) {
        const college = reallearnerData.university_colleges;
    ...
    if (reallearnerData.college_id && reallearnerData.college?.name) {
      return reallearnerData.college.name;
    }

    // Priority 4: Custom entries - college_school_name column
    if (reallearnerData.college_school_name) {
      return reallearnerData.college_school_name;
    }

    // Priority 5: Custom entries - university column
    if (reallearnerData.university) {
      const universityName = typeof reallearnerData.university === 'object'
        ? reallearnerData.university?.name
        : reallearnerData.university;
      if (universityName) return universityName;
    }

    // Priority 6: Profile university
    if (reallearnerData.profile?.university) {
      return reallearnerData.profile.university;
    }
    ...
    if (userRole === 'learner') {
      ...
      if (reallearnerData.experience && reallearnerData.experience.length > 0) {
        const currentJob = reallearnerData.experience.find(exp => exp.end_date === null || exp.end_date === undefined);
        ...
        const sortedExperience = [...reallearnerData.experience].sort((a, b) => {
      ...
      if (reallearnerData.education && reallearnerData.education.length > 0) {
        const sortedEducation = [...reallearnerData.education].sort((a, b) => {
    ...
    return "Institution";
  }, [reallearnerData, fetchedInstitutionName, userRole]);

  // Determine institution location from relationships or fetched data
  const institutionLocation = React.useMemo(() => {
    if (!reallearnerData) return null;

    // For school learners - check schools relationship first
    if (reallearnerData.school_id) {
      if (reallearnerData.schools?.city || reallearnerData.schools?.state) {
        const locationParts = [
          reallearnerData.schools.city,
          reallearnerData.schools.state
        ].filter(Boolean);
    ...
    // For college learners - check university_colleges relationship
    if (reallearnerData.university_college_id) {
      if (reallearnerData.university_colleges?.universities) {
        const university = reallearnerData.university_colleges.universities;
        const locationParts = [
          university.district,
          university.state
        ].filter(Boolean);
    ...
    return null;
  }, [reallearnerData, fetchedInstitutionLocation]);



  // Generate QR code value once and keep it constant
  const qrCodeValue = React.useMemo(() => {
    const learnerId = reallearnerData?.id || "learner";
    return `${window.location.origin}/learner/profile/${learnerId}`;
  }, [reallearnerData?.id]);
  ...
  if (!displayData) {
    return (
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-yellow-700 text-center">
        No learner data found. Please check your email or contact support.
      </div>
    );
  }
              ...
              <div className="w-full lg:w-64 flex-shrink-0 space-y-3 sm:space-y-4 order-2 lg:order-1 px-2 sm:pl-6">
                ...
                <Card variant="blue" className="bg-blue-50/60 backdrop-blur-xl border border-blue-200/60 rounded-xl sm:rounded-2xl shadow-2xl">
                  <CardContent className="p-3 sm:p-6 text-center">
                    ...
                    >
                      SKILL PASSPORT-ID : {
                        reallearnerData?.learner_id ||
                        (reallearnerData?.registration_number ? `SP-${reallearnerData.registration_number}` : null) ||
              ...
              <div className="flex-1 space-y-3 sm:space-y-6 order-1 lg:order-2 px-2 sm:pr-6 w-full">
                ...
                <div className="flex items-start gap-3 sm:gap-4">
                  <div className="relative flex-shrink-0">
                    ...
                    <div
                      className={`absolute -bottom-0.5 sm:-bottom-1 -right-0.5 sm:-right-1 w-5 sm:w-6 h-5 sm:h-6 rounded-full border-2 border-white flex items-center justify-center shadow-md cursor-help ${reallearnerData?.approval_status === 'approved'
                          ? 'bg-green-500'
                          : reallearnerData?.approval_status === 'pending'
                            ? 'bg-yellow-500'
                            : 'bg-red-400'
                        }`}
                      title={
                        reallearnerData?.approval_status === 'approved'
                          ? 'Verified Learner - Profile has been Verified'
                          : reallearnerData?.approval_status === 'pending'
                  ...
                  <div className="flex-1 pt-1 min-w-0">
                    <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                      <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 drop-shadow-sm break-words">
                        {capitalizeName(displayData.name) || "Learner Name"}
                      </h1>
                ...
                <div className="space-y-2 ml-1" style={{ display: 'none' }}>
                  {/* <div className="flex items-center gap-2 text-white">
                    ...
                    <span>
                      Learner ID:{" "}
                      {reallearnerData?.learner_id || "Not Assigned"}
                    </span>
                  </div> */}
                </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      {reallearnerData.grade && (
                        <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-white rounded-full text-xs sm:text-sm font-medium text-slate-900 shadow-sm">
                          <span className="text-gray-900">Grade:</span> {reallearnerData.grade}
                        </span>
                      )}
                      {reallearnerData.section && (
                        <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-white rounded-full text-xs sm:text-sm font-medium text-slate-900 shadow-sm">
                          <span className="text-gray-900">Sec:</span> {reallearnerData.section}
                        </span>
                      )}
                      {reallearnerData.roll_number && (
                        <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-white rounded-full text-xs sm:text-sm font-medium text-slate-900 shadow-sm">
                          <span className="text-gray-900">Roll:</span> {reallearnerData.roll_number}
                        </span>
                      )}
                      {reallearnerData.admission_number && (
                        <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-white rounded-full text-xs sm:text-sm font-medium text-slate-900 shadow-sm">
                          <span className="text-gray-900">Adm:</span> {reallearnerData.admission_number}
                        </span>
                      )}
                    </div>
                  </div>
                 ...
                 (reallearnerData?.registration_number || reallearnerData?.admission_number || reallearnerData?.branch_field || reallearnerData?.learner_id || reallearnerData?.roll_number || reallearnerData?.section) && (
                  <div className="bg-indigo-50/60 backdrop-blur-md rounded-lg sm:rounded-xl p-2 sm:p-3 border border-indigo-200/60 shadow-md">
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      ...
                      {reallearnerData.admission_number && (
                        <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-white rounded-full text-xs font-medium text-gray-700 shadow-sm">
                          <span className="text-gray-500">Adm:</span> {reallearnerData.admission_number}
                        </span>
                      )}
                      {reallearnerData.learner_id && (
                        <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-white rounded-full text-xs font-medium text-gray-700 shadow-sm">
                          <span className="text-gray-500">ID:</span> {reallearnerData.learner_id}
                        </span>
                      )}
                      {reallearnerData.roll_number && (
                        <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-white rounded-full text-xs font-medium text-gray-700 shadow-sm">
                          <span className="text-gray-500">Roll:</span> {reallearnerData.roll_number}
                        </span>
                      )}
                      ...
                      {reallearnerData.section && (
                        <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-white rounded-full text-xs font-medium text-gray-700 shadow-sm">
                          <span className="text-gray-500">Sec:</span> {reallearnerData.section}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {(displayData.classYear || displayData.department || displayData.degree) && (
                  <div className="flex flex-wrap gap-2 sm:gap-3">
                    ...
                    {(displayData.department || displayData.degree) && !reallearnerData?.branch_field && (
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-4 sm:p-6 w-full max-w-sm sm:max-w-lg mx-4 relative max-h-[90vh] overflow-y-auto border border-gray-100">
            ...
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <DetailItem label="Learner ID" value={
                ...
                (reallearnerData?.registration_number ? `SP-${reallearnerData.registration_number}` : null) ||
                displayData?.passportId ||
                displayData?.learnerId ||
                "N/A"
              } />
              <DetailItem
                label={(reallearnerData?.learner_type === 'school' || reallearnerData?.school_id) ? 'Grade/Section' : 'Department'}
                value={
                  (reallearnerData?.learner_type === 'school' || reallearnerData?.school_id)
                    ? (reallearnerData?.grade
                      ? `Grade ${reallearnerData.grade}${reallearnerData?.section ? ` - ${reallearnerData.section}` : ''}`
                      : 'N/A')
