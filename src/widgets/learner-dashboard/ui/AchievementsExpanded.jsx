...
const AchievementsExpanded = ({ learnerId, email }) => {
  const { achievements, badges, loading, error, refresh } = useLearnerAchievements(learnerId, email);
  ...
  const getIconByType = (type) => {
    switch (type) {
      ...
      case 'skill': return <Code className="w-5 h-5" />;
  ...
  const getColorByType = (type) => {
    switch (type) {
      ...
      case 'skill': return 'from-blue-500 to-indigo-500';
      case 'education': return 'from-purple-500 to-violet-500';
      case 'experience': return 'from-amber-500 to-orange-500';
    ...
    return (
      <Card className="bg-red-50 border-red-200">
        <CardContent className="p-6">
          ...
          <Button onClick={refresh} className="mt-4" size="sm">Retry</Button>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6" data-testid="achievements-expanded">
              ...
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 font-normal mt-1">Earned through your achievements</p>
                  </div>
                </CardTitle>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {badges.map((badge, index) => (
                  <motion.div
                    ...
                    className="relative"
                  >
                    <div className={`p-4 rounded-xl bg-gradient-to-br ${getRarityColor(badge.rarity)} shadow-lg hover:shadow-xl transition-all cursor-pointer group`}>
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="p-6">
          {viewMode === 'timeline' ? (
            <TimelineView achievements={filteredAchievements} getIconByType={getIconByType} getColorByType={getColorByType} />
          ) : (
            <CalendarView achievementsByMonth={achievementsByMonth} filterType={filterType} getIconByType={getIconByType} getColorByType={getColorByType} />
          )}
        </CardContent>
      </Card>
    </div>
  ...
  return (
    <div className="space-y-4">
      {achievements.map((achievement, index) => (
        ...
        >
          {/* Timeline dot */}
          <div className={`absolute left-[-17px] top-0 w-8 h-8 rounded-full bg-gradient-to-br ${getColorByType(achievement.type)} flex items-center justify-center shadow-lg`}>
          ...
          <div key={monthKey} className="space-y-3">
            <div className="flex items-center gap-3 sticky top-0 bg-white py-2 z-10">
              ...
              <Badge className="bg-blue-100 text-blue-700">
                {monthAchievements.length} {monthAchievements.length === 1 ? 'achievement' : 'achievements'}
              </Badge>
            </div>
              ...
              {monthAchievements.map((achievement, index) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${getColorByType(achievement.type)} flex items-center justify-center shadow-sm flex-shrink-0`}>
                      {getIconByType(achievement.type)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-gray-900 text-base truncate">{achievement.title}</h4>
                      {achievement.subtitle && (
                        <p className="text-sm text-gray-600 truncate">{achievement.subtitle}</p>
                      )}
                      <p className="text-xs text-gray-500 mt-1">
                        {new Date(achievement.date).toLocaleDateString('default', { day: 'numeric', month: 'short' })}
                      </p>
                      {achievement.verified && (
