...
const ProfileEditSection = ({ profileEmail }) => {
  ...
  const {
    ...
    updateProjects,
    updateCertificates
  } = useLearnerDataByEmail(displayEmail);

  // Extract data from Supabase or use fallback
  const education = learnerData?.education || educationData;
  const training = learnerData?.training || trainingData;
  const experience = learnerData?.experience || experienceData;
  const techSkills = learnerData?.technicalSkills || technicalSkills;
  const soft = learnerData?.softSkills || softSkills;
  ...
  }, [learnerData, education, training, experience, techSkills, soft]);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-slate-900 mb-4 tracking-tight">
            {isOwnProfile ? 'Your Profile' : learnerData?.profile?.name || 'Learner Profile'}
          </h1>
