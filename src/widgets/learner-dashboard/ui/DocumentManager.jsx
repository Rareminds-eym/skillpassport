/**
 * Document Manager Component for Learners
 * Allows learners to upload, view, and manage their documents
 */
...
import { 
  ...
  getlearnerDocumentUrl,
  validatelearnerDocument 
} from '@/features/learner-profile/api';

const DocumentManager = ({ learnerId, className = '' }) => {
  ...
  const loadDocuments = async () => {
    try {
      setLoading(true);
      const docs = await getlearnerDocuments(learnerId);
      setDocuments(docs);
    } catch (err) {
  ...
  const handleFileUpload = async (file, documentType) => {
    try {
      ...
      // Validate file
      const validation = validatelearnerDocument(file, documentType);
      if (!validation.valid) {
      ...
      // Upload document
      const result = await uploadlearnerDocument(learnerId, file, documentType);
  ...
  const handleDeleteDocument = async (documentUrl) => {
    ...
    try {
      const success = await deletelearnerDocument(learnerId, documentUrl);
      if (success) {
  ...
  const handleViewDocument = (documentUrl) => {
    const viewUrl = getlearnerDocumentUrl(documentUrl, 'inline');
    window.open(viewUrl, '_blank');
  };

  const handleDownloadDocument = (documentUrl, fileName) => {
    const downloadUrl = getlearnerDocumentUrl(documentUrl, 'download');
