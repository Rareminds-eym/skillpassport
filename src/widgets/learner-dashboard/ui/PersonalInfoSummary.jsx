...
const PersonalInfoSummary = ({ data, learnerData, isOwnProfile = true }) => {
  if (!data) return null;

  // Determine institution from learnerData
  const institution = React.useMemo(() => {
    ...
    if (learnerData?.college_id && learnerData?.college) {
    ...
    } else if (learnerData?.university || learnerData?.college || learnerData?.college_school_name) {
      ...
      const isUniversityPath = learnerData?.university || learnerData?.branch;
      // Check if it's a school path (has grade but no university)
      const isSchoolPath = learnerData?.grade && !isUniversityPath;
      
      if (isUniversityPath) {
        // University learner - show college name if available, otherwise university
        if (learnerData?.college) {
          type = 'College';
          name = learnerData.college;
        } else if (learnerData?.university) {
          type = 'University';
          name = learnerData.university;
        }
      } else if (isSchoolPath) {
        // School learner - show school name from college field
        if (learnerData?.college) {
          type = 'School';
          name = learnerData.college;
        }
        ...
        name = learnerData.college_school_name || learnerData.college || learnerData.university;
      }
