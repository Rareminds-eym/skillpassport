...
export const generateResumePDF = async (learnerData) => {
  ...
  const profile = { ...learnerData, ...parsedProfile };
...
if (profile.image) {
  ...
  const hasLinks = profile.linkedin || profile.github || profile.portfolio || 
                   (learnerData.link && (learnerData.link.linkedin || learnerData.link.github || learnerData.link.portfolio));
  
  if (hasLinks) {
    addSidebarTitle("Links");
    
    const linkedin = profile.linkedin || learnerData.link?.linkedin;
    if (linkedin) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(7);
      doc.setTextColor(255, 255, 255);
    ...
    const github = profile.github || learnerData.link?.github;
    if (github) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(7);
      doc.setTextColor(255, 255, 255);
    ...
    const portfolio = profile.portfolio || learnerData.link?.portfolio;
    if (portfolio) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(7);
      doc.setTextColor(255, 255, 255);
  ...
  const fullName = profile.name || "LEARNER NAME";
  ...
  const projectData = resolveArray(
    ...
    learnerData.profile?.profile?.projects
  ...
  const certificateData = resolveArray(
    ...
    learnerData.profile?.profile?.certificates
...
/**
 * Helper function to prepare learner data for resume
 */
export const preparelearnerDataForResume = (learnerData) => {
  const parsedProfile = safeParse(learnerData.profile);
  const profile = { ...learnerData, ...parsedProfile };
