...
import { useLearnerDataByEmail, useLearnerCertificates } from '@/entities/learner';
import { useLearnerRealtimeActivities } from '@/entities/learner/model/useLearnerRealtimeActivities';
import { useUser } from '@/shared/model/authStore';
console.log('🔍 [Dashboard] useLearnerDataByEmail imported:', useLearnerDataByEmail);
console.log('🔍 [Dashboard] typeof useLearnerDataByEmail:', typeof useLearnerDataByEmail);
...
const Dashboard = () => {
  ...
  // Get learner ID and email from auth store
  const user = useUser();
  const userEmail = user?.email;

  console.log('🔍 [Dashboard] About to call useLearnerDataByEmail with email:', userEmail);
  console.log('🔍 [Dashboard] useLearnerDataByEmail function:', useLearnerDataByEmail);

  // Fetch learner data by EMAIL (from real Supabase table)
  const {
    learnerData,
    loading,
    error,
    ...
    updateSoftSkills
  } = useLearnerDataByEmail(userEmail, false); // no fallback to mock data

  console.log('🔍 [Dashboard] Hook returned - learnerData:', learnerData, 'loading:', loading, 'error:', error);

  // Fetch real-time learner activities (replaces mock recentUpdates)
  const { 
  ...
  const profile = learnerData?.profile;
  const education = learnerData?.education || mockEducationData;
  const training = learnerData?.training || mockTrainingData;
  const experience = learnerData?.experience || mockExperienceData;
  const technicalSkills = learnerData?.technicalSkills || mockTechnicalSkills;
  const softSkills = learnerData?.softSkills || mockSoftSkills;
  
  // Use real activities instead of mock data
  const recentUpdates = recentActivities.length > 0 ? recentActivities : mockRecentUpdates;
  
  const suggestions = learnerData?.suggestions || mockSuggestions;
  const opportunities = learnerData?.opportunities || mockOpportunities;
  ...
  }, [learnerData, education, training, experience, technicalSkills, softSkills]);
  ...
  const renderCardsByPriority = () => {
    const allCards = {
      ...
      training: (
        <Card key="training" className="h-full shadow-lg hover:shadow-xl transition-shadow bg-gradient-to-br from-[#5378f1]/10 to-[#5378f1]/20 cursor-pointer" onClick={() => navigate('/learner/my-learning')}>
      ...
      experience: (
        <Card key="experience" className="h-full border-t-4 border-t-indigo-500 shadow-lg hover:shadow-xl transition-shadow cursor-pointer" onClick={() => navigate('/learner/my-experience')}>
            ...
            <CardTitle className="flex items-center justify-between text-indigo-700">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    navigate('/learner/my-experience');
                  }}
                >
                  View All →
                </Button>
      ...
      opportunities: (
  <Card key="opportunities" variant="orange" className="h-full border-2 border-[#FFB800] rounded-lg shadow-lg hover:shadow-xl transition-shadow cursor-pointer" onClick={() => navigate('/learner/opportunities')}>
      ...
      technicalSkills: (
        <Card key="technicalSkills" className="h-full border-t-4 border-t-slate-500 shadow-lg hover:shadow-xl transition-shadow cursor-pointer" onClick={() => navigate('/learner/my-skills')}>
          ...
          {loading ? (
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-sm font-medium text-red-700">No learner data found for this email.</span>
              </div>
              <p className="text-xs text-red-600 ml-5">{error}</p>
            </div>
          ) : learnerData && learnerData.profile ? (
