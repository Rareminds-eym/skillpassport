...
const LearnerCard3D = () => {
  ...
  const education = learnerData.education?.[0] || {};
  const employabilityScore = profile.employability_score || profile.employabilityScore || 0;
  const approvalStatus = learnerData.approval_status || 'pending';
  const qrCodeValue = `${window.location.origin}/learner/profile/${learnerData?.id}`;
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-white font-bold">{learnerData.id || 'N/A'}</span>
                    </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 flex flex-col items-center">
                    ...
                    <p className="text-white text-xl font-bold tracking-wider">
                      PASSPORT-ID: {learnerData.passport_id || learnerData.id?.toUpperCase().slice(0, 8) || 'N/A'}
                    </p>
                  ...
                  {/* Skills Summary */}
                  {(learnerData.technicalSkills?.length > 0 || learnerData.softSkills?.length > 0) && (
                    <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20">
                      <h3 className="text-white font-bold text-lg mb-4">Skills Overview</h3>
                      <div className="space-y-3">
                        {learnerData.technicalSkills?.length > 0 && (
                          <div>
                            <p className="text-white/70 text-sm mb-2">Technical Skills</p>
                            <div className="flex flex-wrap gap-2">
                              {learnerData.technicalSkills.slice(0, 5).map((skill, idx) => (
                                <Badge 
                                  key={idx}
                              ...
                              {learnerData.technicalSkills.length > 5 && (
                                <Badge className="bg-blue-600 text-white border-0">
                                  +{learnerData.technicalSkills.length - 5} more
                                </Badge>
                              )}
                            </div>
                          </div>
                        )}
                        {learnerData.softSkills?.length > 0 && (
                          <div>
                            <p className="text-white/70 text-sm mb-2">Soft Skills</p>
                            <div className="flex flex-wrap gap-2">
                              {learnerData.softSkills.slice(0, 5).map((skill, idx) => (
                                <Badge
                                  key={idx}
                              ...
                              {learnerData.softSkills.length > 5 && (
                                <Badge className="bg-indigo-600 text-white border-0">
                                  +{learnerData.softSkills.length - 5} more
                                </Badge>
                              )}
                            </div>
                          </div>
