...
interface DirectoryTreePanelProps {
  ...
  /** Emits the chosen Section id (or null when the selection is cleared). */
  onSelectSection: (sectionId: string | null) => void;
  /**
   * Fires synchronously right after a Section is selected and the panel is
...
/** Recursive node renderer: section leaves are selectable, branches expand. */
function TreeNode({
  node,
  counts,
  expandedIds,
  ...
  if (node.selectable) {
    ...
    return (
      <button
        type="button"
        onClick={() => onSelectSection(isSelected ? null : node.id)}
        className={cn(
  ...
  // Branch (Department / Academic Year): pure navigation, expand/collapse only.
    ...
    <div>
      <button
        ...
        className={cn(
          ...
          isExpanded ? 'border-blue-200 bg-blue-50/60' : 'border-transparent hover:bg-slate-50',
        )}
      >
        <ChevronRight
          className={cn(
        ...
        />
        <Icon
          className={cn('h-4 w-4 shrink-0', isExpanded ? 'text-blue-600' : 'text-slate-400')}
          aria-hidden="true"
        />
        <span
          className={cn(
...
export function DirectoryTreePanel({
  ...
  // Selecting (or deselecting) a Section applies the filter, closes the
  // panel, and reports the applied selection upward in one action — a filter
  // selection, not a persistent panel. `tailwindcss-animate` (the classes
  // driving Sheet's slide/fade transitions in shared/ui/sheet.jsx) is not
  // installed in this project, so those "animation" classes compile to
  // nothing and the sheet closes with no real CSS animation. That means
