...
export async function generateDomainKeywords(fieldOfStudy) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(
        `[Course Recommendations] ⚠️ LAYER 1 (AI Service) FAILED for "${field}": ${response.status}`
      );
    ...
    if (!keywords) {
      console.warn(
        `[Course Recommendations] ⚠️ LAYER 1 (AI Service) returned empty for "${field}"`
      );
      console.log(`[Course Recommendations] → Falling back to LAYER 2 (Pattern Matching)`);
      return getFallbackKeywords(field);
    }
    ...
    console.error(
      `[Course Recommendations] ❌ LAYER 1 (AI Service) ERROR for "${field}":`,
      error.message
    );
    console.log(`[Course Recommendations] → Falling back to LAYER 2 (Pattern Matching)`);
    return getFallbackKeywords(field);
...
function getFallbackKeywords(field) {
  ...
  // Generic fallback
    ...
    console.log(
      `[Course Recommendations] ⚠️ LAYER 2 (Pattern Matching) - No match found for "${field}"`
    );
...
export async function getDomainKeywordsWithCache(fieldOfStudy) {
  ...
  console.log(
    `[Course Recommendations] 💾 CACHE MISS for "${fieldOfStudy}" - generating keywords...`
  );
