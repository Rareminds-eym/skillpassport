  ...
  try {
    ...
    if (storedUser) {
      try {
        ...
        if (userData.role === 'school_admin' && userData.schoolId) {
          console.log(
            '✅ [CompetitionsService] Found school ID from localStorage:',
            userData.schoolId
          );
    ...
    if (user) {
      ...
      if (educatorData?.school_id) {
        console.log(
          '✅ [CompetitionsService] Found school ID from school_educators:',
    ...
    if (userEmail) {
      ...
      if (educatorByEmail?.school_id) {
        console.log(
          '✅ [CompetitionsService] Found school ID from school_educators by email:',
          educatorByEmail.school_id
        );
    ...
    if (user || userEmail) {
      ...
      if (orgData?.id) {
        console.log(
          '✅ [CompetitionsService] Found school ID from organizations table:',
          orgData.id
        );
        return orgData.id;
      }
...
export async function updateCompetitionRegistration(
  ...
  try {
    ...
    if (updateData.teamMembers) {
      updates.team_members = JSON.parse(
        JSON.stringify({
          members: updateData.teamMembers
            .split(',')
            .map((m) => m.trim())
            .filter((m) => m),
        })
  ...
  try {
    ...
    if (updateData.participatingClubs !== undefined) {
      ...
      if (clubsToAdd.length > 0) {
        ...
        const { error: insertError } = await supabase.from('competition_clubs').insert(clubRecords);
...
export async function deleteCompetition(compId: string): Promise<void> {
  try {
    ...
    const { error } = await supabase.from('competitions').delete().eq('comp_id', compId);
