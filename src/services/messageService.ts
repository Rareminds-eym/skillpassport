...
export class MessageService {
  ...
  private static async _getOrCreateConversationInternal(
    ...
    try {
      ...
      let query = supabase
        .from('conversations')
        .select(
          'id, status, deleted_by_student, deleted_by_recruiter, student_id, recruiter_id, application_id, opportunity_id, subject, created_at, updated_at'
        )
  ...
  private static async _getOrCreateStudentEducatorConversationInternal(
    ...
    try {
      ...
      let query = supabase
        .from('conversations')
        .select(
          'id, status, deleted_by_student, deleted_by_educator, student_id, educator_id, class_id, subject, created_at, updated_at'
        )
        .eq('student_id', studentId)
      ...
      if (existing) {
        ...
        if (existing.deleted_by_student || existing.deleted_by_educator) {
          console.log(
            '📥 Restoring previously deleted student-educator conversation:',
            existing.id
          );
  ...
  static async sendMessage(
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert(messageData)
        .select(
          'id, conversation_id, sender_id, sender_type, receiver_id, receiver_type, message_text, is_read, read_at, created_at, updated_at'
        )
        .single();
  ...
  private static async _getUserConversationsInternal(
    ...
    try {
      ...
      query = query.order('last_message_at', { ascending: false, nullsFirst: false }).limit(100); // Reasonable limit for most use cases
   ...
   */
  static subscribeToConversation(conversationId: string, onMessage: (message: Message) => void) {
    return supabase
   ...
   */
  static subscribeToUserMessages(userId: string, onMessage: (message: Message) => void) {
    return supabase
   ...
   */
  static async getConversationWithStudent(conversationId: string): Promise<Conversation | null> {
    try {
      const { data, error } = await supabase
        .from('conversations')
        .select(
  ...
  static async getOrCreateStudentAdminConversation(
    ...
    const request = this._getOrCreateStudentAdminConversationInternal(studentId, schoolId, subject);
  ...
  private static async _getOrCreateStudentAdminConversationInternal(
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('get_or_create_student_admin_conversation', {
  ...
  private static async _getOrCreateStudentCollegeAdminConversationInternal(
    ...
    try {
      ...
      const { data, error } = await supabase.rpc(
        'get_or_create_student_college_admin_conversation',
        {
          p_student_id: studentId,
  ...
  private static async _getOrCreateEducatorAdminConversationInternal(
    ...
    try {
      // Use the database function for consistency
      const { data, error } = await supabase.rpc('get_or_create_educator_admin_conversation', {
