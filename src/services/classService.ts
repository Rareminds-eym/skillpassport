...
const computeAggregates = (classItem: EducatorClass) => {
  ...
  const avg =
    total === 0
      ? 0
      : Math.round(classItem.students.reduce((acc, curr) => acc + curr.progress, 0) / total);
 ...
 */
export const getEducatorAssignedClassIds = async (
  educatorId: string,
...
const transformDBClassToClass = (
  dbClass: any,
  educatorType: 'school' | 'college' = 'school'
): EducatorClass => {
...
const calculateStudentProgress = async (studentId: string, classId: string): Promise<number> => {
  try {
    ...
    const completedCount = studentAssignments.filter(
      (sa) => sa.status === 'graded' || sa.status === 'submitted'
    ).length;
...
export const fetchEducatorClasses = async (
  ...
  try {
    ...
    if (!educatorId || !educatorType) {
      console.warn(
        'fetchEducatorClasses called without educatorId or educatorType - this should not happen for educators'
      );
...
const fetchSchoolEducatorClasses = async (
  schoolId?: string,
  educatorId?: string
): Promise<ServiceResponse<EducatorClass[]>> => {
  try {
...
const fetchCollegeEducatorClasses = async (
  collegeId?: string,
  educatorId?: string
): Promise<ServiceResponse<EducatorClass[]>> => {
  try {
...
export const fetchStudentDirectory = async (
  schoolId?: string,
  collegeId?: string
): Promise<ServiceResponse<StudentDirectoryEntry[]>> => {
  try {
    let query = supabase
      .from('students')
    ...
    if (schoolId) {
      ...
      query = query.eq('school_id', schoolId).is('school_class_id', null);
    } else if (collegeId) {
      // For college: get students without a class assignment
      query = query.eq('college_id', collegeId).is('college_class_id', null);
    } else {
...
export const addStudentToClass = async ({
  ...
  student,
  educatorType = 'school',
}: AddStudentPayload & { educatorType?: 'school' | 'college' }): Promise<
  MutateResponse<EducatorClass>
> => {
  try {
...
export const removeStudentFromClass = async (
  ...
  studentId: string,
  educatorType: 'school' | 'college' = 'school'
): Promise<MutateResponse<EducatorClass>> => {
  try {
    const { data: classData, error: classError } = await getClassById(classId, educatorType);
    if (classError || !classData) {
      return { data: null, error: 'Class not found' };
...
export const createClass = async (
  ...
  try {
    const skills = Array.from(
      new Set(payload.skillAreas.map((skill) => skill.trim()).filter(Boolean))
    );
...
const createCollegeClass = async (
  payload: CreateClassPayload,
  skills: string[]
): Promise<MutateResponse<EducatorClass>> => {
  const { data, error } = await supabase
...
const updateCollegeClass = async (
  classId: string,
  payload: CreateClassPayload,
  skills: string[]
): Promise<MutateResponse<EducatorClass>> => {
...
export const assignTaskToClass = async ({
  classId,
  task,
}: AssignTaskPayload): Promise<MutateResponse<EducatorClass>> => {
  try {
...
export const saveClassNote = async ({
  classId,
  note,
}: SaveNotePayload): Promise<MutateResponse<EducatorClass>> => {
  try {
