...
export async function createClub(clubData: {
  ...
  try {
    ...
    if (!schoolId) {
      ...
      throw new Error(
        'School ID not found. Please ensure you are logged in as a school admin or educator.'
      );
    }
    ...
    const mentorSchoolId = clubData.mentor_school_id || null;
...
export async function markAttendance(
  ...
  try {
    ...
    console.log(
      '🎯 [markAttendance] Starting attendance marking for club:',
      clubId,
      'date:',
      sessionDate
    );
    ...
    if (existingSession) {
      console.log(
        '📝 [markAttendance] Found existing session, will update:',
        existingSession.attendance_id
      );
    ...
    const { error: recordsError } = await supabase.from('club_attendance_records').insert(records);
    ...
    if (recordsError) {
      ...
      console.log(
        '✅ [markAttendance] Successfully marked attendance for',
...
export async function getClubDetails(clubId: string): Promise<any> {
  try {
    const { data, error } = await supabase.rpc('get_club_details', { p_club_id: clubId });
...
export async function updateClub(
  ...
  try {
    ...
    const cleanData = Object.fromEntries(
      Object.entries(clubData).filter(
