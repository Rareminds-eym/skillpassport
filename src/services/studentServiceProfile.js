...
//           total_modules: train.totalModules || 0,
//           hours_spent: train.hoursSpent || 0,
//           // 🔥 FIX: Changed from 'pending' to 'approved' so new trainings show up
//           approval_status: train.approval_status || 'approved',
//           updated_at: nowIso,
//         };
...
//               link: certificateUrl,
//               status: 'active',
//               approval_status: 'approved', // 🔥 Also changed here for consistency
//               enabled: true,
//               created_at: nowIso,
//               updated_at: nowIso,
//             };
...
export const updateProjectsByEmail = async (email, projectsData = []) => {
  try {
    ...
    const formatted = (projectsData || [])
      .filter((proj) => proj && typeof proj.title === 'string' && proj.title.trim().length > 0)
      .map((proj) => {
        const record = {
          student_id: studentId,
          ...
          start_date: normalizeDate(proj.start_date || proj.startDate),
          end_date: normalizeDate(proj.end_date || proj.endDate),
          duration: proj.duration?.trim() || null,
          tech_stack: Array.isArray(proj.tech_stack || proj.tech) ? (proj.tech_stack || proj.tech) : null,
          demo_link: proj.demo_link?.trim() || proj.link?.trim() || null,
          github_link: proj.github_link?.trim() || proj.github?.trim() || proj.github_url?.trim() || null,
          organization: proj.organization?.trim() || null,
          certificate_url: proj.certificate_url?.trim() || null,
          video_url: proj.video_url?.trim() || null,
          ppt_url: proj.ppt_url?.trim() || null,
          approval_status: proj.approval_status || 'pending',
          enabled: proj.enabled !== false,
          updated_at: nowIso,
        };

        const rawId = typeof proj.id === 'string' ? proj.id.trim() : null;
        if (rawId && rawId.length === 36) {
          record.id = rawId;
        } else {
          record.id = generateUuid();
