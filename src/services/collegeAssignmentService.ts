...
/**
 * ============================================
 * COLLEGE STUDENT ASSIGNMENT FUNCTIONS
 * ============================================
 */
 ...
 * 
 * STUDENT ASSIGNMENT MATCHING:
 * - Fetches assignments based on student ID
 * - Uses simple, reliable query structure
 * - Includes student-specific assignment status and submission details
 * 
 * @param studentId - The UUID of the college student
 * @returns Promise<ServiceResponse<CollegeStudentAssignment[]>> - Student's assignments
 */
export const fetchCollegeStudentAssignments = async (studentId: string): Promise<ServiceResponse<CollegeStudentAssignment[]>> => {
  try {
    ...
    // Step 1: Get student assignments
    console.log('📚 SERVICE: Step 1 - Fetching student assignments...');
    ...
    console.log('📚 SERVICE: Step 1 result:', { 
      count: studentAssignments?.length || 0, 
      error: saError
    });

    if (saError) {
      console.error('❌ SERVICE: Error fetching student assignments:', saError);
      throw saError;
    }

    if (!studentAssignments || studentAssignments.length === 0) {
      console.log('⚠️ SERVICE: No student assignments found for student:', studentId);
      return { data: [], error: null };
    }

    console.log('📋 SERVICE: Found', studentAssignments.length, 'student assignments');

    // Step 2: Get assignment details for each student assignment
    const assignmentIds = studentAssignments.map(sa => sa.assignment_id);
    console.log('📚 SERVICE: Step 2 - Fetching assignment details for IDs:', assignmentIds);
    ...
    console.log('📚 SERVICE: Step 2 result:', { 
      count: assignments?.length || 0, 
      error: aError
    });

    if (aError) {
      console.error('❌ SERVICE: Error fetching assignments:', aError);
      throw aError;
    }

    console.log('📋 SERVICE: Found', assignments?.length || 0, 'assignments');

    // Step 3: Combine the data
    console.log('📚 SERVICE: Step 3 - Combining data...');
    const combinedAssignments: CollegeStudentAssignment[] = studentAssignments.map(sa => {
      ...
      if (!assignment) {
        console.warn('⚠️ SERVICE: Assignment not found for student assignment:', sa.student_assignment_id);
        return null;
      }

      console.log('✅ SERVICE: Successfully combined assignment:', assignment.title);
    ...
    console.log('✅ SERVICE: Returning', combinedAssignments.length, 'combined assignments');
    console.log('📋 SERVICE: Final assignments:', combinedAssignments.map(a => ({
      title: a.title,
      status: a.status,
      due_date: a.due_date
    })));
 ...
 * 
 * @param studentId - The UUID of the college student
 * @returns Promise<ServiceResponse<CollegeAssignmentStats>> - Assignment statistics
 */
export const getCollegeStudentAssignmentStats = async (studentId: string): Promise<ServiceResponse<CollegeAssignmentStats>> => {
  try {
    console.log('📊 SERVICE: Fetching assignment stats for student:', studentId);
    ...
    if (error) {
      console.error('❌ SERVICE: Error fetching assignment stats:', error);
      throw error;
    }
 ...
 * 
 * @param studentAssignmentId - The UUID of the student assignment
 * @param newStatus - The new status ('todo', 'in_progress', 'submitted', 'graded')
...
export const updateCollegeStudentAssignmentStatus = async (
  ...
  try {
    ...
    // Set submission_date when submitting
    ...
    if (error) {
      console.error('❌ SERVICE: Error updating assignment status:', error);
      throw error;
    }
 ...
 * 
 * @param studentAssignmentId - The UUID of the student assignment
 * @param submissionData - Submission content and metadata
 * @param submissionFiles - Optional files to upload
 * @returns Promise<ServiceResponse<boolean>> - Success status
 */
export const submitCollegeAssignment = async (
  ...
  try {
    console.log('📤 SERVICE: Submitting college assignment:', { 
      studentAssignmentId, 
      hasContent: !!submissionData.submission_content,
      hasUrl: !!submissionData.submission_url,
      fileCount: submissionFiles?.length || 0 
    });

    // Get assignment and student details for file upload path
    ...
    if (submissionFiles && submissionFiles.length > 0) {
      console.log('📤 Uploading', submissionFiles.length, 'submission files...');
 ...
 * 
 * 5. STUDENT ACCESS:
 *    - fetchCollegeStudentAssignments() gets assignments for a specific student
 *    - getCollegeStudentAssignmentStats() provides assignment statistics
 *    - updateCollegeStudentAssignmentStatus() allows status updates
 *    - submitCollegeAssignment() handles assignment submissions with files
 * 
