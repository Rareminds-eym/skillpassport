...
function validateExclusionListComplete(
  ...
  if (missingAnsweredIds.length > 0) {
    console.error(
      '❌ [AdaptiveAptitudeService] Exclusion list validation failed: Missing answered question IDs:',
      missingAnsweredIds
    );
  }
  ...
  if (missingPhaseIds.length > 0) {
    console.error(
      '❌ [AdaptiveAptitudeService] Exclusion list validation failed: Missing phase question IDs:',
      missingPhaseIds
    );
  }
  ...
  if (isValid) {
    console.log(
      '✅ [AdaptiveAptitudeService] Exclusion list validation passed: All IDs accounted for'
    );
  }
...
export async function initializeTest(
  ...
  const questionResult =
    await QuestionGeneratorService.generateDiagnosticScreenerQuestions(gradeLevel);
  ...
  const session = dbSessionToTestSession(sessionData, [], questionResult.questions);
...
export async function getNextQuestion(sessionId: string): Promise<NextQuestionResult> {
  ...
  const maxTotalQuestions =
    DEFAULT_ADAPTIVE_TEST_CONFIG.phases.diagnostic_screener.maxQuestions +
  ...
  const adaptiveCoreQuestionsAnswered =
    totalQuestionsAnswered - DEFAULT_ADAPTIVE_TEST_CONFIG.phases.diagnostic_screener.maxQuestions;
  if (
    currentPhase === 'adaptive_core' &&
    adaptiveCoreQuestionsAnswered < DEFAULT_ADAPTIVE_TEST_CONFIG.phases.adaptive_core.maxQuestions
  ) {
    ...
    if (!exclusionValidation.isValid) {
      console.error(
        '❌ [AdaptiveAptitudeService] Exclusion list validation failed:',
        exclusionValidation.reason
      );
    }
    ...
    const selectedSubtag =
      availableSubtags[Math.floor(Math.random() * availableSubtags.length)] || ALL_SUBTAGS[0];
    ...
    if (questionResult.questions.length > 0) {
      ...
      if (!validation.isValid) {
        console.warn(
          `⚠️ [AdaptiveAptitudeService] Generated question is duplicate: ${validation.reason}`
        );
        ...
        if (retryResult.questions.length > 0) {
          ...
          if (retryValidation.isValid) {
            ...
            console.error(
              `❌ [AdaptiveAptitudeService] Retry also returned duplicate: ${retryValidation.reason}`
            );
            console.error(
              `❌ [AdaptiveAptitudeService] Retry question ID: ${retryResult.questions[0].id}`
            );
            console.error(
              `❌ [AdaptiveAptitudeService] Retry exhausted (${retryCount}/${maxRetries}), allowing duplicate to avoid blocking test progress`
            );

            // Track retry failures for monitoring
            console.error(
              `❌ [AdaptiveAptitudeService] RETRY_FAILURE: session=${sessionId}, originalQuestionId=${newQuestion.id}, retryQuestionId=${retryResult.questions[0].id}, difficulty=${currentDifficulty}, subtag=${selectedSubtag}`
            );
          ...
          console.error(
            `❌ [AdaptiveAptitudeService] Retry exhausted (${retryCount}/${maxRetries}), using original duplicate to avoid blocking test progress`
          );

          // Track retry failures for monitoring
          console.error(
            `❌ [AdaptiveAptitudeService] RETRY_FAILURE: session=${sessionId}, originalQuestionId=${newQuestion.id}, retryQuestionId=none, difficulty=${currentDifficulty}, subtag=${selectedSubtag}`
          );
  ...
  if (currentPhase !== 'adaptive_core' && currentQuestionIndex < currentPhaseQuestions.length) {
    ...
    console.log(
      '✅ [AdaptiveAptitudeService] Returning question at index',
      currentQuestionIndex,
      ':',
      {
  ...
  console.log(
    '  🔍 All current phase question IDs:',
...
async function validateSessionNoDuplicates(sessionId: string): Promise<{
  ...
  if (error) {
    console.error(
      '❌ [AdaptiveAptitudeService] Failed to fetch responses for duplicate validation:',
      error
    );
  ...
  if (duplicates.length > 0) {
    console.error(
      `❌ [AdaptiveAptitudeService] Session ${sessionId} has duplicate questions:`,
      duplicates
    );
  ...
  console.log(
    `✅ [AdaptiveAptitudeService] Session ${sessionId} validation passed: No duplicate questions found`
  );
  return { isValid: true };
}
...
export async function completeTest(sessionId: string): Promise<TestResults> {
  ...
  if (!validation.isValid) {
    console.error(
      `❌ [AdaptiveAptitudeService] Test completed with duplicates:`,
...
export async function resumeTest(sessionId: string): Promise<ResumeTestResult> {
  ...
  const session = dbSessionToTestSession(sessionData, responses, currentPhaseQuestions);
...
export async function getTestResults(sessionId: string): Promise<TestResults | null> {
  ...
  return {
    ...
    accuracyByDifficulty: data.accuracy_by_difficulty as Record<
      DifficultyLevel,
      { correct: number; total: number; accuracy: number }
    >,
    accuracyBySubtag: data.accuracy_by_subtag as Record<
      Subtag,
      { correct: number; total: number; accuracy: number }
    >,
    difficultyPath: (data.difficulty_path as number[]).map((d) => d as DifficultyLevel),
    pathClassification: data.path_classification as
      | 'ascending'
      | 'descending'
      | 'stable'
      | 'fluctuating',
...
export async function getStudentTestResults(studentId: string): Promise<TestResults[]> {
  ...
  return data.map((record) => ({
    ...
    accuracyByDifficulty: record.accuracy_by_difficulty as Record<
      DifficultyLevel,
      { correct: number; total: number; accuracy: number }
    >,
    accuracyBySubtag: record.accuracy_by_subtag as Record<
      Subtag,
      { correct: number; total: number; accuracy: number }
    >,
    difficultyPath: (record.difficulty_path as number[]).map((d) => d as DifficultyLevel),
    pathClassification: record.path_classification as
      | 'ascending'
      | 'descending'
      | 'stable'
      | 'fluctuating',
