...
export const validateStreamRecommendation = (results) => {
  ...
  try {
    ...
    if (!aiStream || aiStream === 'N/A' || aiStream === 'null' || aiStream === null) {
      ...
      console.log(
        '✅ Generated streamRecommendation:',
...
export const fetchSections = async (gradeLevel = null) => {
  let query = supabase.from('personal_assessment_sections').select('*').eq('is_active', true);
...
export const fetchAllQuestions = async (streamId, gradeLevel = null) => {
  ...
  for (const section of sections) {
    ...
    const questions = await fetchQuestionsBySection(section.id, isStreamSpecific ? streamId : null);
...
export const completeAttempt = async (
  ...
  if (gradeLevel === 'middle' || gradeLevel === 'highschool') {
    ...
    nestedValidation = [
      ...
      {
        path: 'employability.overallReadiness',
        value: geminiResults.employability?.overallReadiness,
      },
      { path: 'careerFit.clusters', value: geminiResults.careerFit?.clusters },
  ...
  const workValuesScores = isSimplifiedAssessment
    ? null
    : geminiResults?.workValues?.scores || null;
  const employabilityScores = isSimplifiedAssessment
    ? null
    : geminiResults?.employability?.skillScores || null;
  const employabilityReadiness = isSimplifiedAssessment
    ? null
    : geminiResults?.employability?.overallReadiness || null;
  ...
  console.log(
    '  workValuesScores:',
    workValuesScores,
    isSimplifiedAssessment ? '(excluded for simplified)' : ''
  );
  console.log(
    '  employabilityScores:',
    employabilityScores,
  ...
  const hasProgress = (attempt) => {
    ...
    if (attempt.all_responses && typeof attempt.all_responses === 'object') {
      ...
      const answerKeys = keys.filter(
        (k) => !k.startsWith('_') && k !== 'adaptive_aptitude_results'
      );
...
export const abandonAttempt = async (attemptId) => {
  try {
    ...
    if (error) throw error;
    console.log('✅ Attempt abandoned:', attemptId);
  } catch (error) {
 ...
 * @param {object} questionsBySection - Questions grouped by section
 * @returns {object} Transformed questions for UI
 */
export const transformQuestionsForUI = (questionsBySection) => {
  // This is a simple pass-through for now
  // Can be enhanced later if UI needs different format
  return questionsBySection;
};
