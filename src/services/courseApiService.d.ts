export interface CourseApiService {
    ...
    submitAiTutorFeedback(data: { conversationId: string; messageIndex: number; rating: number; feedbackText?: string }, token: string): Promise<any>;
    getAiTutorProgress(courseId: string, token: string): Promise<any>;
    updateAiTutorProgress(data: { courseId: string; lessonId: string; status: string }, token: string): Promise<any>;
    summarizeVideo(data: { videoUrl: string; lessonId?: string; courseId?: string; language?: string }, token: string): Promise<any>;
    [key: string]: any;
}
