...
const CAREER_API_URL =
  import.meta.env.VITE_CAREER_API_URL || 'https://career-api.dark-mode-d021.workers.dev';
...
export const fetchStudentStreamData = async (studentId) => {
  const { data, error } = await supabase.rpc('get_student_stream_recommendation_data', {
...
const calculateProjectBasedScores = (projects) => {
  ...
  (projects || []).forEach((project) => {
    ...
    if (
      combined.match(
        /programming|coding|software|app|website|react|python|java|ai|ml|data|algorithm|robot/
      )
    ) {
      scores.science += 15;
    }
...
export const generateStreamRecommendation = async (studentId) => {
  ...
  const finalScores = {
    science:
      marksScores.science * 0.4 +
      interestScores.science * 0.25 +
      projectScores.science * 0.2 +
      experienceScores.science * 0.15,
    commerce:
      marksScores.commerce * 0.4 +
      interestScores.commerce * 0.25 +
      projectScores.commerce * 0.2 +
      experienceScores.commerce * 0.15,
    arts:
      marksScores.arts * 0.4 +
      interestScores.arts * 0.25 +
      projectScores.arts * 0.2 +
      experienceScores.arts * 0.15,
  };

  // Determine recommended stream
  const sortedStreams = Object.entries(finalScores).sort(([, a], [, b]) => b - a);
...
${
  (experiences || []).map((e) => `- ${e.role} at ${e.organization} (${e.duration})`).join('\n') ||
  'No experiences'
}
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      science_breakdown: recommendation.ai_analysis?.science_analysis
        ? { analysis: recommendation.ai_analysis.science_analysis }
        : recommendation.science_breakdown,
      commerce_breakdown: recommendation.ai_analysis?.commerce_analysis
        ? { analysis: recommendation.ai_analysis.commerce_analysis }
        : recommendation.commerce_breakdown,
      arts_breakdown: recommendation.ai_analysis?.arts_analysis
        ? { analysis: recommendation.ai_analysis.arts_analysis }
        : recommendation.arts_breakdown,
