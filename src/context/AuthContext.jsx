...
export const AuthProvider = ({ children }) => {
  ...
  const restoreUserFromStorage = useCallback((sessionUser) => {
    ...
    let sessionRole =
      sessionUser.user_metadata?.user_role || sessionUser.user_metadata?.role || 'user';
    ...
    const {
      ...
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        if (typeof window !== 'undefined' && window.facultyOnboardingInProgress) {
          const newUserRole =
            session.user.user_metadata?.role || session.user.user_metadata?.user_role;
