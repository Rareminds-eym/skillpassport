...
export async function checkRateLimit(
  ...
  // If KV is not available, allow the request (fail open for availability)
  if (!kv) {
    console.warn('[RateLimit] KV namespace not available, allowing request');
    return {
      allowed: true,
  ...
  try {
    ...
    await kv.put(
      ...
      { expirationTtl: config.windowSeconds + 60 } // Add buffer to TTL
    );
...
export async function withRateLimit(
  ...
  const result = await checkRateLimit(kv, userId, config);
