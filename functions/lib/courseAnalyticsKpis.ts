 ...
 * sort by enrollment count (descending). Extracted so neither caller
 * duplicates this loop. Returns every course, unsliced — each caller decides
 * how much of the sorted list it needs (buildEnrollmentOverview takes a
 * fixed top N; buildCoursePerformanceRows paginates).
 */
function groupEnrollmentsByCourse(
  ...
  return Array.from(byCourse.entries())
    ...
    // Sort by enrollment count (descending) — every course, unsliced.
    .sort((a, b) => b.enrollmentCount - a.enrollmentCount);
}

/** Matches the frontend's PaginatedCoursePerformance (entities/course-analytics/model/types.ts). */
...
/**
 * Course Performance Overview rows: every course the organization's learners
 * are enrolled in, ranked by enrollment count (see groupEnrollmentsByCourse
 * above for the shared flow), paginated instead of hard-capped at a fixed top
 * N — `totalCount` is the full course count (pre-pagination), for computing
 * page count on the frontend. `rank` reflects the course's position in the
 * full sorted list, not just its position within the current page.
 *
