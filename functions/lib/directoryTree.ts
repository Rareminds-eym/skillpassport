...
/**
 * Resolves the program_sections.id list for one department's academic year
 * (e.g. "1st Year"), by checking every active section's semester against
 * yearOfStudyLabel — the SAME derivation buildCollegeStyleDirectoryTree uses
 * to build the tree, so a selected Academic Year node always maps back to
 * exactly the sections that node's children represent. Used by the Learner
 * Directory's `academicYear` filter (course-analytics.ts's
 * 'get-learner-directory' action) instead of re-deriving this logic inline.
 */
export async function resolveSectionIdsForYear(
  ...
  return (data || [])
    .filter((row: { semester: number }) => yearOfStudyLabel(row.semester) === yearLabel)
