...
export async function generateReceiptPDF(data: ReceiptData): Promise<Uint8Array> {
  try {
    ...
    // Wrapped in timeout to prevent hanging on image processing
    if (watermarkBytes) {
      try {
        // Set a timeout for watermark processing
        const wmPromise = (async () => {
        ...
        await Promise.race([
          wmPromise,
          new Promise((_, reject) => setTimeout(() => reject(new Error('Watermark timeout')), 3000))
        ]);
      } catch (err) {
    ...
    console.error('[GenerateReceiptPDF] Fatal error during PDF generation:', error);
