...
// ============================================================================
// COLLEGE ANALYSIS (grade_level='college')
// ============================================================================
...
async function analyzeCollege(
  ...
  console.log('[analyzeCollege] Starting college assessment analysis', { attemptId, learnerId });
  ...
  try {
    ...
    if (attemptError || !attempt) {
      console.error('[analyzeCollege] Attempt not found:', attemptError || 'no data');
      return Response.json({ error: 'Attempt not found' }, { status: 404 });
    }
    ...
    if (questionUUIDs.length === 0) {
      console.error('[analyzeCollege] No responses found in attempt');
      return Response.json({ error: 'No responses found in attempt' }, { status: 400 });
    }

    console.log('[analyzeCollege] Found responses', { responseCount: questionUUIDs.length });
    ...
    if (questionsError) {
      console.error('[analyzeCollege] Failed to fetch questions:', questionsError);
      return Response.json(
        { error: 'Failed to fetch question metadata', message: questionsError.message },
        { status: 500 }
      );
    ...
    if (!questions || questions.length === 0) {
      console.error('[analyzeCollege] No questions found for UUIDs:', questionUUIDs);
      return Response.json(
        { error: 'Questions not found', details: 'Could not match responses to question metadata' },
        { status: 400 }
      );
    ...
    console.log('[analyzeCollege] Fetched questions', { questionCount: questions.length });
    ...
    console.log('[analyzeCollege] Section mapping', { 
    ...
    console.log('[analyzeCollege] Calculated scores', {
      riasecTotal: Object.values(riasecScores).reduce((a, b) => a + b, 0),
      bigFiveDimensions: Object.keys(bigFiveScores).filter(k => bigFiveScores[k].length > 0),
      valuesDimensions: Object.keys(valuesScores).length,
      employabilityDimensions: Object.keys(employabilityScores).length,
      aptitudeScore: aptitudeTotal > 0 ? `${aptitudeCorrect}/${aptitudeTotal}` : 'N/A',
      knowledgeScore: knowledgeTotal > 0 ? `${knowledgeCorrect}/${knowledgeTotal}` : 'N/A'
    });
    ...
    console.log('[analyzeCollege] Adaptive aptitude data', { 
      hasAdaptiveData: !!adaptiveData,
    ...
    if (!hasValidRIASEC && !hasValidBigFive && !hasValidValues && !hasValidEmployability) {
      console.error('[analyzeCollege] No valid scores calculated:', {
        riasecScores,
        bigFiveCount: Object.keys(bigFiveAggregated).length,
        valuesCount: Object.keys(valuesAggregated).length,
        employabilityCount: Object.keys(employabilityAggregated).length,
      });
      return Response.json(
        { error: 'Failed to calculate assessment scores', details: 'No valid data could be extracted' },
        { status: 400 }
    ...
    console.log('[analyzeCollege] Validation passed, storing results');
    ...
    if (insertError) {
      console.error('[analyzeCollege] Failed to insert results:', insertError);
      return Response.json(
        { error: 'Failed to store results', message: insertError.message },
        { status: 500 }
      );
    ...
    console.log('[analyzeCollege] Results stored successfully');
    ...
    if (adaptiveData !== null || aptitudeOverall !== null) {
      ...
      if (aptitudeUpdateError) {
        console.error('[analyzeCollege] Failed to update adaptive aptitude scores:', aptitudeUpdateError);
        // Non-fatal — main upsert succeeded; log but continue
      }
    }

    console.log('[analyzeCollege] Analysis complete');
    ...
    console.error('[analyzeCollege] Analysis failed:', error);
    return Response.json(
      {
...
function getTopScores(scores: Record<string, number>, n: number): Array<{ dimension: string; score: number }> {
  return Object.entries(scores)
    .sort(([, a], [, b]) => b - a)
    .slice(0, n)
