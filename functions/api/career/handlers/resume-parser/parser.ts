...
export function parseResumeDeterministic(resumeText: string): ParsedResumeData {
  ...
  // extractPortfolio() below reads the full `text` (its PORTFOLIO_LABEL_PATTERN
  // match needs the injected "Portfolio: <url>" line). detectSections() must
  // NOT see that line — it has no closing boundary for the last section in
  // the document, so the injected line would otherwise be folded into
  // whichever section heading appears last (see stripInjectedPortfolioLabel()
  // doc comment in regex.ts).
  const sections = detectSections(stripInjectedPortfolioLabel(text));
