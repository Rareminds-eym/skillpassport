 ...
 */
export async function handleCollegeAdminSignup(request: Request, env: PagesEnv): Promise<Response> {
  ...
  try {
    ...
    try {
      ...
      // 4. Update user's organizationId with college ID
 ...
 */
export async function handleCollegeEducatorSignup(request: Request, env: PagesEnv): Promise<Response> {
  const supabaseAdmin = createSupabaseAdminClient(env);
  ...
  try {
    ...
    try {
      ...
      const { error: userError } = await supabaseAdmin.from('users').insert({
        ...
        phone: body.phone,
        metadata: {
          source: 'college_educator_signup',
 ...
 * Verifies college exists in organizations table
 */
export async function handleCollegeStudentSignup(request: Request, env: PagesEnv): Promise<Response> {
  const supabaseAdmin = createSupabaseAdminClient(env);
