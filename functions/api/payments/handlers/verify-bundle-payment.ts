  ...
  try {
    ...
    try {
      ...
      await ssoRecordTransaction(env, authToken, {
        ...
        metadata: { bundle_id: body.bundle_id, bundle_name: body.bundle_name },
      });
    } catch (txError) {
      console.error('[VerifyBundlePayment] Transaction recording in auth DB failed (non-critical):', txError);
    }
