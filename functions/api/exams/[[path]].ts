...
import { onRequestPost as handleExamActions } from './actions';
...
export const onRequest: PagesFunction<PagesEnv> = async (context) => {
  ...
  try {
    if ((path === '' || path === '/' || path === '/health') && request.method === 'GET') {
      return apiSuccess({
        status: 'ok', service: 'exams-api', version: '1.0.0',
        timestamp: new Date().toISOString(),
        endpoints: ['GET /health - Health check', 'POST /actions - Exam data CRUD'],
      }, request);
    }

    if (path === '/actions' && request.method === 'POST') {
      return handleExamActions(context);
    }
