...
// .wrangler/tmp/bundle-XSOArk/checked-fetch.js
...
// .wrangler/tmp/pages-Vk8hLG/functionsWorker-0.046393787279255694.mjs
...
__name2(handleCorsPreflightRequest, "handleCorsPreflightRequest");
...
async function generateDiagnosticScreener(env, gradeLevel, excludeQuestionIds = [], excludeQuestionTexts = []) {
  throw new Error("Diagnostic screener generation not yet implemented. See README.md for implementation details.");
}
__name(generateDiagnosticScreener, "generateDiagnosticScreener");
__name2(generateDiagnosticScreener, "generateDiagnosticScreener");
async function generateAdaptiveCore(env, gradeLevel, startingDifficulty, count = 10, excludeQuestionIds = [], excludeQuestionTexts = []) {
  throw new Error("Adaptive core generation not yet implemented. See README.md for implementation details.");
}
__name(generateAdaptiveCore, "generateAdaptiveCore");
__name2(generateAdaptiveCore, "generateAdaptiveCore");
async function generateStabilityConfirmation(env, gradeLevel, provisionalBand, count = 4, excludeQuestionIds = [], excludeQuestionTexts = []) {
  throw new Error("Stability confirmation generation not yet implemented. See README.md for implementation details.");
}
__name(generateStabilityConfirmation, "generateStabilityConfirmation");
__name2(generateStabilityConfirmation, "generateStabilityConfirmation");
async function generateSingle(env, gradeLevel, phase, difficulty, subtag, excludeQuestionIds = []) {
  throw new Error("Single question generation not yet implemented. See README.md for implementation details.");
}
__name(generateSingle, "generateSingle");
__name2(generateSingle, "generateSingle");
async function generateDiagnosticScreenerQuestions(env, gradeLevel, excludeQuestionIds = [], excludeQuestionTexts = []) {
  return generateDiagnosticScreener(env, gradeLevel, excludeQuestionIds, excludeQuestionTexts);
}
__name(generateDiagnosticScreenerQuestions, "generateDiagnosticScreenerQuestions");
__name2(generateDiagnosticScreenerQuestions, "generateDiagnosticScreenerQuestions");
async function generateAdaptiveCoreQuestions(env, gradeLevel, startingDifficulty, count = 10, excludeQuestionIds = [], excludeQuestionTexts = []) {
  return generateAdaptiveCore(env, gradeLevel, startingDifficulty, count, excludeQuestionIds, excludeQuestionTexts);
}
__name(generateAdaptiveCoreQuestions, "generateAdaptiveCoreQuestions");
__name2(generateAdaptiveCoreQuestions, "generateAdaptiveCoreQuestions");
async function generateStabilityConfirmationQuestions(env, gradeLevel, provisionalBand, count = 4, excludeQuestionIds = [], excludeQuestionTexts = []) {
...
__name(generateStabilityConfirmationQuestions, "generateStabilityConfirmationQuestions");
__name2(generateStabilityConfirmationQuestions, "generateStabilityConfirmationQuestions");
async function generateSingleQuestion(env, gradeLevel, phase, difficulty, subtag, excludeQuestionIds = []) {
  return generateSingle(env, gradeLevel, phase, difficulty, subtag, excludeQuestionIds);
}
__name(generateSingleQuestion, "generateSingleQuestion");
__name2(generateSingleQuestion, "generateSingleQuestion");
  ...
  try {
    ...
    if (path === "/generate-program-career-paths" && request.method === "POST") {
      return jsonResponse(
        {
          error: "Not implemented",
          message: "Generate program career paths endpoint not yet implemented. See README.md for implementation details."
        },
        501
      );
    }
...
__name2(createSupabaseClient, "createSupabaseClient");
var SCHOOL_SUBJECT_PROMPT = `You are an expert educational assessment creator for 10th grade students in India who are about to choose their stream (Science/Commerce/Arts) for 11th-12th.
...
var APTITUDE_PROMPT = `You are an expert psychometric assessment creator. Generate aptitude test questions for {{STREAM_NAME}} stream career assessment.
...
var SYSTEM_PROMPT = `You are an expert assessment creator. Generate a skill assessment SPECIFICALLY for the course: {{COURSE_NAME}}
...
function repairAndParseJSON(text) {
  ...
  cleaned = cleaned.replace(/,\s*]/g, "]").replace(/,\s*}/g, "}").replace(/[\x00-\x1F\x7F]/g, " ").replace(/\n/g, " ").replace(/\r/g, "").replace(/\t/g, " ").replace(/"\s*\n\s*"/g, '", "').replace(/}\s*{/g, "},{").replace(/]\s*\[/g, "],[").replace(/"\s+"/g, '","');
  try {
    return JSON.parse(cleaned);
  } catch (e) {
...
__name(repairAndParseJSON, "repairAndParseJSON");
__name2(repairAndParseJSON, "repairAndParseJSON");
var FREE_MODELS = [
  "xiaomi/mimo-v2-flash:free"
];
async function callOpenRouterWithRetry(openRouterKey, messages, maxRetries = 3) {
  let lastError = null;
  for (const model of FREE_MODELS) {
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        console.log(`\u{1F504} Trying ${model} (attempt ${attempt + 1}/${maxRetries})`);
        ...
        if (response.status === 429) {
          const waitTime = Math.pow(2, attempt) * 2e3;
...
__name(callOpenRouterWithRetry, "callOpenRouterWithRetry");
__name2(callOpenRouterWithRetry, "callOpenRouterWithRetry");
var APTITUDE_CATEGORIES = [
...
async function generateAptitudeQuestions(env, streamId, questionsPerCategory = 5, studentId, attemptId, gradeLevel) {
  const supabase = createSupabaseClient(env);
  const isAfter10 = gradeLevel === "after10";
  console.log(`\u{1F4DA} Grade level detection: streamId=${streamId}, gradeLevel=${gradeLevel}, isAfter10=${isAfter10}`);
  if (studentId) {
    try {
      const { data: existing, error } = await supabase.from("career_assessment_ai_questions").select("*").eq("student_id", studentId).eq("stream_id", streamId).eq("question_type", "aptitude").eq("is_active", true).maybeSingle();
  ...
  console.log("\u{1F4DD} Generating fresh aptitude questions in 2 batches for stream:", streamId);
  ...
  async function generateBatch(batchNum, batchCategories) {
    ...
    let prompt;
    if (isAfter10) {
      prompt = promptTemplate.replace(/\{\{CATEGORIES\}\}/g, categoriesText).replace(/\{\{STREAM_NAME\}\}/g, streamContext.name);
    } else {
      prompt = promptTemplate.replace(/\{\{CATEGORIES\}\}/g, categoriesText).replace(/\{\{STREAM_NAME\}\}/g, streamContext.name).replace(/\{\{STREAM_CONTEXT\}\}/g, streamContext.context).replace(/\{\{CLERICAL_EXAMPLE\}\}/g, streamContext.clericalExample);
    }
    let jsonText;
    const systemPrompt = isAfter10 ? `You are an expert educational assessment creator for 10th grade students. Generate EXACTLY ${totalQuestions} questions total covering school subjects. Generate ONLY valid JSON.` : `You are an expert psychometric assessment creator. Generate EXACTLY ${totalQuestions} questions total. Generate ONLY valid JSON.`;
    if (claudeKey) {
      console.log(`\u{1F511} Batch ${batchNum}: Using Claude API for ${totalQuestions} questions`);
      try {
        ...
        if (openRouterKey) {
          console.log(`\u{1F511} Batch ${batchNum}: Claude failed, trying OpenRouter with retry`);
      ...
      console.log(`\u{1F511} Batch ${batchNum}: Using OpenRouter with retry for ${totalQuestions} questions`);
      jsonText = await callOpenRouterWithRetry(openRouterKey, [
        { role: "system", content: systemPrompt },
        { role: "user", content: prompt }
      ]);
  ...
  __name(generateBatch, "generateBatch");
  __name2(generateBatch, "generateBatch");
  let batch1;
  let batch2;
  if (isAfter10) {
    const batch1Categories = categories.filter((c) => ["mathematics", "science", "english"].includes(c.id));
    console.log("\u{1F4E6} Generating batch 1 (Mathematics + Science + English = 30 questions)...");
    ...
    console.log("\u{1F4E6} Generating batch 2 (Social Studies + Computer = 20 questions)...");
    ...
    console.log("\u{1F4E6} Generating batch 1 (verbal + numerical + abstract = 24 questions)...");
    batch1 = await generateBatch(1, batch1Categories);
    console.log(`\u2705 Batch 1 complete: ${batch1.length} questions`);
    console.log("\u23F3 Waiting 3s before batch 2...");
    await delay(3e3);
    const batch2Categories = categories.filter((c) => ["spatial", "clerical"].includes(c.id));
    console.log("\u{1F4E6} Generating batch 2 (spatial + clerical = 26 questions)...");
    batch2 = await generateBatch(2, batch2Categories);
    console.log(`\u2705 Batch 2 complete: ${batch2.length} questions`);
  }
  const categoryToModule = isAfter10 ? {
...
__name(generateAptitudeQuestions, "generateAptitudeQuestions");
__name2(generateAptitudeQuestions, "generateAptitudeQuestions");
async function generateKnowledgeQuestions(env, streamId, streamName, topics, questionCount = 20, studentId, attemptId, gradeLevel) {
  const supabase = createSupabaseClient(env);
  if (studentId) {
    try {
      const { data: existing, error } = await supabase.from("career_assessment_ai_questions").select("*").eq("student_id", studentId).eq("stream_id", streamId).eq("question_type", "knowledge").eq("is_active", true).maybeSingle();
  ...
  console.log("\u{1F4DD} Generating fresh knowledge questions in 2 batches for:", streamName);
  const openRouterKey = env.OPENROUTER_API_KEY || env.VITE_OPENROUTER_API_KEY;
  const claudeKey = env.CLAUDE_API_KEY || env.VITE_CLAUDE_API_KEY;
  if (!openRouterKey && !claudeKey) {
    throw new Error("No API key configured (OpenRouter or Claude)");
  }
  async function generateBatch(batchNum, count, topicSubset) {
...
{"questions":[{"id":1,"type":"mcq","difficulty":"easy","question":"Question text","options":["A","B","C","D"],"correct_answer":"A","skill_tag":"topic"}]}`;
    let jsonText;
    if (claudeKey) {
      console.log(`\u{1F511} Knowledge Batch ${batchNum}: Using Claude API`);
      try {
        const response = await fetch("https://api.anthropic.com/v1/messages", {
          method: "POST",
          headers: {
        ...
        console.error("Claude error:", claudeError.message);
        if (openRouterKey) {
          console.log(`\u{1F511} Knowledge Batch ${batchNum}: Claude failed, trying OpenRouter with retry`);
          jsonText = await callOpenRouterWithRetry(openRouterKey, [
      ...
      console.log(`\u{1F511} Knowledge Batch ${batchNum}: Using OpenRouter with retry`);
      jsonText = await callOpenRouterWithRetry(openRouterKey, [
        { role: "system", content: `Generate ONLY valid JSON with ${count} questions. No markdown.` },
        { role: "user", content: prompt }
      ]);
  ...
  console.log("\u{1F4E6} Generating knowledge batch 1 (10 questions)...");
  ...
  console.log("\u23F3 Waiting 3s before batch 2...");
  await delay(3e3);
  console.log("\u{1F4E6} Generating knowledge batch 2 (10 questions)...");
...
__name(generateKnowledgeQuestions, "generateKnowledgeQuestions");
__name2(generateKnowledgeQuestions, "generateKnowledgeQuestions");
async function generateAssessment(env, courseName, level, questionCount) {
  const supabase = createSupabaseClient(env);
  try {
    const { data: existing, error: cacheError } = await supabase.from("generated_external_assessment").select("*").eq("certificate_name", courseName).single();
  ...
  console.log("\u{1F4DD} Generating new questions for:", courseName);
  const apiKey = env.CLAUDE_API_KEY || env.VITE_CLAUDE_API_KEY;
  if (!apiKey) {
    throw new Error("Claude API key not configured");
  }
  const prompt = SYSTEM_PROMPT.replace(/\{\{COURSE_NAME\}\}/g, courseName).replace(/\{\{LEVEL\}\}/g, level).replace(/\{\{QUESTION_COUNT\}\}/g, String(questionCount));
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
  ...
  assessmentJSON = assessmentJSON.replace(/```json\n?/gi, "").replace(/```\n?/g, "").trim();
  ...
  assessmentJSON = assessmentJSON.replace(/,\s*]/g, "]").replace(/,\s*}/g, "}").replace(/[\x00-\x1F\x7F]/g, " ");
  let assessment;
  try {
...
__name(generateAssessment, "generateAssessment");
__name2(generateAssessment, "generateAssessment");
var onRequest3 = /* @__PURE__ */ __name2(async (context) => {
  ...
  if (path.includes("/career-assessment/generate-aptitude") && request.method === "POST") {
    try {
      ...
      console.log("\u{1F4DA} Aptitude request:", { streamId, gradeLevel, studentId });
  ...
  if (path.includes("/career-assessment/generate-knowledge") && request.method === "POST") {
    try {
      const body = await request.json();
      console.log("\u{1F4E5} Knowledge request body:", JSON.stringify(body));
      ...
      console.log("\u{1F3AF} Generating knowledge questions for:", streamName, "topics:", topics.length, "grade:", gradeLevel);
...
async function authenticateUser(request, env) {
  ...
  if (!userId && env.SUPABASE_SERVICE_ROLE_KEY) {
    try {
      const supabaseAdmin2 = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);
      const { data: { user: authUser }, error: authError } = await supabaseAdmin2.auth.getUser(token);
...
async function handleCareerChat(request, env) {
...
__name(handleCareerChat, "handleCareerChat");
__name2(handleCareerChat, "handleCareerChat");
var RECOMMEND_CONFIG = {
  MATCH_THRESHOLD: 0.01,
  MAX_RECOMMENDATIONS: 50,
  DEFAULT_LIMIT: 20,
  CACHE_DURATION_HOURS: 24
};
async function handleRecommendOpportunities(request, env) {
  if (request.method !== "POST") {
    return jsonResponse({ error: "Method not allowed" }, 405);
  }
  let body;
  ...
  try {
    if (!forceRefresh) {
      const { data: cached } = await supabase.from("recommendation_cache").select("recommendations, created_at").eq("student_id", studentId).gt("expires_at", (/* @__PURE__ */ new Date()).toISOString()).single();
      if (cached?.recommendations) {
        const recommendations2 = Array.isArray(cached.recommendations) ? cached.recommendations.slice(0, safeLimit) : [];
        return jsonResponse({
    ...
    const { data: student } = await supabase.from("students").select("embedding").eq("id", studentId).single();
    ...
    const { data: matches, error: matchError } = await supabase.rpc("match_opportunities_enhanced", {
    ...
    if (recommendations.length > 0) {
      ...
      expiresAt2.setHours(expiresAt2.getHours() + RECOMMEND_CONFIG.CACHE_DURATION_HOURS);
      void supabase.from("recommendation_cache").upsert({
        student_id: studentId,
        recommendations,
...
__name(handleRecommendOpportunities, "handleRecommendOpportunities");
__name2(handleRecommendOpportunities, "handleRecommendOpportunities");
async function handleAnalyzeAssessment(request, env) {
  if (request.method !== "POST") {
    return jsonResponse({ error: "Method not allowed" }, 405);
  }
  const auth = await authenticateUser(request, env);
...
__name(handleAnalyzeAssessment, "handleAnalyzeAssessment");
__name2(handleAnalyzeAssessment, "handleAnalyzeAssessment");
async function handleGenerateEmbedding(request, env) {
  if (request.method !== "POST") {
    return jsonResponse({ error: "Method not allowed" }, 405);
  }
  const auth = await authenticateUser(request, env);
  ...
  try {
    ...
    const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);
    const { error: updateError } = await supabase.from(table).update({ embedding }).eq("id", id);
    if (updateError) {
...
__name(handleGenerateEmbedding, "handleGenerateEmbedding");
__name2(handleGenerateEmbedding, "handleGenerateEmbedding");
async function handleGenerateFieldKeywords(request, env) {
  if (request.method !== "POST") {
    return jsonResponse({ error: "Method not allowed" }, 405);
  }
  let body;
...
__name(handleGenerateFieldKeywords, "handleGenerateFieldKeywords");
__name2(handleGenerateFieldKeywords, "handleGenerateFieldKeywords");
...
__name(handleParseResume, "handleParseResume");
__name2(handleParseResume, "handleParseResume");
var getOpenRouterKey = /* @__PURE__ */ __name2((env) => {
...
async function handleGetFileUrl(request, env) {
  if (request.method !== "POST") {
    return jsonResponse({ error: "Method not allowed" }, 405);
  }
  const body = await request.json();
  ...
  const expiration = Math.floor(Date.now() / 1e3) + expiresIn;
...
__name2(handleGetFileUrl, "handleGetFileUrl");
async function handleAiTutorSuggestions(request, env) {
  if (request.method !== "POST") {
    return jsonResponse({ error: "Method not allowed" }, 405);
  }
  return jsonResponse({
...
__name(handleAiTutorSuggestions, "handleAiTutorSuggestions");
__name2(handleAiTutorSuggestions, "handleAiTutorSuggestions");
async function handleAiTutorChat(request, env) {
  if (request.method !== "POST") {
    return jsonResponse({ error: "Method not allowed" }, 405);
  }
  return jsonResponse({
...
__name(handleAiTutorChat, "handleAiTutorChat");
__name2(handleAiTutorChat, "handleAiTutorChat");
async function handleAiTutorFeedback(request, env) {
  if (request.method !== "POST") {
    return jsonResponse({ error: "Method not allowed" }, 405);
  }
  return jsonResponse({
...
__name(handleAiTutorFeedback, "handleAiTutorFeedback");
__name2(handleAiTutorFeedback, "handleAiTutorFeedback");
async function handleAiTutorProgress(request, env) {
  return jsonResponse({
...
__name(handleAiTutorProgress, "handleAiTutorProgress");
__name2(handleAiTutorProgress, "handleAiTutorProgress");
async function handleAiVideoSummarizer(request, env, waitUntil) {
  if (request.method !== "POST") {
    return jsonResponse({ error: "Method not allowed" }, 405);
  }
  return jsonResponse({
...
__name(handleAiVideoSummarizer, "handleAiVideoSummarizer");
__name2(handleAiVideoSummarizer, "handleAiVideoSummarizer");
var onRequest5 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    if (path === "/ai-video-summarizer") {
      return await handleAiVideoSummarizer(request, env, context.waitUntil);
    }
    if (path === "/health" || path === "/") {
      return jsonResponse({
        status: "ok",
...
async function handleFetchCertificate(request) {
  if (request.method !== "POST") {
    return jsonResponse({ error: "Method not allowed" }, 405);
  }
  const body = await request.json();
  ...
  const extractMeta = /* @__PURE__ */ __name2((html2, property) => {
    const regex = new RegExp(`<meta[^>]*(?:property|name)=["']${property}["'][^>]*content=["']([^"']*)["']`, "i");
    const match2 = html2.match(regex);
    return match2 ? match2[1] : "";
  }, "extractMeta");
  const extractMetaReverse = /* @__PURE__ */ __name2((html2, property) => {
    const regex = new RegExp(`<meta[^>]*content=["']([^"']*)["'][^>]*(?:property|name)=["']${property}["']`, "i");
    const match2 = html2.match(regex);
    return match2 ? match2[1] : "";
  }, "extractMetaReverse");
  const extractTitle = /* @__PURE__ */ __name2((html2) => {
  ...
  const bodySnippet = html.replace(/<script[\s\S]*?<\/script>/gi, "").replace(/<style[\s\S]*?<\/style>/gi, "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().slice(0, 5e3);
  return jsonResponse({
    success: true,
    metadata,
    platformData,
...
__name(handleFetchCertificate, "handleFetchCertificate");
__name2(handleFetchCertificate, "handleFetchCertificate");
...
async function hashOtp(otp) {
  const encoder2 = new TextEncoder();
  const data = encoder2.encode(otp);
  const hashBuffer = await crypto.subtle.digest("SHA-256", data);
  const hashArray = Array.from(new Uint8Array(hashBuffer));
  return hashArray.map((b) => b.toString(16).padStart(2, "0")).join("");
}
__name(hashOtp, "hashOtp");
__name2(hashOtp, "hashOtp");
async function verifyOtpHash(otp, storedHash) {
  const inputHash = await hashOtp(otp);
  return inputHash === storedHash;
}
__name(verifyOtpHash, "verifyOtpHash");
__name2(verifyOtpHash, "verifyOtpHash");
async function sha2562(message) {
  const encoder2 = new TextEncoder();
  const data = encoder2.encode(message);
  const hashBuffer = await crypto.subtle.digest("SHA-256", data);
...
async function hmacSha256(key, message) {
  ...
  return await crypto.subtle.sign("HMAC", cryptoKey, encoder2.encode(message));
}
...
async function createAwsSignature(method, host, path, queryString, headers, payload, config, timestamp, dateStamp) {
  const encoder2 = new TextEncoder();
  ...
  const payloadHash = await sha2562(payload);
  ...
  const canonicalRequestHash = await sha2562(canonicalRequest);
  const stringToSign = [
    algorithm,
    timestamp,
    credentialScope,
  ...
  const kDate = await hmacSha256(encoder2.encode(`AWS4${config.secretAccessKey}`), dateStamp);
...
__name(createAwsSignature, "createAwsSignature");
__name2(createAwsSignature, "createAwsSignature");
async function sendSms(phoneNumber, message, env) {
  ...
  try {
    ...
    console.error("[SNS DEBUG] Request Error:", error.message);
...
async function storeOtp(supabase, phone, otpHash, expiryMinutes) {
  const expiresAt2 = new Date(Date.now() + expiryMinutes * 60 * 1e3).toISOString();
...
__name2(storeOtp, "storeOtp");
async function getOtpRecord(supabase, phone) {
  const { data, error } = await supabase.from("phone_otps").select("*").eq("phone", phone).single();
  if (error || !data) {
    return null;
  }
  return data;
}
__name(getOtpRecord, "getOtpRecord");
__name2(getOtpRecord, "getOtpRecord");
async function incrementAttempts(supabase, phone) {
  const { data } = await supabase.from("phone_otps").select("attempts").eq("phone", phone).single();
  if (data) {
    await supabase.from("phone_otps").update({ attempts: (data.attempts || 0) + 1 }).eq("phone", phone);
  }
}
__name(incrementAttempts, "incrementAttempts");
__name2(incrementAttempts, "incrementAttempts");
async function markOtpVerified(supabase, phone) {
  await supabase.from("phone_otps").update({ verified: true }).eq("phone", phone);
}
__name(markOtpVerified, "markOtpVerified");
__name2(markOtpVerified, "markOtpVerified");
async function deleteOtp(supabase, phone) {
  await supabase.from("phone_otps").delete().eq("phone", phone);
}
__name(deleteOtp, "deleteOtp");
__name2(deleteOtp, "deleteOtp");
async function isRateLimited(supabase, phone, maxRequestsPerHour = 5) {
  const oneHourAgo = new Date(Date.now() - 60 * 60 * 1e3).toISOString();
  const { count } = await supabase.from("otp_requests_log").select("*", { count: "exact", head: true }).eq("phone", phone).gte("created_at", oneHourAgo);
  return (count || 0) >= maxRequestsPerHour;
}
__name(isRateLimited, "isRateLimited");
__name2(isRateLimited, "isRateLimited");
async function logOtpRequest(supabase, phone) {
  await supabase.from("otp_requests_log").insert({ phone });
}
__name(logOtpRequest, "logOtpRequest");
__name2(logOtpRequest, "logOtpRequest");
...
__name2(formatPhoneNumber, "formatPhoneNumber");
async function sendOtpHandler(body, env, supabase) {
  try {
...
__name(sendOtpHandler, "sendOtpHandler");
__name2(sendOtpHandler, "sendOtpHandler");
function formatPhoneNumber2(phone, countryCode = "+91") {
  let cleaned = phone.replace(/\D/g, "");
  cleaned = cleaned.replace(/^0+/, "");
  if (countryCode === "+91" && cleaned.startsWith("91") && cleaned.length > 10) {
    cleaned = cleaned.slice(2);
...
__name(formatPhoneNumber2, "formatPhoneNumber2");
__name2(formatPhoneNumber2, "formatPhoneNumber");
var MAX_ATTEMPTS = 5;
async function verifyOtpHandler(body, env, supabase) {
  try {
    ...
    const formattedPhone = formatPhoneNumber2(phone, countryCode);
...
__name(verifyOtpHandler, "verifyOtpHandler");
__name2(verifyOtpHandler, "verifyOtpHandler");
function formatPhoneNumber3(phone, countryCode = "+91") {
  let cleaned = phone.replace(/\D/g, "");
  cleaned = cleaned.replace(/^0+/, "");
  if (countryCode === "+91" && cleaned.startsWith("91") && cleaned.length > 10) {
    cleaned = cleaned.slice(2);
...
__name(formatPhoneNumber3, "formatPhoneNumber3");
__name2(formatPhoneNumber3, "formatPhoneNumber");
var RESEND_COOLDOWN = 30;
async function resendOtpHandler(body, env, supabase) {
  try {
    const { phone, countryCode = "+91" } = body;
    if (!phone) {
      return jsonResponse({ success: false, error: "Phone number is required" }, 400);
    ...
    let formattedPhone;
    try {
      formattedPhone = formatPhoneNumber3(phone, countryCode);
    } catch (error) {
      return jsonResponse({ success: false, error: error.message }, 400);
    }
    const rateLimited = await isRateLimited(supabase, formattedPhone);
    ...
    if (existingOtp && existingOtp.created_at) {
      ...
      const now = Date.now();
      const secondsSinceCreation = (now - createdAt) / 1e3;
...
__name(resendOtpHandler, "resendOtpHandler");
__name2(resendOtpHandler, "resendOtpHandler");
...
async function handleGetStreak(supabase, studentId) {
  if (!studentId) {
    return jsonResponse({ error: "Student ID is required" }, 400);
  }
  const { data, error } = await supabase.from("student_streaks").select("*").eq("student_id", studentId).single();
  if (error) {
...
__name(handleGetStreak, "handleGetStreak");
__name2(handleGetStreak, "handleGetStreak");
async function handleCompleteStreak(supabase, studentId) {
  if (!studentId) {
    return jsonResponse({ error: "Student ID is required" }, 400);
  }
  const { data, error } = await supabase.rpc("update_student_streak", {
    p_student_id: studentId,
    p_activity_date: (/* @__PURE__ */ new Date()).toISOString().split("T")[0]
  });
  if (error) {
...
__name(handleCompleteStreak, "handleCompleteStreak");
__name2(handleCompleteStreak, "handleCompleteStreak");
async function handleGetNotifications(supabase, studentId, limit = 10) {
  if (!studentId) {
    return jsonResponse({ error: "Student ID is required" }, 400);
  }
  const { data, error } = await supabase.from("streak_notification_log").select("*").eq("student_id", studentId).order("sent_at", { ascending: false }).limit(limit);
  if (error) {
...
__name2(handleGetNotifications, "handleGetNotifications");
async function checkStudentActivityToday(supabase, studentId) {
  const today = /* @__PURE__ */ new Date();
  today.setHours(0, 0, 0, 0);
  const todayEnd = /* @__PURE__ */ new Date();
  todayEnd.setHours(23, 59, 59, 999);
  const { data, error } = await supabase.from("student_course_progress").select("id").eq("student_id", studentId).gte("last_accessed", today.toISOString()).lte("last_accessed", todayEnd.toISOString()).limit(1);
  if (error) {
...
__name(checkStudentActivityToday, "checkStudentActivityToday");
__name2(checkStudentActivityToday, "checkStudentActivityToday");
async function handleProcessStreak(supabase, studentId) {
  if (!studentId) {
    return jsonResponse({ error: "Student ID is required" }, 400);
  }
  try {
...
__name(handleProcessStreak, "handleProcessStreak");
__name2(handleProcessStreak, "handleProcessStreak");
async function handleResetDailyFlags(supabase) {
  const { data, error } = await supabase.rpc("reset_daily_streak_flags");
  if (error) {
...
__name(handleResetDailyFlags, "handleResetDailyFlags");
__name2(handleResetDailyFlags, "handleResetDailyFlags");
...
// ../../../../../home/gokul/.nvm/versions/node/v20.19.5/lib/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../../../../home/gokul/.nvm/versions/node/v20.19.5/lib/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-XSOArk/middleware-insertion-facade.js
...
// ../../../../../home/gokul/.nvm/versions/node/v20.19.5/lib/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-XSOArk/middleware-loader.entry.ts
...
aws4fetch/dist/aws4fetch.esm.mjs:
   ...
   *)
*/
//# sourceMappingURL=functionsWorker-0.046393787279255694.js.map
