...
CREATE UNIQUE INDEX IF NOT EXISTS idx_sub_cache_active_user
  ON public.subscription_cache(user_id)
  WHERE status IN ('active', 'pending');

CREATE INDEX IF NOT EXISTS idx_sub_cache_user ON public.subscription_cache(user_id);
CREATE INDEX IF NOT EXISTS idx_sub_cache_status ON public.subscription_cache(status);
CREATE INDEX IF NOT EXISTS idx_sub_cache_org ON public.subscription_cache(organization_id)
  WHERE organization_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_sub_cache_staleness ON public.subscription_cache(synced_at);
