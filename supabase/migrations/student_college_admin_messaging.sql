...
DO $ 
BEGIN
    -- Add college_id column if it doesn't exist (references colleges table)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'conversations' AND column_name = 'college_id') THEN
        ALTER TABLE conversations ADD COLUMN college_id UUID REFERENCES colleges(id);
    END IF;
    
    -- Add college_admin_unread_count column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'conversations' AND column_name = 'college_admin_unread_count') THEN
        ALTER TABLE conversations ADD COLUMN college_admin_unread_count INTEGER DEFAULT 0;
    END IF;
    
    -- Add soft delete columns for college admin if they don't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'conversations' AND column_name = 'deleted_by_college_admin') THEN
        ALTER TABLE conversations ADD COLUMN deleted_by_college_admin BOOLEAN DEFAULT FALSE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'conversations' AND column_name = 'college_admin_deleted_at') THEN
        ALTER TABLE conversations ADD COLUMN college_admin_deleted_at TIMESTAMP WITH TIME ZONE;
    END IF;
END $;
...
-- Update the participant check constraint to include student_college_admin conversations
DO $ 
BEGIN
...
-- Update sender_type and receiver_type constraints to include 'college_admin'
DO $ 
BEGIN
...
-- Create RLS policies for student-college_admin conversations
...
DO $
BEGIN
    RAISE NOTICE 'Student-College Admin messaging migration completed successfully!';
    RAISE NOTICE 'New conversation type supported: student_college_admin';
    RAISE NOTICE 'New columns added: college_id, college_admin_unread_count, deleted_by_college_admin, college_admin_deleted_at';
    RAISE NOTICE 'Uses colleges table - all admins in the college can see student messages';
    RAISE NOTICE 'RLS policies created for secure access';
    RAISE NOTICE 'Helper function created: get_or_create_student_college_admin_conversation';
END $;