...
serve(async (req) => {
  ...
  try {
    ...
    const { student, userEmail, schoolId: requestSchoolId } = await req.json() as { student: StudentData; userEmail?: string; schoolId?: string }
    ...
    const { data: currentUserData, error: currentUserError } = await supabaseAdmin
      .from('users')
      .select('id, organizationId, role, entity_type')
      .eq('email', user.email)
      .maybeSingle()
    ...
    console.log('✅ Creating student for school_id:', schoolId)
    ...
    try {
      ...
      const { data: publicUser, error: publicUserError } = await supabaseAdmin
        ...
        .insert({
          ...
          lastName: lastName,
          role: 'student',
          organizationId: schoolId, // Use schoolId instead of organizationId
          isActive: true,
          entity_type: 'student',
          metadata: {
      ...
      return new Response(
        JSON.stringify({
          ...
          data: {
            ...
            email: student.email,
            name: student.name,
            password: studentPassword, // Return password so admin can share with student
