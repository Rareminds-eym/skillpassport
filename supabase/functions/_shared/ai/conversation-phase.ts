// Conversation Phase Management - Production Ready v2.0

import type { ConversationPhase, CareerIntent } from '../types/career-ai.ts';
...
export function getPhaseParameters(
  ...
  const intentTemperatures: Record<CareerIntent, number> = {
    'find-jobs': 0.2,        // Very factual - must match real jobs
  ...
  if (intent) {
    ...
    if (factualIntents.includes(intent)) {
      ...
      result.top_p = 0.75; // More focused sampling
    }
  }
