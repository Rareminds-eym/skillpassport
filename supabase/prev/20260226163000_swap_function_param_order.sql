-- Swap parameter order to match Supabase's positional parameter resolution
-- Supabase RPC resolves parameters positionally when using named parameters

DROP FUNCTION IF EXISTS get_plan_for_entity(text, text, text);
DROP FUNCTION IF EXISTS get_all_plans_for_entity(text, text);

-- Swap order: business_type comes BEFORE entity_type to match RPC call order
...
-- Swap order for get_plan_for_entity too
