#!/usr/bin/env python3
"""
Find Missing Exports Script
Identifies all imports that are missing from their export files
"""
        ...
        for imp in imports:
            ...
            if is_default:
                ...
                if imp['name'] not in exports['named']:
                    # Check if it might be re-exported
...
def main():
    ...
    for export_file, items in sorted(by_file.items()):
        ...
        for name in names:
            examples = [item['importing_file'] for item in items if item['name'] == name][:2]
            print(f"   - {name}")
            for ex in examples:
                print(f"     imported by: {ex}")
        print()
    
    # Summary
    print(f"\n📊 Summary:")
    print(f"   Total missing exports: {len(missing)}")
    print(f"   Files needing fixes: {len(by_file)}")
