...
const StudentHistoryModal = ({
    ...
    onExportHistory: (student: Student, session: AttendanceSession, allRecords: AttendanceRecord[]) => void;
}) => {
    ...
    const stats = {
        ...
        percentage: studentHistory.length > 0 ? ((studentHistory.filter((r) => r.status === "present" || r.status === "late" || r.status === "excused").length / studentHistory.length) * 100).toFixed(1) : "0",
    };
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                            ...
                            <ReactApexChart
                                ...
                                series={[
                                    {
                                        name: "Attendance",
                                        data: studentHistory.slice(-7).map(r => r.status === "present" || r.status === "late" || r.status === "excused" ? 1 : 0),
                                    },
                                ]}
                                type="line"
                                height={200}
