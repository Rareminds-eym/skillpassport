-- Alternative migration script with manual duplicate handling
-- Run this AFTER checking duplicates with check_duplicates.sql

-- Step 1: Add new columns if they don't exist (same as before)
DO $$ 
BEGIN
...
-- Step 3: MANUAL DUPLICATE HANDLING SECTION
-- IMPORTANT: Review the output of check_duplicates.sql before proceeding
-- Uncomment and modify the DELETE statements below based on your needs

-- Option A: Keep the record with the most recent createdAt (or newest ID if createdAt is null)
/*
DELETE FROM public.students 
WHERE id NOT IN (
    SELECT DISTINCT ON (COALESCE(email, profile->>'email')) id
    FROM public.students 
    WHERE COALESCE(email, profile->>'email') IS NOT NULL
    ORDER BY COALESCE(email, profile->>'email'), 
             COALESCE("createdAt", '1970-01-01'::timestamp) DESC, 
             id DESC
) AND COALESCE(email, profile->>'email') IS NOT NULL;
*/

-- Option B: Keep records with the most complete profile data
/*
DELETE FROM public.students 
WHERE id NOT IN (
    SELECT DISTINCT ON (COALESCE(email, profile->>'email')) id
    ...
    WHERE COALESCE(email, profile->>'email') IS NOT NULL
    ORDER BY COALESCE(email, profile->>'email'), 
             jsonb_array_length(COALESCE(profile->'education', '[]'::jsonb)) DESC,
             jsonb_array_length(COALESCE(profile->'experience', '[]'::jsonb)) DESC,
             jsonb_array_length(COALESCE(profile->'technicalSkills', '[]'::jsonb)) DESC,
             id DESC
) AND COALESCE(email, profile->>'email') IS NOT NULL;
*/

-- Option C: Manual deletion - Delete specific records by ID
-- Replace the IDs below with the actual IDs you want to remove
/*
DELETE FROM public.students WHERE id IN (
    'replace-with-actual-id-1',
    'replace-with-actual-id-2'
    -- Add more IDs as needed
);
*/

-- Step 4: Add constraints (only after duplicates are resolved)
DO $$
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Duplicate emails still exist. Please resolve duplicates before adding unique constraint.';
    END IF;
...
-- Step 6: Create or replace trigger function for updated_at
...
-- Step 10: Create or replace function to handle new user registration
...
-- Step 12: Update the specific user record with the correct user_id
...
BEGIN
    ...
    IF auth_user_id IS NOT NULL THEN
        ...
        AND user_id IS NULL
        LIMIT 1; -- Only update one record in case there are still duplicates
